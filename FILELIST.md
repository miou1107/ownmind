...
新增檔：
```
tests/real-db-start-failure-detail.test.js — 兩條：docker 起不來的時候，丟出來的訊息
                                 要帶著 docker 自己給的原因；容器起來了卻一直不回話的
                                 時候，要帶著最後一次試探的錯誤與容器的日誌
```

修改檔：
```
tests/helpers/real-db.js       — docker run 的 stderr 原本整個丟掉，訊息只剩下了哪一行
                                 指令。埠被占、映像拉不下來、磁碟滿了、registry 擋流量，
                                 四種長得一模一樣。現在 stderr 接回來、跟著往外丟。
                                 「一直不回話」那一條也一樣：留下最後一次試探的錯誤，
                                 移除容器之前先讀 docker logs 的最後 20 行。
                                 試探次數可以用 OWNMIND_TEST_DB_READY_ATTEMPTS 蓋掉，
                                 只為了讓那條測試不用等 40 秒。只收正整數，-1 跟 1e400
                                 這種會被忽略（前者會變成一次都不試就放棄，後者是永遠不結束）
```
...
CHANGELOG.md, FILELIST.md              — 這一段
package.json / package-lock.json / README* / docs/README* — 1.30.19 → 1.30.20
```

前兩份是 dependabot 開的 PR 合進來的（#116、#118），nanoid 這次手動升。四份
`package.json` 的相依套件宣告一個字都沒動，根目錄那份只動版號。
...
tests/secret-detect-wp-prose.test.js（再改）
                                     — 第二組真的密碼，3 處。用產品自己的偵測器掃過每個
                                       進版控的檔案才找到，人眼那一輪漏了它
...
## v1.30.15 修改之四（company host 搬遷的說明書與搬資料腳本）

新增檔：
```
scripts/migrate-to-company host.sh           — 一次性資料搬遷。在本機跑、自己 SSH 出去，
                                       check / cutover / rollback 三個子指令。
                                       來源 API 先停再 dump（產品沒有唯讀模式，
                                       邊寫邊 dump 會靜靜漏行）、dump 三道驗證、
                                       目的地先重建 schema 再匯入、逐表比對筆數，
                                       對不上就中止。加密金鑰一併帶過去（只搬資料
                                       不搬金鑰，記憶會回來、存的密碼全解不開，
                                       而且沒有一步會報錯），寫完比對指紋。
...
docs/deploy-to-company host.html             — 給 Vin 與 Eric 看的部署說明書。發版流程圖、
                                       ...
                                       CI 四個值）
docs/superpowers/plans/2026-08-13-ownmind-to-company host-migration.md
...
```
hooks/lib/render-session-context.js  — 廣播內文本來走這一行：只留前 5 行、把換行接成空格、
                                       再砍到 400 字。三件會掉東西的事寫在同一行，
                                       而且砍完什麼都不說 —— 寫的人跟收的人都不知道少了。
                                       現在整篇送出去、換行保住；真的砍到的話明講砍了幾行幾字。
                                       上限訂成 2000 字，跟伺服器「超過 2000 字就不收」同一個
                                       數字：伺服器收得下的，這邊就一個字都不會少
tests/session-start-render.test.js（再改）
                                     — 四條新的，兩條舊的重寫。舊那兩條本來釘的是「只留 5 行」
                                       跟「砍到 400 字」—— 它們釘的正是這個 bug。
                                       新的其中一條會自己去讀伺服器的上限，兩邊不准再各走各的
```


## v1.30.12 修改之二（判斷的時間上限訂太短，實際上每一輪都會逾時）

發版當天第一輪就撞到：**判斷實際要 150 秒，而上限訂在 90 秒。**
當初那個 90 秒是拿「1 條規矩」量出來的；正式帳號一輪會挑出 9 條、提示 12,349 字。

```
hooks/lib/local-judge.js（再改）      — 判斷上限 90 秒 → 300 秒。順便把檔案裡另外兩處
                                       寫著「18 秒」的舊數字改掉 —— 就是那個數字害它訂太短的
hooks/lib/verdict-store.js（再改）     — 「判官沒回來」的門檻 180 秒 → 420 秒。
                                       不改的話，跑滿上限的判斷會被當成死掉的，結果被丟掉
tests/reply-check-review-round-two.test.js（再改）
                                     — 兩條。真正沒被守住的不是「數字夠不夠大」，
                                       是「門檻必須大於判斷上限加上網路跟啟動時間」——
...
1. 一個對話的**最後一段回話**永遠不會被檢查結果撿走 —— 撿的動作發生在「下一次你打字」，
   而最後一段後面沒有下一次。那一輪的判決有跑、有存檔，但沒有人會看到。
   要送出去的話，等於下一個對話一開頭跟你講上一個對話的事，那是產品決定、不是修 bug。
2. 「判官超過三分鐘沒回來」的門檻設在 3 分鐘，而實測最慢的情況是 115 秒。
   現在超過門檻會先再讀一次檔，所以就算判官晚到也不會弄丟判決；但如果哪天實際變慢，
   第一個症狀會是「判官沒回來」跳在其實只是比較慢的判官上，那時候要調的就是這個數字。

**發版順序：伺服器要先上（或同時上），不能讓用戶端先更新。**
用戶端會跟伺服器要「這一輪該對哪幾條規矩」。舊版伺服器不認得這個要求，
會回一個沒有規矩清單的答案 —— 新版用戶端看到就會判定「這次沒檢查」，
於是每一輪都跳紅字。資料庫那條新狀態（`pending`）是伺服器啟動時自己跑的，
所以伺服器上了就好，不用另外手動跑。

計畫：`docs/superpowers/plans/2026-08-16-reply-check-on-user-quota.md`
三塊都做完了。

修改：
```
src/routes/compliance.js            — 多一種模式：只挑出這一輪該比對哪幾條規矩、附上內文，
                                       不叫任何模型。判斷要搬到使用者自己的電腦上跑
                                       （額度在那裡），但規矩內文不在那裡 ——
                                       實測使用者電腦上只有 318 條的編號跟標籤、內文一條都沒有。
                                       沒帶 mode 的舊呼叫走原本的路，一個位元組都沒變
```

新增：
```
db/026_compliance_check_pending.sql  — 檢查的狀態多一種「還在判」。因為現在是先開一列、
                                       等使用者電腦判完再回填；中間那段如果沿用「略過」，
                                       等於把還在跑的檢查算成「跑完沒問題」
tests/compliance-select-only.test.js — 六條。其中一條專門用「一被呼叫就爆炸」的假模型當斷言，
                                       否則這條路偷偷還在打中轉，其他斷言照樣會過
tests/compliance-pending-outcome-real-db.test.js — 用真的資料庫跑。上面那六條是接假的資料庫，
                                       所以就算新狀態被資料庫擋掉也照樣全綠 ——
                                       而它本來真的會被擋掉。把 026 拿掉這條就會紅
shared/judge-prompt.js               — 判官要問什麼、答案怎麼讀。放在共用區是因為兩邊都要用：
                                       伺服器還在判的時候用它，之後改成你電腦上判也用它。
                                       同一段話存兩份會走鐘，而走鐘看起來會像判官改變心意
hooks/lib/local-judge.js             — 你電腦上的判官：用你自己的訂閱額度跑。
                                       關鍵是加了「安全模式」—— 沒加的話它會把你的 CLAUDE.md、
                                       你的規矩、還有 OwnMind 自己的掛勾整包載進去，
                                       結果它把規矩當成在講自己、開始道歉而不是判斷；
                                       而且 OwnMind 的掛勾會再叫一個判官出來，無限繞下去
tests/local-judge.test.js            — 十二條。每一種失敗（叫不動、逾時、答得不成形、
                                       這台沒裝 claude）都必須講出是哪一種，
                                       不准變成一個「沒有違規」的空答案 ——
                                       那跟「檢查過了沒問題」在畫面上長得一模一樣
hooks/lib/verdict-store.js           — 判決結果的交接處。判官要跑 29～54 秒，所以是
                                       AI 講完就丟出去背景跑、下一輪再撿。
                                       寫的人跟讀的人是兩個各自獨立的程式、中間沒有鎖，
                                       所以用「先寫暫存再改名」——讀的人只會看到舊的或
                                       完整的新的，不會看到寫到一半的。讀走就刪：
                                       同一件事報第二次，讀的人就學會跳過它
tests/verdict-store.test.js          — 八條，含「寫到一半的檔案永遠讀不到」跟
                                       「工作單帶著金鑰，用完就刪」
hooks/lib/run-local-judge.js         — 在背景跑的那支：跟伺服器要規矩 → 用你的額度判 →
                                       把結果寫下來 → 回報伺服器。先寫結果再回報，
                                       因為它跑完的時候那一輪早就結束了，網路最可能已經不在，
                                       而結果是要給你看的、回報是給資料庫看的
tests/run-local-judge.test.js        — 八條。它跑起來沒有人在看（沒有畫面、沒有人檢查它的
                                       結束代碼），所以每一條驗的都是同一件事：
                                       不管發生什麼，都要留下一個講清楚發生什麼的檔案
hooks/lib/verdict-collect.js         — 下一輪把結果撿起來：給你看的那句不帶任何行話，
                                       給 AI 的那段帶原因跟引用，而且明講「不要道歉、
                                       不要把發現再覆述一遍給使用者」——
                                       否則它會用一整段自我檢討洗版
tests/verdict-collect.test.js        — 十條，每一種狀態各一條，加上「判官沒跑」不准變成沉默
src/routes/compliance.js（再改）      — 多一個「回填判決」的入口。只認還在等的那一列、
                                       只認自己帳號的，免得客戶端把已經定案的紀錄改掉
hooks/locales/*（再改）               — 兩句新的：判官沒跑、以及事後抓到違規
hooks/lib/start-local-judge.js       — 把判官丟出去、自己馬上回來。丟出去用「分離」模式、
                                       而且不接管畫面輸出 —— 接了的話掛勾的輸出還開著，
                                       等於 Claude Code 還在等它
tests/start-local-judge.test.js      — 五條。最後一條是真的 spawn 一支出去：
                                       確認呼叫的人 0.4 秒內就脫身，而那支後來真的把事做完了
tests/reply-check-async.test.js      — 十條，整條路一次跑完（AI 講完那支 → 背景判官 →
                                       結果檔 → 下一輪那支 → 你的畫面）。用一台「接了電話
                                       不講話」的假伺服器證明掛勾真的沒在等：實測 89 毫秒回來。
                                       十條全部各弄壞一次確認會紅
```

修改（第三塊：接到兩支掛勾上）：
```
hooks/ownmind-reply-lint.js         — AI 講完話那支。以前它站在那裡等伺服器判完，
                                       現在改成把判官丟到背景就回來。等於這支不再有能力
                                       把 AI 的回話擋下來重寫 —— 這是真的變弱了，換來的是
                                       「檢查真的會跑」：舊的那條路一半的時候會失敗，
                                       失敗之後還會靜音五分鐘
hooks/ownmind-prompt-inject.js      — 下一輪開頭那支。多做一件事：把上一輪的判決撿起來，
                                       給你看的那句跟給 AI 的那段一次送出。
                                       兩件事分兩個欄位、但只能寫一次 —— 寫第二次的話
                                       Claude Code 兩份都讀不到
hooks/lib/compliance-step.js        — 本來就有的「這台能不能檢查」四道關卡抽出來共用
                                       （沒登入、沒抓規矩、被關掉、沒有一條規矩跟這輪有關），
                                       後面接的東西從「呼叫伺服器判」換成「起一個判官」
hooks/lib/verdict-collect.js（再改） — 兩件事：金鑰被拒絕要講「重新登入」而不是「等一下再說」
                                       （等待永遠修不好被撤銷的金鑰）；以及帶上編號，
                                       你才有辦法說「這條判錯了」
hooks/lib/run-local-judge.js（再改）  — 分得出「金鑰被拒絕」跟「伺服器連不上」
hooks/locales/*（再改）              — 再一句：判官連起都起不來
```

修改（審查抓到五個嚴重問題，這一輪修完）：
```
hooks/lib/verdict-store.js（再改）    — 一輪一個檔，不是一個對話一個檔。本來這一輪的判決會
                                       蓋掉上一輪的：上一輪抓到違規、這一輪乾淨，違規就沒了，
                                       而且畫面上跟「檢查過沒問題」一模一樣。工作單也一樣，
                                       而且更糟 —— 被蓋掉的那個判官讀不到工作單，
                                       不寫、不講、直接結束。改成一個對話一個資料夾、
                                       裡面一輪一個檔；讀不動的檔案也算「這輪沒檢查到」，
                                       不當作沒事
hooks/lib/start-local-judge.js（再改）— 丟判官出去之前，先在硬碟上留一筆「開始檢查了」。
                                       判官如果一起來就死掉，它什麼都不會寫，
                                       而什麼都沒寫正是「檢查過沒問題」的樣子。
                                       留這筆的必須是丟它出去的那支，不能是它自己。
                                       另外同一段回話不會被判兩次 —— 判兩次是花你自己的額度
hooks/lib/run-local-judge.js（再改）  — 每一條出口都要寫下結果。本來「你的帳號把檢查關掉了」
                                       跟「這輪沒有規矩要對」都是直接結束、什麼都不寫：
                                       前者讓 OwnMind 最該大聲的狀態變成最安靜的，
                                       後者會讓三分鐘後跳出「判官沒回來」——
                                       那是一句關於死掉的判官的真話、關於這一輪的假話
hooks/lib/verdict-collect.js（再改）  — 一次撿所有等著的判決，不是只撿一個。四件事跟著改：
                                       ①給 AI 的那段會引用「是哪一段回話」——
                                       慢了兩輪的判決本來會說「你上一段回話」，那是別段，
                                       AI 會去改錯的東西，而且從此一直錯一輪
                                       ②開始檢查後超過三分鐘還沒下文，明講那一段沒檢查
                                       ③講違規的那句改成 🟡 而且明說「這只是提醒、擋不住它」，
                                       並且講出總共幾條，不是只講第一條
...
tests/reply-check-turn-identity.test.js
                                     — 二十四條，五個嚴重問題各自的重現。全部用真的檔案跑，
                                       ...
                                       全部介面。十個修法各弄壞一次確認會紅
tests/check-failure-log-not-polluted-by-tests.test.js（再改）
                                     — 這道防線自己以同樣的方式失守了一次。它本來盯的是一個
                                       函式名字；判官搬家之後寫紀錄的換成另一支，
                                       新測試往我自己機器上的診斷紀錄塞了 30 筆假資料才被發現。
                                       現在「哪些程式會寫這個紀錄」是問出來的，不是寫死的清單
```

移除（判官搬到你自己的電腦之後，這條路沒有人走了）：
```
hooks/lib/compliance-client.js       — 整支刪掉。它是「叫伺服器判」用的，而現在沒有人叫了
hooks/lib/compliance-step.js（再改）  — 刪掉同步那半邊：擋下回話要 AI 重寫、重寫次數上限、
                                       以及跟著它們的三種訊息。留下來的是「這台能不能檢查」
                                       那四道關卡。刪之前先把一件會掉的事搬過去：
                                       團隊規範不是使用者自己的規矩，他說沒關係不算數 ——
                                       這句話舊那條路有講、新的沒有
hooks/locales/*（再改）              — 四句一起刪：伺服器沒做完、連不上伺服器、
                                       已經要 AI 重寫、重寫幾次還是不過
```

修改（第二輪審查：四個嚴重問題，其中一個是我自己刪掉的防護）：
```
hooks/lib/redact.js                  — 新的。送出去之前把「看起來像密碼／金鑰」的字遮掉。
                                       這段本來住在 compliance-client.js 裡面，判官搬到你電腦上
                                       之後那個檔沒人用了、被我刪掉 —— 遮蔽也跟著沒了，
                                       而新寫的那支送出去的程式沒人提醒它要帶這件事。
                                       所以現在它自己一個檔：下一個要送東西出去的人有東西可以引用，
                                       而不是自己重寫一份
hooks/lib/run-local-judge.js（再改）  — 三件事：①回話跟你的提問送出去之前先遮蔽
                                       ②伺服器「答了但沒做完」要先判，不能先看「開關是不是關的」——
                                       伺服器抓不到規矩的時候回的是「開著、但失敗了、而且沒有規矩」，
                                       照舊的順序會掉進「拿空的規矩表去判」，判出來是「略過」，
                                       而略過就是沉默；伺服器讀不到帳號的時候回的是「關閉」，
                                       照舊的順序會跟你說「你的帳號把檢查關掉了」——
                                       那是在講你自己的設定，而且是假的
                                       ③看不懂的回應也算失敗，不准掉進「檢查過沒問題」
hooks/lib/verdict-collect.js（再改）  — 三件事：①伺服器沒做完有自己的一句話（不用你做什麼）
                                       ②抓到違規的那一輪也要把節流狀態撥回正常 ——
                                       不撥的話它會卡在上一個失敗上，下一次同樣的失敗會被當成
                                       「沒變化，安靜」而不講，那一輪就是沒檢查又沒人講
                                       ③超過三分鐘要宣告「判官沒回來」之前，再讀一次檔 ——
                                       判官晚一秒回來的話，本來會被刪掉、還被講成沒回來
hooks/lib/verdict-store.js（再改）     — 三件事：①同一段話回答不同問題算兩次檢查（規矩是看兩邊挑的）
                                       ②清掃連工作單一起清 —— 工作單裡有你的金鑰，
                                       而它只有判官讀走才會刪，判官被強制關掉就永遠留在那
                                       ③空資料夾看它自己的時間，不是看它沒有的檔案 ——
                                       不然正在用的對話每一輪都被砍掉重建
hooks/ownmind-reply-lint.js（再改）   — 拿掉「已經照你的規矩檢查過 AI 這段回話」那句。
                                       講這句的時候它最多只是「把判官丟出去了」，
                                       離有答案還有 30～50 秒。這句改由真的拿到判決的那一輪講
hooks/lib/local-judge.js（再改）      — 判官引用回話當證據，而那段引用會被回報到伺服器。
                                       回話本身出去之前已經遮過了，這是同一段話走另一條路，
                                       所以也要遮：不然回話裡那行 api_key=… 剛好就是違規那行的話，
                                       它會原封不動送出去
hooks/locales/*（再改）              — 一句新的（伺服器沒做完），一句補上「你不用做什麼」
tests/reply-check-review-round-two.test.js
                                     — 二十一條。每一條都是「有綠燈、有做過破壞測試」的程式碼裡
                                       還是漏掉的狀態。十五個修法各弄壞一次確認會紅 ——
                                       其中一次發現我的破壞測試腳本自己判斷寫反了，
                                       把「抓到了」全部印成「沒有測試對到」

第三輪審查（再兩個嚴重的）：
```
hooks/lib/verdict-collect.js（三改）  — 三件事：①「這一輪沒有規矩要對」不算「檢查是好的」。
                                       伺服器說沒規矩要對，判官根本沒被叫起來 ——
                                       本來會在一台根本沒裝 Claude Code 的機器上印綠燈說
                                       「OwnMind 又在檢查了」，而且跟紅字一輪一輪交替出現，
                                       等於節流也失效
                                       ②再讀一次的時候，「檔案不見了」跟「檔案讀不動」要分開：
                                       不見了是別人已經處理掉了，不是判官沒回來
                                       ③叫不起 Claude Code（權限不對之類）也算 Claude Code 那邊的事，
                                       不要叫使用者去跑 OwnMind 的更新指令
hooks/lib/verdict-store.js（三改）     — 兩件事：①判決檔改成只有你讀得到（裡面有 AI 講的話的前 160 字
                                       跟判官從裡面引的句子）②讀單一輪的時候分得出「不見了」跟「讀不動」
tests/*（再改）                       — 三個測試檔本來會去讀開發者自己機器上的狀態目錄，
                                       答案會因為在誰的電腦上跑而不同
```
```

## v1.30.10 修改（測試安裝不准冒充真機、以及那兩個紅字的真相）
...
tests/guard-failure-is-not-silence.test.js
                                     — 三條。防護壞掉的時候不准長得像防護通過。
                                       其中一條一開始驗錯了東西（只驗「沒當掉」，
                                       而沒當掉在修之前本來就成立），弄壞一次才發現，
                                       改成驗「擋門真的出聲了」
```

修改（防護消失的時候要講出來）：
```
hooks/ownmind-git-pre-commit.js      — 它自己壞掉的時候，本來只印一行小字然後 commit 就過了，
                                       畫面跟「檢查過沒問題」幾乎一樣。現在明講：這次沒有掃密碼、
                                       也沒有比對規矩。還是不擋 —— 壞掉的檢查不該擋住人做事
hooks/ownmind-edit-reminder.js       — 讀憑證讀失敗的時候，整個擋改檔的門就不跑了，而且完全沒聲音。
                                       憑證是給另一件事用的，門根本不需要它。現在讀不到就算了，
                                       門照跑
```
...
修改：
```
tests/iron-rule-trigger-parity.test.js          — 30 個指令改成先平行分類（一次四個）再逐條
                                                   判斷，55 秒降到 16 秒。維持一個指令一條測試，
                                                   真的不一致時仍會用該指令的名字報錯。
                                                   每次執行給不同的對話編號：掛鉤的「一小時
                                                   只列一次」是用它當鑰匙，原本三十次共用同一把
```
...
hooks/locales/ja.json                            — Task 6: generated by `npm run
                                                   ...
                                                   shared legacy-server.example/llm-switch proxy —
