...
hooks/lib/verdict-store.js           — 判決結果的交接處。判官要跑 29～54 秒，所以是
                                       ...
                                       同一件事報第二次，讀的人就學會跳過它
tests/verdict-store.test.js          — 七條，含「寫到一半的檔案永遠讀不到」跟
                                       「工作單帶著金鑰，用完就刪」
...
hooks/lib/verdict-collect.js         — 下一輪把結果撿起來：給你看的那句不帶任何行話，
                                       ...
                                       不要把發現再覆述一遍給使用者」——
                                       否則它會用一整段自我檢討洗版
tests/verdict-collect.test.js        — 五條，四種狀態各一條，加上「判官沒跑」不准變成沉默
...
tests/reply-check-async.test.js      — 八條，整條路一次跑完（AI 講完那支 → 背景判官 →
