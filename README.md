...
- **Half the memory index had stopped reaching the session, and nothing said so** — the index file grows one line per memory and the thing that reads it stops at 200 lines, asking for under 140. Measured on a real install: 283 lines and 52 KB, 143 iron rules plus 130 projects, longest line 305 characters. Everything past the limit was being dropped at load time, with no mark on the file and no warning from either side, so it went on reading as a complete index. What pushed it over was listing every iron rule, which the SessionStart hook already injects into the session in full, so those lines bought nothing and squeezed out projects instead. The index now has a line budget it cannot cross: entries are chosen most-recently-updated first, a type that needs fewer lines than its share releases the rest to the others, entry lines are capped at 200 characters with a visible `…`, and **each section states how many it left out and where to find them**. Same data: 140 lines and 28 KB, 127 of 273 memories listed, the other 146 named out loud. No memory file is deleted and nothing becomes unreachable — only the listing is capped `v1.26.100`
