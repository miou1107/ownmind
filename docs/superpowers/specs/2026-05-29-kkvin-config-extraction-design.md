# Extract hardcoded `legacy-server.example` into config — design
...
`legacy-server.example` is hardcoded in ~60 places across source, docs, tests, and history.
...
3. The author can still run one hosted server (`legacy-server.example`) for themselves and
...
- **小明 (existing user):** installed months ago; his `~/.ownmind` already holds
  ...
  upgrade one-liner that hits `legacy-server.example` must keep working.
- **阿華 (stranger who clones to self-host):** opens the repo, sees
  `YOUR_OWNMIND_URL` placeholders, fills his own host. No `legacy-server.example` anywhere
  in what he reads.
- **阿明 (friend who becomes a user):** the author privately hands him one
  install command containing the real `legacy-server.example` API URL + his API key. This
  command is NOT in the public repo.
- **admin = the author deploying to legacy-server.example:** real values live in a
...
- **E2** — the install script is fetched from the public **GitHub repo**, not
  from `legacy-server.example`. The author's "serve bootstrap" route stays alive for
...
- **Historical files included** — `CHANGELOG.md`, `docs/superpowers/**`, and
  `openspec/changes/archive/**` also get `legacy-server.example` → `example.com`.
...
- `hooks/ownmind-iron-rule-check.sh:97-98` — drop the hardcoded
  `|| 'https://legacy-server.example/ownmind'` fallback; if env missing, no-op (小明 has it).
...
- `src/lib/llm-narrative.js:81` — read `process.env.OWNMIND_LLM_API_BASE`; no
  `legacy-server.example` default; error clearly if unset.
...
- `.env.example` — document `OWNMIND_LLM_API_BASE` (replaces the
  `legacy-server.example/llm-switch/dashboard` comment at line 25).
...
- **Keep** `src/app.js:183-186` (`/bootstrap.sh`, `/bootstrap.ps1` routes) —
  domain-agnostic, so 小明's old `legacy-server.example` one-liner keeps working untouched.

### ⑤ brand display
- `src/public/me/index.html:555` — "資料即時取自 legacy-server.example" → inject
...
- History: `CHANGELOG.md`, `docs/superpowers/**`, `openspec/changes/archive/**`
  → `legacy-server.example` → `example.com` (and `root@legacy-server.example` → `root@example.com`).

## Backward compatibility

- 小明's client reads his own `~/.ownmind` env → unaffected by repo changes.
- The bootstrap-serving route stays, so his old `legacy-server.example` upgrade one-liner
  still works.
- The hosted `legacy-server.example` service keeps running; only its references leave the
...
- The `shared/language-lint.js` proper-noun whitelist of internal project
  ...
  lint behavior and the tokens are low-exposure). Revisit during the legacy host
...
- Renaming/retiring the actual `legacy-server.example` domain (it keeps running).
...
- After the change: `grep -rn 'legacy host' . --include=... | grep -v node_modules`
