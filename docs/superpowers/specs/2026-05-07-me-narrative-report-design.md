...
* 走 OpenAI-compatible LLM Switch — `https://legacy-server.example/llm-switch/v1/chat/completions`，`model: 'auto'`，Bearer auth
...
```js
POST https://legacy-server.example/llm-switch/v1/chat/completions
...
* 手動驗測（IR-020）：部署後 legacy-server.example/ownmind/me 切到敘事報告 tab，確認 12 section 都 render、按鈕能觸發 LLM、刷新走 cache
...
* Production env 由 Vin 手動補 key 到 legacy-server.example 主機 `.env`，不寫進任何 git-tracked 檔案
