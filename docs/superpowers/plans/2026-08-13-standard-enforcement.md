...
const keywordRule = {
  id: 2, type: 'team_standard', title: 'company host onboarding',
  content: 'do not edit ci/projects.yml', tags: [],
  metadata: { enforcement: { keywords: ['company host', 'onboarding'] } },
};
const repoRule = {
...
test('a keyword in the user prompt selects the rule', () => {
  const { selected } = selectRules([keywordRule], { ...baseCtx, userPrompts: ['把 ownmind 搬到 company host'] });
...
test('keyword matching is case-insensitive', () => {
  const { selected } = selectRules([keywordRule], { ...baseCtx, userPrompts: ['company host migration'] });
  assert.deepEqual(selected.map((r) => r.id), [2]);
});
...
const payload = {
  ...
  user_prompts: ['migrate ownmind to company host'], repo_remote: null, tools_used: [],
};
...
const standard = { id: 412, type: 'team_standard', tags: [], keywords: ['company host'], always_check: false, repo_match: '' };

const base = {
  disabled: false, mode: 'block', apiKey: 'k', apiUrl: 'http://x',
  sessionId: 's1', assistantText: 'I will edit ci/projects.yml for company host',
...
**致命點（spec §4.1）：** repo 一定要由 `path.dirname(file_path)` 解析。事故當時 cwd 在 OwnMind repo、目標檔在 company host repo，用 cwd 會直接放行。
...
const execImpl = fakeExec({
  '/work/company host': 'https://git.fontrip.com/fontrip/fontrip-agentic-process-automation.git\n/work/company host\n',
...
test('the repo is resolved from the edited file, not the working directory', () => {
  const remote = resolveRepoRemote('/work/company host/ci/projects.yml', { execImpl });
...
test('a forbidden path in the guarded repo is caught even when the session is in another repo', () => {
  // The 2026-08-13 topology exactly: cwd was the OwnMind checkout, the target was company host.
...
test('a non-forbidden path in the guarded repo is not caught', () => {
  const v = findGuardViolation('/work/company host/Projects/ownmind/src/index.js', [standard], { execImpl });
  assert.equal(v, null);
});

test('the root config file pattern matches exactly, not as a prefix', () => {
  assert.ok(findGuardViolation('/work/company host/.gitlab-ci.yml', [standard], { execImpl }));
  assert.equal(findGuardViolation('/work/company host/docs/.gitlab-ci.yml.md', [standard], { execImpl }), null);
});
...
test('a standard with no guard block is ignored', () => {
  const v = findGuardViolation('/work/company host/ci/projects.yml', [{ id: 1, metadata: {} }], { execImpl });
  assert.equal(v, null);
});

test('running twice gives the same answer', () => {
  // IR-135: an operation that will be applied repeatedly must be tested twice.
  const a = findGuardViolation('/work/company host/ci/projects.yml', [standard], { execImpl });
  const b = findGuardViolation('/work/company host/ci/projects.yml', [standard], { execImpl });
...
test('a standard already injected this session is not injected again', () => {
  const { text, injectedIds } = buildInjection([standard], 'company host', null, [412]);
...
test('dropping the guard while keeping keywords is still a removal', () => {
  ...
  const existing = { enforcement: { guard: { paths: ['ci/**'] }, keywords: ['company host'] } };
  const r = checkEnforcementRemoval(existing, { enforcement: { keywords: ['company host'] } });
  assert.equal(r.blocked, true);
...
test('layer 1: the standard is injected when the user mentions company host', () => {
  const { text, injectedIds } = buildInjection(BUNDLE_INJECTABLES, 'ownmind 專案要遷移到 company host', null, []);
  assert.deepEqual(injectedIds, [412]);
...
test('layer 2: the bad reply selects the standard for judging', () => {
  const { selected } = selectRules([std412], {
    assistantText: THE_BAD_REPLY, userPrompts: ['migrate to company host'], repoRemote: null, toolsUsed: [],
  });
...
test('layer 3: the edit itself is blocked, from a session running in a different repo', () => {
  const company host = mkdtempSync(path.join(os.tmpdir(), 'incident-fixture-company host-'));
  execFileSync('git', ['init', '-q', company host]);
  execFileSync('git', ['-C', company host, 'remote', 'add', 'origin', 'https://example.com/incident-fixture-company host.git']);
  mkdirSync(path.join(company host, 'ci'), { recursive: true });
  writeFileSync(path.join(company host, 'ci', 'projects.yml'), 'projects: {}\n');

  const v = findGuardViolation(path.join(company host, 'ci', 'projects.yml'), BUNDLE_GUARDS);
  assert.ok(v, 'the edit must be blocked regardless of where the session started');
  assert.equal(v.standard.id, 412);
  rmSync(company host, { recursive: true, force: true });
});
```
