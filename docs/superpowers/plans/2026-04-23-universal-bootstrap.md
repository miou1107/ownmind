...
- Express route serving the scripts at `https://legacy-server.example/ownmind/bootstrap.sh|.ps1` for one-line curl/iwr install on fresh machines.
...
#   Remote: curl -fsSL https://legacy-server.example/ownmind/bootstrap.sh | bash
#
# Branches:
...
#   Remote: iwr -useb https://legacy-server.example/ownmind/bootstrap.ps1 | iex
#
# Branches:
#   1. ~/.ownmind not present         → fresh clone + install
#   2. ~/.ownmind present, no .git    → backup + re-clone + install (repair)
...
// `curl -fsSL https://legacy-server.example/ownmind/bootstrap.sh | bash` before they
...
- Runs: `curl -fsSL https://legacy-server.example/ownmind/bootstrap.sh | bash` (Mac/Linux) or `iwr -useb https://legacy-server.example/ownmind/bootstrap.ps1 | iex` (Windows)
...
- [ ] SSH deploy: `ssh root@legacy-server.example "cd /srv/ownmind && git pull && docker compose build --no-cache api && docker compose up -d api"`
- [ ] Verify `/api/memory/init` returns `server_version: 1.17.6`
- [ ] Smoke test: `curl -sS https://legacy-server.example/ownmind/bootstrap.sh | head -5` — should stream the bash script
...
- Host at legacy-server.example → Express route (Task 3) ✓
