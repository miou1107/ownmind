...
```bash
ssh root@example.com "cd /srv/ownmind && docker exec ownmind-db psql -U ownmind -d ownmind -f /dev/stdin" < db/004_weekly_summary_marker.sql
```
...
```bash
ssh root@example.com "cd /srv/ownmind && git pull && npm test"
```
...
```bash
ssh root@example.com "cd /srv/ownmind && npm test"
```
...
```bash
ssh root@example.com "cd /srv/ownmind && git pull && docker compose build --no-cache api && docker compose up -d api"
```
...
```bash
ssh root@example.com "cd /srv/ownmind && git pull && docker compose build --no-cache api && docker compose up -d api && docker logs ownmind-api --tail=20"
```
