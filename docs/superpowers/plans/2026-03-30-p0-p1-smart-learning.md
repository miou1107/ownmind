...
- [ ] **Step 2：在 legacy-server.example 執行 migration**

```bash
ssh root@legacy-server.example "cd /srv/ownmind && docker exec ownmind-db psql -U ownmind -d ownmind -f /dev/stdin" < db/004_weekly_summary_marker.sql
```
...
```bash
ssh root@legacy-server.example "docker exec ownmind-db psql -U ownmind -d ownmind -c '\d users'"
```
...
```bash
ssh root@legacy-server.example "cd /srv/ownmind && git pull && npm test"
```
...
```bash
ssh root@legacy-server.example "cd /srv/ownmind && npm test"
```
...
- [ ] **Step 4：部署到 legacy-server.example 並驗證**

```bash
ssh root@legacy-server.example "cd /srv/ownmind && git pull && docker compose build --no-cache api && docker compose up -d api"
```

驗證（換成實際 api_key）：
```bash
curl "http://legacy-server.example:3100/api/session/report?period=week&offset=1" \
...
```bash
ssh root@legacy-server.example "cd /srv/ownmind && git pull && docker compose build --no-cache api && docker compose up -d api && docker logs ownmind-api --tail=20"
```
...
驗證：
```bash
curl "http://legacy-server.example:3100/api/memory/init?compact=true" \
...
開瀏覽器到 `http://legacy-server.example:3100/admin`，點「週/月報」頁籤，選期間，點「載入」，確認資料顯示正確。
