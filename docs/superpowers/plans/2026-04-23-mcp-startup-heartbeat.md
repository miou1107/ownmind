...
curl -sS -X POST \
  ...
  https://legacy-server.example/ownmind/api/broadcast/admin
```
...
Open `https://legacy-server.example/ownmind/admin`, scroll to 廣播管理, confirm the new row appears with severity=warning.
