...
**Tech Stack:** Node.js + Express + node:test + PostgreSQL + Mermaid (CDN) + llm-switch (https://legacy-server.example/llm-switch/v1，OpenAI-compatible，model='auto')
...
export async function callLLMSwitch({ apiKey, messages, fetchImpl = fetch, timeoutMs = 30_000 }) {
  ...
  try {
    const res = await fetchImpl('https://legacy-server.example/llm-switch/v1/chat/completions', {
      method: 'POST',
      headers: {
...
# 管理者請到 https://legacy-server.example/llm-switch/dashboard 申請 key
LLM_SWITCH_API_KEY=
```
...
- [ ] **Step 3（部署時人工）：到 legacy-server.example 主機 `.env` 加真 key**

```bash
ssh legacy host 'echo "LLM_SWITCH_API_KEY=<從 user 對話拿到的 key>" >> /path/to/ownmind/.env'
...
到 https://legacy-server.example/ownmind/me：
...
提醒 Vin 到 https://legacy-server.example/llm-switch dashboard rotate 之前在 conversation 貼出來的 key，然後改 production `.env` + restart。
