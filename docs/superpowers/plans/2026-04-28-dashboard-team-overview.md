...
如果走 PR + review 流程，先開 PR、merge 後再 tag。tag 推上去觸發 CI/CD 部署到 legacy-server.example。
...
```bash
curl -s https://legacy-server.example/api/health | grep version
# 或
curl -s https://legacy-server.example/api/memory/init -H "x-api-key: ..." | jq .server_version
```
