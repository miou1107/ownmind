...
- **安裝檢測警報 job** — `runInstallCheckAlerts` 讀每台機器最新一份自我檢測回報（用伺服器給的流水號排新舊，不用客戶端送來的時間戳），記錄新增／已解決的失敗，把 48 小時有效的廣播草稿發給資歷最久的一位 super admin。每一筆新失敗都先認領、再寫廣播，廣播寫失敗就把認領放掉，所以兩支同時跑的評估只會講一次、廣播失敗也不會讓失敗從此靜音。每次報告存檔後執行一次（`POST /api/debug/install-check`，失敗只記日誌不拋出），伺服器每次開機也補跑一次、掃過既有回報 `v1.26.87`
