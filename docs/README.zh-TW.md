...
- **安裝檢測失敗渲染** — 純函式把彙整後的安裝檢測失敗渲染成看得懂的訊息。多台機器同一個問題合併成一行；訊息超過廣播字數上限會自動截斷、附上遺漏筆數 `v1.26.87`
- **安裝檢測警報 job** — `runInstallCheckAlerts` 讀每台機器最新一份自我檢測回報，記錄新增／已解決的失敗，把廣播草稿發給資歷最久的一位 super admin。每次報告存檔後執行一次（`POST /api/debug/install-check`，失敗只記日誌不拋出），伺服器每次開機也補跑一次、掃過既有回報 `v1.26.87`
