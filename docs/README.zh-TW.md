...
- **安裝檢測警報 job** — `runInstallCheckAlerts` 讀每台機器最新一份自我檢測回報（用伺服器給的流水號排新舊，不用客戶端送來的時間戳），記錄新增／已解決的失敗，把 48 小時有效的廣播草稿發給資歷最久的一位 super admin。每一筆新失敗都先認領、再寫廣播；從第一次認領到寫廣播之間任何一步失敗，就把這一輪真的認領到的那些放掉再往上拋。所以兩支同時跑的評估只會講一次，而且不會有「寫到一半」留下一筆標記成已公告、卻沒有任何廣播的失敗。每次報告存檔後執行一次（`POST /api/debug/install-check`，失敗只記日誌不拋出），伺服器每次開機也補跑一次、掃過既有回報 `v1.26.87`
