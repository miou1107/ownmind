...
- **記憶索引有一半沒進到 session，而且沒有人講** — 索引每多一筆記憶就多一行，但讀它的那一端 200 行就停，還要求壓在 140 行以內。真實安裝量到的是 283 行、52 KB，143 條鐵律加 130 個專案，最長的一行 305 字。超過的部分在載入當下就被丟掉，檔案上沒有記號、兩邊都不出聲，所以它一直讀起來像一份完整的索引。把它推過上限的，是「把每一條鐵律都列一次」，而 SessionStart 掛勾本來就已經把整組鐵律注入 session 了，那些行沒有多給任何東西，卻把專案擠了出去。現在索引有一個跨不過去的行數預算：每一類挑最近更新的排前面，用不完的額度讓給別類，每行最多 200 字、截斷處補 `…`，而且**每一類都會講出自己少列了幾筆、去哪裡找**。同一份資料：140 行 28 KB，273 筆裡列出 127 筆，另外 146 筆被明確標示。沒有刪掉任何記憶檔，也沒有任何一筆變得找不到，被限制的只有索引的列表 `v1.26.100`
