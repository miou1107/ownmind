...
OwnMind 透過 SessionStart hook 自動載入記憶，你會在 context 中看到【OwnMind vX.X.X】標記的記憶內容。
如果沒有看到，手動呼叫 OwnMind API 載入：
- Endpoint：`GET YOUR_OWNMIND_URL/api/memory/init`
...
格式：`{ "summary": "做了什麼（1-2句）", "tool": "gemini-cli", "model": "模型名" }`
不需使用者同意，直接記錄。
