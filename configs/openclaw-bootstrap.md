...
> 如果 shell 不可用，呼叫 OwnMind API: `GET /api/memory/init`（Header: `Authorization: Bearer <OWNMIND_API_KEY>`）

載入後：
1. 顯示摘要（【OwnMind vX.X.X】{類型}：{內容} 格式）
2. 將所有 iron_rules 內化為工作準則
3. 如有待接手交接，先摘要給使用者確認
...
即將違反鐵律時立即停止：
【OwnMind vX.X.X】鐵律觸發：你提醒過「XXX」，我要遵守，不能再犯

## 衝突偵測（強制）

矛盾時不自行決定：【OwnMind vX.X.X】衝突偵測：列出雙方規則，問使用者
