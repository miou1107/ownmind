...
10. 輸入如果有 \`_condensed\` 這個 key，代表**那幾區的資料在送給你之前被摘要過**，你看到的不是全部。
   \`_condensed\` 是一份中文說明，逐條講哪一區被動了什麼手腳。
   - 寫到被摘要的那一區時，**先講清楚你看到的是哪一部分**（例如「這裡只列出有違反或跳過的規則」）。
   - **禁止**對那一區推斷整體比例、總數、或用「大部分」「多數」「普遍」這類全稱說法。
   - 例：規則遵守紀錄只留下有違反的那幾條時，
     ✗ 「鐵律遵守狀況不佳，多數規則都被違反」 ← 你看到的本來就只有違反的那些，這句是假的
     ✓ 「這裡只列出有違反或跳過的規則，其中 X 條是 ___」
   同一條規則的精神跟第 9 點一樣：看不到全部就不要講全部。`;
