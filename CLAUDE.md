...
## 要叫模型判斷東西的時候，用使用者自己的額度（IR-160，2026-08-16 Vin 第二次交代）

**任何「叫一個模型來判斷／產生東西」的功能，用使用者自己訂閱的 Claude Code 額度。
不准打 llm switch 中轉。**

判準：程式碼裡出現 `callLLMSwitch`、`LLM_SWITCH_API_KEY`、`OWNMIND_LLM_API_BASE`，
而且那段是新寫或正在改的 → 停下來，這條不允許。

不准用的理由：
- 「既有的功能就是這樣打的，我照抄」→ 既有的是還沒改的，不是對的
- 「先用中轉做出來再說，之後再搬」→ 已經發生兩次了，沒有之後

**為什麼寫在這裡：** Vin 之前就交代過一次，而那次沒有落地到任何一層 ——
OwnMind 記憶裡沒有、repo 裡也沒有。於是 v1.30.3（替判官選模型）跟 v1.30.10
（修判官一半會失敗）兩輪都繼續建在中轉上面，第二輪還花了三次診斷、四次連進正式機。
這正是下一節第 4 條講的事故，只是更糟：規範根本沒被存起來，所以連「沒讀」都談不上。

技術現況與尚未驗證的部分見 OwnMind 記憶 `project id=944`。
...
起源：2026-08-13 company host 搬遷規劃事故（規範已送達 AI 仍違反），詳見 OwnMind 記憶 principle id=904。
...
**這四道檢查也管表格的欄位標題跟列標籤，不是只管句子。**
2026-08-16 回報稽核結果時，我把「那台機器有沒有一個叫 `timeout` 的小工具」寫成表格的欄位標題 ——
主詞是機制，而且 `timeout` 本來就寫在上面第 4 條的禁用字清單裡。Vin 讀不懂。

會漏掉的原因很單純：我把「檢查每一句話」執行成「檢查每一個句號結尾的東西」，
而表格標題沒有句號。**送出前把表格的每個欄位標題跟列標籤單獨拉出來，當成一句話重跑這四道檢查。**
