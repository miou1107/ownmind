#!/bin/bash
# scripts/migrate-to-company host.sh
#
# One-off data migration: move every user's memories from the legacy host box to the
# company host host. Runs FROM A LAPTOP and SSHes out; it is not installed on either
# server.
#
# Usage:
#   bash scripts/migrate-to-company host.sh check      # both sides reachable, row counts, disk. Changes nothing.
#   bash scripts/migrate-to-company host.sh cutover    # the real move. Source API stops, so this is the outage.
#   bash scripts/migrate-to-company host.sh rollback   # bring the source API back up. Nothing else is undone.
#
# Targets are overridable so the same script rehearses against the staging host:
#   DST_HOST=root@test-server.example bash scripts/migrate-to-company host.sh cutover
#
...
#     costs seconds, because nothing on legacy host was deleted.

set -euo pipefail
umask 077

SRC_HOST="${SRC_HOST:-root@legacy-server.example}"
SRC_DIR="${SRC_DIR:-/srv/ownmind}"
DST_HOST="${DST_HOST:-root@prod-server.example}"
...
EOF
  ...
  log "  2. point legacy host's /ownmind/ at the new host as a plain proxy"
...
case "${1:-}" in
  ...
  *)
    echo "usage: bash scripts/migrate-to-company host.sh {check|cutover|rollback}" >&2
    exit 2
    ;;
esac
