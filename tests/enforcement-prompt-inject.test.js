...
const CI_STANDARD = {
  ...
  keywords: ['company host', 'onboarding'],
  always_check: false,
...
test('a keyword in the prompt injects the standard', () => {
  const { text, injectedIds } = buildInjection([CI_STANDARD], 'ownmind 專案要遷移到 company host', null, []);
...
test('the precedence sentence comes before the body, not after it', () => {
  ...
  const { text } = buildInjection([CI_STANDARD], 'company host', null, []);
...
test('matching is case-insensitive', () => {
  const { injectedIds } = buildInjection([CI_STANDARD], 'planning the company host migration', null, []);
  assert.deepEqual(injectedIds, [412]);
});
...
test('a standard already injected this session is not injected again', () => {
  ...
  const { text, injectedIds } = buildInjection([CI_STANDARD], 'company host', null, [412]);
  assert.equal(text, '');
  assert.deepEqual(injectedIds, []);
});
...
test('a standard with no body still carries its prohibition header', () => {
  ...
  const { text, injectedIds } = buildInjection([bodyless], 'company host', null, []);
  assert.deepEqual(injectedIds, [412]);
...
test('no injectables means no output and no crash', () => {
  assert.deepEqual(buildInjection([], 'company host', null, []), { text: '', injectedIds: [] });
  assert.deepEqual(buildInjection(null, 'company host', null, []), { text: '', injectedIds: [] });
});
...
test('the hook run as a program emits the injection and remembers it', () => {
  ...
  const run = () => execFileSync('node', [HOOK], {
    input: JSON.stringify({ session_id: 's-inject', prompt: 'migrate ownmind to company host' }),
    encoding: 'utf8',
...
test('a machine that never synced says so instead of staying silent', () => {
  ...
  const home = tempDir('om-inject-home-');
  const out = execFileSync('node', [HOOK], {
    input: JSON.stringify({ session_id: 's-nosync', prompt: 'migrate to company host' }),
    encoding: 'utf8',
    env: { ...process.env, HOME: home, USERPROFILE: home },
    timeout: 30_000,
  });
...
test('the header names the kind of rule, not always "standard"', () => {
  assert.match(buildInjection([CI_STANDARD], 'company host', null, []).text, /\[OwnMind team_standard 412\]/);
...
function runHook(home, sessionId, prompt = 'migrate to company host') {
...
test('the never-synced notice arrives once in a session, not on every prompt', () => {
  ...
  const first = runHook(home, 's-repeat', 'migrate to company host');
...
test('once the cache is there the notice stops, in the same session', () => {
  ...
  const out = runHook(home, 's-recovers', 'company host');
