...
describe('v1.19.13 — value-side keyword requires assignment style (S1)', () => {
  ...
  it('S1.1: dotted identifier anydesk.bot_legacy-host.unattended_password → allow', () => {
    const r = detectSecretLike('anydesk.bot_legacy-host.unattended_password');
  ...
  it('extra: reference doc containing multiple secret names (no assignment) → all allowed', () => {
    const refDoc =
      '相關 OwnMind secret：\n' +
      '- ssh.bot-server.example.vin.password\n' +
      '- anydesk.bot_legacy-host.unattended_password\n' +
...
describe('v1.19.13 review I-2: dotted identifier requires ≥ 3 segments', () => {
  ...
  it('3-segment identifier path anydesk.bot_legacy-host.unattended_password → allow (unchanged)', () => {
    const r = detectSecretLike('anydesk.bot_legacy-host.unattended_password');
    assert.equal(r.detected, false);
  });
