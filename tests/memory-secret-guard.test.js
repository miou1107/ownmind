...
// ============================================================
// v1.19.13 — matched_text + bot-server.example regression
...
describe('v1.19.13 — 400 response contains matched_text (S3)', () => {
  ...
  // S3.2
  it('S3.2: dotted identifier anydesk.bot_legacy-host.unattended_password → allow (regression)', () => {
    const result = validateMemoryContent({
      type: 'env',
      title: 'test',
      content: 'anydesk.bot_legacy-host.unattended_password',
    });
    assert.equal(result.ok, true,
...
describe('v1.19.13 — bot-server.example full-content regression (S4)', () => {
  // S4.1: the full 2026-05-23/24 conversation content that kept getting blocked
  it('S4.1: bot-server.example remote-access overview full text → allow', () => {
    const content =
      '## bot-server.example 可以怎麼遠端連進來\n\n' +
      '| 方式 | URL / IP / Port | 認證 | 用途 |\n' +
      '|---|---|---|---|\n' +
      '| **SSH** | `ssh vin@bot-server.example`（port 22）| 密碼 or key | 終端機指令、AI 自動化 |\n' +
      '| **xrdp** | `bot-server.example:3389` | ssh 同密碼 | 圖形桌面 |\n' +
      '| **AnyDesk** | ID `1901091212` | unattended password（在 OwnMind `anydesk.bot_legacy-host.unattended_password`）| 跨平台桌面 |\n' +
      '| **Tailscale** | bot-legacy host 或 `100.72.72.60` | Tailscale 私網 | 私網存取 |\n' +
      '| **Guacamole** | `https://app.legacy-server.example/guacamole/` | Guacamole 自己的帳密 | 瀏覽器內桌面 |\n' +
      '\n## 相關記憶\n\n' +
      '- OwnMind secret：`anydesk.bot_legacy-host.unattended_password`、' +
      '`ssh.bot-server.example.vin.password`、`hermes.telegram.bot_token`\n';

    const result = validateMemoryContent({
      type: 'env',
      title: 'bot-server.example 遠端訪問方式總覽',
      content,
    });
    assert.equal(result.ok, true,
      `bot-server.example full text must not be blocked, actual body=${result.body ? JSON.stringify(result.body) : 'none'}`);
  });
