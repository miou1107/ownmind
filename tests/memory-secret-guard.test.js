...
describe('v1.19.13 — 400 回應含 matched_text（S3）', () => {
  ...
  // S3.2
  it('S3.2：點分隔識別字 anydesk.bot_legacy-host.unattended_password → 放行（regression）', () => {
    const result = validateMemoryContent({
      type: 'env',
      title: 'test',
...
describe('v1.19.13 — bot-server.example 整段內容 regression（S4）', () => {
  // S4.1：完整 2026-05-23/24 對話中被連續擋下的內容
  it('S4.1：bot-server.example 遠端訪問方式總覽全文 → 放行', () => {
    const content =
    ...
    assert.equal(result.ok, true,
      `bot-server.example 全文不該被擋、實際 body=${result.body ? JSON.stringify(result.body) : 'none'}`);
  });
