...
describe('checkApiCredentials (v1.17.64 regression)', () => {
  it('打 /api/memory/init（不是 /api/init）+ 帶 Authorization Bearer header', async () => {
    ...
    try {
      const r = await selfCheck.checkApiCredentials('https://legacy-server.example/ownmind', 'k_test_apikey');
  ...
  it('401 回應仍判為 fail（避免 server 換認證後回 false pass）', async () => {
    ...
    try {
      const r = await selfCheck.checkApiCredentials('https://legacy-server.example/ownmind', 'bad');
