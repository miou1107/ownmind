...
test('a keyword in the user prompt selects the rule', () => {
  const { selected } = selectRules([KEYWORD], { ...CTX, userPrompts: ['把 ownmind 搬到 company host'] });
...
test('a keyword only in what the assistant said also selects it', () => {
  ...
  const { selected } = selectRules([KEYWORD], { ...CTX, assistantText: 'stage 0 of the company host move' });
  assert.deepEqual(selected.map((r) => r.id), [412]);
});
...
test('fragments are merged into the text the judge will read', () => {
  ...
  const { selected } = selectRules([fragmented], { ...CTX, userPrompts: ['company host'] });
...
test('the prompt carries each rule id, title and full text, plus the reply to audit', () => {
  ...
  const messages = buildJudgeMessages({
    rules, assistantText: 'I will edit ci/projects.yml', userPrompts: ['move to company host'],
  });
  ...
  assert.match(all, /move to company host/);
});
