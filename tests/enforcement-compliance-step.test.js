...
const BUNDLE = {
  present: true,
  selectors: [
    { id: 412, type: 'team_standard', tags: [], keywords: ['company host'], always_check: false, repo_match: '' },
  ],
  guards: [],
  injectables: [],
};
...
const BASE = {
  ...
  apiUrl: 'http://server',
  sessionId: 's1',
  assistantText: 'Stage 0 of the company host migration: I will add an entry to ci/projects.yml',
  userPrompts: [],
  repoRemote: null,
  trigger: '',
  bundle: BUNDLE,
...
test('a turn a rule bears on starts a judge, and says nothing about it', async () => {
  ...
  assert.match(started.assistantText, /company host/);
});
...
test('the local pre-filter matches the same four ways the server does', () => {
  ...
  assert.equal(anySelectorMatches([{ keywords: ['company host'] }], { ...ctx, userPrompts: ['go to company host'] }), true);
  assert.equal(anySelectorMatches([{ keywords: ['company host'] }], { ...ctx, assistantText: 'the company host move' }), true);
