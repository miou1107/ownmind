...
```
GIVEN  detectSecretLike() 第一個參數 value = "anydesk.bot_legacy-host.unattended_password"
...
```
GIVEN  type = 'env'、title = 'test'、content = 'anydesk.bot_legacy-host.unattended_password'
WHEN   呼叫 validateMemoryContent({ type, title, content })
...
## S4：bot-server.example 整段內容 regression
...
GIVEN  type = 'env'
       ...
       content 為 2026-05-24 對話中 AI 試著存的全文（含 AnyDesk ID、
              Tailscale IP、ssh.bot-server.example.vin.password 等密鑰名稱）
WHEN   呼叫 validateMemoryContent({ type, title, content })
THEN   回傳 ok = true
```
