...
- [x] Confirm project memories are absent from the context — `legacy host` appears 0 times in 18 KB
...
- [x] Re-run 「幫我看一下 legacy-server.example 那台的部署狀況」 → searches first, quotes the new line
...
- [x] Measured the reviewer's claim that the `[團隊] ` prefix breaks the search: it does not.
      `[團隊] 發布網頁到 pages.fontrip.com`, `公司 pages` and `legacy-server.example` all resolve
