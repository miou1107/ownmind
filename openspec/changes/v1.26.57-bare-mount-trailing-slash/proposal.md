...
`https://legacy-server.example/ownmind/dashboard` — the console URL without a trailing slash —
answers `301 Location: /dashboard/`. That is an absolute path, so the browser drops the
`/ownmind` prefix and lands on `https://legacy-server.example/dashboard/`, an **unrelated page**
titled "Vin WorkSpace" that has nothing to do with OwnMind. `/ownmind/admin` does the
same thing, to `https://legacy-server.example/admin/`.
...
rewritten `^/ownmind/(.*)` → `/$1` (`/etc/nginx/sites-enabled/legacy-server.example:54-55`), so
