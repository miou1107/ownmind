# Critical 鐵律卡控（提醒層升級到邏輯層）— 漸進推 v1.19.6 → v1.19.10

- **Author**: Vin
- **Date**: 2026-05-22（原版）/ 2026-05-22（依 Gemini 對抗審查重定 10 條候選）
- **Status**: 拍板執行中（v1.19.6 進行）
- **策略**: 拆成 v1.19.6 ~ v1.19.10 五個小版本、永遠停在 v1.19.x、不升大版號
- **原 v1.20 目錄保留作為「總藍圖」**：本檔即拍板版
...
別人的記憶系統：「我幫你記住事情」。
OwnMind v1.19.6+ 後的記憶系統：「我幫你記住規則、而且會擋下違反」。
...
這三條留在「警告層」、由現有 verification engine 處理、不上 v1.19.6+ 的硬擋。
...
## 4. 漸進切法（v1.19.6 → v1.19.10）

| 版本 | 範圍 | 原則 |
|---|---|---|
| **v1.19.6** | 共用判定核心 `hooks/lib/rule-enforcer.js` + 放行通道 `hooks/lib/bypass-handler.js` + 審計紀錄擴充 | 基礎建設、不擋任何規則 |
| **v1.19.7** | IR-041（隱私）+ IR-002（密碼進 commit）+ reply-lint 切擋下模式（IR-036 / IR-037） | 高誤判 + 已驗證有效的先上、邊用邊磨 |
| **v1.19.8** | 指令樣式類 5 條：IR-044 / IR-023 / IR-018 / IR-046 / IR-043 | 一波處理 PreToolUse 指令攔截 |
| **v1.19.9** | 靜態檢查收尾：IR-009 / IR-024 / IR-031 | 最後上零誤判 |
| **v1.19.10** | 兩週觀察期、根據誤判紀錄調規則 | 不新增功能、純調校 |
...
- ❌ 任何特定鐵律的 detect 邏輯（v1.19.7 起、IR-041 + IR-002 先上）
- ❌ Reply-lint 切擋下模式（v1.19.7 一起）
- ❌ PreToolUse 指令攔截（v1.19.8）
- ❌ Git pre-commit / pre-tag 整合到 rule-enforcer（v1.19.9）
- ❌ Admin UI Bypass 紀錄分頁（v1.19.10 觀察期結束後再評估）
...
✅ 既有 user 升級：v1.19.6 純基礎建設、不影響既有 hook、不需 migrate
...
| 風險 | 機率 | 影響 | 緩解 |
|---|---|---|---|
| Hook 誤擋、卡死工作流 | 中 | 高 | Bypass 機制 + 完整 audit log + v1.19.7~9 漸進上線、每版兩週觀察 |
...
> v1.19 給鐵律掛了等級標籤。v1.19.6 開始讓標籤真的有意義。
