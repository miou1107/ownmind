...
- [x] Deployed to `legacy-server.example` 2026-07-31. Production checkout is at
      `/srv/ownmind` — `/root/.ownmind/` I hit first is an unrelated
...
- [x] Post-deploy browser check on production (unauthenticated path, since I
      ...
      - `/ownmind/` and `/ownmind/dashboard` both land on the console
        (`https://legacy-server.example/ownmind/dashboard/login` — auth guard sending an
