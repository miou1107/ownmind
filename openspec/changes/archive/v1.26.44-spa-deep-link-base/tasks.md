...
- [x] Re-measured the "Filed, not fixed here" claim on production rather than
      ...
      `https://legacy-server.example/dashboard/` → 200. The claim holds; no user-visible break.
...
- [ ] `GET /ownmind/dashboard` (no trailing slash) 301s to `/dashboard/`, dropping
      ...
      path Express sees. On legacy-server.example the bare `/dashboard/` is also proxied, so it
