...
AI 試著存 type=`env` 的記憶「bot-server.example 遠端訪問方式總覽」、內容含：

- AnyDesk 連線編號 `1901091212`（公開資訊、不是密碼）
- Tailscale 私網位址 `100.72.72.60`（白話：公司內網才看得到的虛擬位址）
- 密鑰**名稱**字串 `anydesk.bot_legacy-host.unattended_password`（白話：「鑰匙的名字」而不是鑰匙本身、真鑰匙在 OwnMind 密鑰管理工具裡）
...
| Title／description keyword | 沒命中（title 就「bot-server.example 遠端訪問方式總覽」、不含密碼字樣） |
| **Value keyword 掃描** | **命中 `keyword:password`** — value 裡的 `anydesk.bot_legacy-host.unattended_password` 含 `password` 字樣 |
...
讓 AI 看到「`anydesk.bot_legacy-host.unattended_password` 觸發」就能判斷「這是 reference 不是密碼、改寫法」、第一次就改對。
