...
- [x] ④ Internal ecosystem names neutralized in openspec (FUNIT→ExampleClient,
      RING/company host/ima→"other internal projects", whitelist example list genericized,
...
- [x] ① remote-access incident data scrubbed across openspec + tests + shared +
      FILELIST + CHANGELOG: bot-server.example→bot.example.com, app.legacy-server.example→
      app.example.com, AnyDesk ID 1901091212→123456789, Tailscale 100.72.72.60→
      100.64.0.1, bot_legacy-host→bot_example, bot-legacy host→bot-example. Behavioral test
      fixtures only; npm test stays 2012/0/0. Public endpoint legacy-server.example NOT touched.
- [ ] DEFERRED (Vin still deciding):
      ② `legacy-server.example` bare public install endpoint (README / .mcp.json) — it is the
