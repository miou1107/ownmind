...
### 搬到公司 FAPA 環境的說明書與搬資料腳本

伺服器要從 kkvin 換到公司的測試機與正式機。這一版還沒有任何使用者看得到的改動，
進來的是兩件人手做不好的事。
...
另外有一份給人看的說明書 `docs/deploy-to-fapa.html`，兩個讀者一份文件：
...
| FAPA onboarding | 7 個自創標籤 | 沒有對應動作 |
...
「我沒有 kkvin.com 的資訊」，被叫去查之後立刻就找到了。
...
| `公司 pages` | **#869** |
| `kkvin.com` | #653、**#745**（那台伺服器）、#702、#632 |
...
- 「幫我看一下 kkvin.com 那台的部署狀況」→ 第一個動作就是 `ownmind_search("kkvin.com")`；
