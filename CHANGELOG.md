...
- 把先前「記錄真名↔代稱對照」的三處文件（v1.26.21 CHANGELOG 條目、legacy host 設計稿、v1.26.21 tasks）改寫成不揭露對照關係——**代稱對照表不寫進任何 commit 檔**，否則讀者可反推、化名形同白做。
- 保留 `Vin`（公開代號）。

**刻意未做**：`shared/language-lint.js` 的內部專案代號白名單（功能性 allowlist、低曝險）、本機路徑與 `.mcp.json` 主機（屬 `legacy-server.example` 設定化那版、`.mcp.json` 是功能設定）。
...
- 遠端存取事故樣本資料中性化（主機名 `bot-server.example`→`bot.example.com`、AnyDesk/Tailscale 等假值）+ FUNIT 1Password vault 樣本（`FUNIT-prod`/`wp-vin`→泛稱），橫跨 openspec + `tests/memory-secret-guard`、`tests/secret-detect-unit` + `shared/secret-detect.js`，皆為行為不變的測試樣本。

**v1.26.21 當下未做、留待後續「開源前全倉去品牌化」**：散在約 40 個檔（tests/、src/、shared/、hooks/、scripts/）的回報者單名註解、`shared/language-lint.js` 專案名白名單、`llm-narrative.js` 提示詞人名範例（→ 人名部分已於 v1.26.22 完成）；以及公開安裝網址 `legacy-server.example` 設定化（已有設計稿 `docs/superpowers/specs/2026-05-29-legacy host-config-extraction-design.md`，仍待辦）。
...
- prod 部署到 https://legacy-server.example/ownmind/dashboard/ 後手動 smoke test `document.baseURI` 拿到預期值（reviewer 建議、basename 動態偵測理論上 nginx reverse proxy 不會干擾 baseURI、但實測 30 秒 100% 安心）
...
- 線上 `/ownmind/dashboard/login` → baseURI `https://legacy-server.example/ownmind/dashboard/` → basename `/ownmind/dashboard`
...
v1.20.1 prod deploy 後 Claude in Chrome 線上實測登入立即踩到：填表單按登入後沒反應、停留在 /login 頁、無錯誤訊息。直接 curl `https://legacy-server.example/ownmind/api/me/login` 成功拿到 api_key、確認後端正常 → 前端 fetch URL 錯。

原因：`client/src/api/client.js` 直接 `fetch('/api/me/login')`、瀏覽器解析成 `https://legacy-server.example/api/me/login`（同 host 絕對路徑）、不會自動帶 nginx 反向代理的 `/ownmind/` 前綴。線上 endpoint 在 `/ownmind/api/...`、所以打到不存在的 URL、靜默失敗。本機 docker 部署在 `/dashboard/`（無 /ownmind 前綴）/api 同 host root 就 work、沒踩到。

修：跟 main.jsx basename 動態偵測同源邏輯、`client/src/api/client.js` 新增 `API_BASE` 從 `document.baseURI` 抽 dashboard 之前的 pathname、`resolveUrl()` helper 對 `/api/...` 開頭的 path 自動加前綴。線上 baseURI `https://legacy-server.example/ownmind/dashboard/...` → API_BASE = `/ownmind`、fetch 拼成 `/ownmind/api/me/login`。本機 docker baseURI `http://localhost:3100/dashboard/...` → API_BASE = ''、行為不變。
...
- `client/src/scripts/translate.mjs`：增量翻譯腳本、支援 OpenAI 相容 API（legacy-server.example llm-switch / OpenAI / Anthropic）、`temperature=0` 降低隨機性、hash 比對未變動就跳過
...
**P0 緊急修補。** v1.19.14/15 部署後 Vin 從瀏覽器打開 `https://legacy-server.example/ownmind/admin/`、輸入帳密按登入沒反應。打開 DevTools 看 console 顯示：
...
3. **白名單從 80 詞擴到 200+ 詞**（基於 30 天 audit Top 30 真實違規詞）
   ...
   - Vin 個人專案名：adog、company host、fontrip、ring、ownmind、auto、speech、ima、funit 等
...
**新增鐵律 IR-048：** deploy OwnMind / RING / 任何 server 前必須跑 db/ 下所有未套用 migration（人工版本、適用所有專案）。本版 OwnMind 自動化後、IR-048 在 OwnMind 場景降為 advisory、但對 RING / company host / ima 等專案仍是 default 強度。
...
**升級指引**:
1. SSH prod: `ssh root@legacy-server.example`
...
**升級指引**（**這版部署 server**）：
1. SSH 到 prod：`ssh root@legacy-server.example`
2. `cd /srv/ownmind && git pull origin main`
...
**背景**：Vin 回報 `https://legacy-server.example/ownmind/me`（沒尾斜線）連不上、必須打 `/ownmind/me/`。Express `app.use('/me', express.static(...))` 對 mount path 本身不自動補 trailing slash — `/me` 直接 404、`/me/` 才 hit index.html。
...
**為什麼相對路徑**：用 `'me/'` 而非 `'/me/'`。legacy-server.example 跑 nginx 反代到 `/ownmind`、Express 內部 URL 看不到 `/ownmind` prefix。如果用絕對 `/me/`、Location header 會是 `/me/`、user browser 拼到 origin 變成 `https://legacy-server.example/me/`（沒 prefix）。用相對 `me/` 對當前 URL `/ownmind/me` 來說、browser 拼出 `/ownmind/me/`。
...
[ OK ]  server_health        https://legacy-server.example/ownmind/health -> 200
...
- 新表 `install_check_logs`。Deploy 時要手動跑 migration（OwnMind 沒有 auto-migration runner）：
  ```
  ssh root@legacy-server.example "docker exec -i ownmind-db psql -U ownmind -d ownmind" < db/011_install_check_logs.sql
  ```
...
- 走 llm-switch（OpenAI-compatible）`https://legacy-server.example/llm-switch/v1`，`model='auto'`，`response_format=json_object`
...
但 nginx 只 rewrite `/ownmind/...` → `/...`。前端打 `https://legacy-server.example/api/me/login`
...
1. 開 https://legacy-server.example/ownmind/me/
...
Alice / Dana / Bob（user role）現在可以開 https://legacy-server.example/ownmind/me/，
...
iwr -useb https://legacy-server.example/ownmind/bootstrap.ps1 | iex
```
...
```bash
curl -fsSL https://legacy-server.example/ownmind/bootstrap.sh | bash
```

Mac / Linux / Git Bash（**首次安裝**，要提供 API key + URL）：
```bash
curl -fsSL https://legacy-server.example/ownmind/bootstrap.sh | bash -s -- YOUR_API_KEY YOUR_API_URL
```
...
```powershell
$env:OWNMIND_API_KEY='YOUR_API_KEY'; $env:OWNMIND_API_URL='YOUR_API_URL'; iwr -useb https://legacy-server.example/ownmind/bootstrap.ps1 | iex
```
...
### 核心功能
- API Server（Node.js + Express）上線，部署於 legacy-server.example
...
- 預設 API URL 指向 https://legacy-server.example/ownmind
...
### 部署
- Docker Compose 部署
- nginx reverse proxy（https://legacy-server.example/ownmind/）
