...
## v1.30.24 — 伺服器搬家，改成由伺服器自己告訴每台電腦

你不用做任何事，下次自動更新就會換過去。

上一版是把新位置寫死在程式裡。這版拿掉了：更新的時候，你的電腦會問現在連的那台伺服器
「我該連去哪」，伺服器回一個位置才跟著換，沒回就什麼都不動。自己架 OwnMind 的人不受影響，
因為那台伺服器不會叫任何人搬家。

這樣改有兩個好處。程式庫裡不會留下任何一台機器的網域，這個庫是公開的。以後再搬一次，
也不用再改程式、再發一版。

伺服器那邊怎麼開：在環境設定裡填 `CANONICAL_URL`，填了才會告訴客戶端；沒填就是沿用現狀。

一次性的搬家腳本跟它的說明書一起刪掉了，那次搬家已經做完。

### 自動更新學會了一件事：遠端歷史被重寫之後自己接回去

把不該進版本庫的東西（金鑰、內部主機名稱）事後清掉，代價是每台已經安裝的機器手上那份
歷史跟遠端對不上。原本這會讓自動更新每一輪都失敗，而且沒有人坐在那些機器前面，症狀就是
更新安靜地停住。

現在更新程式會先確認兩邊真的一個共同提交都沒有——那才是重寫，不是一般的衝突——才把這份
複本重新對齊到遠端，並且留一筆紀錄。還有共同提交的時候什麼都不動。

## v1.30.23 — 伺服器搬家了，你的電腦會自己改網址

你不用做任何事，下次自動更新就會換過去。

OwnMind 的伺服器從舊主機搬到公司的正式機了。舊網址現在還通，是因為舊主機前面有一段轉送
把請求接到新機器；那段轉送只是一個檔案裡的一行設定，一旦有人動到它或那台舊主機關掉，
所有人的 OwnMind 就會一起斷。

伺服器碰不到你的電腦，網址寫在你自己機器的設定檔裡，只有你這台改得動。所以改成由更新程式
來做：它看到還寫著舊網址就換成新的，換完會在畫面上講一句。你自己架的 OwnMind 不會被動到，
它只認得要退休的那一個網址。

## v1.30.22 — 搜不到不代表沒有：連不上的時候，搜尋不再安靜回「0 筆」

你不用做任何事，升級之後就會拿到。

一段對話開久了，中間電腦睡過一次，OwnMind 就可能再也連不上伺服器。寫東西進去的時候看得
出來，它會直接報錯；搜尋看不出來。搜尋會安靜改讀本機那份舊資料，回你「0 筆」，而這句話
跟「你從來沒存過這筆」在畫面上長得一模一樣。

9 月 14 日真的這樣過了一輪。AI 搜一台主機的設定放在哪，回 0 筆，那筆記憶好好地在伺服器
上，AI 差一點就跟人說你沒存過。

原因在搜尋同時問兩個地方：你存的記憶，還有你的工作紀錄。兩邊的錯誤都被當成「查不到結果」
吞掉了，所以錯誤從來沒有浮上來，而準備好的那句離線說明是掛在錯誤後面的，於是也永遠不會
出現。

### 現在連不上會看到什麼

搜尋會先自己再試一次，因為睡醒之後第一次連線失敗、第二次常常就通了。兩次都失敗，結果會
講明這批是本機的舊資料、舊到什麼時候；這台機器上連那份舊資料都沒有的話，它也會直接說沒有，
不會讓你以為查過了。要是接下來每次搜尋都這樣，那就是這個對話的連線接不回去了，開一個新的
對話就會恢復。

一半連得上一半連不上的時候，連得上那半的結果照樣給你，旁邊註明少了哪一半、為什麼少。
兩邊都因為別的原因失敗，你會看到錯誤本身，不會看到一個乾淨的 0。

## v1.30.21 — 「今天」在這套程式裡有兩個答案，這一版把它們對齊

你不用做任何事，升級之後就會拿到。

事情從 CI 全紅開始。有一個測試把 2026-08-06 寫死在裡面，再拿執行當下的真實時間去比，而
門檻是 30 天。9 月 5 日一過，這個測試就注定失敗，之後每一個送上去的 PR 都是紅的。
Dependabot 那筆 hono 安全更新剛好排在那之後，只動了一個版本號，看起來卻像是它弄壞了測試。

把時鐘固定住之後才發現，同一種毛病在別的地方也還在。

### 記錄檔用 UTC 命名，台灣時間每天早上八點以前寫進昨天

`hooks/lib/hook-context-fetch.js` 在伺服器認不得新端點的時候會留一筆紀錄，檔名取的是 UTC
的日期。台灣是 UTC+8，所以本地凌晨到早上八點之間寫的每一筆都落在昨天那個檔案裡，而同一個
資料夾裡別的程式寫的是今天。這正是 `shared/local-date.js` 當初為了終結而寫的那個分裂，
發生在它點名的那個資料夾。

守衛測試本來就是為這件事寫的，它沒攔到，原因是名單裡沒有這個檔案，而這個檔案在名單寫好
之後才加進來。清單不會告訴你它漏了什麼。名單補齊了，守衛本身也補強：它原本會把網址裡的
雙斜線當成註解開頭，把那一行後面整段吃掉，而這支程式整支都在處理網址。

### 兩個測試每天有八小時什麼都沒檢查，卻回報通過

一個測試拿 UTC 日期去找檔案，找不到就跳過它唯一那句斷言。寫檔那邊用的是本地日期，所以每天
前八小時，它找的是一個從來不存在的檔案。現在兩邊用同一個算法，找不到檔案就算失敗，而且
錯誤訊息會把「想找哪個檔案」跟「資料夾裡實際有什麼」一起印出來，因為少了那份對照，
日期對不上會長得跟檔案不見一模一樣。

同一個檔案裡還有一個測試，名字寫的是兩個檔案都不該被寫出來，實際只檢查了其中一個。另一個
測試算了一個 UTC 日期放著沒人用，就擺在一個記錄檔資料夾旁邊，這次一起清掉。

### 兩批安全通報，一批真的關掉，一批本來就打不到

hono 那三則合併之後關掉。js-yaml 那則是高風險，而且掛在直接相依的套件上，所以查得比較
仔細：讀鐵律檔頭的程式指定用 `JSON_SCHEMA` 載入，那個 schema 裡沒有 merge、也沒有 omap，
通報要繞過的計數器根本不會被叫到，拿 CVE-2026-84375 的攻擊寫法實測，0.8 毫秒就被「重複的
鍵」擋掉。

| 動到什麼 | 從 | 到 |
|---|---|---|
| js-yaml | 4.3.1 | 4.3.2 |
| update.sh 與 update.ps1 的門檻 | 4.3.1 | 4.3.2 |
| dep-floor-guard 綁在通報上的那條測試 | 4.3.0 | 4.3.2 |

門檻要跟著升，升版才有用。只改 package.json 的話，已經裝好的機器不會再碰那個套件，會一直
停在舊版本；4.1.1 那次就是這樣拖了很久。那條測試是整份檔案裡唯一綁在特定通報上的，上一次
升到 4.3.1 沒跟著拉，所以把 package.json 改回 4.3.0 也照樣全綠。

## v1.30.20 — 清掉三個套件的安全通報，都不在跑得到的路徑上

這批不用急著發版。三個套件確實在相依樹裡，但沒有一個是服務跑起來會呼叫到的。GitHub 報的
8 個（4 個高風險、4 個中風險）全部修完，另外自己掃出一個 GitHub 還沒開單的也一起清掉；
只動 lockfile，四份 `package.json` 的相依套件宣告一個字都沒改。

| 套件 | 在哪 | 問題 | 打不打得到 |
|---|---|---|---|
| `fast-uri` 3.1.5→3.1.7 | MCP 那支的 `ajv` 底下 | 連接埠沒驗證就組回網址、IP 的中括號不成對造成主機名混淆 | 不會。只用在 JSON schema 的 `$ref` 解析，而 MCP 走本機管道、沒有對外開埠 |
| `qs` 6.15→6.16 | 伺服器的 `express` 底下 | 中括號鍵夾逗號可以繞過陣列數量上限、`isBuffer` 被動過手腳的輸入拖住 | 不會。qs 有兩條入口。網址參數那條，Express 5 預設走 Node 內建的 `querystring`，這個服務沒有改掉；表單那條要自己掛 `express.urlencoded()` 才會有，整個 repo 一處都沒掛 |
| `nanoid` 3.3.16→3.3.18 | 後台前端的 `postcss` 底下 | 自訂產生器碰到長度 0 會空轉 | 不會。只有打包後台畫面的時候跑得到，送到瀏覽器的檔案裡沒有它 |

前兩個是 dependabot 自己開的 PR，fast-uri 是 #116、qs 是 #118，三個平台七項檢查全綠。
nanoid 那個 GitHub 還沒開單、`npm audit` 掃得到，這次一起升上去，升完重打包一次，出來的
檔案跟原本一模一樣。qs 那次順帶把 `hasown`、`side-channel`、`side-channel-list`、
`es-object-atoms` 也升到新版；根目錄 lockfile 自己那行版號本來還停在 1.30.4，這次跟著
版號一起對回來。

## v1.30.19 — 中文使用者名稱的 Windows 被寫壞設定檔、查版本被誤判、修好了沒人通知

### 使用者名稱有中文的 Windows，裝一次就可能把設定檔寫壞

安裝程式會把 OwnMind 寫進 `~/.claude/settings.json`：整份讀進來、加上自己那一段、再整份
寫回去。Windows PowerShell 5.1 讀不帶 BOM 的檔案時照系統的 ANSI 代碼頁解碼，繁體中文的
機器是 cp950，所以檔案裡的中文在讀進來那一刻就已經變成亂碼，寫回去等於把原本好好的內容
換掉。

會不會踩到，看那個檔案裡有沒有中文。安裝程式自己就會放一段進去：MCP 的啟動路徑帶著
Windows 使用者名稱，名稱是中文就中。別的工具寫進同一個檔的內容也算。

原本三處讀檔都沒有指定編碼，交給機器的代碼頁決定；現在補上 `-Encoding UTF8`，讀進來的就是
檔案本來的樣子。升級腳本跟版本顯示那四處一起補，它們只讀不寫，最壞的情況是版本顯示不出來、
或升級回報漏送。另外兩處記路徑用的小快取也一起補：路徑裡有中文就讀不回來，之後每次都得重新
找一遍 node 跟 Git Bash。

已經被改壞的檔案這次救不回來。裝完發現 `~/.claude/settings.json` 裡有亂碼，那一段要自己改
回來，或是把 ownmind 那一段刪掉重裝一次。

上一批修的 bug-report 30 是同一個毛病的唯讀那一半：Windows 上查版本被誤判成需要升級。

開發的這台機器沒有 PowerShell，所以新增的測試是掃腳本文字，誰再寫出沒指定編碼的整檔讀取
就會紅。至於在 cp950 的 Windows 上實際裝一次，沒有驗過。
### Windows 上查版本，一直被告知要升級

Windows 的人查 OwnMind 版本，答案是「需要升級」。照著跑完一輪升級才發現本來就是最新版，
下次查還是同一句話。

查版本會看四層。前三層都答已是最新，第四層檢查掛勾實際讀的那份規範快取，它回報讀不到，
整份結果就被拉成需要升級。

那份快取是不帶 BOM 的 UTF-8 檔。Windows PowerShell 5.1 讀不帶 BOM 的檔案，會照系統的
ANSI 代碼頁解碼，繁體中文的機器是 cp950，所以 2026-09 新增的兩條記憶帶進中文標籤之後，
中文被解成亂碼，其中一段吃掉了字串的結束引號，JSON 解析當場失敗。掛勾自己走 Node 讀同一個
檔完全正常，41 條規範一條都沒少，壞的只有這支檢查腳本。

原本用 PowerShell 的 `Get-Content` 讀，編碼交給機器的代碼頁決定；現在三處讀 JSON 的地方
都走同一支 `Read-Utf8Text`，固定以 UTF-8 讀進來，遇到別人留下的 BOM 也照樣認得。這支
helper 自己開檔而不是用現成的 `ReadAllText`，因為 `ReadAllText` 不允許別人同時寫入，而規範
快取正是在 session 開場同步時被重寫的，撞上就會變成同一句「讀不到」。mac 與 Linux 的
`check-sync.sh` 走 `cat`，本來就沒有這個問題。

開發的這台機器沒有 PowerShell 可以實際跑一次，所以新增的測試改成掃腳本文字：誰再把
`Get-Content` 接回 `ConvertFrom-Json`，測試就會紅。

回報者：bug-report 30。

### 報告修好了，只有 Windows 的人會知道
...
`EnterWorktree` 在這台機器的每個 repo 都開不起來，錯誤訊息是「hook succeeded but returned
no worktree path」。同一個機制 7/17、7/30 已經回報過三次。

OwnMind 原本註冊了一個 `WorktreeCreate` hook，只想在新 worktree 裡放一份 `.mcp.json`。
但那個事件不是通知：Claude Code 把「建立 worktree」整件事交給註冊在上面的 hook，等它把新
路徑印到 stdout。OwnMind 那支只做事、不印路徑，所以 Claude Code 認定建立失敗，連原本用
git 建的那條路都不走了。

放 `.mcp.json` 這件事本來也不需要。每支安裝腳本都會把 ownmind 這台 MCP 伺服器註冊到
`~/.claude.json` 的使用者層級，而且註冊完會讀回來確認；使用者層級本來就涵蓋每一個目錄。

所以整個註冊拿掉，`hooks/ownmind-worktree-setup.sh` 也刪掉。升級的時候會主動把舊的註冊從
`~/.claude/settings.json` 移掉：會踩到的人全都是已經裝好的人，只停止新增等於一個都沒修到。
別人自己註冊在同一個事件上的 hook 不動。

### Google 地圖的地點代碼被當成金鑰，提交被擋

一份設計文件引了三個 Google Maps Place ID 當佐證資料，commit 直接被擋下來。Place ID 是
27 個字的隨機字串、沒有單字結構，所以最後那道長度啟發式把它讀成金鑰。它其實是公開識別碼，
誰拿到都只查得到一個地點，遮掉反而讓人沒辦法複查。

擋下來的訊息也沒說有別條路可以走。放行用的環境變數一直都在，只是從來沒印出來，所以看得到
的出口只剩 `--no-verify`，而那會把這次 commit 的每一項檢查一起關掉，也不留紀錄。

兩邊都補：

- `ChIJ` 開頭、只含 base64url 字元的字串列為公開識別碼，只放行長度啟發式那一條。真金鑰
  格式各自有專屬規則、排在前面，照樣擋
- 因為疑似金鑰而擋下來的訊息，現在會寫出這次要放行用哪些代碼，並且說明 `--no-verify`
  的代價。代碼一定含 BASELINE：自訂規則關掉之後，底下的預設掃描還是會攔同一筆，只寫規則
  代碼的話，照著做還是會再被擋一次

### 「這台機器沒有規範」的警告每一輪都重講一次

一整個工作階段，每一則訊息前面都掛著同一段警告。連跟規範無關的也照掛。回報的人查了兩次：
`ownmind_search` 抓得回團隊規範的全文，`check-sync.sh` 三層全報 in_sync。兩邊都跟警告對不
起來，於是他判定警告是舊的，開始略過。

那句話其實是真的，只是沒有人查得到。掛勾讀的是 `~/.ownmind/cache/enforcement.json`，而
`ownmind_search` 直接問伺服器、`check-sync.sh` 比的是版號跟部署檔，兩者都不碰這個檔。所以
原本一個查得到的人都沒有，現在三個地方一起改：

- 警告寫出缺的是哪一個檔，並且點名那兩個工具為什麼會給出不一樣的答案
- 一個工作階段只講一次。使用者那一側沒有變安靜：回話檢查的橫幅照舊在狀態變化時、以及狀態
  沒變的每十輪講一次
- `check-sync.sh` 與 `check-sync.ps1` 多一層 `L4_STANDARDS`，讀的就是掛勾讀的那個檔。
  沒有這份快取的機器，整體結論不會再回 in_sync

## v1.30.17 — 排程跑的更新在 mac 上停在 npm，程式碼換了、掛勾沒換
...
那條規則就是「不要中英夾雜」。產品違反了它正在回報的規則。

v1.21.0 把檢查搬進 `shared/validators/`，每支驗證器自己把英文句子寫死；字典裡的
`lint.banner.violationLine` 只是「  ・{規則}: {訊息}」這個外框，它照單全收。所以每一種
語言都是翻好的外框包一句沒翻的英文。被它取代掉的舊路徑（`shared/language-lint.js`）
一直都有中文 —— 這是回退，卻長得像一個從來沒做過的功能。

三支驗證器現在都交出字典鍵和參數，英文句子原封不動留在稽核紀錄裡（伺服器和模型讀那個，
按規定是英文）。字典少一個鍵不會退回英文，`t()` 會把鍵本身印給使用者，所以新的測試盯著
三件事：每支驗證器都有鍵、三種語言都有那個鍵、英文算出來跟稽核紀錄那句一模一樣。

**同一次自我檢查找到的第二個問題：安裝腳本把 npm 的錯誤丟掉了。**
`install.sh` 跑 `npm install -q 2>/dev/null`，位置就在它自己那句「stderr 一律不准丟到
/dev/null」的註解上面 25 行。原因是那份日誌檔在那一行還沒被建立 —— 建立它的區塊排在
下面。代理伺服器拒絕、node_modules 權限不足、套件庫掛掉、鎖定檔衝突，四種都被壓成
同一句「試試看 npm install -g npm@latest」，四種都不是那句話能解決的。日誌區塊往上搬，
npm 的輸出寫進去，失敗時印最後 20 行。

現有那道「安裝腳本不准丟掉 Node 的錯誤訊息」的檢查只看 `node -e` 那種寫法，`npm install`
不屬於它，所以它從頭到尾沒話說。這次把 npm 也納進同一道檢查。

送審時抓到這個修法自己的問題：日誌檔建不起來時，`INSTALL_LOG` 會退回 `/dev/stderr`。
那個值寫得進去、讀起來卻是錯的 —— 在終端機上 `-s` 判定為空，腳本會宣稱 npm 什麼都沒吐，
而錯誤就在畫面上；把輸出導進檔案跑的話 `-s` 為真，`tail` 會把整份腳本的最後 20 行當成
npm 的錯誤印出來。更糟的是實測發現 Git Bash 下 `>>/dev/stderr` 直接失敗，重導向失敗等於
npm 根本沒執行 —— 為了保住 npm 診斷訊息而做的那一步，反而讓 npm 沒機會產生診斷訊息。
現在寫不進日誌就不重導向，npm 直接印在畫面上，訊息也照實說「錯誤在上面」。

### 兩組真的密碼被當成測試樣本寫在程式碼裡

密鑰偵測器的測試樣本用的是真的網站後台密碼，兩組、散在八個檔案。
這個倉庫是公開的，所以那兩組密碼跟著公開了一段時間。

第二組是拿產品自己的偵測器把每個進版控的檔案掃過一遍才找到的。第一組是靠
人眼追字串，那一輪只找到它自己 —— 手寫的清單不會報錯，它只會安靜地漏。
掃出來的 19 組疑似目標裡，只有這兩組是真的，其餘本來就是假樣本。

已經換成假的樣本，密碼本身也換過了。樣本挑的形狀跟真的一樣（六組四碼、
其中幾組帶數字），所以偵測器該抓到的還是抓得到，相關的 163 條測試全過。

樣本在檔案裡是用陣列接起來的，不寫成一整串。這個倉庫就是偵測器的家，
一段完整的鑰匙形狀寫在它自己的原始碼裡，會被它自己的送出前檢查擋下來。

**同類的東西以後不要用真的。** 偵測器的測試需要的是「長得像」，不是「就是」。

### 搬到公司 company host 環境的說明書與搬資料腳本

伺服器要從 legacy host 換到公司的測試機與正式機。這一版還沒有任何使用者看得到的改動，
進來的是兩件人手做不好的事。

搬資料的那一趟寫成腳本了，`check` 先看兩邊狀況、`cutover` 真的搬、`rollback` 退回。
它在自己電腦上跑、自己連兩台機器。來源那一步是把服務整個停掉而不是切唯讀，
因為 OwnMind 沒有唯讀模式，邊寫邊匯出會靜靜漏掉幾筆，而那幾筆要好幾週後才有人發現。
匯進去之前先重建結構、匯完逐表比對筆數，對不上就中止，不會留下一個半套的資料庫。

加密金鑰也由腳本一起帶過去。使用者存放的密碼是用舊機器上那把鎖的，只搬資料不搬金鑰，
記憶會完好回來、存的密碼卻全部解不開，而且沒有任何一步會報錯。腳本在還原之後、
服務起來之前把金鑰寫進新機器的設定檔，寫完比對指紋，對不上就中止。設定檔是軟連結，
所以是改連結指到的那個檔，不是把連結蓋成一般檔案。

另外有一份給人看的說明書 `docs/deploy-to-company host.html`，兩個讀者一份文件：
前段是全貌，中段是伺服器規格。裡面附了兩台機器的設定檔全文、網站要加的那段轉送設定，
以及設定檔的軟連結要落在 compose 專案目錄下、名字得叫 `.env` 這個地雷。

## v1.30.14 — 一條測試綁著今天的日期，時間到了就自己壞掉

自檢時發現的：`tests/selfcheck-roundtrip-weekly.test.js` 從 2026-08-17 開始，
每一次跑都會失敗一條。程式沒壞，**是那條測試綁到了真正的今天**。

那組測試講的是「上傳回讀這個檢查每 7 天才跑一次」。測試把日期凍在 2026-08-11，
照這個日期寫下標記檔；可是被測的 `checkNamesFor` 沒有時鐘可以餵，它只能去看真正的今天。
兩個日期一開始差得夠近，測試就過；等真實日期走到剛好差滿 7 天，它就開始天天紅。

修法放在程式這一邊，不是測試那一邊：這個判斷本來就是在問一個時鐘，
那時鐘就該是一個可以餵的參數 —— 旁邊的 `roundtripDue` 從第一天就是這樣寫的。

會綁到日期的測試不是測試。現在多一條測試專門走「不餵時鐘」的預設路徑，
用一個久到不可能過期的日期，所以它不會再用同樣的方式爛掉。

## v1.30.13 — 廣播只送得出前 5 行，其餘無聲丟掉

你在後台發一則公告給團隊，收到的人只會看到前 5 行、而且換行被接成一整句，
再從第 400 個字砍掉。**砍完不會說。** 你不知道少了，收的人也不知道少了。

實際上是這一行同時做三件會掉東西的事：只留 5 行、換行變空格、砍到 400 字。

現在整篇送出去、換行保住。真的太長被砍的話，會明講砍了幾行、幾個字，
就跟旁邊「還有 N 則公告沒顯示」一樣的講法。

上限訂成 2000 字，跟伺服器「超過 2000 字就不收」同一個數字。
兩邊一樣的意思是：**伺服器收得下的公告，收到的人一個字都不會少。**
兩個上限不一樣正是這個問題的成因：寫的人在一個上限裡面，另一個更小的上限安靜地砍掉其餘。
現在有測試會自己去讀伺服器那個數字，兩邊不准再各走各的。

## v1.30.12 — 回話檢查在 Windows 上一次都沒跑成功過，而它回報的理由是假的

上一版把「檢查 AI 回話」搬到你自己的電腦上跑，用 `claude` 這個指令。
在 Windows 上，那個指令**從來沒有啟動成功過**，一次都沒有。

### 症狀：它說「這台機器沒有裝 claude」，但明明裝了

Windows 10、node v25.8.1、用 npm 裝的 Claude Code，實測兩條路都死：

```
spawn('claude')      → ENOENT   node 不會自己去找 .cmd 這個副檔名，
                                 而旁邊那個沒有副檔名的 claude 是 shell 腳本，Windows 執行不了
spawn('claude.cmd')  → EINVAL   node 拒絕在沒有 shell 的情況下啟動 .cmd
                                 （這是 CVE-2024-27980 的修補，不是 bug）
```

ENOENT 這個錯誤被歸類成「機器上沒有這個程式」，所以每一輪檢查都回報
`claude 不在這台機器上` —— 對著一台 `C:\Users\…\AppData\Roaming\npm\claude.cmd`
明明就在那裡的電腦。

**錯的診斷比沒有診斷更糟**：它會叫人去裝一個根本沒缺的東西。
今天那份稽核文件裡「O4：回話檢查最近失敗率變高，原因不明」，原因就是這個。

### 為什麼不用 `shell: true`（一般人的第一直覺）

判官的參數裡帶著 `--system-prompt`，而那是一段多行的長文字。
`shell: true` 會把整串參數交給 `cmd.exe /d /s /c`，而且**不做任何跳脫**——
文字裡的引號、`%變數%`、`^` 全部會被 cmd 重新解讀一次。
用弄壞內容的方式修好啟動，不算修好。

### 改成怎麼做

新的 `hooks/lib/resolve-claude-bin.js`：找出真正的執行檔，
如果找到的是 npm 產生的 `.cmd` 轉接檔，就讀出它背後真正要跑的東西，直接跑那個。
全程沒有 shell，所以參數一個位元組都不會變形。

Windows 以外的系統原樣放行，一條程式碼路徑走天下。

### 一個教訓，寫下來因為它差點就過關了

第一版改完，**13 個測試全綠**，然後對真正的 CLI 一跑就爆。
原因：第一版只會認 `.js` 目標，而 Claude Code 現在的轉接檔指向的是 `.exe`。

第二個坑同一天出現：npm 的轉接檔裡有一行 `IF EXIST "%dp0%\node.exe"`，
整份檔案掃過去的話，會把那個「沒被選到的分支」裡的直譯器當成目標。
所以現在只讀帶著 `%*` 的那一行 —— 也就是轉接檔真正拿去執行的那一行。

兩個都有測試釘住。真機驗證：解析到
`…\node_modules\@anthropic-ai\claude-code\bin\claude.exe`，啟動成功，回報 2.1.218。

### 順帶：多一種失敗理由

「裝了但這裡啟動不了」（`bad-cli-shape`）跟「根本沒裝」（`no-cli`）現在是兩句不同的話。
它們的修法不一樣，混在一起講就是把人送到錯的地方。

### 同一版還修了：時間上限訂太短，Mac 上每一輪都逾時

Windows 是一次都沒跑成功；Mac 是跑了但每次都來不及。

上限訂在 90 秒，那個數字是拿「1 條規矩」量出來的。實際上正式帳號一輪會挑出 9 條規矩、
提示 12,349 字，判斷要跑 **150 秒**。所以發版當天第一輪就逾時，之後每一輪都是。

上限改成 300 秒，「判斷沒回來」的門檻從 180 秒改成 420 秒，後面這個數字不改不行。
不改的話，一個跑滿上限的判斷會在還在跑的時候被當成死掉的，結果被丟掉。

真正缺的不是更大的數字，是一道檢查。這兩個數字住在不同檔案、沒有東西把它們綁在一起，
所以它們往同一個危險的方向各走各的。現在有測試盯著：門檻必須大於判斷上限加上網路跟啟動時間。

### 三種失敗，以前都給你同一句沒用的話

「重跑一次 OwnMind 的更新指令」。那支指令裝的是 OwnMind，它碰不到 Claude Code，
也不會讓判斷變快。三種情況本來都掉進這句話：

| 實際狀況 | 現在跟你講 |
|---|---|
| 有 Claude Code，但 OwnMind 叫不動它 | 你自己在終端機打一次 `claude` 試試看 |
| 檢查超過時間上限 | 跟我說一聲，那是上限訂得不夠 |
| 看不懂檢查回來的答案 | 你不用做什麼，下一段回話會重新檢查 |

抓到後面兩個是因為寫了一條會自己去找的檢查：它掃過判斷程式會產生哪幾種失敗，
逐一確認每一種都有自己的句子。手寫清單不會抓到，因為漏掉的人跟補清單的是同一個。

## v1.30.11 — 回話檢查改用你自己的額度，而且不再「其實沒檢查」卻裝得像檢查過

這一版做兩件事。一件是把「檢查 AI 回話」搬到你自己的 Claude Code 訂閱上跑；
另一件是把搬過去之後冒出來的十一種「沒檢查卻看不出來」全部堵掉。

### 一、判斷 AI 回話的模型，改用你自己的訂閱

以前是伺服器叫一個外部模型來判，有兩個問題。一是你交代過「任何要叫模型判斷的功能，
一律用使用者自己訂閱的額度」；二是那條路本來就不太會動，實測 1006 次檢查裡失敗 474 次。

現在改成：伺服器只負責挑「這一輪該對哪幾條規矩」，判斷本身在你的電腦上，用 `claude` 跑。

**代價要講清楚：它不再有能力把 AI 的回話擋下來重寫。**
判斷一次要 29～54 秒，沒有人願意每講完一句話等半分鐘，所以改成 AI 講完就丟到背景跑、
下一輪才把結果撿給你看。那時候你早就讀過那段回話了。
所以抓到違規的那句話從綠燈改成黃燈，而且明講「這只是提醒、OwnMind 擋不住它，你自己再看一眼」。

### 二、十一種「其實沒檢查」，畫面上跟「檢查過沒問題」長得一樣

搬到背景跑之後，「沒有聲音」變得很危險：它同時代表「檢查過、沒問題」跟「檢查根本沒跑」。
三輪審查挖出十一種會走到後者的路，全部堵掉。

| 原本會發生 | 現在 |
|---|---|
| 上一輪抓到的違規被下一輪的結果蓋掉 | 一輪一份紀錄，不會互相蓋 |
| 檢查跑到一半掛掉，什麼都不會寫、也不會講 | 丟出去之前先留紀錄，超過三分鐘沒下文就明講 |
| 你的帳號把規矩檢查關掉了，完全沒聲音 | 直接跟你說是關掉的 |
| 伺服器算不出該用哪些規矩，被當成「沒有規矩要對」 | 算不出來就是沒檢查，明講 |
| 伺服器讀不到帳號，被講成「你自己把檢查關掉了」 | 不再拿伺服器的毛病講成你的設定 |
| 這台沒裝 Claude Code，卻叫你重跑 OwnMind 更新指令 | 叫你裝 Claude Code，因為更新指令裝不了它 |
| Claude Code 沒登入或不肯做，也叫你跑更新指令 | 指向 Claude Code 那邊，不是 OwnMind |
| 綠燈出現在「這一輪根本沒規矩要對」的時候 | 只有判斷真的跑完才亮綠燈 |
| 同一個問題每一輪都在畫面上跳一次 | 會節流：狀態變了才講，否則每十輪一次 |
| 慢一秒回來的判斷結果被刪掉，還被說「沒回來」 | 宣告之前再讀一次，晚到的照樣給你看 |
| 同一段話回答不同問題，被算成同一次檢查 | 兩次都檢查 |

### 三、順手補回來的一道防護

回話送去伺服器之前會先把「看起來像密碼／金鑰」的字遮掉。這段程式碼原本住在舊的那條路上，
搬家的時候整個檔案沒人用了、被刪掉，遮蔽也跟著不見了。現在它自己一個檔，
而且連「AI 講的話被引用回來當證據」那條路也一起遮。

另外，你打了 `/ownmind-off` 之後不會再叫判斷跑，因為那花的是你自己的額度。

### 已知限制（設計上就這樣，不是壞掉）

- 一個對話的**最後一段回話**不會被撿走。撿的動作發生在「你下一次打字」，最後一段後面沒有下一次。
- 「三分鐘沒回來」的門檻對上實測最慢 115 秒。晚到不會弄丟結果了，但真的變慢的話要調這個數字。

## v1.30.10 — 掃過跟沒掃過，畫面上長得一樣

這一版全部來自一輪實測回報。六個問題，一句話講完就是：**OwnMind 說「檢查過了」的時候，
有好幾種情況它根本沒檢查 —— 而畫面上兩者長得一模一樣。**

### 一、有檔案根本沒被掃過，綠勾照樣印

改動超過 5 MB 的檔案，掃描器讀不完就當作沒事。實測：6 MB 的檔案裡放一把 AWS 金鑰，
commit 直接過、什麼都沒說；同一把金鑰放在 4 MB 的檔案裡會被擋。二進位檔也一樣 ——
掃描器看不進去，於是報「沒發現」。

現在讀不完、看不進去的檔案會擋下來，而且畫面上會指名是哪一個檔案、為什麼。
**掃描器不該有「太大所以不看」這種狀態。**

### 二、合併別人的分支，沒有任何人在檢查

git 做一般 commit 走一支掛勾，做合併走另一支，不會兩支都跑。以前只裝了前者。
所以你合併一個帶著金鑰的分支，完全沒人檢查 —— 而按下合併的人，不是寫出那把金鑰的人。

新增了合併專用的那支。已經在用的機器升級之後會自動補上。

（cherry-pick 跟 rebase 重播還是沒有防護。git 在那兩條路上沒有提供掛勾可以掛。）

### 三、OwnMind 自己壞掉的時候，看起來像檢查通過

安裝到一半、更新拉到一半、或任何一個檔案壞掉，都會讓 commit 前的檢查整個消失 ——
以前只印一行小字然後 commit 就過了。改檔的防護更糟，什麼都不印，跟「沒有規矩管到這個檔案」
完全一樣。

現在兩邊都會明講「這次沒有檢查」。**還是不擋** —— 壞掉的檢查不該擋住你做事。

另外，讀不到登入資訊的時候，擋改檔那道門整個不跑了。那道門根本不需要登入資訊。

### 四、測試安裝會冒充成真機，害你收到假警告

在自己電腦上另外開一個假的家目錄測安裝，那次測試的成績會被當成這台機器的真實健康狀態
上傳，然後你就收到一則「你的電腦少了設定」的警告。機器一直是好的。

現在用別人家目錄跑的自我檢查不會上傳，也不會排隊等下次上傳，而且畫面上會明講為什麼。

### 五、「commit 前會幫你擋密碼」這句話，改成它做得到的樣子

實測 31 種常見的密碼格式，14 種可以直接 commit 進去 —— 包括最常見的資料庫連線字串、
設定檔裡的 `password=`。範圍維持不動（擴大會一直誤擋，被誤擋的人會把它關掉），
但綠勾旁邊現在寫清楚：**只認得幾種常見格式，通過不等於沒外洩。**


## v1.30.9 — 擋住了，但你看不到；還有一把 AI 自己配得出來的鑰匙

### 一、OwnMind 擋下 AI 的時候，你跟 AI 都不知道發生什麼事

上一版發版的時候，OwnMind 照你的規矩把它擋了下來 —— 這是對的。
但你的畫面上什麼都沒有，AI 那邊只看到一行字：`Hook PreToolUse:Bash denied this tool`。

沒有規矩名稱、沒有原因，也看不出「回一句 go 就會過」。
AI 只能自己去翻程式碼才知道 OwnMind 想要什麼，你根本沒得翻。

現在擋下來的時候，AI 會拿到完整的原因（哪一條規矩、你要回什麼、它接下來該做什麼），
它會照著轉述給你。實測過的：同一個擋，改之前只有那一行，改之後整段都出得來。

**為什麼這件事比它看起來重要：** 看不到的擋是解不開的擋，
下一個人只好去找繞過去的方法 —— 而繞過去的方法就是下面那件事。

### 二、AI 可以自己寫一張同意書，然後自己蓋章放行

OwnMind 擋下 AI 之後，會在硬碟上留一張「等你同意」的紙。
以前 AI 只要自己在那張紙上寫「已同意」，OwnMind 就認了 —— 檢查只看紙在不在，不看是誰寫的。

還有那組六位數同意碼：硬碟上存的等於把答案寫在旁邊，900,000 個數字全掃過只要 0.7 秒。
「猜錯 5 次就作廢」擋不到這條路，因為它一次都不用猜錯。

現在三件事都改了：

- **紙上有 OwnMind 的印章。** AI 自己寫的、或把發下去的那張改一個字的，都不算數。
- **同意碼推不回來了。** 每試一個數字從幾乎免費變成要 67 毫秒，而且同意碼過一小時就作廢。
  一台普通筆電開滿八個核心去猜，**猜完全部要 3.8 小時** —— 同意碼早就過期了。
  （這一版剛發時這裡寫「16.8 小時」，那是只用一個核心算出來的，多開幾個核心就縮成 3.8 小時。
  數字更正過了，但結論一樣：**真正擋住的是一小時就過期，不是那個計算難度。**）
- **有人動過那張紙，你會看到。** OwnMind 不採用它，改成重新問你，並且告訴你有這件事。

**第一版的印章蓋錯地方，審查抓到了。** 蓋章的那道手續自己不看印章 ——
AI 只要把紙換成自己寫的，再走一次正常的同意流程，OwnMind 就會幫它蓋上真章。
不用偷鑰匙，四種寫法都成功，其中三種完全沒出聲。
另外印章原本只蓋住「誰、哪條規矩、有沒有同意」，沒蓋住「哪一組同意碼算數」，
所以把真的那張紙留著、只換掉同意碼，印章照樣是真的。兩個都修了，
四種寫法各補一條測試，拿修之前的程式碼跑全部會紅。

**說清楚沒做到的部分：** 印章的鑰匙跟 AI 放在同一台電腦、同一個帳號底下，
AI 真要偷鑰匙來蓋章，還是做得到。這一版把「寫一行字」變成「要刻意去偷鑰匙」，
不是變成不可能。而「你口頭說 go」這種同意本來就沒有密碼可以驗 ——
所以現在 AI 說你同意了、OwnMind 放行的時候，會直接跟你講一句「AI 說你同意了 ___」。
你沒說過，當場就看得出來。

**另外分清楚了兩句話：** 「這張紙不是 OwnMind 發的」是在指控人，
「OwnMind 認不出這張紙是不是自己發的」是機器自己有狀況。
原本兩種情況講的是同一句，於是一份完全正常的同意書，會因為機器上少了一個檔案而被說成偽造。

### 三、金鑰進了 commit，畫面上卻打勾說通過

你的機密規矩如果設成「不擋」（或是根本沒設擋不擋），
commit 會帶著 `AWS_ACCESS_KEY_ID=...` 進去，而終端機上寫的是
`Pre-commit check: all 1 rules passed ✓`。

放不放行還可以討論，**說它通過了不行，那是假的。**

而且更糟的是：那條設成「不擋」的規矩，會連 OwnMind 的預設防線一起關掉。
預設防線的用途就是「你什麼都沒設也擋得住金鑰」，不該被任何一條規矩的設定關掉。

現在：金鑰一律擋，不管你的規矩怎麼設；
規矩沒過就明講哪一條沒過、OwnMind 為什麼還是放行、你要改哪個設定。真的都過了才打勾。

---

## v1.30.8 — 「別人的檔案你不能改」，有兩扇門是開著的

OwnMind 有一條最硬的保證：**同事負責的檔案，AI 改不下去。**
這一版之前，有兩種情況它會安靜地放行 —— 不是擋不住，是根本沒去擋，而且不出聲。

### 第一扇門：AI 要新增的檔案，它的資料夾還不存在

你叫 AI 在 `ci/templates/` 底下開一個新檔，而那個資料夾本來就沒有，
AI 是連資料夾帶檔案一起建的。

OwnMind 去問「這個資料夾屬於哪個專案」，資料夾還不存在，問不到，
於是它當成「這個檔案不屬於任何專案」，放行。

而「在別人管的路徑底下新增檔案」，正好就是這條規矩要擋的那件事。

### 第二扇門：同一個資料夾，有兩種寫法

Windows 上同一個資料夾常常有兩個名字：一個長的 `C:\Users\runneradmin\`，
一個短的 `C:\Users\RUNNER~1\`，兩個都打得開，指的是同一個地方。

OwnMind 拿到的是其中一種寫法，去問 git 拿回來的是另一種，
兩串字對不起來，它就判斷「這個檔案不在這個專案裡」，放行。
Mac 上換成大小寫寫法不同，結果一樣。

這也是 Windows 那套測試從 8/13 紅到今天的原因。
不是測試寫壞了 —— 是這個功能在 Windows 上真的沒有在擋。

### 第三扇門：規矩檔讀不到的時候，它不講

存在你電腦上的那份規矩清單如果壞掉、讀不出來，OwnMind 就沒有規矩可以比對。
這時候還是要讓你繼續改檔案，這是對的，不能因為一個檢查掛了就把人卡住。
但它以前是**默默**放行的，於是「保護關掉了」跟「保護跑過、沒問題」在畫面上長得一模一樣。

現在 OwnMind 會講出來：這次沒檢查、規矩現在沒在生效、重跑更新腳本可以修好。
訊息是接在原本那行提醒後面，不是取代它。

**還沒同步過的新機器不會被吵。** 那個檔案根本還沒建立，跟「建了但壞掉」是兩件事，
只有後者會出聲。

### 改了什麼

OwnMind 不再自己拿兩串路徑相減來判斷檔案在不在專案裡 —— 改成直接問 git，
git 是站在那個資料夾裡回答的，所以怎麼拼寫都不影響答案。
資料夾還不存在的時候，它會往上找到第一個存在的資料夾再問，然後把中間缺的補回去。

### 你要做什麼

升級之後就生效，你不用設定任何東西。

## v1.30.7 — 一個只代表「機器很忙」的紅字

驗收 v1.30.6 時，有兩支測試檔紅了。兩支單獨跑都過。
它們回報的耗時是「大約九小時」——那是這個測試工具在說「時間到了它還沒跑完」。

`tests/iron-rule-trigger-parity.test.js` 單獨跑就要 **55 秒**。
它有 30 個案例，每個案例都要開一次殼層、殼層再啟動掛鉤、掛鉤又啟動好幾次 Node。
全套並行的時候，這個速度足以撞上 300 秒的上限，然後那支檔案就會紅，
訊息寫著「殼層版的分類器跟主程式不一致」——**而它其實一致**。

這是假紅字，而且更麻煩的是：它會落在哪支檔案，取決於那一分鐘剛好哪四支一起跑。
v1.26.158 整版就是在清掉這種形狀。它從另一扇門回來了——
上次是抽到不能用的埠號，這次是抽到很忙的一分鐘。

### 改了什麼

**所有指令改成一次先跑完，四個四個平行跑**，每條測試再去讀自己的答案來判斷。
維持「一個指令一條測試」，所以真的不一致時，還是會用那個指令的名字報錯——
這也正是這支檔案寫成三十條而不是一個迴圈的理由。

55 秒變 16 秒（連跑三次：17、16、15 秒）。

**每次執行給不同的對話編號。**
掛鉤那個「一小時只列一次」的視窗是用對話編號當鑰匙的；沒給的話，三十次全部共用同一把。
串著跑的時候，這是一個沒人注意到的牽連——只有第一次會看到完整名單；
平行跑的話，就會變成三十個行程同時寫同一個檔案。
給各自的編號，既是修這件事，也比較貼近真實：真實的對話不會共用同一個視窗。

**限制在四個而不是全部一起衝**，因為三十棵行程樹同時開，只是換一種方式把機器壓垮。

### 沒有動的部分

另一支一起逾時的 `reset-admin-password-script`，單獨跑不到一秒，是被同一分鐘掃到的。
沒有一起改：**兩件事一起改，就分不出來是哪一個止住了假紅字。**
如果這支變快之後它還會逾時，那才是新的情報。

## v1.30.6 — 修復要放在故障會經過的那條路上

`ensure-scanner-schedule.sh` 這支腳本，開頭就寫著它為什麼存在：

> 2026-08-06 在正式環境量到：Adam 的用量回報停在 2026-07-15。
> 他的記憶服務一直活著、每天自動更新、每天心跳。**只有排程死了。**
> 三個星期，沒有人發現，因為沒有任何東西在看。
>
> Windows 那邊其實早就有修復程式（而且註解裡直接寫著 Adam 的名字）。
> 但它從來沒送到他手上：只有 `bootstrap.ps1` 會叫它，而沒有人會手動去跑 bootstrap。
> **修復必須放在故障會經過的那條路上，也就是每天的自動更新。**

Windows 上每天的自動更新是 `bash update.sh`——
那正是伺服器自己發給使用者的升級指令，而 Git Bash 跑得動它。
它呼叫了這支腳本，這支腳本比對 Mac、比對 Linux，然後掉進「其他」那一格，印出：

```
OK:schedule:skipped_unsupported_os
```

而 `update.sh` 收到 0 就往下走了——上一行還印著 `[ OK ] Usage scanner ready`。

2026-08-15 在 Windows 上、跑 OwnMind 自己給的升級指令實測到。

**所以那次事故的修復，從來沒有接到事故真正發生的那個平台上。**
Windows 的修復程式今天還在，還是只有那個沒人會手動跑的東西會叫它。

### 改了什麼

**加一個 Windows 分支，轉呼叫 PowerShell 版的修復程式。**
輸出格式完全一樣，所以 `update.sh` 讀得到真正的結果，
它原本就寫好的失敗處理（警告 + 回報伺服器）**在 Windows 上第一次真的會動**。

**看不懂的回覆算失敗，不算成功。**
如果那支程式當掉、或吐出這支腳本讀不懂的東西，就報錯，並把它說的原話帶出來。
「我看不出來」跟「沒問題」是兩件不同的事，
而只有其中一件可以印在寫著 `ready` 的那行上面。

**加了一個測試用的開關**，讓測試可以換掉 PowerShell。
把開發者自己真正的排程刪掉、看它會不會長回來——那不叫測試，那叫製造事故。

**「其他平台」那一格維持原樣**，但註解不再宣稱 Windows 是其中之一。

### 驗證

新測試六案，拿改動前的程式跑：紅五案。
這台實測：`OK:schedule:already_registered`，exit 0。
Mac 與 Linux 行為完全沒變。

## v1.30.5 — 沒人跑的那一半

這個專案在 Mac 上開發，在 Windows 上使用。2026-08-15 第一次**在 Windows 上跑完整套測試**，
紅了 **15 條**。這 15 條在 Mac 上全部是綠的——所以它們才活到今天。

其中 2 條是產品真的壞了，其他是測試在 Windows 上本來就不可能對，而從來沒人告訴它。

### 產品壞掉的兩件事

**一、Windows 從來沒同步過規範檢查包。**

負責下載的那個函式只在一個地方被呼叫：`conditional-sync-cli.js`，
而那支只有 `ownmind-session-start.sh`（Mac／Linux）會跑。
Windows 註冊的是 `.js` 那支，它直接引用別的東西，從來沒碰過那個 CLI。

結果：`~/.ownmind/cache/enforcement.json` 永遠不存在，
**這台電腦上所有的規範檢查都是關的**，而且每一輪對話都誠實地講出來：

> [OwnMind] This machine has never synced its standards…

講得完全正確，而且永遠不會變。實測：直接跟伺服器要，它回 37 條；
用全新的家目錄跑一次開場，那個檔案還是不存在。

那段程式的註解自己把理由寫得清清楚楚：

> .js 那支是 Windows 專用的。把同步寫進 .js，在它服務的那台機器上永遠不會被執行。

兩句話對 Mac 都成立，對 Windows 都不成立。**一條關於兩個平台的規則，在其中一個平台上寫成、也只在那個平台上驗過。**

**二、`translate.mjs` 在 Windows 上是個安靜的空轉。**
...
在 Mac 上這是對的，但是**碰巧對的**——路徑 `/repo/x.mjs` 開頭那條斜線剛好補上了網址需要的第三條斜線。
在 Windows 上路徑是 `C:\repo\x.mjs`，兩邊永遠不相等。
所以這支腳本跑起來：什麼都沒比對到、什麼都沒印、什麼都沒寫，然後**回報成功**。

同樣一行還有第二份，在 `hooks/lib/sync-memory-files.js`。
那份沒出事，純粹因為 Windows 上沒有任何東西會用命令列去叫它。

### 那些不可能過的測試

- **10 條**在啟動掛鉤時開了一條輸入管道卻沒關。掛鉤會一直等到輸入結束——
  這是 Claude Code 的規矩——所以它就一直等。每條卡 25 秒、沒有輸出、沒有錯誤，
  最後吐出來的訊息在講鎖跟初始化，指向完全不相干的地方。
- **4 條**把 Windows 路徑當網址用（`C:\` 被當成一種通訊協定）、
  用 `:` 接 PATH（Windows 要用 `;`）、還把假的 `docker` 寫成 Linux 腳本。這三件 Windows 都做不到。
- **1 條**要求檔案權限是 `0400`。Windows 沒有這種權限位元，Node 對唯讀檔一律回 `0444`，
  怎麼改都不可能是 `0400`。

### 改了什麼

**Windows 的開場程式會同步規範檢查包了**，而且是引用同一個函式，不是自己再寫一份——
再寫一份正是這兩個平台當初走散的原因。

**兩處判斷都改用正規做法**，並加了一個防呆：
`tests/no-file-url-concatenation.test.js` 只要有人再用字串拼網址就紅。
它還會**分平台各驗一次自己的前提**：在 Windows 上兩種寫法真的不同、在 Mac 上真的相同。
後面那半才是重點——不然 Mac 上的人讀到這條規則，只會覺得是多此一舉。

**那 10 條測試改成關閉輸入**，這也正是 Claude Code 的行為，所以它們現在測的是真正的約定。

**那 4 條在 Windows 上標示跳過，並寫明理由。**
一個代表「這裡不適用」的紅字，只會訓練大家忽略紅字。

**權限那條改成驗 Windows 真的能保證的事**（檔案不可寫），
並在同一個地方寫明它保證不了的事：在 Windows 上這把金鑰仍然能被同一台機器的其他帳號讀取，
要擋住得用存取控制清單，不是權限位元。**講出來，而不是偷偷放寬數字。**

### 還有兩條其實從來不是壞掉

`bare-mount-trailing-slash` 那兩條需要前端編譯產物，而那個產物沒進版控。
跑一次 `node scripts/ensure-console-build.js` 就過了。
它們被當成「本來就紅的」好幾週——其實只是東西沒建。

### 同一個形狀，第六週

這次六個問題裡有五個**不會報錯**：
沒寫出來的快取、跑完回報成功卻什麼都沒做的腳本、永遠在等的掛鉤、長得像失敗的跳過。

新的地方是**它們在哪裡**——全都在那張平台對照表裡、沒人跑的那一半。

### 驗證

Windows 上全套：5450 條、5426 過、0 紅、24 跳過。跑完前後暫存資料夾都是 6 個。

## v1.30.4 — 交接功能照說明書用會失敗

要把手上沒做完的事交給另一個工具或另一個對話接手，AI 會呼叫 OwnMind 的交接功能。
**照它自己的說明書填，一定失敗。**

說明書寫「專案」跟「內容」兩欄必填，其他三欄可以不填。伺服器那邊卻要四欄，其中兩欄
正好就是說明書說可以不填的。所以 AI 老實照著填，換回來一句「必填欄位缺少」，那份交接
就沒了；只有在 AI 剛好多填了兩欄不必填的欄位時才會成功。

現在：**工具用的是哪一個（Claude Code、Cursor 之類）OwnMind 自己填**，那本來就是它知道
的事，不需要問 AI。**模型名稱不編**——這台電腦上沒有任何地方知道那個值，硬填一個
「不明」會讓後台的統計多出一個假分類。伺服器改成只要「專案」跟「內容」，跟資料庫本來
的設計一致（那四欄除了內容以外，本來就允許空白）。

這跟 2026 年 6 月那次「工作紀錄因為缺一個欄位整筆被丟掉」是同一個毛病，換了一張表。

## v1.30.3 — 判你回話的那顆腦袋，之前是隨機抽的

檢查 AI 回話的時候，OwnMind 會請一個判斷模型讀你的規矩、再讀那段回話。**它之前沒有指定要
哪一顆**，等於每次隨機抽。實際量到的：抽到的池子裡有 71 個，包含專門辨識圖片文字的、
做語音的。正式機的紀錄裡真的抽到過辨識圖片文字那顆去讀你的規矩。

看得到的症狀是：**同一段話，這次說沒問題，下次說違規。** 我拿同一段文字連跑四次，兩次過、
兩次不過；兩次不過給的建議還互相打架，一次叫「只留第一句」，另一次叫「第一句就是問題」。

還有一件更難發現的：最常被抽到的那一顆（16 次裡 13 次）**幾乎見人就抓** —— 我拿 8 段
照你規矩寫的回話去測，8 段全被判違規。判斷一旦每次都說違規，「違規」這兩個字就沒有資訊了。

改成三件事：

- **指定模型**，不再隨機抽。指定的那顆在同一份測試裡，8 段裡只誤判 1 段，該抓的 3 段都抓到。
- **判斷時不留隨機性。** 之前沿用了寫敘述文那邊的設定，會刻意讓答案有變化；那對寫文章是優點，
  對判對錯是缺點。
- **指定的那顆被拒絕時，同一顆重送一次**（這台伺服器被拒之後重送通常就會過），而不是換一顆
  來頂。**故意不設備援模型**：唯一會被認的第二顆就是那顆見人就抓的，讓它接手等於在多數情況下
  給你一個有信心的錯答案，而且事後沒有任何紀錄分得出來那次是誰判的。真的判不成就明講判不成
  —— 那句話你看得到，也不會混進誤判率裡。

一個實測到的陷阱寫進程式裡了：**跟它要哪顆，它不一定給哪顆。** 要 gemini、llama、gpt-4o
這三個，回來的全是同一顆別的。所以現在每次都會核對「實際回答的是不是我要的那顆」，不一樣
就留紀錄，否則設定寫得很漂亮、實際上完全沒生效。

誠實說清楚：判準是我自己標的 8 段樣本、每段跑 2 次。這足夠讓我換掉隨機抽，不足以說誤判
問題已經解決。要真的知道，得靠你回「誤判」累積出來的數字（v1.30.1 接上的那條路）。

## v1.30.2 — 檢查沒跑成的時候，講的是真正的原因

OwnMind 檢查不了 AI 的回話時，只會講一句「這次連不上伺服器」。**不管真正的原因是什麼。**

伺服器不認這台電腦、網路斷一秒、伺服器有接到但它自己那邊沒做完 —— 你讀到的都是同一句。
其中兩種等一下就好，一種等到下禮拜還是壞的，而那句話叫你等。

現在分三種講：

- **伺服器不認這台電腦** → 「OwnMind 認不得這台電腦了…你要重新登入：拿新的登入資料重跑一次
  安裝指令。」你知道要動手，也知道要動什麼手。
- **伺服器有回話，但它自己那邊沒做完** → 「OwnMind 這次沒把檢查做完…這是 OwnMind 那邊的問題，
  通常會自己好，你不用做什麼。」這其實是最常發生的一種，原本卻被說成「連不上」，等於叫你去
  查自己的網路。
- **真的連不上** → 維持原本那句。

三句是三個不同的狀態，所以狀態一變你就會馬上讀到，不會被當成「同一個狀態、繼續安靜」。

而且「認不得這台電腦」這一句**不會在你重新登入之後繼續糾纏你**。連不上伺服器的時候 OwnMind
會停五分鐘不再重試（省下每一輪等五秒），但登入被拒是秒回的，停五分鐘只會讓你剛登入好、它還在
叫你去登入。這種狀況改成一分鐘就再試一次 —— 比你重跑安裝指令的時間還短，你回到鍵盤前它就恢復
檢查了。

還有一件你看不到但查得到的：**檢查失敗的真正原因現在會被寫下來**，寫在這台電腦的
`~/.ownmind/logs/check-failures.jsonl`。之前它哪裡都沒去 —— 訊息裡不能寫（那些是內部
代號，不是講給你聽的字），從訊息拿掉之後就沒有第二個地方存了。要查「什麼時候開始壞的、
現在還在壞嗎」，答案在這個檔案裡；它只留在這台電腦，因為它要記的正好是「送不出去」那件事。

同時修掉的兩件：

- 原本只有真的去連伺服器的那一輪知道原因，接下來五分鐘不重試的每一輪都退化成「不知道為
  什麼」。而你會讀到的那些訊息，正好都落在那五分鐘裡。
- 伺服器那邊沒做完的四種情況，原本連分類都沒有，寫下來就是一行「不知道／不知道」。現在其中
  三種帶著伺服器那筆紀錄的編號，兩邊對得起來；剩下那一種（連「這個帳號要不要檢查」都讀不到，
  所以根本沒留紀錄）靠它自己的原因字串認得出來。

## v1.30.1 — 「回覆誤判 770」現在真的會記下來

被擋的訊息一直叫你「如果判斷有誤，請回覆『誤判 770』」。**你照做了，什麼都不會發生。**

資料庫有欄位存這個判定、伺服器也有收它的端點，但沒有任何東西把你打的那句話送過去 ——
只有測試在呼叫那個端點。所以那句話等於叫你做一件做不到的事，而且看起來像有在運作。

代價不只是白打一行字：**誤判率是「要不要開放給其他人用」的門檻**，而它一直是從空集合算出來的。

補上 `ownmind_report_check_feedback`：你回「誤判 770」，AI 就呼叫它，判定真的寫進那筆檢查。
說「這次擋得對」也可以記。被擋時 AI 讀到的指示也改了，直接寫明「不呼叫這個工具，使用者
講的話就不會被記下來」。

三件刻意的設計：

- **AI 不准自己判斷一次攔截對不對。** 這個工具記的是**你說的話**；AI 自己填的誤判率，量的是
  AI 不是規矩。
- **失敗就說失敗，不排隊重送。** 這個端點是照編號改一筆資料，晚幾分鐘無聲送達，跟根本沒送
  分不出來 —— 你要知道它沒進去，才會再講一次。
- **只動得到你自己的紀錄。** 別人回報你的檢查，你的判定不會被蓋掉（有真資料庫測試釘住）。

## v1.30.1（同版）— 訊息重寫：每一句都要講「誰做了什麼、你要不要管」

系統跟你講的每一句話都重寫了，24 句，中英日三版。

原本的寫法是站在系統的角度描述機制，句子裡沒有人：「合規檢查沒有執行（unknown）— 這一輪
「沒有」被檢查」。同一件事講兩次、漏一個內部代號出來、而且完全沒說你要不要做什麼。現在是
「OwnMind 這次連不上伺服器，沒有檢查 AI 這段回話。」

四條規則，寫進 CLAUDE.md，之後新訊息一律照這個：

1. **主詞是「你」或「AI」，不是機制。**
2. **動作要指名是誰做的** —— 不准「已經擋下來」「已經要它重寫」這種沒主詞的句子，是 OwnMind
   做的就寫 OwnMind。
3. **要講「所以呢」** —— 你要不要做什麼；不用做就明講不用。
4. **不准出現行話跟內部代號** —— 合規、lint、回執、把關、憑證、模式、`unknown`、`timeout`。

**⛔ 換成三顆狀態燈。** 原本每一則訊息都掛同一個紅色禁止標誌，不管 OwnMind 是**成功擋住了**
還是**整個沒跑起來** —— 兩者長得一模一樣。現在：

- 🟢 **OwnMind 做到了它該做的事** —— 擋下 AI、挑到違規、檢查過了。**擋住是綠燈，因為擋住
  就是它在正常運作。**
- 🟡 還在運作，但保護變弱、或該看一眼 —— 只提醒不擋、重寫幾次放棄、確認不了 AI 有沒有讀規矩。
- 🔴 沒運作 —— 沒檢查到、連不上、沒登入、被關掉、設定錯。

程式碼裡那些寫死的英文備援句（字典載不進來時出的就是它）一起改了。那是最容易漏的一層：
跟字典對不上的時候，平常完全看不出來，只有字典壞掉那天才會現形，而那天你正需要它講對話。

`compliance.notChecked.checkFailed` 也不再把伺服器的原始原因印給你看。原本會出現「（unknown）」
—— 一個對讀的人毫無意義的內部字串。

**但要講清楚：那個原因目前沒有被記到任何地方。** 拿掉之後它就沒有去處了。實際代價是，金鑰被撤銷
（`http 401`）跟真的連不上，你看到的是同一句「這次連不上伺服器」，而前者會每一輪都出現、永遠不會好。
把原因接進紀錄是下一版要做的事。

## v1.30.1（同版）— 被拒絕的時候，至少要說是為什麼

上一版修好了「兩個視窗就永遠點不了頭」，但沒動到讓那個 bug 藏了整整一版的原因：
**登記失敗只印一句 `REJECTED`，不講任何理由。** 現在會講 —— 是那個對話沒有待審紀錄、
已經批准過了、代碼不對、還是根本沒給對話代號。理由印在錯誤輸出，`REJECTED` 這個字本身
維持原樣，讀它的程式不用改。

另外三件是獨立審查抓到的，都不影響已經在跑的正式機，但每一件都是「哪天會咬人」：

- **對話代號格式不對時，以前不是被擋掉，是被改成 `unknown`。** 而 `unknown` 是一個公用桶子
  —— 所有拿不到正常代號的對話都掉在裡面。所以格式不對的代號不會失敗，會落到別人的紀錄上。
  現在在登記程式這一層就直接擋掉。
- **`--session` 後面沒接值時，以前會悄悄退回舊的指標檔**，也就是把上一版修掉的 bug 原封不動
  裝回去，而且看不出來。現在直接拒絕：參數給了就要嘛認、要嘛拒，不准當作沒看見。
- **測試用的資料庫互斥鎖有三個洞**：拿著鎖的人只要工作超過十分鐘就會被判定「死了」而被搶走
  （活著沒有心跳、判定順序又把年齡放在生死前面）；被搶走的人離開時還會把別人的鎖一起刪掉；
  而拿到鎖之後如果容器啟動失敗，鎖就再也放不掉。全部修掉，並補上四項測試 —— 這個鎖上一版
  是完全沒有測試的。

上一版的一項測試也是錯的：它宣稱在驗「不合法的對話代號會被拒絕」，實際上那個路徑會被正規化
成一個不存在的檔名，所以有沒有防護它都是綠的；而它宣稱的那個性質當時根本不成立。已換成會
真的失敗的版本。

## v1.26.174 — 開兩個視窗，閘門就永遠點不了頭

閘門擋下動作、使用者說「go」、AI 去登記那個「go」—— 只要你同時開著兩個 Claude 視窗，
最後一步就一定失敗，而且畫面上不會說為什麼。

原因是「現在是哪個對話」被存成**一個檔案**，每個對話一開始就把它覆蓋掉。所以閘門把
「等待點頭」的紀錄寫在**被擋的那個對話**底下，登記程式卻去問那個檔案、拿到**最後開的
那個對話**，找不到紀錄，回一句 `REJECTED` 就結束。使用者明明說了 go，動作還是過不去；
而閘門的設計裡沒有第二條路可以點頭。

2026-08-14 實測抓到：這條閘門第一次真的擋下一次發版，就卡在這裡。被擋的是
`2fc0ce05…`、指標指著五分鐘前開的 `54e53a3e…`。**開一個視窗的人碰不到，開兩個的人
一定碰到** —— 而這個產品的作者本人就是天天開兩個以上的那種使用者。

改法：閘門在「叫 AI 去跑哪一行指令」的時候，直接把對話代號寫進那行指令裡
（`--session <id>`），登記程式收下它。沒帶就退回原本那個指標檔，單一視窗的行為完全不變。
AI 沒有別的辦法知道自己的對話代號 —— 環境變數裡沒有，所以非得由閘門講出來不可。

另外修掉一個測試環境的坑，兩層：

- 等資料庫起來的判斷改成「真的下一句查詢問得到答案」。原本只問 `pg_isready`，而資料庫容器
  初始化時會先開一個**臨時**伺服器，那支問答會提早回答 yes；等臨時的收掉、正式的還沒起來
  的那一瞬間，測試就會撞上「連不到」而整批遷移失敗。
- 要資料庫的測試檔改成一次只跑一個容器（跨行程互斥鎖）。測試框架會照 CPU 數量平行跑檔案，
  而這種檔案現在有四個；四個資料庫容器加上其餘五千多項測試的負載，實測連續兩輪各中一次、
  每次中的還是不同檔案 —— 一次是啟動時整批遷移失敗，一次是跑到一半查詢回 500。單獨把那四
  個檔案抓出來跑則穩定全過，所以問題在負載不在程式。改成排隊之後全程只會有一個容器，整套
  測試多花大約二十秒。

鎖是用「建目錄」這個原子操作做的，裡面記著持有者的行程編號；持有者掛掉沒解鎖時會被判定
失效並接手，不會卡死。

**這不會讓任何人多拿到權限。** 登記還是必須有閘門自己寫下的那筆「等待點頭」紀錄、
還是一次性、代碼模式還是要輸入正確的六位數字，而且不合法的代號會先被收斂成 `unknown`
才碰到檔案路徑，穿不出狀態目錄。能傳這個參數的人，本來就能直接改那個指標檔 ——
它是查表用的，從來不是一道鎖。
...
```
[OwnMind v1.26.172] Memory search ┃ Tip: 可以說「搜尋記憶」
```

事件尾端的冒號一併去掉（兩種全形半形都收）。那個冒號原本是要帶出下一行的內容，
現在下一行不存在了，留著只會變成一個沒有下文的標籤。

2026-08-14 實測過四種「不是 `\n` 的換行字元」能不能繞過：U+2028、U+2029、U+000B、U+0085
在 Claude Code 一樣會斷行，而且斷出來的新行照樣被蓋章。沒有免費的換行，所以一行就是下限。

順這條線查下去，發現這些字元本來就會弄壞這支程式，而且是兩種壞法：

| 字元 | 原本的下場 |
|---|---|
| `\r`、U+2028、U+2029 | 對 JS 正規表達式來說是換行，比對整條失敗，**那則訊息整個消失** |
| U+000B、U+0085 | 不是換行，原封不動流進輸出，在畫面上斷行、多被蓋一次章 |

前者違反規格第 1 條（每個動作都要被使用者看到），後者違反第 2 條。現在 `extractBanners`
在比對之前先把這五個字元一律換成空白，只留 `\n` 當唯一的分行依據，兩種壞法一起收掉。
測試改用「內文真的帶這些字元」的樣本，斷言訊息沒少、字元沒溜出去；把修正拿掉會變紅。

事件尾端冒號的處理也收窄了：只有在整行**只有那一個冒號**時才拿掉（`Memory search:` 這種
純標籤），內文自然結尾在冒號的（`Tip: try this:`）保持原樣，不再被吃掉一個字。

### 自動更新的結果重新找回送達路徑

背景自動更新跑在一個比對話還久的獨立程序裡，結果出來的時候已經沒有回合可以講了。原本靠
「下次開場時把佇列印出來」補送，但 v1.26.171 把那個補送拿掉了 —— 拿得對，因為開場那條
管道是講給**模型**聽的、不是給人看的，而且它印完就把稽核紀錄一起清掉。

問題是拿掉之後沒人補上替代路徑。**更新失敗的訊息從此寫進一個沒人讀的檔案**，而那正是最
需要被看到的一種。
...
| 檔案 | job |
|---|---|
| `logs/update-pending.jsonl` | 待送佇列。送出去之後才把送掉的那幾行刪掉 |
| `logs/banner-pending.jsonl` | 只增不減的稽核紀錄，不再被任何人清空 |

送達點改成 Stop 掛勾（每個回合結尾都會跑，也是目前唯一真的會顯示給使用者的管道）。
排進去的時機在所有提早結束的分支之前，所以更新失敗不會因為這回合的檢查結果而被卡住；
清佇列則放在「訊息真的寫進 stdout 之後」，清理永遠排在證據後面。

清的時候是按「已送出幾行」刪，不是整檔清掉 —— 更新程序隨時可能在讀取與清除之間再寫一筆，
整檔清掉會刪掉一則從來沒人看過的結果，那就是這次要修的毛病本身。
...
(`gpt-4o-mini`, then `gpt-4o`) hit the shared `legacy-server.example/llm-switch` proxy's broken
...
一條已存的規範安靜地不被執行。這一版把三個都補掉。

**一、挑選：310 條只有 6 條真的進判斷。** v1.26.170 修了 API 那層讓標籤清單傳得過去，但挑選
函式本身還是把清單插進字串模板，變成 `trigger:respond,report`，對不上任何標籤 —— 只有
`trigger:always` 那 7 條活下來。現在清單裡任何一個標籤對上就算對上；同時把 `trigger:always`
升到跟 `always_check` 同級，數量預算擠不掉它們（審查抓到的陷阱：只修比對不修排序，
反而會把唯一在執行的那幾條擠出去）。

**二、關掉不准長得像通過。** 伺服器回「這個帳號的執行功能是關的」（skipped + enabled:false）
以前被當成乾淨判定；機器沒有憑證以前是完全安靜。兩種現在都會明講「這一輪沒有檢查」。

**三、通知走了一條永遠到不了的路。** 所有給使用者看的訊息都寫 /dev/tty，但 hook 子行程在任何
平台都沒有控制終端機，那個寫入從第一天起每一次都失敗；備援檔下次開對話時被灌進模型的腦袋，
不是人的眼睛。這個使用者的備援檔裡躺著三條「擋不住了、改讓你看」的通知，他一條都沒看過。
改走官方文件寫明會渲染給人看的通道：stdout 上的 `{"systemMessage": …}` JSON（exit 0）。
降級警告原本用 exit 1 + stderr（哪裡都不顯示），一併改成 exit 0 + systemMessage。
擋下重寫的路（exit 2 + stderr）不動 —— 那條本來就會以「擋下原因」的形式呈現，
而且現在會帶上查核編號，使用者看到誤判可以直接回「誤判 <編號>」。

**工具橫幅那支鉤子同病同修。** 記憶讀寫、鐵律觸發時的品牌橫幅（PostToolUse 那支）也是寫
/dev/tty 的設計，同一版一起改走 systemMessage；稽核檔照寫。

**「沒檢查」提醒有節流（Vin 拍板）。** 狀態類提醒（斷線、沒憑證、沒同步、帳號關閉）在
狀態改變時講一次、恢復時講一次、同一狀態持續期間每 10 輪提醒一次，不會整段斷線期間
每句話都掛一行。事件類（違規退回、擋到上限）不節流。被靜音的輪次照樣寫進稽核檔。

## v1.26.170 — 標籤清單在最後一哩被丟掉

v1.26.169 上線之後，我打了一段明顯違反「第一句就是結論」的話去試，查核回「沒挑到任何規範」。

原因：挑選函式已經改成可以吃多個標籤（一則回覆同時是「回話」也是「回報」），但 API 那一層還是
只認單一字串，收到清單就當成空的。結果是**每一條靠標籤選中的規範全部被丟掉**，而回應是
「skipped」—— 跟「這一輪很正常」長得一模一樣。

線上實測的數字：309 條規範送到用戶端，符合標籤的一條都沒進判斷。

順帶量到的事實：這個帳號的規範標籤裡，`trigger:edit` 有 72 條、`trigger:deploy` 27 條、
`trigger:report` 22 條、`trigger:respond` 18 條、`trigger:always` 7 條，另外還有中文標籤
（`trigger:回話`、`trigger:回報`）。中文那些目前不會被回話這條路選到，之後要一起處理。

## v1.26.169 — 第三道關卡接上了

判斷者接到對話上：AI 每講完一輪，系統拿當下相關的規範去比對它剛剛講了什麼，違反就退回要求更正。

**接在「這一輪是不是重寫」的判斷之前。** 那個判斷本來是為了避免無限迴圈，但它也代表「因為被退回
才產生的那一版」永遠不會被檢查 —— 退回一次就等於開一個永久後門。改成用自己的計數器設上限（兩次），
既不會無限來回，也不會有檢查不到的死角。把它移回原位，一條測試轉紅。

**大部分輪次完全不連網。** 本機先用同步下來的選擇鍵比對，沒有任何規範跟這一輪相關就直接結束。

**寫這段的時候踩了自己警告過的坑。** 這支 hook 的共用函式是在 main() 裡動態載入到區域變數的，
我在外面的輔助函式直接呼叫 `readCredentials()`，它不在作用域裡。ReferenceError 被外層的 catch
吞掉，結果是：檢查在每一輪都沒有執行，而且完全安靜。

抓到它的是「把 hook 當程式跑起來看它到底做了什麼」那條測試。任何只驗函式的測試都看不到，因為函式
本身是對的 —— 錯的是它被放進去的那個位置。這正是這整個功能存在的理由，在它自己身上發生了一次。

**已知未解：** 全套測試約每十次會出現一次紅，重跑就綠，連跑八次都抓不到是哪一條。已排除容器埠互撞
那個原因（上一版修掉），目前沒有第二個線索，先記在這裡。

## v1.26.168 — 判斷者（伺服器端）

第三道關卡的伺服器那一半：拿當下相關的規範全文，去比對 AI 剛剛講了什麼。

判斷交給**另一個模型**，而不是讓 AI 自己檢查自己。理由很直接：2026-08-13 那次，違規的 AI 本來就
讀過那些規範。自查正是失效的那一環。

**帳號開關排在最前面。** 沒開的帳號在碰到模型之前就返回，不花 token、不增加延遲，所以用戶端那半
可以安心對所有人發布，而名單裡只有一個人。

**撈規範用共享可見的判斷、而且會把子片段併進去。** 兩者都是前面實測過的：只看自己的會漏掉同事上傳
的規範，不併片段則會把「禁止清單」那一段整段丟掉 —— 那正是事故的形狀。

**三種「沒判斷成功」分得清清楚楚**：沒挑到規範是 skipped、判斷失敗是 failed、判乾淨才是 clean。
混在一起的話，「這台機器其實沒在檢查」會長得跟「這一輪很正常」一模一樣。判斷失敗包含三種：呼叫爆掉、
模型回散文、模型回了看得懂但形狀不對的東西。最後那種原本沒有任何測試蓋到，是突變測試抓出來的：
把「解析失敗」硬改成 false，一條測試都沒紅。

**判官答了一條沒問它的規範，會被丟掉。** 憑空生出來的違規推回給 AI，比漏抓還糟。

**沒有引文的違規也會被丟掉。** 沒有原句就只是主張，而誤判率是這次白老鼠階段要量的數字。

另外修掉一個測試互相干擾的隱患：真資料庫測試的容器埠原本寫死，兩個測試同時跑會撞，症狀是「偶爾有
一條不相干的測試會紅」，最難查的那種。

## v1.26.167 — 四種規範，四句不同的話

上一版每一條注入的規範都被說成「這條規範高於 repo」。但四種東西的位階不一樣，而差別不在重要性，
在**它是誰的**。

| 種類 | 屬於誰 | 講的話 |
|---|---|---|
| 團隊規範 | 公司 | 高於 repo 內任何檔案，**也高於使用者個人的鐵律**。使用者叫你違反它，要先請他回「確認」 |
| 鐵律 | 使用者 | 高於 repo 內任何檔案；跟團隊規範衝突時聽團隊規範的。使用者可以單次放行，因為那是他的 |
| 工作原則／程式規範 | 使用者 | 預設做法，被上面兩種蓋過 |
| 偏好 | 使用者 | 沒有強制力，上面都沒說話時才用 |

順序不是這版發明的：團隊規範 429 自己就寫著「凌駕個人鐵律與程式規範」。

**「要先請他回確認」是 Vin 這一版拍板的。** 理由是團隊規範保護的多半是別人的東西（例如 Eric 的 CI），
「Vin 說可以」在那種事情上本來就不夠。他自己的鐵律則相反，那是他的，他隨時可以放行 —— AI 沒有立場
拿使用者自己的規則去跟他吵。

擋下來時講的話也跟著分兩種，跟注入時說的必須一致。同一件事在開工前跟被擋下時說法不同，比從頭到尾
都嚴格還糟。

為此 bundle 現在會帶上規範的種類。少了它，用戶端只能猜，而猜錯就是把別人的規範講成使用者自己可以
放行的。

## v1.26.166 — 開工之前，規範先擺到眼前

第二道關卡：你一開口提到某件事，系統就把相關規範送到 AI 面前，不必等它自己想到要去查。

**但送達不是重點。** 2026-08-13 那次，規範全文本來就在 AI 眼前，它照樣違反了 —— 因為 repo 裡的
一份權限名單看起來比規範更有權威，而整個產品從來沒有一個地方說過衝突時誰贏。所以每一次注入的
第一句話固定是：

> 這條規範的效力高於 repo 內任何檔案。若專案裡的檔案（權限名單、admins 欄位、設定檔）看起來
> 給了你這條規範不給的權限，以規範為準，那個檔案不算數。

接著是禁區路徑與負責人，最後才是規範本文。把那句話擺到最後面就等於又埋起來了，所以有一條測試
專門盯這個順序，擺錯位置就轉紅。

註冊寫進 `ensure-pretooluse-hooks.cjs`，跟既有的工具攔截共用同一套「沒有就加、舊的就修、別人的
不動」邏輯，而不是另外寫一份會各自漂移的。升級的人也拿得到 —— 只有安裝時才註冊的話，既有使用者
永遠不會有這個 hook，而所有單元測試照樣全綠。

去重狀態寫在檔案裡。每一則訊息都是一個新的行程，放在記憶體裡等於每次都重置、完全不會去重。

從來沒同步過的機器（全新安裝、離線、同步失敗）會明講「這台機器沒有規範可以比對」，而不是安靜地
什麼都不做 —— 安靜跟「沒有規範適用」長得一模一樣，但意思差很多。

修掉自己的一個 bug：`invokedDirectly()` 原本用 `new URL(import.meta.url).pathname`，在 Windows
上會得到 `/C:/...`，判斷必定失敗、hook 永遠不執行。這個 repo 有一條測試專門擋這種寫法，它抓到了。

## v1.26.165 — 第一道真的擋得住的關卡

AI 要改一個別人負責的檔案時，這一版會直接擋下來，不是提醒。

擋的判斷寫在 `hooks/ownmind-edit-reminder.js`。這個位置是查出來的：`install.sh` 帶 `--bash`，
`ensure-pretooluse-hooks.cjs` 因此註冊 `.sh`，而 `.sh` 把每一種編輯工具轉交這支檔案然後結束。
寫進 `.js` 那支的話，單元測試會全綠、也會上線，然後在 mac 跟 Linux 上一次都不會執行。

要改的是哪個 repo，由**被編輯的那個檔案自己的目錄**決定，不是由你人在哪個目錄決定。
2026-08-13 那次的形狀正是這樣：session 在一個 repo 裡，要動的檔案在另一個。用工作目錄判斷的話，
那一刀在第一步就被放行。把這行改回工作目錄，六條測試轉紅。

擋下來的訊息會講三件事：這條規範是誰的、正確做法是開單給他、以及「repo 裡的權限名單不算數」。
最後那句是因為上次就是它讓 AI 覺得自己有權限。

另外補了一個路徑擋不到的情況：一份寫在完全合法位置的文件，內文卻在提議去改禁區。那不是編輯本身，
所以是警告不是阻擋 —— 但它正是上次真正產出的東西，而等到有人發現時，已經有人讀完那份計畫了。

擋的動作跑在節流之前。底下那份清單一小時只列一次是刻意的；擋如果也跟著一小時一次，那不是保證，
只是延後。

實測：把整段流程從 Claude Code 真正會跑的那支 `.sh` 灌進去，`{"decision":"block"}` 原封不動
出現在輸出裡。

## v1.26.164 — 規範終於會走到你的電腦上

上一版建好了配送端點，這一版把最後一段接起來：使用者的電腦真的去把規範抓回來、存下來。

同步寫在 `hooks/lib/conditional-sync-cli.js`，不是 `ownmind-session-start.js`。這個位置是
量出來的，不是選出來的：`session-hook-command.cjs` 第 38 行寫著 mac 與 Linux 註冊的是 `.sh`，
而 `.sh` 呼叫的是這支 CLI，`.js` 那支只有 Windows 會跑。寫進 `.js` 的話，這台機器永遠不會
執行到它，快取永遠是空的，而且沒有任何地方會說話。

它也刻意排在既有的 init 同步**之前**、而且不依賴它成功。兩者的有效期與失敗方式都不一樣，
讓其中一個決定規範能不能到這台機器，等於讓一個不相干的故障悄悄關掉整套強制執行。

快取讀取端分得出「同步過但這個帳號沒有規範」跟「這台機器從來沒同步過」。前者是正常的安靜
一輪，後者代表這台機器上所有檢查都是關的。把兩者混為一談，就是讓全新安裝看起來一切正常而
其實什麼都沒在擋。

空回應不准覆蓋已經有東西的快取。空的結果多半是請求壞掉，不是使用者把規範全刪了；過期的快取
至少還擋得住東西，空的什麼都擋不住。

測試分兩層：行為的部分在行程內對真的 HTTP 伺服器跑，走真的抓取、真的寫入、真的讀取；
「CLI 當成程式跑的時候真的會走到同步」則是把它當程式跑一次，看它在同步紀錄裡留下的那一行。
把 main() 裡那行呼叫拿掉，第二層當場轉紅。

## v1.26.163 — 那條規範不是你上傳的，所以你看不見它

規範強制執行機制的第一塊：把規範送到用戶端的那條路，還有一組不准「兩端都造假」的測試。

這一版沒有任何使用者看得到的行為改變。它存在的理由是計畫階段量到的兩件事。

**第一件：判官問模型的那條路，寫法是錯的。** `callLLMSwitch` 回傳的是已經解析好的物件，
而第一版的程式把它當字串接。實跑的結果是每一次查核都會被記成「失敗」，永遠報不出任何違規。
路由的測試注入了一個回字串的假模型，所以兩邊互相同意，沒有任何東西會發現。

**第二件：撈規範的查詢看不到共享的團隊規範。** 起一個真的資料庫、套上本 repo 全部的
migration、放進事故當時的資料形狀（規範屬於同事、禁止清單在子片段、查詢的是另一個人），
量到的結果是：

```
WHERE user_id = $1              → 只回自己那條，看不到 412
加上 buildReadableWhere 的述詞  → 兩條都回
```

412 正是這套機制要防的那條規範，而它是同事上傳的。也就是說，第一版的查詢對它全盲。

新增的端點 `GET /api/memory/enforcement-bundle` 回三份清單：`selectors`（每一條規範，不含
內文，供用戶端判斷這一輪要不要問伺服器）、`guards`（禁區路徑規則）、`injectables`（帶內文，
只含被標註的規範）。內文留在伺服器，判官直接查資料庫，所以判官的涵蓋範圍由資料庫決定，
不受用戶端快取到多少限制。

它註冊在 `memory.js` 的 `/:id` **之上**。掛在下面的話，Express 會把 `enforcement-bundle`
當成一個 id 交給那條路由，整數轉型失敗、回 500，而用戶端只會讀成「伺服器壞了」。

測試接的是真東西：真的資料庫、真的路由、真的認證中介層。把查詢換回 `user_id = $1`，那條
測試當場轉紅。
## v1.26.162 — 兩 GB 的那個檔案

Amiee 的用量從五月起就沒再進來過。她的機器每一項看起來都健康：排程是 Ready、版本是最新的、
金鑰有效、伺服器連得到。今天請她手動跑一次掃描，node 直接崩了：

    Assertion failed: args[3]->IsInt32()
    at defaultReadIncremental (.../shared/scanners/claude-code.js:221)

掃描器一次讀「從上次位置到檔尾」的全部位元組。node 的檔案介面規定那個長度必須放得進 32 位元
整數，所以只要某個對話檔長到 2 GB，這個要求就會讓**整個行程當場中止**。中止不是可以接住的錯誤 ——
外面那層「一個檔案讀不了就跳過」的保護，一行都沒機會執行。掃描死在同一個位元組上，每一次，
而且不留任何痕跡。

**伺服器上的心跳把死掉的位置指得很準。** 掃描依序跑 claude-code、codex、opencode、cursor、
antigravity。她的 claude-code 今天還在回報，後面三個停在 2026-07-27，中間的 codex 一次都
沒有出現過 —— 那正是行程中止的形狀，而她本來就是用 Codex 工作的人。這個讀取函式是兩個
adapter 共用的，所以修一處，兩邊都好。

**改成分段讀，而且一次掃描最多吃 64 MB。** 分段是為了不再把過大的長度交出去；上限是因為它前面還有
三道天花板：那麼大的緩衝區本身就是記憶體尖峰、轉字串會超過 node 的字串長度上限、真的送出去也會被
伺服器擋掉。超過上限就停在某一行的結尾，下一次從那裡接著讀，所以積了很久的檔案會分幾次排空，
而不是永遠卡在同一個地方。

**切點改成在位元組上找，不是在文字上找。** 一旦讀取可以停在任意位元組，遲早會停在一個多位元組
字元的中間；先解碼再算長度的話，那幾個位元組會變成一個替代字元，而它自己的編碼長度是 3，位置
記錄就這樣一點一點偏掉。換行符不可能是任何多位元組字元的一部分，所以改用它當唯一的地標。

**壞掉的位置記錄也不會再讓它讀 NaN 個位元組。** 存下來的位置只要不是合法的非負整數就從頭讀，
重複的部分伺服器本來就會去重。

**還有一種罕見情況會直接說出來**：如果單獨一行就超過上限，這個檔案會以看得懂的錯誤被跳過並記進
紀錄檔，而不是每次都回報「沒有新東西」然後永遠不動。

### 排程是 Ready，不代表它跑起來是好的

同一台機器的自我檢測，把 `scheduler` 這一項判成通過，理由是「Task Scheduler state=Ready」。
Ready 的意思只是「這個排程存在而且沒被停用」，跟「它最後一次跑完的結果」是兩個問題。

而答案其實一直躺在同一份回報裡：`LastTaskResult` 是 `0x86`，也就是 SIGABRT ——
正是上面那個崩潰留下的退出碼。沒有任何一段程式讀過它。

**現在會讀了。** 0 是成功，`0x000413xx` 那一段是排程系統在講自己的狀態（還沒跑過、正在跑、
沒有下一次），其餘一律當作我們啟動的那個程式的退出碼，判成失敗，並附上去哪裡看它印了什麼。
讀不到就不判 —— 會對「看不出來」發警報的檢查，只會訓練大家忽略它。

## v1.26.161 — 窗戶只關了一半

v1.26.154 給指令這條路加了節流：同一個 session、同一種操作，完整清單一小時只列一次。
程式算出一個 `listing` 值代表「這次是那個一小時一次的完整輸出」，然後把它用在**一個地方**：
數字那行裡的名字要不要跟著出去。

底下那個 ⚠️ 規則清單沒有被它管。它的條件只有「有比對到東西嗎」。

實際跑出來長這樣。兩個 `gh issue comment` 差一分鐘：

```
第一次  [OwnMind v1.26.160] … Memories found: Team standards 3/32, Iron rules 9/150, …
          Team standards: 發布網頁到 pages.fontrip.com, …      ← 名字
        ⚠️  IR-154 / IR-111 / IR-143 …（九條）                 ← 清單

第二次  [OwnMind v1.26.160] … Memories found: Team standards 3/32, Iron rules 9/150, …
                                                               ← 名字正確地被擋住了
        ⚠️  IR-154 / IR-111 / IR-143 …（九條）                 ← 清單照印
```

第二次那個節流**確實在運作**——它把名字擋下來了。然後底下把同樣那九條又印了一次。

所以讀數字那行會覺得節流有效，看螢幕會覺得沒效。
**一個回報自己成功、而被它擋的東西照樣過去的防護，比沒有防護還糟**，因為沒有防護是看得見的。

### 改了什麼

**⚠️ 清單改成看同一個 `listing`。** 一行。
它跟名字本來就是同一個決定，拆成兩個判斷才是讓長的那半漏出去的原因。

**那行字加上排版指示。** 現在它會要求用「引用區塊 ＋ 斜體」呈現，理由是它應該退到回覆後面，
而不是跟使用者要的答案搶版面。哪些語法有效是實際量過的，不是猜的：

| 寫法 | 結果 |
|---|---|
| `<sub>` / `<small>` | 沒反應，終端機的 markdown 做不出字級 |
| 行內 code | 有效，但會被主題上強調色，**反而更大聲** |
| 斜體 | 有效，不改顏色 |
| 引用區塊 | 有效，變淡加左側直條 |

排版指示放在「叫 AI 轉述」那句裡，不是包在字串外面：節流那條路會在同一個字串後面接東西，
markdown 寫進字串會讓收尾的 `*` 卡在句子中間。

**編輯路徑五個中文字串改成英文原稿，而且每一個都配了轉述指示。**
最要緊的是 `· 本小時第 N 次`：它被接在一句「請你翻成使用者的語言」的話後面，自己卻已經是中文。

「配轉述指示」這半是審查退回來才補的。第一版只把字串翻成英文就送出去，
但這個專案的規矩是**英文原稿 ＋ 一句叫 AI 翻譯的指示**，兩件事缺一不可。
只做前半，等於把一個中文使用者本來讀得懂的訊息換成他讀不懂的英文——比原本的中文還糟。
漏掉的有三處：舊伺服器的節流單行、⚠️ 框框的頭尾、狀態寫入失敗的提示，
它們都不在「請轉述上面那一行」的涵蓋範圍內。

順帶修掉一個從 v1.26.154 就在的錯位：`· 本小時第 N 次` 是接在**轉述指示後面**，
而那句指示只交代「數字跟版號要照抄」，沒交代這個。照著做的 AI 會把它丟掉。
現在它進到句子裡面，指示也改成「數字、次數、版號都要照抄」。

使用者自己寫的鐵律標題一個字都沒動，那是使用者的資料，而且框框的指示裡明講不准翻。

### 測試

三支新的，加起來 15 個案例。改動前跑過：該紅的紅。

順手修掉一個測試污染：`hook-context-five-categories` 那支跑渲染器時沒有隔離狀態檔，
一直在讀寫**開發者自己**的 `~/.ownmind/state/edit-reminder.json`。
以前只是留下垃圾，這次改完之後，它的斷言會變成「取決於你上一個小時做過什麼」。

### 影響範圍

兩支 hook、兩個共用檔、三支新測試、兩支既有測試跟著改斷言。
每小時一次的節流本身、數字那行的內容、⚠️ 框框的格式，都沒變。

指令路徑的舊伺服器相容分支（`ownmind-render-context.js` 那段 `/type/iron_rule`）完全沒動，
包含它的中文。**編輯路徑的舊伺服器分支有動**：那條節流單行從中文改成英文原稿＋轉述指示。
兩者不一樣，是因為前者整段是要淘汰的相容碼，後者跟新路徑共用同一個函式。

## v1.26.160 — 有找到的，就要念出名字

v1.26.154 讓提示訊息多了「每小時列一次名單」：數字底下把比對到的記憶名字列出來。
當時三個呼叫端都把**鐵律**從名單裡拿掉，理由是下面那個 ⚠️ 框框已經印過了，
同一份清單隔三十行印兩次，會被讀成兩件不同的事。

用了一天之後，實際長這樣：

```
[OwnMind v1.26.159] Memories found: Team standards 8/32, Iron rules 2/4, …
  Team standards: config-placement-rule, 寫入/部署要驗證落地, 不要 blind edit, …
```

**有比對到的類別都念了名字，只有最重要的那個沒有**——它只顯示 `2/4`，然後就沒了。
整段看下來，鐵律看起來像是「什麼都沒找到」的那一類。

當初「怕重複」的理由沒錯，錯的是**搞錯名單在回答什麼問題**。
名單回答的是「OwnMind 找到了什麼」；框框不是在回答這個問題，**框框是用來擋你的**。
兩件不同的事，重疊不算浪費。

### 改了什麼

**三個呼叫端全部改成整份傳過去，鐵律也在裡面。** 各一行。
負責排版的那支本來就沒意見——你給它什麼它就列什麼——所以共用程式只有註解要改。

**框框完全沒動。** 一樣的框、一樣的編號、一樣「AI 回話第一行要複誦」的規定。

**補了一個測試，這才是重點。**
原本那個排除鐵律的邏輯是三個檔案各一行，而且**沒有任何測試在驗它**——
刪掉不會有任何一條變紅，這就是「決定」變成「意外」的過程。
新的 `tests/names-include-iron-rules.test.js` 三個呼叫端各測一次，
驗鐵律同時出現在名單**和**框框裡。
拿改動前的程式跑過：四個案例紅了三個。

三個都測，是因為「三份實作只改了兩份」是這個專案一週內找到六次的同一種毛病。

### 影響範圍

三支 hook 各一行、一個共用檔的註解、一支新測試。
每小時只列一次的節流、數字那行、框框，都沒變。

## v1.26.159 — 5264 個沒人負責的資料夾

v1.26.153 修好了一支會亂丟垃圾目錄的測試檔，也寫了一個防護盯著**那一支**。
那次的工作紀錄自己寫得很清楚：

> **把 `tests/` 其他檔案也查一遍。** 該做，但不在這次範圍。

2026-08-13 把那一遍查完了。這台跑測試的機器，系統暫存資料夾裡躺著：
...
**5264 個目錄、10 種前綴，最舊的六天前。**
v1.26.153 修掉的是至少 11 支漏水的檔案裡的 1 支，
另外 10 支之所以看不見，是因為**一個空目錄不會弄壞任何東西**——
沒有測試變紅、沒有日誌講話，唯一的症狀是暫存資料夾一直長，長到有人剛好去數。

這是一週內第五個同樣形狀的毛病：**解法早就有了，只蓋到一部分，
而且沒有任何東西會說它沒蓋到哪些。**

### 改了什麼

**要暫存空間只剩一條路。** `tests/helpers/temp-dir.js` 提供 `tempDir(前綴)`，
借出目錄，並在借它的那支檔案跑完時收回去。
清理的邏輯集中在一個地方，不再是 80 支檔案各自要記得的習慣。

**80 支檔案、106 個地方全部改掉。**
`fs.mkdtempSync(path.join(os.tmpdir(), 'x-'))` 一律換成 `tempDir('x-')`。
本來就有自己收尾的檔案保留原本的收尾，不會少任何東西——同一個目錄刪兩次不算錯。

**一個讓「偷跑」變大聲的防護。** `tests/no-unregistered-temp-dir.test.js`
只要有任何 `tests/` 底下的檔案自己去要暫存目錄就會失敗，並且指名是哪一支。
它還會驗自己兩件事：一是它的比對規則還認得出它禁止的寫法
（否則哪天改壞了，它會因為什麼都沒比對到而變綠），
二是那條唯一許可的路真的會收乾淨——做法是另開一個行程、把它的暫存位置指到一個空資料夾，
先確認那個行程真的有跑，再看那個資料夾裡還剩什麼。

改 80 支檔案就是修 80 支檔案。**防護才是用來擋第 81 支的**——
畢竟這 10 支漏水的檔案，全都是在第 1 支被查清楚之後才寫出來的。

### 那 5264 個

刪掉了。這部分是一個下午的打掃，不是這一版的重點；
沒有那支共用工具和那個防護，它們一週內就會長回來——本來也就是一週長出來的。

### 影響範圍

`tests/` 以內。沒有動到產品程式。

## v1.26.158 — 藥早就配好了，只發給一半的人

v1.26.143 量出了一個在測試套件裡飄了兩版的紅字：

`listen(0)` 是向系統要「任何一個空的埠號」，大概每幾百次會拿到一個**在規格封鎖清單上**的
（5060、6000、6566 那些）。`fetch` 一律拒絕這些號碼，不管有沒有東西在聽。
當時就寫好了解法 `tests/helpers/app-server.js`——抽到就重抽——
而且那個檔案自己的說明寫著「搬檔案是機械式的」。

**有 12 支檔案從來沒搬。** 2026-08-12 做全功能實測時，其中一支在整套跑的時候紅了：
...
被測的程式沒問題。斷言也沒問題。**只是那個埠號在一張清單上。**

這跟那天找到的其他毛病是同一個形狀（測試留下的垃圾目錄、從沒響過的規範、
中文搜尋回 0 筆、沒人檢查的標籤）——**防護存在、只蓋到一部分，
而且沒有任何東西會說它沒蓋到哪些。**

### 改了什麼

**12 支全部改用共用的那支 helper。** 每一支的寫法都不一樣
（有的每次請求開一台、有的在 `before` 開、有的包在 Promise 裡、
有的整個請求都寫在 listen 的回呼裡），所以是一支一支改的，不是套樣板。

**加一條守門測試。** `tests/no-raw-listen-with-fetch.test.js`：
只要有測試檔同時用了 `.listen(0)` 跟 `fetch(` 卻沒有引用那支 helper，就紅。

**守門測試才是這一版真正的產出。** 搬 12 支只是修好 12 支；
守門是為了擋住下星期寫的第 13 支——而這 12 支**本來就是在問題解決之後才寫出來的**，
之所以做得到，正是因為沒有東西在檢查。

**守門測試會講出自己的前提**：它會斷言「`fetch` 真的會拒絕 6000 埠」。
哪天執行環境改了行為，這條會紅並說明原因，helper 跟守門就都可以刪掉——
而不是讓前提悄悄失效，留下一段沒人講得清楚為什麼要重抽的程式碼。

### 刻意沒改的

`bare-mount-trailing-slash` 裡留了一個原始的 `listen(0)`，
那段是**直接往 socket 寫一行手工的請求**。原始 socket 沒有封鎖清單，
那是 `fetch` 才有的規則。守門測試接受這支檔案，因為它有引用 helper——
規則是「這支檔案不能用 fetch 打沒檢查過的埠號」，不是「這支檔案不准呼叫 listen」。

## v1.26.157 — 那張「認得哪些字」的表，從來沒人在寫入時查過

`TRIGGER_TAG_ALIASES` 決定掛勾認不認得一個 `trigger:` 標籤。
在這之前，**全專案只有一個地方讀它**——掛勾決定要不要跳出來的那一刻。

**存記憶的時候完全不查。**

所以貼一個系統不認得的標籤，會被開開心心存起來、回報「已儲存」，然後**永遠不會被問到**。

2026-08-12 對正式帳號實測：**9 條記憶**貼了一種動作都對不上的標籤。
其中一條是「對外送出前先獨立審查」——存在兩個星期，**一次都沒響過**，
而那天下午開了一張 issue，它沒有出現。

它能活這麼久，是因為**兩端都看不出有問題**：
...
這跟同一天找到的其他毛病是同一個形狀（測試留下的垃圾目錄、從沒響過的規範、
中文搜尋回 0 筆）——**沒有一個會報錯**。
這一版是第一個修「讓它們能安靜發生的那個機制」，而不是修其中一個案例。

### 改了什麼

存記憶與更新記憶的時候，把 `trigger:` 開頭的標籤拿去對照那張表，認不得就當場講：
...
**兩條路都要，不是只有新增。** 更新的時候 `tags` 是**整組取代不是合併**，
所以更新正是「原本會動的標籤被弄掉」或「死標籤被帶進來」的那個時刻——
那天下午補標籤走的就是這條路。

### 幾個決定

**只警告，不擋下來。** 那 9 條裡有 7 條是**刻意不標**的：
它們管的是一整類工作（收工、除錯、開新專案），不是一個動作，
本來就靠開場載入的規範標題被找到。擋下來會讓那幾條**因為任何不相干的理由都改不動**，
那比原本的問題更糟。

**訊息講「會發生什麼」，不是講「你錯了」。** 因為通常沒有錯。
靠名字被找到的記憶本來就不需要時機標籤——所以那句話最後會說「可以不加」。

**offending 的標籤原字照抄。** 正規化過會害作者去找一個他從來沒打過的字串。

**認得的字彙從那張表推導出來，不另外寫一份。**
手寫第二份清單就是同一個缺陷升一層：它會跟表分家，然後開始警告那些其實會動的標籤。
測試會走過表裡每一個字，確認這個檢查接受它、**而且** `ruleMatchesTrigger` 仍然認它。

### 沒做的

**它不會去找已經帶著死標籤的那些記憶。** 這版只擋住「以後新的不會安靜地壞掉」。

**沒有任何東西會掃過現有記憶、告訴你「這條永遠不會響」**——
那 9 條是我用手一條條挖出來的。那個偵測器是下一件值得做的事，不在這一版。

## v1.26.156 — 搜尋用空白切字，而中文沒有空白

在一次全功能實測中發現的，不是測試抓到的。
...
存進去的標題是「[團隊] 收工**／交接**六項自檢」。**憑印象打，少打中間兩個字，就完全找不到。**

`tokenize` 是用空白切開查詢字串的。英文本來就是分開的——「wrap up」是兩個詞，中一個就算。
**中文沒有空白**，整句話變成一整塊，被當成一個連續字串去比對。
使用者必須一字不差地重現存起來的其中一段。

這件事比一般的「搜尋不夠好」嚴重，因為：

**「找不到」跟「你根本沒有這條記憶」長得一模一樣。**
而這個產品的全部主張就是它記得。半記半忘地問一句就得到「沒有」，
是它唯一輸不起的那種壞法。

### 改了什麼

整塊比對不到的時候，改用**兩字一組、互相重疊**的窗格再試一次：

```
收工六項自檢  →  收工  工六  六項  項自  自檢
```

對上那個標題，五組中四組落在裡面，只有「工六」被中間的「／交接」切斷。
門檻設在**六成**（五組要中三組），所以過。

**適用範圍抓得很窄：**

- 只有**含中文**、而且**至少四個字**的詞才會用窗格。
  兩個字就是詞本身，三個字不足以當證據。英文本來就用空白分開，完全不碰。
- 只比對**標題與內文**，絕不碰 `code` 與標籤。
  那兩個放的是識別碼（`IR-003`、`trigger:commit`），部分吻合毫無意義。
- **每個欄位分開數。** 窗格散落在標題和一段不相干的內文裡，
  不能算成「這條就是你要的」。

**線上跟離線一起改。** 兩邊本來就共用 `shared/memory-search-tokens.js`，
就是為了不讓它們講不一樣的話；線上那份是同一條規則寫成 SQL 的數數版本。

### 為什麼不做同義詞表

使用者直接問的就是這個：收工 / 收尾 / 結束 / 下班 / 88 / bye 要不要收錄。**先量再決定：**

| 查詢 | 找得到嗎 | 為什麼 |
|---|---|---|
| 收工 | ✅ | 標題裡有 |
| 收尾、下班、wrap up | ✅ | **規範自己的第一句就列了這些詞** |
| 結束、88、bye、要走了、工作做完了 | ❌ | 沒人寫過這些字 |

所以同義詞問題比看起來小——**寫規範的人本來就會自己盤點觸發詞**。
而同義詞表只是把負擔從「作者要想到」換成「維護表的人要想到」，
而且**永遠救不了「工作做完了」這種整句話**。所以沒做。

### 沒做：語意搜尋（值得寫下來，因為地基早就在了）
...
**沒有任何一行程式在寫這個欄位。** 每一筆記憶的 `embedding` 都是空的，
索引從 schema 寫下那天起就在空轉。有人第一天就設計了語意搜尋，然後停在那裡。

它是唯一能接住「工作做完了」的做法，但需要：接外部服務算向量、
把現有每一筆補算一次、每次寫入重算、還要有算不出來時的退路——而且**按次計費**。
2026-08-12 因為成本因素暫緩。

## v1.26.155 — 有一條團隊規範，從來沒有響過

**「對外送出前先獨立審查」**——給客戶或主管的報告、對外文件、公開發布的內容、
**issue 或 PR 的正式回覆**，送出前都要先換一個獨立視角重看一遍。

它自己寫的理由是：**送出去就收不回來。錯字可以改，錯的事實跟不該講的內部細節改不掉。**

寫它的人貼了 `trigger:send`。**但系統從來沒有問過這個標籤。** 2026-08-12 對正式帳號實測：

```
gh issue create   → （沒有中）
gh issue comment  → （沒有中）
gh pr create      → （沒有中）
```

當天下午開了一張 issue，這條規範沒有出現。

而且它不孤單。把帳號上每一個 `trigger:` 標籤掃過一遍，有 **9 條記憶貼了標籤卻一種動作都中不到**。
這種壞法兩頭都看不見：寫的人看到自己貼的標籤還在，看提醒的人只會看到某一類的數字比較小。

### 改了什麼

**新增一種動作：「對外送出」。** `gh issue|pr create|comment|edit|review|close|reopen`
與 `git send-email`。

**故意抓得窄**——`gh issue list`、`gh pr view` 沒有送出任何東西。
在每一次列清單前面都跳提醒，只會讓人學會跳過它，真正要提醒的那次也一起跳過了。

**沒有併進「部署」，雖然那樣比較省事。** 名稱是給人看的，
你只是回一則 issue 留言卻跳出「本動作為部署工作程序」，
跟以前有人 curl 帶金鑰、畫面跳「鐵律觸發（install）」然後問「到底在裝什麼」是同一個毛病。
這個動作叫**「對外送出」**。

**`gh release create` 歸「部署」，不是「對外送出」。** 它發布的是一份建置產物，
把它叫成對外送出是同一個貼錯標籤、只是方向相反。
...
同一個字，兩件事，而**寫記憶的人不會知道系統把它歸到哪邊**。

所以 `publish` / `發布` / `發佈` **兩邊都算**。

多算一邊的代價是：在用不到它的那個動作上多一行字。
少算一邊的代價是：該提醒的那條，在它當初就是為了那個時刻而寫的動作上，安靜不動。
這條規範自己給了判準——**送出去就收不回來。**

（部署專用的字 `部署`、`上線`、`升級`、`kubectl` **沒有**跟著過去，
不然每條部署規範都會在你回 issue 留言時跳出來。）

### 另外 7 條為什麼不修

它們貼的標籤講的是**一整類工作**，不是一個動作，擴充字彙表救不了：

| 記憶 | 標籤 | 為什麼 |
|---|---|---|
| 回話要用白話中文那條鐵律 | `reply` `language` | 「回話」不是工具呼叫，掛勾看不到 |
| 收工／交接六項自檢 | `wrap-up` `handoff` | 是使用者開口講的，沒有指令 |
| 系統性除錯 | `bug` `debug` | `debug` 是**故意**被排除的 |
| 學到東西全層同步 | `save` `learn` | 沒有對應動作 |
| 專案架構規範 | `architecture` `design` | 沒有對應動作 |
| company host onboarding | 7 個自創標籤 | 沒有對應動作 |
| GitLab 移交 | 4 個 gitlab 系列 | 沒有對應動作 |

**它們本來就有另一條路在管：** 開場會載入全部團隊規範的標題，
所以你一講「我要收工」，那條規範就靠名字被找到。
這些標籤是多餘，不是壞掉。

**要讓它們出現的唯一辦法是標成 `trigger:command`（什麼都中）**，
那就等於七條全部擠在每一次 commit、每一次改檔案前面——正是 v1.26.151 移掉的雜訊。

順帶：`trigger:push` 看起來像漏了，但貼著它的三條已經靠別的標籤中了 commit，
補下去今天不會有任何差別，所以沒補。

## v1.26.154 — 那個數字沒有讓任何人去讀它

v1.26.151 把「鐵律觸發（commit）」換成五類記憶的條數，v1.26.152 讓它能翻譯。
數字回答了「OwnMind 到底有沒有去查」。**但它沒有讓任何一條規範被讀到。**

在這個 repo 上量到的，就在上一版：commit 前面那行寫著

```
[OwnMind v1.26.153] Commit · Team standards 4, Iron rules 0, …
```

讀到這行的 AI，四條**一條都沒點開**。其中一條是**「Commit 前品管三步驟」**，
第二步是「commit 前先請求 code review」。那一步被跳過了，commit 照樣送出去。
數字是對的、位置很顯眼、完全沒有作用。

使用者看到同一行的第一個反應是另外半個問題：**「這數字對嗎？我以為它會列出我全部的記憶。」**
`4` 分不出「有四條管到」跟「總共只有四條」。兩個問題其實是同一個形狀——
**一個沒有東西可以對照的數字。**

### 改了什麼

**加分母。** `團隊規範 4/32`。總數是本來就查回來的那批列數出來的，沒有多打一次資料庫。

**把名字列出來。** 不只鐵律，每一條中的記憶都列名。名字比數字難視而不見，
而且你要說「不對，不是那條」的時候，需要的是名字。

**加一個時間窗，讓名單不會變成雜訊。** 名單一小時出現一次，
**每個對話、每種動作各自算**。中間的每一次操作只印數字那一行。
用的是改檔案那條路從 v1.26.92 就有的機制，不是再寫一套。

**時間窗的鍵值加上動作種類。** 本來只有對話。
管到 commit 的記憶跟管到部署的記憶不是同一批，共用一個窗等於
10:00 看過 commit 名單之後，10:20 的部署名單被當成「已經看過了」——它沒有。

**措辭改成實際發生的事。** 是「已查到」，不是「已載入」：
掛勾只是數過、列名，並沒有把內容讀進誰的腦袋裡。
使用者原本提的是「已載入」，聽過這行字實際做了什麼之後改成「已查到」。

### 幾個做了決定的地方

**名單不設上限。** 真實帳號 commit 會中大約 38 條鐵律，
而「…另有 28 條」是 28 條沒有人能拿它做任何事的記憶。
時間窗才是用來換長度的東西；截斷只會變成第二種、更安靜的遺失方式。

**沒中的類別有數字、沒有名單那一行。** `個人偏好 0/4` 照印——
那個 0 就是有意義的部分，也正是 issue #94 要分辨的事。
但一個底下什麼都沒有的標題不是。

**時間窗以對話為單位，不是以電腦為單位。** 使用者原本要求以電腦為單位，
看到後果之後改了：名單存在的目的是把名字放進**某一個** AI 的腦袋，
而另一個從沒看過名單的對話，會被當成看過了。

**鐵律的名字只會出現在一個地方。** 橫幅要印或名單要印，
只有呼叫端知道，所以 `names` 是傳進去的，不是在渲染那層決定的。

### 這次撞到自己家的兩道檢查

程式碼註解裡寫了具體鐵律編號、也寫了擁有者的名字，兩件都是這個 repo 本來就在擋的，
被既有測試抓到，不是被人看出來的。已修。
...
其中 5 條有明確的時機（3 條 commit、1 條部署、1 條改檔案），本來要在這版一起補完，
但**開發用的這個帳號讀得到、寫不了**——它不是這些規範的擁有者，也不是管理員，
每次寫入都是 404。需要有權限的帳號來補。

另外 5 條**刻意不標**。它們管的是一整類工作（把專案上線、跑一次審查、開一個新專案），
不是一個動作。唯一會讓它們出現的標籤是「什麼都中」，
那就等於每次 commit 都跳出來——正是 v1.26.151 要移除的雜訊。

## v1.26.153 — 一支測試每跑一次就留 23 個垃圾目錄，一天累積 391 個

`tests/sync-rules-block.test.js` 每條測試建一個暫存目錄，**一個都沒刪過**。

2026-08-12 在跑這套測試最勤的那台機器上量到：跑一次留 **23 個**，
`C:\Users\Vin\AppData\Local\Temp` 裡躺著 **391 個**，最舊的是前一天的。

沒有東西壞掉，而那正是它活這麼久的原因。空目錄不會讓任何斷言變紅、不會明顯拖慢什麼、
也不會出現在任何日誌裡。唯一的症狀是暫存資料夾一直長，直到有人剛好去看一眼
—— 這次是收工自檢時翻到的，不是任何測試報出來的。

這個 repo 其他測試檔本來就都會清（`fs.mkdtempSync` 配 `finally` 裡的 `fs.rmSync`，
`tests/update-lock-mutual-exclusion.test.js` 一個檔案裡就做了十五次）。只有這支沒有，
而且沒有任何機制會發現。

### 改了什麼

`fixture()` 把每次建的目錄記進一份清單，檔案層級的 `after` 掛勾統一刪掉。

**沒有採用「每條測試各自 `t.after()`」**：要拿到 `t` 就得幫 23 條 `it()` 都加一個
它們用不到的參數，而在一個檔案裡對每條測試做機械式修改，本身就是另一種出錯的方式。
`after` 不管測試過或不過都會跑，這正是關鍵 —— **斷言失敗的那一次才是漏最兇的時候**，
因為它是從中間跳出去的。

清理失敗會講出來（寫 stderr，但不讓整套測試變紅）。安靜地吞掉等於把原本的缺陷原樣裝回去：
目錄照樣累積，上面還掛著一條寫著「不留東西」的綠燈測試。

### 守門測試為什麼另開一個檔案

新增 `tests/sync-rules-block-no-temp-leak.test.js`：把受測檔案當子行程跑，並用
`TMPDIR` / `TEMP` / `TMP` 把它的暫存位置導到一個空目錄，跑完之後那個目錄裡剩下什麼，
就正好是它漏掉的東西。**受測檔案不需要為了被量測而改任何一行。**

它不能放在被量測的那個檔案裡：測試沒辦法觀察自己的清理掛勾，那是全檔跑完才執行的。

**它自帶一個反向檢查。** 子行程如果根本沒跑起來（路徑錯、語法錯、篩選條件沒中），
留下的也是一個空目錄，漏洩斷言照樣會過，而它什麼都沒量到。所以守門測試會先確認
「這一趟真的跑了，而且跑的是整個檔案（至少 20 條過）」，才相信目錄講的話。

這個反向檢查馬上就派上用場。第一版守門測試就是紅在「分不出子行程有沒有跑」：
Node 的測試執行器會在每個測試行程裡設 `NODE_TEST_CONTEXT=child-v8`，
從測試裡再開一個 `node --test` 會把它繼承下去，子行程於是從給人看的格式切換成
序列化的內部協定 —— 輸出裡根本沒有 `pass N` 這一行。現在會把
`NODE_TEST_CONTEXT` 跟 `NODE_TEST_WORKER_ID` 從子行程環境裡拿掉。

### 兩個方向都驗過

| | 沒修的版本（`HEAD` 開在另一個工作區） | 修好的版本 |
|---|---|---|
| 守門測試 | ✖ `23 temp entries survived the run: ownmind-block-0G9KbR, …` | ✔ |

沒修的那一側是把 `HEAD` 檢出到獨立工作區、只把新的守門測試複製進去跑的，
所以受測的是原本那個檔案，不是我重建的仿製品。

那 391 個累積的目錄，是在數量與最舊日期都記錄下來之後才刪的（IR-003）。

### 沒做的

**沒有把 `tests/` 其他檔案掃一遍找同樣的問題。** 值得做，但不在這次範圍：
這次的修法跟守門都是針對單一檔案的，全面清掃是另一件事、風險也不同。
這次加的守門測試可以當成那件事的樣板。
...
1. **OwnMind 覺得你在做什麼？**
2. **它這次到底有沒有真的看你的記憶？** —— 每一類各撈到幾條。
3. **想深究的話怎麼查？**

三件都答不了。它只印 `鐵律觸發（install）`：一個內部代號、沒有條數、沒有下一步。

而第 2 點**在資料層就辦不到**：兩支掛勾都只打 `/api/memory/type/iron_rule`，其他四類
**從來沒被撈過**。所以使用者無法分辨

- 「這次操作沒有任何團隊規範適用」，和
- 「團隊規範根本沒被查」。

這是兩件不同的事，其中一件是缺陷，而那個顯示在結構上就沒辦法講出是哪一件。

`install` 這個標籤是同一個毛病的另一面。有人在 production 上除錯 auth header，每一條帶
`X-API-KEY` 的 curl 後面都掛 `鐵律觸發（install）`，他來問「我又沒在安裝東西」。他沒有 ——
`install` 是內部分類，剛好把碰金鑰也裝在裡面，本來就不該拿去給人看。

### 改了什麼

**新端點 `GET /api/memory/hook-context?trigger=X`**，一次回全部五類，而且已經在伺服器端
依 trigger 篩好：各類條數，加上鐵律的標題。

**一次請求，不是五次。** 這東西擋在每個危險指令前面，shell 掛勾那支 curl 是同步的、
上限 5 秒，五支循序跑最糟就是在 `git commit` 前面卡 25 秒。這種延遲的下場就是整個機制被關掉，
而被關掉的安全機制什麼也擋不住。

**四類只給條數，不給內容。** 鐵律保留清單。五類的內容全送等於在指令前面塞幾百行標題，
那是提醒變成大家自動略過的東西的方式 —— 代價比那四份清單本身還大。缺口由「怎麼查」那行補：
它會告訴你怎麼把任何一類叫出來看。

**未標適用時機的記憶不再淹進來。** 「沒標籤 = 適用全部」這個既有契約保留，鐵律照舊。
但它撐不住另外四類 —— 那四類本來就沒標 trigger，因為從來沒人要求它們標。實測（同一帳號，
150 鐵律 / 32 團隊規範 / 33 程式規範 / 92 工作原則 / 14 個人偏好）：
...
多出來的全是跟當下操作無關的筆記。改嚴之後**鐵律一條都沒少**（commit 33/33、install 12/12、
edit 71/71），這才是它安全的原因。

**shell 掛勾裡那段內嵌 `node -e` 的渲染搬出去了**，變成 `hooks/ownmind-render-context.js`。
那段內嵌就是 shell 版必須自己抄一份 `TRIGGER_TAG_ALIASES` 的原因：`node -e` 裡要 import 模組
就得把路徑內插進原始碼，而那正是兩次 Windows 靜默失敗的成因。**用 argv 把檔案路徑傳給 node
是另一回事**，這支掛勾本來就這樣跑它其他 helper。所以那份副本沒了，兩支掛勾現在印同一行字，
不再是同一個意思的兩種翻譯。

### 伺服器還沒更新怎麼辦

掛勾和伺服器是分開部署的，版本本來就常常不一致。一支只認得新網址的掛勾，碰到還沒更新的
伺服器就會沉默，而沉默跟「沒有規則適用」長得一模一樣 —— 這個專案一直在防的就是這件事。

所以只要 `/hook-context` 回的不是 200，就退回 `/type/iron_rule`（v1.19 之後每台伺服器都有），
並且把這次退回寫進活動記錄（`hook_context_fallback`）—— **兩支掛勾都記**。原本只有 shell 那支記，
那會讓「記錄有沒有出現」取決於平台裝的是哪一份：在 Linux 上「沒有 fallback 事件」代表伺服器正常，
在 Windows 上什麼都不代表。**一個永久降級卻從不吭聲的提醒，
看起來就跟正常的一模一樣。**

退回時**不會**把另外四類印成 0。0 的意思是「查了，沒中」；那四類是根本沒被問。印出來就是
用這個修正去講同一個謊。

### 兩個當場拍板的問題

**條數 0 的類別要不要印 → 印。** issue 說這跟編輯提醒「0 條就不要說話」相反。其實不衝突：
那條規則講的是**總數**（沒東西可講就別講），這裡講的是**已經有內容的清單裡的其中一列**。
那個 0 本身就是資訊。在一個 14 條個人偏好只有 2 條標了時機的帳號上，`個人偏好 0`
講的是標籤的實話 —— 而那正是舊顯示造不出來的句子。

**「怎麼查」那行只掛在低頻 trigger 上。** 指令路徑沒有 session 狀態可以節流，
掛在 commit 上就會每次提交都印一次，提示就變壁紙了。

### 測試

`npm test`：4767 條，紅 2 條 —— 都是 `bare-mount-trailing-slash`（需要被 gitignore 的
`src/public/dashboard/`），與這次無關。

有兩個舊測試是用「掃 shell 檔的原始碼字串」在守的，而那些字串搬家了。它們改成**跑真的模組**；
其中別名那個從「比對兩份副本是否一致」改成「斷言根本沒有第二份」—— 這是比原本更強的保證。

### 還沒做

**類別顯示名稱的 i18n。** issue 說走 #91 講的路線，但 #91 已關閉、內容是把 credential 從
install 拆出來跟指令路徑節流，裡面沒有 i18n 路線可循。標籤目前是
`shared/hook-context.js` 裡一張表的中文字串，翻譯層要接就是接在那裡。

## v1.26.150 — 兩份判斷變成一份（issue #92 收尾）

v1.26.149 讓兩份指令判斷講出同一個答案，但**沒讓它們變成同一個東西**。兩份還在，還是可以
被改到分岔，守門測試也只有在有人跑的時候才會發現。剩下的問題只有一個：shell 那份到底還需
不需要在「沒有 node」的機器上跑？

**答案是不需要，而且早就不需要了。** `hooks/ownmind-iron-rule-check.sh` 呼叫 node 十幾次，
第一次在檔案最上面、在所有判斷之前，只是為了從 `package.json` 撈一個版號字串出來：

```sh
VERSION=$(node -e "…package.json…version…")
```

讀金鑰、抓規則、解析輸入、跑驗證、印輸出，全都走 node。一台沒有 node 的機器拿到的不是「功能
打折的掛勾」，是**在第 23 行就掛掉的掛勾**。所以「消除重複」的代價其實一直都在付，而那份重複
什麼也沒買到。

這也讓 v1.26.149 留下的選擇題失效了：A 案（樣式抽成共用資料）只有在 shell 那份必須獨立時才
比較好。它不必獨立，所以 A 案是為一個不存在的特性多加一層解析。

### 改了什麼

- **新增 `hooks/ownmind-detect-trigger.js`** — 從 stdin 讀指令，印出 `detectCommandTrigger()`
  給的答案。它是一層包裝，不是第二份實作。
- **`.sh` 裡那條 grep 鏈整個刪掉**，改成把指令用管線餵進這支包裝。兩支掛勾現在走同一個函式。
- **兩支安裝腳本**的掛勾檔清單都補上這支新檔。
- **新增 `tests/helpers/hook-home.js`** — 把「幫 shell 掛勾搭一個拋棄式 $HOME」這件事收成一處，
  取代原本在五個測試檔各寫一遍的同一段。

指令走 stdin 不走 argv 是刻意的：`git commit -m` 的訊息常常是多行的，stdin 沒有 argv 的長度
上限，也沒有引號規則要處理。而且那支掛勾裡其他每一個 node 呼叫本來就是這樣做的。

失敗是被檢查的，不是被假設不會發生。包裝的離開狀態有檢查，非零會寫進活動記錄；stderr **刻意
不導掉** —— 一個安靜壞掉的判斷器回傳空答案，跟「這條指令本來就不觸發任何東西」長得一模一樣，
而後者是最常見的真實答案，也就是缺陷最好的藏身處。

### 這次沒改的

**觸發樣式本身。** 守門測試那 18 條指令，每一條的答案都跟 v1.26.149 完全一樣。這次動的是
「答案從哪裡來」，不是「答案是什麼」。

### 一個現場示範

改完第一次跑守門測試：**每一條指令都回「沒觸發」。** 測試用的假 $HOME 裡沒有那支新檔的連結，
所以 node 找不到模組。這正是這個缺陷的原本形狀 —— 掛勾沒有大聲壞掉，它安靜地把每條指令都判成
無關，而那看起來就跟平安無事的一天一樣。八個測試橫跨四個檔案同時紅，全部同一個原因：同一段
搭建程式被手抄了五遍，而沒人記得五個地方都要改。**這就是這次要消滅的那個缺陷，只是低了一層。**
所以那段搭建也收進 `tests/helpers/hook-home.js` 了。

### 測試

`npm test`：4745 條，紅 2 條 —— 都是 `tests/bare-mount-trailing-slash.test.js`，需要
`src/public/dashboard/`（被 gitignore、由 `scripts/ensure-console-build.js` 產生、乾淨的
clone 裡本來就沒有）。與這次改動無關，這次沒有碰任何路由或中介層。

## v1.26.149 — 同一個判斷寫了兩份，而它們講的不一樣（issue #92）

你要執行危險指令之前，會有一個掛勾先把相關鐵律列給你看。這個掛勾有兩份實作：一份走
`shared/helpers.js` 的 `detectCommandTrigger()`，一份是 `.sh` 裡自己手寫的一條 grep 鏈。

兩份都有存在的理由 —— `.sh` 不需要 node 在 PATH 上，也避開了「把路徑交給 node -e」這個
動作，那正是 v1.26.88 與 v1.26.90 兩次 Windows 靜默失敗的成因。

沒有的是**任何東西在確認兩份講的是同一件事**。`KEEP IN SYNC` 註記只涵蓋 `TRIGGER_TAG_ALIASES`
那張表，旁邊的漂移測試也只比對那張表。而 `install.sh` 在 mac 與 Linux 上註冊的是 `.sh` ——
**沒人守的那一份，正是大多數人實際在跑的那一份。**

拿 17 條指令同時餵給兩份，**7 條答案不一樣**。其中三個是實際的失效，不是見解不同：

- **`git tag` 在 mac / Linux 上完全不觸發。** 打版本標籤是版號同步那類鐵律唯一想講話的時刻，
  而掛勾在那一刻是啞的。
- **`docker compose build` 和 `docker compose push` 也不觸發。** `.sh` 的部署樣式是
  `docker.*deploy|docker.*up`，這兩個都不含。有人專門為某個指令寫了鐵律，而實際在跑的那份
  實作看不見那個指令。
- **`docker.*up` 會誤中。** `docker logs backup`、`docker ps | grep uptime` 都被判成部署 ——
  因為 `backup` 和 `uptime` 裡面有 `up`。只是看個 log，就被塞一整份部署鐵律清單。

### 先寫守門測試，再修

`tests/iron-rule-trigger-parity.test.js` 把那張表同時餵給兩份，答案不同就紅。**它是跑真的
那支 shell 腳本**，從掛勾自己印出來的橫幅把判斷結果讀回來，不在測試裡重抄 grep 規則 ——
重抄等於在「防止副本漂移」的東西裡面再造一份副本。先跑一次：7 條紅，正是 issue 預測的那 7 條。

然後 `.sh` 的判斷鏈改成 `detectCommandTrigger` 的逐條轉寫，連順序都照抄。參考實作是
`shared/helpers.js`：它是 `KEEP IN SYNC` 註記指名的那份、帶著每條樣式理由的那份、也是
本來就有測試的那份。

三個細節：

- **有一個樣式是往反方向搬的。** `docker.*deploy`（也就是 `docker stack deploy`）只有 `.sh`
  認得。拉齊的時候可以刪掉也可以升上去，選了升上去 —— Swarm 部署就是部署，刪掉等於在本來
  認得的平台上把它弄丟。
- **`del ` 拿掉了。** 那是 cmd.exe 的刪除指令，在 `.sh` 跑的平台上不可能出現，而 PowerShell
  真正的名字 `Remove-Item` 已經認得。留著就等於要教測試容忍一個差異，那是下一次漂移的開頭。
- **順序也是差異的一種。** `docker compose up -d && rm -rf ./old` 兩邊家族都命中，參考實作
  先判部署、`.sh` 先判刪除。這是這次唯一「哪個答案才對」屬於判斷而非缺陷的一列：跟著參考走，
  所以那也是每個 Windows 使用者一直以來看到的行為，是讓平台一致、不是發明第三種行為。

### 這一版沒有做的事

**重複本身還在。** issue #92 提的兩條消除路線（樣式資料化、或 `.sh` 改成呼叫 `.js`）都取決於
「`.sh` 不依賴 node」這個理由現在還成不成立 —— 那是決策不是實作細節，而且值得在守門測試已經
就位之後再談：兩條路線都得先通過它。

**兩份印出來的字還是不一樣。** `.js` 印英文、`.sh` 印中文，而 mac 與 Linux 上跑的是中文那份，
所以講英文或日文的使用者會拿到一行中文。那是 issue #91 的範圍。

## v1.26.148 — 提示句唸出你們公司自己「可以開口叫」的規範（issue #85）

每則回應最後那一行提示，原本有一句是：

> OwnMind has team standards: every member's AI follows the same rules automatically…

它講的是「這個機制存在」，不是「你可以叫它做什麼」。

真實案例（2026-08-11）：建了一條團隊規範「發布網頁到 pages.fontrip.com」，從那之後
任何同事只要說「幫我發 pages」，她的 AI 就會走完整條流程並回傳網址。**但除非有人口頭
告訴她，她永遠不會知道。** 而那行提示是產品每天唯一會跟她說話的地方。

### 為什麼不能直接唸標題

issue 最初的想法就是直接唸。拿正式機 32 條規範實際跑一遍，會變成這樣：

```
Tip: 你們公司有一條規範「不要 blind edit」，直接開口叫我做就行
Tip: 你們公司有一條規範「pages 發布工具 pages.py 全文（配合「發布網頁到 pages.」，直接開口…
```

逐條看過的分佈：17 條是紀律（AI 自己遵守，使用者不會開口講）、8 條是給 AI 讀的內容、
6 條才是使用者叫得動的。**六次有五次是雜訊**；而且標題是寫給管理規範的人辨識用的，
塞進句子裡會被截斷、語氣也不對。標籤也救不了 —— 那些是 `pages`、`commit` 這種關鍵字，
不是「幫我發 pages」這種人話。

### 改法：由規範自己說

一條團隊規範可以帶兩個欄位：這條使用者可以開口叫（`user_invocable`），以及要給他看的
那句話（`invocation_hint`，例如「想把東西變成網址傳給人看？直接說『幫我發 pages』」）。

- **標了卻沒寫台詞會被擋下來**（400，而且錯誤裡直接附上該怎麼寫）。沒有台詞就會退化成
  唸標題，那正是這組欄位存在的理由；而且會發現的人是看到提示的那個，不是設定的那個。
- **是取代不是新增。** 有標記的話，那些句子取代原本那句靜態的團隊規範提示。否則一間公司
  標六條，25 句的池子就有六句是它自己的；標二十條就把產品自己的提示淹掉了。
- **沒有標記任何規範的帳號，一個字都不會變。**
- 一律不猜。沒有標記的規範永遠不會出現在提示裡 —— 漏掉一條的代價是一個能力沒被宣傳，
  猜錯的代價是每一則回應都在對每一個人講一句他做不到的事。

### 兩輪審查抓到的：這個功能本來會「上線了但沒有人碰得到」

- **沒有任何地方告訴管理員這兩個欄位存在。** 工具說明沒寫、手冊沒寫、後台也沒有介面。
  管理員叫 AI「把 pages 那條設成可以叫的」，AI 手上什麼都沒有。現在兩支工具的說明都寫了
  這一組欄位跟範例句；順帶寫上「更新記憶的附加欄位是整包取代、不是合併」，因為這一版
  正是第一次會讓管理員手動去改規範附加欄位的改動。
- **每則回應的提示其實永遠不會變。** 原本只在 AI 呼叫 `ownmind_init` 時記住那份清單，
  但 Claude Code 是靠開場掛勾載入記憶、根本不會呼叫那支工具 —— 也就是使用者看最多的那個
  介面，清單會一直是空的。現在改成從掛勾寫在本機的那份資料讀，一個行程只讀一次。
- **多行的台詞會出現在畫面上。** 寫入時擋了換行，讀出來的時候只檢查空字串跟長度，而去頭尾
  空白不會動到中間的換行。那句話會被放進別人的 AI context、而且是「請照著轉述」的內容，
  等於可以夾帶沒有人寫過的行。現在讀寫兩邊套用同一組規則。
- 另外修掉：一句寫錯的錯誤訊息（說超長會被截斷，實際上是整條不顯示）、一句不成立的理由、
  三條「怎麼改都不會紅」的測試（含一個只擋得住某一種寫法的守衛），以及手冊裡那句會讓 AI
  懷疑公司句子的措辭。

### 這一版沒有做的事

程式碼不會去標記任何規範，那是資料，部署之後另外標。後台也沒有加編輯這兩個欄位的介面
（後台本來就沒有記憶編輯器），設定方式跟現在管理規範的方式一樣。


## v1.26.147 — 一條團隊規範，只有當初建立它的人改得動（issue #85）

要做 issue #85（提示句唸出使用者自己「可以開口叫」的規範）得先在每一條規範上標一個記號，
標第一條就撞牆。對正式機實測，用最高權限的帳號：**讀規範 108 是 200，改它是 404。**

那條規範全公司都讀得到，但除了當初上傳它的那個帳號以外沒有人改得動。

### 為什麼那道「只有管理員能改」的檢查從來沒跑到

每一支寫入的程式都是同一個開頭：先用「這一列是不是你的」去撈，撈不到就 404。而那句
「團隊規範只有管理員能編輯」寫在三十行之後 —— 不是他的東西的人，早在上一句就被判 404 了。

所以那道檢查實際上管的是**「非管理員改自己建的規範」**，而它看起來要管的那件事
（管理員可以管全公司的規範）從來不存在。

實際代價，正式機實測：32 條團隊規範分屬 2 個帳號，issue #85 要標的 6 條裡有 2 條在
一個人名下、3 條在另一個人名下，**沒有任何一個帳號標得完**。而且建立者離職之後，
他留下的規範沒有人改得動、也沒有人停得掉（停用同樣綁擁有者），還會繼續發給每個人的 AI
當成公司規定。另外，停用要管理員、重新啟用卻不用 —— 一道只有單向的門。

### 改法

管理員（含最高權限）可以寫入任何人建立的團隊規範跟它的段落。放寬的範圍就是**本來就全公司
讀得到的那兩種**，其他類型（鐵律、專案、環境）一律不變，而且被拒絕時回的東西跟「這個編號
不存在」一模一樣 —— 不能拿這支 API 去掃別人有哪些記憶。

- **那道本來跑不到的檢查現在真的會跑，內容一個字沒改。** 自己建的規範但不是管理員，
  照樣 403。
- **重新啟用跟還原也補上同一道檢查**（本來沒有）。還原是把舊版內容寫回去，那就是編輯。
- **寫入時綁的是那一列的擁有者，不是呼叫的人。** 綁成呼叫者等於把同一個 404 往後挪一句，
  而且變成安靜的：什麼都沒寫、也不會有人知道。
- **管理員動了別人的東西會留紀錄。** 更新、停用、啟用、還原都會把「誰動的、原本是誰的」
  寫進版本歷史；本人動自己的東西則完全不留這個欄位，既有紀錄的形狀不變。
- **版本歷史比照辦理**：能還原卻看不到有哪些版本可還原，等於只給了一半。

### 這一版沒有做的事

issue #85 本身還沒做 —— 提示句還是原來那句，那兩個新欄位還不存在，一條規範都還沒標。
這一版只是把「開始做」的那道牆拆掉。也沒有動讀取權限、沒有回填、沒有改動任何一筆正式資料。

### 兩輪審查抓到的：新加的那個紀錄，被記錄的人自己就能偽造

兩個審查員都試著找「有沒有辦法碰到不該碰的東西」，兩個都沒找到。但抓到四件別的：

- **「誰動的」那筆紀錄可以被偽造。** 記憶的附加欄位本來就是使用者自己寫得進去的，而寫版本
  歷史的時候是整包照抄過去 —— 所以自己在自己的規範上先塞一筆「某某管理員改過」，下次一存
  就進了歷史。稽核紀錄不能由被稽核的人來寫。現在照抄之前先把那個欄位拿掉。
- **那幾條測試不會失敗。** 它只檢查程式碼裡有沒有出現那個欄位名，所以把條件改成「永遠不記」
  也是綠的。現在改成盯住條件本身，並實測：條件關掉會紅、兩個 id 對調會紅。
- **停用那一支的檢查沒有人守。** 那句斷言是拿整份檔案去比對，而這一版讓另外兩支也有同一句，
  所以把停用的檢查整個刪掉，會在別支的複本上通過。把它反過來（成員放行、管理員擋掉）整套
  測試也全綠。現在改成只看那一支，而且連那個「不是」一起比對。
- **沒有任何東西保證檢查跑在寫入之前** —— 而這一版要修的就是這個毛病。現在每一支都比對位置，
  實測把檢查搬到寫入後面會紅。

驗證：37 個新測試、全套 4,676 個測試全綠、6 種破壞法逐一實測會變紅，以及正式機資料庫上
的三項對照（新的查法找得到別人的那一列、綁擁有者的寫入打得中、舊的綁法 0 筆 —— 那正是
當初 404 的原因）。**要重建 legacy-server.example 才會生效。**

已知限制（本來就有，這一版沒改）：把一條規範停用之後，同事那邊的本機副本最久要等一天才會
消失。判斷「有沒有變」的那個值只看還在啟用中的規範，停用剛好會讓它離開那個集合。

## v1.26.146 — 一條讀起來只有一行的規範（issue #89）

團隊規範有兩種存法。一種是內容就寫在那一列上，另一種是上傳時被切成很多小段掛在底下、
那一列只留一句「由 ownmind-upload 自動建立的規範摘要」。

兩種都用同一個方式讀，而第二種讀回來就只有那句話。**沒有任何跡象顯示還有東西沒讀到** ——
它看起來就是一條寫得很短的規範，不像一條讀漏的規範。

這個帳號上有 7 條是第二種，包括 `git-push`。所以同事說「幫我 push 一下」，AI 認出這條規範、
照指示去讀、拿到一句話、然後照自己的習慣 push。那條規範真正寫的六個步驟（先掃 `.env` 跟金鑰、
補 README 跟 CHANGELOG、下 tag 前先問測試機還正式機）一個都沒套用，而且不會有人發現。

### 改法：讀的時候直接給，不再交代讀法

`GET /api/memory/:id` 讀到一條有子段落的團隊規範時，會把那些段落一起回傳（新的 `fragments`
欄位）。讀的人不需要知道它是哪一種存法。

這件事已經用「寫一句話交代讀法」修過兩次。v1.26.141 把讀法從 A 改成 B，修好了第一種、
同時弄壞了第二種 —— 一句指示不可能同時對兩種形狀成立，而 context 裡只放得下一句。

- **那一列的 `content` 不動。** 合併進去的話，`ownmind_get` 自己的說明就會變成一句「請把
  整包內容寫回母列」的指示（它現在寫著「更新前先讀完整份，否則你會蓋掉其餘部分」），
  一次儲存就把所有段落壓平、原本的段落變孤兒。
- **只有「讀某一條」會合併，「列出全部」不會。** 列出全部現在是 32 列、38,108 字；合併的話
  會變成約 74,000 字，而那是一個只想要目錄的呼叫。
- **順序改由上傳時記錄。** 原本靠建立順序排，但改一個標題就等於「舊的下架、新的插到最後」，
  改一個大標題會讓底下整段都跑到文件結尾。現在同步時把段落在文件裡的位置寫進去
  （`metadata.ord`），讀的時候照它排，沒有的就退回舊行為，下次同步自動補上。
- **超過上限會講出來，而且講出下一步怎麼走。** 上限 20,000 字（目前最大一條 14,091 字）。

另外，單獨讀到一個段落時，會附上它屬於哪一條規範、那條規範共有幾段。

### 審查抓到的兩個自製新問題

- **我給的逃生路會把文件倒著交出來。** 超過上限時叫人改打「拿全部段落」那支，但那支
  照「最後修改時間由新到舊」排；而新加的重排是一列一列蓋時間，所以第一次重新同步之後，
  那支回傳的剛好是文件反序。救援的路自己犯了這次要修的毛病。已改成照位置排。
- **離線讀到的還是那句話。** 本機快取只存七種類型，不含這些段落。而新的工具說明卻
  無條件保證「用 id 讀就拿得到全文」。現在離線拿到團隊規範時會明講：它的段落不在
  本機、不要當成完整的。

### 驗證

- 27 個單元測試，12 種破壞法逐一實測會變紅（拿掉排序、`ord || id` 讓第 0 段跑到最後、
  把過長的段落切掉、型別條件放寬、參數對調、上限改成無限大、丟掉沒有內文的段落、
  拿掉重排分支、插入時不寫位置、用真假值判斷位置、不下架消失的段落、計數參數對調）
- 真的 Postgres 上 13 項：文件順序、沒有內文的段落、已下架的段落、非擁有者讀得到、
  選擇退出的人讀不到、沒有 `ord` 時退回建立順序、壞掉的 `parent_id` 不會讓查詢爆掉、
  逃生路回傳的是文件順序而不是寫入順序
- 全套測試 4,615 綠

順手修一個被這次改動絆倒的舊測試：它在固定位元組數的視窗裡找一道防呆，上面多寫八行
就把防呆擠出視窗、測試變紅但沒有東西壞掉。改成切到下一個分支。

## v1.26.145 — 守門的那個東西，被門外的人刪掉了

三支程式共用同一把鎖，確保同一時間只有一個在更新資料夾。鎖如果超過十分鐘沒動，
就當作「拿著它的那次執行已經死了」，可以收回。

收回這個動作要先刪掉舊鎖再建新的，這兩步沒辦法合成一步，所以外面再包一道門閂
（一個叫 `.reclaim` 的檔）：同一時間只有一個人能進去刪東西。

CI 抓到有兩個人同時在門裡面。重跑同一份程式碼四條腿全綠，所以是跟機器負載有關。

### 實際重現

2026-08-11，十六個競爭者 + 十二支吃滿 CPU 的背景程式：**240 回合裡有 10 次兩個人都拿到鎖**。
加上追蹤之後看到原因：

```
17460 進門
17530 把一個「還活著的」門閂當成沒人要的，刪掉了
17516 進門          ← 已經沒有東西擋著
17460 刪鎖   17516 刪鎖   17572 建鎖並確認   ← 17572 拿到
17460 建鎖並確認                            ← 17460 也拿到
```

**清掉一個沒人要的門閂，本身也是「先刪再建」，所以它會刪到還活著的那一個。**
v1.26.111 看到了一半：它讓「刪到活的」那個人自己退出。那管住了那個人，
但門閂已經刪掉了 —— 而門閂就是門，所以原本在裡面的人現在待在一扇沒有門的房間裡，
下一個人就跟著走進來。

`shared/update-lock.js` 的註解本來寫這個殘留風險是「檢查完之後的幾微秒」。
實際量到是每回合 11%，而且原因不是幾微秒，是**一間設計給一個人的房間裡站了三個人**。

### 改了什麼

**門閂也帶上主人的名字**，跟鎖本來就有的做法一樣。進門時寫進去，
在刪任何東西之前先確認「門閂還是我的那一個」 —— 刪舊鎖之前確認一次，離開刪門閂之前再確認一次。

丟了門閂的人就不是門裡的人，不可以再照著舊判斷去刪東西；
被換掉的人離開時也不會順手刪掉接手者的門閂。

兩份實作一起改（shell 跟 Node 是同一套協定的兩個版本）。

### 試過、量出來更糟的兩條路（都寫進程式碼註解了）

- **把門閂放回去**：120 回合 45 次，比原本的 5 次還糟。放回去的過程本身又是一段「門不見了」的空窗，
  而且 `mv` 會蓋掉這段期間別人放的東西。
- **先把鎖拿出來再量它**，這樣只會刪到自己量過的那一個：120 回合 53 次。
  它讓一把活著的鎖在每次收回嘗試時都短暫消失，那是邀請不是防護。

### 結果

同一個情境，**500 回合 0 次**（原本 500 回合 56 次）。剩下的是「沒有人收回」的回合（400 回合 3 次），
代價是少更新一次，下一個 session 或兩小時後的排程會接手 —— 那是安全的方向。

一般情況（鎖過期、沒有殘留門閂）每一回合都剛好一個人拿到。

### 沒有解決的部分，直說

這不是把競爭消滅了，用現有的工具也做不到。從「確認門閂還是我的」到「真的把鎖刪掉」
中間永遠有一小段時間，作業系統可以在那裡把行程切走。兩個確認都已經擺在刪除動作的
正前方，讓那段時間短到語言允許的極限 —— 但它還在。

之所以還算安全，是因為被踢掉的人自己會發現（他再讀那個檔，內容已經不是他寫的）。
上面那些數字是「十六個競爭者 + 十二支吃滿 CPU 的程式」量出來的，核心數不同的機器
會不一樣。**下次要判斷，請重新量，不要用推論的。**

---

## v1.26.144 — 升級程式把自己寫出來的東西當成「你改的」

升級的時候，OwnMind 會先問一句「這個資料夾有沒有被人改過」。有的話，它會先備份，
回報一則錯誤，然後用遠端版本把整個資料夾蓋回去。

問題是這句話從安裝完的第一天就一直是「有」。2026-08-11 在一台沒有人動過的機器上量到：

```
 M hooks/ownmind-usage-scanner.js
?? bin/
?? reports/
```
...
**第一個會一直重複。** 那個檔案在 repo 裡記錄成「不可執行」，安裝腳本跟同步腳本
兩邊都會把它改成「可執行」。蓋回去會改回不可執行，同一次升級的最後一步又會把它
改成可執行 —— 所以每一台 Mac 跟 Linux，每一次升級，都會走到那條會覆蓋東西的路。

**後兩個根本蓋不掉。** `git reset --hard` 動不了沒被追蹤的檔案，所以蓋完它們還在，
下次升級再來一次。有位同事的機器就是這樣每次都回報 `?? standards/`。

真正的代價不是那次覆蓋（有備份）。是那條會覆蓋東西的路變成**每次都走的預設路**，
它印的警告變成每次都出現的雜訊 —— 於是哪天你真的改了什麼，那行字也一起被滑過去。

### 改了什麼

- 那個檔案在 repo 裡改成可執行，跟兩支安裝腳本設定的一致
- 判斷「有沒有被改過」只看被追蹤的檔案。沒被追蹤的檔案照樣寫進升級紀錄給人看，
  但不再觸發覆蓋 —— 因為覆蓋這個動作對它們無效
- `bin/`、`reports/` 加進 `.gitignore`。`standards/` 沒加 —— 這個 repo 裡沒有東西
  會產生它，所以那是使用者自己的目錄，把別人的目錄從他自己的 `git status` 藏起來
  不是我們該做的事。上面那條改動已經讓它不會再觸發覆蓋
- 新增一條測試：安裝腳本裡每一個 `chmod +x` 的檔案，都必須在 repo 裡記錄成可執行。
  這份清單是從安裝腳本讀出來的，不是手寫的 —— `.gitignore` 那份手寫清單已經漏掉兩次了

### 還有一個順著查出來的，比上面嚴重

開場掛勾在拉新版之前會先 `git stash`，**整段程式沒有任何一個地方把它放回來** ——
包括成功的那條路。你當時沒存檔的東西會進到暫存區，然後留在那裡。

同一個 bug v1.17.22 在另一支程式上出過，那次真的把人的東西弄不見了；修好了，
但沒有修到這一支。

2026-08-11 在一台機器上數到 **30 筆暫存紀錄**，一次升級一筆，一路回到 v1.17.x。
裡面裝的是上面講的權限變更、不是誰的稿子 —— 但這個機制分不出差別，
哪天裡面裝的是真的東西，它一樣留著。

改成 `--autostash`（會自己放回來的那個），跟另一支程式一致。
另外加一條測試：這個 repo 裡任何腳本只要 `git stash` 而沒有放回來，測試就會紅。

---

## v1.26.143 — v1.26.139 修錯了：那不是競態，是 fetch 拒絕某些埠號

### 先更正上一版

v1.26.139 說測試的隨機紅字是「並行負載下 `srv.address()` 回不出可用位址、`undefined` 進了網址」，
加了等待與位址檢查，然後**連跑三輪結果一致，就宣告偶發消失了**。

那是運氣，不是證據。下一輪它就回來了——而且出現在一支**已經改過**的檔案裡，
位置在位址檢查通過**之後**的那個 fetch。

### 真正的原因

失敗的網址是 `http://127.0.0.1:5060`、`:5061`、`:6000`、`:6566`。這些都是合法埠號，
但它們在 fetch 規格的**封鎖埠號清單**上（5060 是 SIP、6000 是 X11、6566 是 SANE），
`fetch` 一律拒絕，不管有沒有東西在聽：

```
await fetch('http://127.0.0.1:5060/')   → bad port
await fetch('http://127.0.0.1:54321/')  → ECONNREFUSED   （允許，只是沒人在聽）
```

`listen(0)` 是向系統要「任何一個空的埠號」，偶爾就會拿到清單上的那幾個。
壓力探測可重現：並行 400 台伺服器中 2 個、800 台也中 2 個。

這解釋了全部現象：為什麼隨機、為什麼每次紅的不一樣、為什麼單獨跑會過（抽的次數少），
以及為什麼三輪乾淨完全不代表修好了。

### 修法

`startServer` 抽到封鎖清單上的埠號就關掉重抽（上限 20 次，超過就明確報錯而不是無限重試）。
被跳過的埠號記在 `rejectedPorts` 裡，需要時看得到。

v1.26.139 加的「等 listening 才讀位址」與位址檢查**保留**——解釋錯了，但那個行為本身是對的：
在 `listen(0)` 的下一行讀 `address()` 真的可能拿到 null，而一個講清楚原因的失敗
勝過一個「測試超時」。

### 這次怎麼避免再自欺

重抽那條路徑用**假物件強制觸發**來驗，不靠壓力探測的運氣。理由就寫在測試檔裡：
一次剛好沒抽到封鎖埠號的探測，對那個分支什麼都沒證明——而那正是上一版犯的錯。

另外加了一條測試直接對 undici 斷言「這四個埠號真的會被拒絕」。
如果哪天執行環境改了行為，那條會紅，重抽的程式碼就可以刪掉；
而不是讓前提悄悄失效、缺陷換個地方冒出來。
## v1.26.142 — 壞在沒人看得到的地方

三件事，同一種形狀：**機器上出了狀況，它自己知道，而且只寫在自己的硬碟裡。**

---

### 一、掃描器某個工具掛掉時，伺服器完全看不到

先講看到的東西。一位同事整天都在 Codex 裡工作，她的機器每兩小時準時回報一次，
帳號裡卻**從來沒有出現過一筆 codex 的紀錄**。

那不是任何一種「正常沒東西」能產生的狀態。掃描完沒裝這個工具、有裝但沒用過、
檔案讀不到 —— 每一種收尾都會留下一筆紀錄講明原因。要讓一個工具完全不留痕跡，
只有三條路：程式丟出錯誤、卡住不回來、或被設定跳過。

這三條路以前的結局都一樣：在那台電腦的 `logs/scanner.log` 裡寫一行字。
內容完全正確，位置在唯一沒有人會去看的地方。從伺服器看過去，它跟「這個人沒裝過
這個工具」長得一模一樣 —— 而這正是它被誤讀了六個星期的原因。

現在這三種情況都會回報：

| 情況 | 回報的原因 |
|---|---|
| 程式丟出錯誤 | `adapter_error`，附上錯誤訊息 |
| 卡住超過十分鐘 | `adapter_timeout` |
| 被設定跳過 | `skipped_by_config` |

「卡住」是這次新加的一道。丟出錯誤會被接住、記錄、回報；**卡住不會** —— 迴圈停在
那個工具上，排在它後面的四個永遠輪不到，整輪掃描等排程放棄時一起死掉。
同一位同事的紀錄就是這個形狀：第一個工具每兩小時更新，後面四個停在七月某一天。

錯誤訊息本身不塞進原因欄位（那是一個長度有限的固定清單），改寫進稽核紀錄，
截到 1000 字。

### 二、唯一會回答「資料到底有沒有送到」的那項檢查，等於沒在跑

自我檢查有十五項。其中十四項問的是「東西裝好了沒」，只有 `usage_roundtrip` 這一項
會實際掃一次、再回頭跟伺服器對帳，回答「這台機器的資料到底有沒有進去」。

v1.26.81 把它從「每日自動更新」那條路上拿掉了，理由是每天把本機所有資料庫掃一遍
太重 —— 這個理由到今天仍然成立。沒有被量到的是另一半：**每日自動更新那條路，
對於不會自己重跑安裝程式的人來說，是唯一的一條路。** 於是「不要每天跑」變成了
「永遠不跑」。前面那位同事的機器，每天上傳十四項檢查、連續好幾個星期，
一次都沒跑過第十五項。

現在改成每七天跑一次。讀不到紀錄、日期壞掉、時鐘往回跳，一律當成「該跑了」——
連自己都說不清楚狀況的機器，正是最需要被聽見的那一種。

### 三、機器要有人開 AI 工具，才會升級

查上面那件事的時候看到的。另一位同事的五筆紀錄全部是今天的，版本全部是她
**六月十八號安裝當天**的那一版。

完整的升級動作（抓新版、更新套件、同步檔案）以前只寫在 MCP 裡，而 MCP 要有人開一個
會跟 OwnMind 講話的 AI 工具才會啟動。開場掛勾只負責同步檔案，那是修復、不是升級。

她用的編輯器不在那個名單裡。所以那台機器沒有壞，只是**兩個月來沒有人叫它去更新**,
這兩個月發的每一個修正都到不了她手上。

現在改由每兩小時跑一次的排程負責：不需要 AI、不需要模型、跟今天用哪個編輯器無關。
升級排在掃描之後 —— 升級會改寫這支程式自己的檔案，讓這一輪跑完手上原本那份，
下一輪再換新的。

> 有一件事這次修不了自己：停在六月那台收不到這個版本，因為要送達它的，
> 正是這次要修的東西。那一台需要人工升級一次，之後就會自己跟上。

### 對抗審查抓到的三件事（都已修）

因為第三項開始在沒人看著的時候跑，所以審查特別針對它。

**升級升到一半失敗，隔天會報告一切正常。** `git pull` 成功之後，安裝套件或同步腳本
還是可能失敗，而這時程式碼已經換新了。下一輪看到「沒有新 commit」就當作乾淨、
蓋掉當日戳記，那台機器從此帶著新程式碼配舊套件跑，而且每天回報健康 —— 這正是這一版
要消滅的壞法，不能由這一版自己製造。現在後半段有自己的標記：開始前寫、做完才刪，
只要它還在，下一輪就重做一次，不管 git 怎麼說。

**git 卡在 rebase 中間會永久卡死。** 有衝突時 rebase 會停下來等人處理，但排程那邊
沒有人。之後每一次升級都會失敗（包括本來要接手的那條退路），使用者自己沒提交的修改
被鎖在暫存裡。現在拉之前先還原，拉失敗留下半截也再還原一次 —— 還原這個動作本身就會
把暫存放回來。

**錯誤紀錄可以繞過流量限制。** 心跳本身有 30 秒限流，新加的那筆錯誤紀錄卻無條件寫，
壞掉的機器可以一直灌。改成心跳真的有寫入才記錄。因為原因一改變心跳一定會寫，
所以第一次失敗永遠留得下來，被擋掉的只有窗口內的重複。

另外兩個發現實測後推翻：Promise.race 的敗方晚一點才失敗不會變成未處理例外；
開場掛勾也不會餓死排程，它自己每天只跑一次。

### 第二輪對抗審查：兩個會讓這一版整體變差的東西

**升級被放錯地方了。** 掃描器的 `main()` 不只是排程的進入點 —— 自我檢查也把它當成
「跑一次掃描告訴我結果」在用，而且限它 60 秒跑完。升級寫在 `main()` 裡面，等於把
`git pull` 跟 `npm install` 塞進那個檢查裡；更糟的是，那個自我檢查本身就是升級腳本
啟動的。在落後兩個月的機器上（正好就是這一版要救的那些），光 npm 就超時：檢查什麼都
沒證明就逾時、還照樣蓋掉每週戳記，然後在還握著更新鎖的情況下結束行程 —— 五分鐘後
另一個行程把那把鎖當成死鎖收走，開始第二次 pull 灌進同一個目錄。
現在只有排程叫起來的那一次會升級，其他情況一律只掃描。

**新的失敗心跳會把「找出這件事的警報」關掉。** 那個警報靠的是同一台機器裡的不一致：
有的工具是新的、有的凍在很久以前。它會有效，正是因為壞掉的採集器**不再寫紀錄**。
這一版之後它每兩小時就寫一筆說自己失敗了 —— 不處理的話，每個壞掉的採集器看起來都
永遠是最新的，而引發這整件事的那台機器會顯示五筆全新紀錄、一個警報都不發。
現在失敗心跳算「沒有回報」：它不會讓機器看起來還活著，而且不管多新都算在停掉那一邊。
被使用者自己設定跳過的不算 —— 每兩小時為別人自己的設定發一次警報，是警報被關掉的
標準流程。

另外修的：慢但沒卡死的工具在期限後才寫檔，會蓋掉排在它後面已經寫好的進度（改成寫入
前重讀再合併）；放鎖改成要對得上取鎖時拿到的憑證，超時的持有者不能再刪掉接手者的鎖；
鎖的死亡判定從 5 分鐘拉到 10 分鐘（升級本身最壞情況要 280 秒，留 20 秒不叫餘裕），
shell 那份同步改掉、而且那條「兩邊要一致」的測試改成去讀兩邊的數字而不是自己再寫一次；
每週戳記改成只有真的問出答案才蓋；路徑遮蔽補上編碼過的專案資料夾名，以及錯誤訊息裡
接在路徑後面的標點。

### CI 抓到的一個：那道十分鐘上限本來可能永遠不會觸發

本機測試全綠，CI 的 Node 20 紅了。原因不是測試環境的差異，是真的有問題。

那個計時器原本設成「不要讓行程為了它而活著」，想法是掃描做完就不用再等剩下的幾分鐘。
但清掉計時器這件事本來就已經另外做了，而設成不影響行程存活的計時器**也就沒有力量把
行程留下來**。卡住的工具如果卡在一個永遠不會有結果的等待上（而不是卡在一個還開著的
檔案上），事件迴圈就會排空、行程直接結束 —— 在十分鐘到之前。那個卡住的工具還是什麼
都不會回報，正是這道上限要打破的沉默。

Node 24 的測試執行器自己會把行程留著，所以本機看不出來；Node 20 會直接講出來。
**本機綠色，而且是錯的。**

> 已知限制（沒修）：Windows 上 `npm install` 逾時只殺得掉外層那支殼、殺不掉底下真正
> 在寫檔的那支。Node 沒有可攜的作法，而代價不成比例 —— 那個窗口是 120 秒、排程間隔
> 兩小時、裝的是幾個套件。寫下來，不粉飾。

---

## v1.26.141 — 只有在你剛好講對詞的時候才答得出來的記憶

兩個回報，同一個根因：**開場給的東西全部是「推」——它告訴 AI 已知的有哪些。
裡面沒有任何一句是「拉」——沒有一句告訴它什麼時候該去查。**

所以只要使用者講的話剛好撞到清單上的某個標題，系統就會動；撞不到就安靜地失效。
而這裡的「安靜失效」長得像一個講話很有把握、只是剛好不知情的 AI ——
在使用者眼裡就是「這個 AI 又忘記有 OwnMind 這個東西」。

---

### 一、那句「怎麼讀規範全文」的指示，讀回來是空的
...
第 869 條是當天早上新增的「[團隊] 發布網頁到 pages.fontrip.com」。它的內文存在自己的
紀錄上、沒有子片段，所以那句唯一寫在畫面上的指示，**對使用者正在問的那條規範回傳空的**。
真正找得到它的是 `ownmind_search("發 pages")`，而且是第一筆。

#### 怎麼發現的

拿兩個獨立的 AI 來模擬同事：只給它們同事開場會拿到的東西、本機沒有裝任何相關工具，
使用者說「幫我做一個網頁，上面只寫 hi，然後發 pages」。
...
| 決定先去讀它再動手 | 兩個都有 |
| 照畫面上那句去撈 | 兩個都是 |
| 自己察覺那句可疑 | 兩個都有 ——「這看起來不是真的編號」 |

它們是對的，而且是靠自己夠謹慎才沒出事。**設計上沒有任何東西攔得住。**

### 二、從頭到尾沒有一句話叫它去查

開場列的是標題，然後給了兩種「你已經知道名字的時候怎麼拿全文」的用法。
「搜尋」這個動作，整份開場一次都沒出現過。
...
| 使用者說 | 會怎樣 |
|---|---|
| 「發 pages」 | 撞到標題「發布網頁到 pages.fontrip.com」 → 會去查 |
| 「公司 pages」 | 撞不到 → 不會去查，AI 自己編一套做法 |

#### 同一天回報的、更嚴重的那個

使用者一台正式伺服器的存取資訊，在記憶裡放了好幾週。AI 還是回答
「我沒有 legacy-server.example 的資訊」，被叫去查之後立刻就找到了。

**這個比「漏查」嚴重，因為它是一句講得很有把握的假話。** 而且完全解釋得通：
開場畫面render 的是 profile、鐵律、團隊規範標題、原則標題——**專案記憶一條都沒有**，
而那台伺服器就是存在專案記憶裡。從 AI 坐的位置看，「我沒有資料」跟「根本沒有資料」
長得一模一樣。

`legacy host` 在那 18 KB 的開場內容裡，出現 **0 次**。

### 修法：三句話，兩個地方都放

1. **規範清單改成講「怎麼真的讀得到」**：先 `ownmind_search("<標題>")`，
   再對回傳的那一列 `ownmind_get({ id })`。
2. **碰到不認得的公司用語就去查** —— 工具、網站、流程、縮寫、名字 —— 在回答或動手之前。
   刻意只限「不認得的詞」：叫它每則訊息都查是噪音，而噪音就是會被忽略的那種東西。
3. **「我沒有這個資料」要先查過才准講。** 對使用者自己的東西（伺服器、專案、憑證、決策）
   一律先 `ownmind_search`。那是一句關於**他的記憶**的斷言，不是關於世界的。

#### 放在哪裡，以及花了一輪 review 才放對

`hooks/lib/render-session-context.js` 是 Claude Code 載記憶的路。這一版第一次寫的時候，
把同樣三條規則放進 `INSTRUCTIONS_SOP`，並且宣稱這樣就涵蓋了其他工具。**沒有。**
review 抓到了，而且程式碼寫得很清楚：
...
repo 裡每一個呼叫端都帶 compact —— `ownmind_init`、條件同步、shell 掛勾。
那個欄位在離開伺服器之前就被拿掉了，所以修法被放在**整包資料裡唯一沒有人收得到的地方**。
那正是它自己寫著要避開的 v1.26.128 形狀。而測試看不到，因為它們是把 `memory.js` 當文字檔讀：
只證明了原始碼裡有那些字，沒證明任何一個回應帶著它們。

真正活得下來、現在規則放的地方：

| 放哪裡 | 誰收得到 | 什麼時候讀 |
|---|---|---|
| `hooks/lib/render-session-context.js` | Claude Code | 開場一次 |
| `mcp/index.js` 的工具說明 | 每一個工具 | 每一輪 |
| `configs/*.md`（7 份範本） | 每一台安裝 | 每一輪 |

`ownmind_search` 的說明現在寫的是**什麼時候該叫它**，不只是它會回什麼；
`ownmind_get` 不再指向 `standard_detail` —— 工具說明每一輪都在模型面前，
本來就壓過開場那份講同一句錯話的內容。

#### 一個 review 提出、但量完之後不成立的說法

review 另外說：畫面上那一行帶著 `[團隊] ` 前綴，而那不是標題的一部分，加上搜尋是 AND，
所以照那行去搜會是 0 筆。它是讀 tokenizer 原始碼推的，不是打端點量的。實測正式機：

| 查什麼 | 第一筆 |
|---|---|
| `[團隊] 發布網頁到 pages.fontrip.com` | **#869**，就是那條規範 |
| `公司 pages` | **#869** |
| `legacy-server.example` | #653、**#745**（那台伺服器）、#702、#632 |

三個都找得到，包括這一版命名的那個講法。照實記下來，因為那段推論很仔細、很具體，
而且是錯的 —— 照著改的話，這一版會去追一個根本不存在的問題。

### 驗證

拿新的開場內容重跑模擬，兩個情境各一次：

- 「幫我看一下 legacy-server.example 那台的部署狀況」→ 第一個動作就是 `ownmind_search("legacy-server.example")`；
  問它會不會說「沒有這台的資訊」，答「不會」，並引用了新加的那句。
- 「幫我做一個網頁寫 hi，然後放到公司 pages」→ 去查，而不是自己編。

**第二次那個要打折：** 那個 AI 同時也看到了它自己環境裡裝著的一個相關工具，
所以那次結果被污染，只有第一次是乾淨的證據。照實記，不當成兩次乾淨的。

### 四、規則寫進誰的檔案裡（Vin 追問之後才發現的另一半）

三句話只是一半。Vin 問了那個找出另一半的問題：**安裝／升級的時候，這些不是應該寫進
CLAUDE.md 嗎？**

應該，而且沒有做到——原因跟這一版其他每一件事同一個形狀。`install.sh` / `install.ps1`
用 heredoc 寫下五行，然後**只要檔案裡出現過「OwnMind」這個字就永遠跳過**；
`update.sh` / `update.ps1` 從頭到尾沒碰過這個檔案。**每一台機器的內容都凍結在安裝當天。**
2026-08-11 實測：有一台還帶著已經被取代三次的四行版本。

所以今天寫的規則，走這條路是一個人都收不到。

#### 改法

`configs/ownmind-rules-block.md` 變成唯一來源，用標記區塊注入使用者自己的指示檔
（CLAUDE.md 加另外六個工具的），由**一支 node 腳本**負責，四支安裝／升級腳本都呼叫它。
那七份 `configs/*.md` 範本**沒有**也放一份——一個範本一份拷貝，就是七份會各自走鐘的拷貝。
每次升級重寫標記中間；標記外面唯一會動到的是檔案最尾端的空行（統一收成一行），其餘一個字不動。

標記區塊這件事本來寫過兩份（shell 一份、PowerShell 一份），而 v1.26.140 抓到兩份已經
走鐘到「一份對空檔案會炸、另一份不會」。現在只有一份，放在三個平台本來就都依賴的執行環境裡。

#### 規則本文，經過一輪 review

有四個反對意見是對的：

- **「動手之前先查」會自廢武功。** 寫程式的 AI 幾乎每一輪都在改檔案，這條等於每輪都查、
  每輪都查不到東西，然後**連帶教會模型這整段可以忽略**。而且兩個回報的狀況它一個都沒多擋到。
  改成用「指的是誰」當判準：這件事指向這個人，還是指向世界？「公司 pages」每個字都認得、
  但從眼前的程式碼裡完全查不出來——那正是「看詞認不認得」抓不到的情況。
- **「發現錯了就更正」根本執行不了。** 更正要帶數字編號，AI 手上只有標題。現在把整串講完：
  搜尋 → 讀全文 → 才更新。這正是這一版在修的同一個坑，換一層又犯一次。
- **拿搜尋結果去更新會蓋掉其餘的內容。** 搜尋只回 400 字。現在明講：先讀全文。
- **沒有任何東西擋「一個人的 AI 去改全公司的團規」。** 更正限縮在他自己的 `project` 跟
  `env`；碰到 `team_standard` 或 `iron_rule`（那是使用者本人在真實事件裡寫下的話），
  改成回報給他、由他決定。

同一輪砍掉兩段：規則類型之間的優先順序（是死條文，而且跟更底層的強制邏輯打架），
以及密鑰那段四行裡的三行（伺服器本來就用 HTTP 400 擋掉，兩個寫入工具的說明也都寫了）。

#### 已經有舊區塊的機器怎麼辦

舊區塊沒有標記，升級時認不出來，會變成新的加在下面、舊的留在上面。所以那支腳本會拿它
**逐行**比對本專案出貨過的每一種寫法：

- 每一行都是我們出的 → 直接換掉
- 只要有一行不是 → **不動它**，並且印一行告訴使用者上面那段可以自己刪

那個檔案是使用者放個人鐵律的地方。為了少一個重複標題而吃掉他手寫的一行，不划算。

### 這一版沒有改的

沒有新增任何清單，也沒有多抓任何資料。開場內容本身**會變大** —— 用最小樣本量是
448 → 1,069 位元組 —— 因為三句話就是三句話。這份紀錄的初稿寫「沒有任何東西變大」，那是錯的。

### 這一版沒有修的

**專案記憶到現在還是沒有出現在開場畫面裡。** 這一版改的是「AI 會去找，而不是直接斷言沒有」，
那正是被回報的那個症狀。它沒有讓使用者的 133 條專案記憶在一開場就看得到。
要做到那個，每次開場要多塞大約 5.8 KB 的標題 —— 對 18 KB 的開場內容來說是真實的代價，
而且那是「注意力怎麼分配」的取捨，不是對錯問題。留給使用者決定，數字是量出來的不是猜的。

## v1.26.140 — 兩個都印了 OK 的失敗

一個是使用者回報的，一個是檢查發版時抓到的。形狀一樣：東西壞了，看著它的那個人說沒事。

---

### 一、更新腳本遇到空檔案會炸，然後回報同步完成

2026-08-11 Windows 使用者回報：
...
`Get-Content -Raw` 讀 0 位元組的檔案，回的是 `$null` 不是 `''`：

```powershell
$existing = if (Test-Path $TargetFile) {
  [regex]::Replace((Get-Content -Raw -Path $TargetFile), '…', '')   # 176 行
} else { '' }
…
Set-Content -Path $TargetFile -Value ($existing.TrimEnd() + $block)  # 180 行
```

`[regex]::Replace()` 不收 null 輸入，所以那個賦值整句沒完成，`$existing` 還是 null，
下一行對 null 呼叫 `.TrimEnd()` —— 那就是第二個錯誤。

動手改之前先在真的 PowerShell 裡重現三種狀況：
...
所以那台機器六個 AI 工具裡有一個的指示檔是空的，而且只有那一個受影響。

#### 為什麼沒人發現

兩個錯誤都是非終止的，腳本照樣跑到下一個工具，最後印出寫死的
`[ OK ] Upgrade rules synced to detected AI tools`。**那行字不依賴任何實際發生的事，
所以它本來就不可能講出別的。**

#### 修法

`Add-OwnMindUpgradeRule` 搬到 `scripts/windows/lib/append-upgrade-rule.ps1`
（`find-git-bash.ps1` 本來就住在那），這樣才測得到——不用整套更新腳本跑在一台
剛好有那種檔案的機器上。

- 讀到 null 一律當成 `''`，那本來就是每個呼叫端的原意
- 回 `written` 或 `skipped`，其他狀況一律丟例外
- `update.ps1` 逐一計數、失敗的指名、結尾印實際數量
- 改用 `WriteAllText` 而不是 `Set-Content -Encoding UTF8`（後者在 PowerShell 5.1
  會加 UTF-8 BOM，而這些檔案是別家工具在讀）

`scripts/update.sh` 不會這樣炸——它透過 Node 讀，`readFileSync` 讀空檔案回的是 `''`。
但那行寫死的結尾它也有，見下面第三節。

---

### 二、整體分析那頁被歸咎於一個不存在的大小上限

v1.26.137 修好了 14 天與 30 天報告失敗，把原因記成「LLM 閘道有 40 KiB 硬上限，
二分法量出來的」。**那是錯的。** 值得記下來，因為那次量測看起來很紮實：
探測用的內容是同一句短句重複貼成的，每個位元組耗掉的 token 遠少於真實報告，
所以二分法找到的邊界根本不是位元組的邊界。
...
|---|---|
| 40,214 位元組的探測內容 | 200 |
| 路徑實際送出的 14 天內容，35,301 位元組，失敗時段 | **502**，連四次 |
| 同一份內容二十分鐘後，連續送三次 | 200、200、200 |
| 同一份內容，並行送三次 | 200、200、200 |
| 端點本身連叫五次 | 200 × 5 |
...
40,214 位元組過得去，35,301 位元組被擋。**決定的不是大小，是閘道當下還剩多少量能**，
而比較大的內容比較容易超過剩下的額度。7 天報告（31,929 位元組）一路都正常就只是
因為它比較小——而那正是這件事看起來像大小上限的原因。

#### 修法

`callLLMSwitch` 會重試。閘道現在沒量能，三秒後通常就有了；讀者該付的代價是多等幾秒，
不是看不到報告。

- 重試 408、429、5xx，以及連線直接失敗
- 不重試其餘的 4xx —— 那是閘道只會重複同一句話的答案
- 重兩次、間隔三秒，總共 60 秒為限；這個期限同時夾住每一次嘗試的逾時，
  所以不會有人為了三次卡死等九十秒
- 錯誤訊息帶實際用掉幾次嘗試

上游回應保留 2,000 字而不是 200 字。查這次的原因得從伺服器手動重放請求，因為紀錄
剛好斷在第二、第三個供應商為什麼失敗的地方。v1.26.137 把「加寬它」列為已知缺口，
這一版補上。

v1.26.137 的精簡維持原樣、額度一樣是 38,000 位元組。它買的一直都是「比較不容易被擋」，
這點沒變；變的是程式碼不再宣稱它買到保證——那些寫著 40 KiB 上限的註解換成量到的事實。

---

### 三、review 回合抓到的東西

四件是這一版自己寫出來的，而且一開始的測試全部看不到。

#### 重試把它要註記的那個錯誤毀掉了

「(after N attempts)」原本是直接改 `lastError.message`。真正逾時的 fetch 丟出來的是
`DOMException`，它的 `message` 是唯讀存取子，而這是模組（嚴格模式），所以那一行會炸。
用真的會理會 signal 的 fetch 實測：

```
THROWN: TypeError | Cannot set property message of  which has only a getter
```

也就是說每一次逾時進到紀錄裡的都會是這個 TypeError，而那 2,000 字的上游原因——這一版
第二件事的全部重點——會被丟掉。

**測試當時是綠的**，因為它自己造了一個 `new Error()` 再把 `name` 設成 `'AbortError'`，
而普通 Error 的 `message` 是可寫的。介面兩端都是假的，所以那個測試只證明了兩個假的
彼此同意。現在改成用會理會 signal 的 fetch，那才是「閘道卡住」真正的樣子。

迴圈也不再改動它接到的任何錯誤：連線層的失敗一律重新包成自己的 Error，最後的訊息是
一個新的 `Error`，原因掛在 `cause` 上。

#### 要不要重試，原本是靠在錯誤訊息裡找關鍵字決定的

`isTransportError` 去比對訊息裡有沒有 `network|socket|fetch failed|ECONN`，而解析失敗的
訊息裡本來就夾著模型自己吐出來的內容。一份剛好提到「network error」的報告會被重試三次，
然後被回報成連線失敗。現在分類發生在失敗當下（fetch 呼叫獨立包一層 try），
只要回應已經回來了，後面出的問題一律直接回報。

#### 期限會放一次注定失敗的嘗試進場

期限原本只在失敗之後、睡覺之前檢查，所以睡完可能已經超過期限，下一次嘗試就帶著
一毫秒的額度進場、燒掉一個名額，還把有用的 502 換成一個逾時。改成開始之前先檢查。

#### 讀 BOM-less UTF-8 卻用系統編碼

不寫 BOM 是對的，但 `Get-Content -Raw` 在 Windows PowerShell 5.1 讀沒有 BOM 的檔案
會用系統 ANSI 編碼——在回報這個問題的繁體中文 Windows 上就是 cp950。寫跟讀合起來，
會在下一次更新時把使用者自己寫的中文當成 Big5 解，弄壞之後再寫回去。

這條流程上的兩個讀取都改成 `[System.IO.File]::ReadAllText`（UTF-8，且舊版留下的 BOM
照樣認得）。

順帶挖到一個**比這一版更早就存在的缺陷**：`$snippet` 自己也是用 `Get-Content -Raw` 讀的，
而 `skills/ownmind-upgrade-agents-snippet.md` 是 UTF-8、含中文、沒有 BOM。
**每一台繁體中文 Windows 一直都在把一份亂碼的升級規則寫進它找到的每個 AI 工具。**

#### 同一個逾時，換一道門進來

第二輪 review 發現上面那個修法漏了一個雙胞胎。`res.json()` 落在分類那層 try 的外面，
所以閘道先回 200 標頭、然後在傳輸內容時卡住（同一種量能不足，而且在負載高的時候很常見），
結果是：
...
不重試，而且紀錄裡沒有嘗試次數、沒有網址、也沒提到逾時——**正是第一個修法要消滅的那個樣子。**
現在讀取內容也在分類裡面，而 `parseLLMJson` 留在外面：內容送到了卻解不開，下次送到的還是同一份；
內容根本沒送到，則不是。

#### 同一輪的三件小的

- **兩次嘗試之間的等待沒有被期限夾住**，所以「60 秒為限」實際上是最多 63 秒。現在夾住了，
  文件上那個數字才是真的。
- **`retries: NaN`** 會讓次數變成 NaN、迴圈一次都不跑，最後那行去取一個沒人設定過的錯誤。
  改用 `Number.isFinite` 擋掉。
- **網址設定壞掉**會被當成連線失敗重試三次，白白花掉讀者六秒鐘去換同一個答案。
  網址改成迴圈之前就組好。

#### `update.sh` 有同一個寫死的結尾

會炸的那件事只發生在 Windows，但這一版命名的那件事不是：`update.sh` 同樣用寫死的
`[ OK ] Upgrade rules synced to detected AI tools` 結尾，而且它清除舊區塊那一步後面掛著
`|| true`，所以 node 跑不起來的時候，舊區塊留在原地、下面的附加又加了一個新的。
第二輪還找到第三件：規則本文是在一個命令群組裡 `cat` 出來的，而群組的結束狀態取的是
**最後一個指令**的——所以一份存在但讀不到的規則本文，會在兩個標記中間寫下一塊空的，
然後回報成功。三個都修了，測試是把真正的腳本片段抽出來跑，不是抄一份。

---

### 這一版沒有改的

畫面上看不出差別，除了閘道忙的時候 14 天與 30 天會拿到報告而不是錯誤。報告內容、
摘要提示、機械報表都沒動。

### 這一版沒有修的

- `admin.js`、`broadcast.js`、`bug-reports.js` 還是用 `parseInt` 解析列編號，
  是 v1.26.136 那個缺陷的輕微版本。仍然待辦。
- `redactPIIDeep` 會把 `Date` 物件變成 `{}`，所以 `last_activity` 與 `last_reported_at`
  送到模型手上是空物件。既有問題，這次沒動。

## v1.26.139 — 每次跑測試都有紅字，而且每次紅的都不是同一個

### 量到的事實

2026-08-10 連跑四次 `npm test`：失敗數 3、4、4、2，**每次紅的都是不同的測試**。
把每一支單獨拿出來跑，全部通過。最常紅的那四支放在一起跑三輪，43 項全過。
再用 `--test-concurrency=1`（一次只跑一支）跑完整套件 4375 項——只剩下兩個
「需要建置前端」的既有失敗。

所以那些測試從頭到尾都沒壞。壞的是它們共用的那個寫法。

### 根因：測試自己的輔助函式有競態

```js
const srv = app.listen(0, async () => {
  const port = srv.address().port;          // 或者：在 listen() 的下一行就同步讀
  await fetch(`http://127.0.0.1:${port}${path}`);
  srv.close();
});
```

32 支測試用這個樣式，其中好幾支是**每發一個請求就開關一台伺服器**，而 `node --test`
是把檔案並行跑的。負載一高，`address()` 就不保證回得出可用的位址；沒有人檢查它，
於是埠號以 `undefined` 進到網址裡：
...
那四支裡有兩支更直接：它們在 `listen(0)` 的**下一行就同步讀 `address()`**，
根本沒等 'listening' 事件。那是確定的競態，只要機器忙就會中。

### 為什麼值得修

一個查不出歸屬的紅字比一個真的紅字更糟，因為它教出來的習慣是「紅了先重跑」——
而那正是真正的回歸被放過去的方式。

### 修法

新增 `tests/helpers/app-server.js` 的 `startServer(app)`：

- **等 'listening' 事件**才讀位址，不在 listen 的下一行就讀
- **檢查位址可用**，不可用就用「實際看到什麼」報錯，而不是拿 `undefined` 去組網址
- 一併處理 `listen` 的 'error' 事件（原本沒接，會變成「測試超時」這種毫無資訊的失敗）
- `close()` 會先 `closeAllConnections()`，否則 fetch 留下的長連線會讓關閉的回呼不觸發

四支實際紅過的測試改用它。**沒有一次改 32 支**——那是為了沒看到失敗的檔案付出大量改動，
不划算。這四支是真的紅過的，`tests/test-server-helper.test.js` 會釘住它們不准改回去。

### 順帶修的兩件事

**1. `update.sh` 只同步 `hooks/*.sh`，最上層的 `.js` 掛勾不同步。**
實測：更新到 1.26.137 之後，`~/.claude/hooks/ownmind-session-start.js` 還是 1.26.132 的版本，
落後三版、少兩個修法，而同步腳本印的是「Hook scripts synced」。

現行安裝不會跑那些副本（Windows 走 `~/.ownmind/hooks/` 的路徑），所以是舊檔而不是活的故障。
**選擇一併同步而不是刪掉**：夠舊的安裝可能真的把掛勾指向 `~/.claude/hooks/*.js`，
刪掉會讓那台機器默默失效。複製的代價是幾個檔案，刪除的代價是一台沒人在看的機器。

**2. 同一段的 `cp … 2>/dev/null || true`。**
`lib/` 複製失敗（權限、磁碟滿）會讓掛勾繼續用磁碟上的舊模組，而上面那行照樣印
「synced」。改成失敗就明說。

### 驗證

修法後連跑三輪完整套件，**每一輪的失敗完全一樣**（只剩那兩個需要建置前端的）——
不再漂移。這才是「偶發消失了」的證據，而不是「這次剛好綠」。
...
那頁要 AI 讀的資料，是整段期間的完整紀錄一次送出去。送出去的量：7 天 32KB、14 天 48KB、
30 天 53KB。而承接的服務對單次請求有 40KB 上限，超過就直接退回，三家供應商都退。

所以那頁提供三個區間，其中兩個從來沒有真的做出來過。

上限是實測出來的，不是猜的：每 20 秒送一次（避免把流量限制誤讀成大小限制），39,600 位元組
會過、41,025 位元組被退。

### 這一版改什麼

**送出去之前先精簡，而且只在需要時精簡。** 每做一步就重新量一次，一放得下就停手 ——
最便宜的那一步就是損失最少的那一步。7 天本來就在額度內，所以原封不動，跟以前一模一樣。

精簡的順序是照「資訊密度」排的，不是照大小：

1. **逐筆踩坑紀錄太長的截短**，但一則都不刪。它是文章，重點通常在前面。
2. **完全遵守的規則不列出來**，只留有違反或跳過的，並附上省略了幾條。一條大家都遵守的
   規則是整份資料裡最沒有訊息的一列。
3. **版本清單每台機器收成一列**，而且留的是那台**最舊**的版本。這一區是拿來看「誰還沒更新」的，
   留最新的等於把要看的東西藏起來。
4. 萬一還是太大，就從當下最大的那一區開始一口一口裁，並說明少列了幾列。這一步是給沒人預料到
   會變大的區塊用的。

**精簡了什麼會寫在報告裡。** 一份被摘要過卻被當成完整紀錄來讀的報告，比一個錯誤訊息更糟。

### 驗證

拿正式機當下的真實資料跑：7 天完全沒被動；14 天只收了規則與版本兩區、踩坑紀錄全文保留；
30 天踩坑紀錄截到每則 250 字，62 則一則沒少、12 個專案一個沒漏。

然後把精簡後的內容真的送去給模型：14 天與 30 天都回來了完整的分析（結論、洞察、建議動作、
6 個專案的踩坑）。不是只證明「變小了」，是證明「真的做得出報告」。

三處各弄壞一次確認測試擋得住：路由不呼叫精簡 → 紅 2；拿掉最後手段的裁切 → 紅 1；
回覆不帶「哪些被摘要了」 → 紅 1。

### review 回合抓到五件，其中三件都是同一件事：這一版說要誠實，但誠實只做了一半

**第一件：說明文字沒被算進大小。** 那幾句「哪些被摘要了」是跟著資料一起送出去的，但它們是在
量完之後才貼上去的 —— 於是實際送出去的比通過檢查的那個數字大。這正是這一版要消滅的毛病，只是
換一層出現。現在量的就是要送出去的那個東西。

**第二件：說明文字會自打嘴巴。** 實測：100 則踩坑紀錄壓到很小的額度時，同一份說明裡會同時出現
「共 100 則全部保留」跟「只保留前 5 列，另外 95 列沒有列出」。改法不是去改那句字，是改結構：
說明改成從「壓縮前後的差異」推導出來，而不是每一步各自附加一句。這樣「描述一個後來被推翻的
步驟」在結構上就不可能發生。

**第三件（最重要）：沒有告訴模型它讀的是摘要。** 遵守紀錄被精簡成「只剩有違反的那幾條」之後，
模型看到的畫面就是「每一條都有違反」，很自然會寫出「鐵律遵守狀況不佳，多數規則都被違反」——
一句很有自信的假話，而且這份報告是給被分析的人看的。現在指示裡明講：看到摘要標記就要先說清楚
自己看到的是哪一部分，禁止推斷整體比例。實測驗過：模型現在開頭寫「這裡只列出有違反或跳過的
7 條紀錄」，整段沒有出現「大部分」「多數」「普遍」。

**第四件：那些說明沒有顯示在畫面上。** 伺服器有回傳、頁面沒有讀。結果是一段「只讀了部分資料」
寫出來的 AI 說明，旁邊擺著一張完整的統計表，兩邊對不起來而讀者不知道為什麼。現在顯示在報告
最上面，三種語言都有。

**第五件：壓不下去的時候沒有留下任何線索。** 上一次查這個問題，是把請求搬到伺服器上手動重放
才看到完整錯誤。現在會直接記下還剩幾位元組。

另外四件小的：讀不出來的版本（null 或 unknown）會蓋掉真的版本號，實測一台機器的 1.26.135
會被同機另一個工具的 null 蓋成空白，而那正是這一區要回答的問題；最後手段裁切時間序列時砍錯邊
（砍掉最近的日子）；說明文字裡混進了程式內部的欄位名稱；「全部遵從」用詞不對（只被觀察、
沒有任何計數的規則也會被歸進去）。

## v1.26.136 — 打錯一個網址，系統回答「伺服器壞了」

### 量到的事實

2026-08-10 全功能巡檢，對正式機逐一打過每一支端點時量到的：

| 網址 | 回應 |
|---|---|
| `/api/memory/stats` | **500** 伺服器錯誤 |
| `/api/memory/recent` | **500** 伺服器錯誤 |
| `/api/memory/abc` | **500** 伺服器錯誤 |
| `/api/memory/999999`（不存在的編號） | 404 找不到 |

伺服器日誌對得上：`invalid input syntax for type integer: "stats"`。

### 為什麼

記憶那組網址裡，「用編號取一筆記憶」是最後才登記的一條，所以任何前面沒對上的路徑
都會掉到它手上、被當成編號。那個值直接送進資料庫查詢，資料庫拒絕，而那個拒絕被包成
「Query failed」的 500 回去。

**500 的意思是「伺服器自己壞了」。** 但伺服器沒壞，是那個網址不存在 —— 那是 404。
差別不只是好看：500 會進錯誤監控、會讓人以為服務出事，而真正的原因是有人打錯字。

同一個毛病也在「取歷史版本」「更新」「停用」「啟用」「還原」上，因為它們都吃同一個編號。

### 這一版改什麼

編號在進到任何一條路由之前先被檢查一次，不是六條各檢查一次 —— 之後再加新的路由也不會忘記。
只有純數字、1 以上、不超過資料庫整數上限的才算編號，其餘一律 404。

比 `parseInt` 嚴格是有原因的：`parseInt('12abc')` 會得到 12，於是一個打錯的網址會安靜地
讀出別人的第 12 筆資料。

**先講清楚：沒有任何在用的程式在打那兩個網址。** `memory/stats` 跟 `memory/recent` 在整個
專案裡找不到呼叫端，是我巡檢時自己打出來的。所以沒有功能是壞的。真正有問題的是那個形狀：
任何打錯字、任何還停在舊路由的客戶端、以及未來任何被 `/:id` 蓋住的新路徑，都會產生一個
說「伺服器壞了」的錯誤。

### review 回合抓到四件，其中兩件是同一個毛病還活著

**第一件：沒有任何自動化測試在測真正壞掉的那件事。** 原本只測了「數字怎麼判斷」，但壞的
從來不是數字判斷，是路由登記的順序。把整段掛載刪掉，4323 條測試照樣全綠。補了一支會真的
發出請求的測試（12 個情境，打真的伺服器、真的資料庫）。

**第二件：同一個毛病在「還原舊版本」那條路上還活著。** 要還原到哪一版的編號是從請求內容
帶進來的，一樣直接送進資料庫。實測 `history_id` 給 `abc` → 500「還原失敗」，給超大數字也是。
對照組：格式正確但不存在的版本編號本來就回 404。用同一支檢查擋掉。

**第三件：「其他路由沒有這個形狀」講太滿。** 實測 `PUT /api/handoff/abc/accept` → 500
「接受交接失敗」。那邊沒有被蓋住的固定路徑，所以沒有真的端點在壞，但話不能那樣寫。同一道
檢查也裝上去了。

**第四件：有個變數設了卻沒人用。** 它看起來像「以後大家都該用的那個編號」，於是下一個人
可能在其中一條路由用它、其他條繼續用舊的，兩邊會悄悄不一致。刪掉。

### 驗證

三道檢查各弄壞一次：拿掉記憶那道 → 12 個情境紅 6 個；拿掉還原那道 → 紅 1 個；拿掉交接那道
→ 紅 1 個；全部裝回去 12/12 綠。不是在假的資料庫上測的，是真的起一套伺服器打進去。
全套測試過、端對端過。

## v1.26.135 — 那張稽核表裡的二十五萬筆，沒有一筆是真的

### 量到的事實

2026-08-10 正式機，直接數 `usage_audit_log`：

| | 筆數 |
|---|---|
| 整張表（2026-04-22 起） | **253,409** |
| 其中「不認識的模型」 | **253,409** |
| 其他任何類型 | **0** |
| 最近 14 天 | 94,713 |

最近 14 天照模型拆開：opus-5 有 62,405 筆、opus-4-8 有 18,484 筆、sonnet-5 有 10,397 筆、
fable-5 有 2,317 筆、gpt-5.5 有 1,100 筆。**這張清單上每一個模型，都是團隊每天在用的。**

一天長 6,700 筆，而且 100% 是假警報。

### 為什麼

判斷「這個模型我認不認得」是去查價目表。在成本還是功能的年代，這個代用品很合理。

**v1.26.60 把成本計算拔掉了。** `cost_usd` 從此刻意寫 NULL、儀表板也不再讀它。沒有人拔掉的
是那個「查價目表」的動作 —— 價目表還在、內容還停在 2026 年 4 月、而它仍然是整條收資料的路上
唯一一份「認得的模型」名單。它裡面最新的 Anthropic 模型是 opus-4-7。

所以從團隊換到 opus-5 的那一天起，每一則訊息都過不了那份名單，每一則都寫一筆稽核。
而 token 倒退、指紋不符、指紋碰撞這些真正的異常，寫的是同一張表 —— 真出一筆，就是二十五萬
分之一。

一個每則訊息都響的訊號，不是比較大聲的訊號，是沒有訊號。

### 這一版改什麼

**同一個模型只記第一次，之後閉嘴。** 寫之前先問這張表：這批裡面哪些模型已經有紀錄了，有的
就跳過；同一批裡面同一個模型，也只由第一則訊息去記。新模型第一次出現時你還是看得到那一筆，
那正是原本想要的通知。

`db/024` 加一道只管「不認識的模型」這一類的唯一索引，寫入改成撞到就安靜放掉。兩份上傳同時
帶著同一個沒見過的模型時，兩邊都會先讀到「還沒記過」；索引把那個空窗關掉，而安靜放掉是為了
不要讓輸的那邊去記一筆「寫入失敗」——它本來就不需要寫。

其他類型的稽核不受影響，它們本來就該每則訊息記一次，索引也管不到它們。

**沒有一起做的事：** 沒有把價目表補新。補了等於又把稽核綁回一張早就沒有存在理由的表。改成
一個模型一筆之後，名單過期的代價是「每個新模型多一筆」，不是「每則訊息多一筆」。

既有那 253,409 筆已備份後清空（2026-08-10，Vin 指示）。

### 第一輪 review 抓到四件事，其中一件會讓這一版做不到它自己要做的事

**第一件：重送的那一則會把名額吃掉。** 原本是在寫入之前就先決定「這批裡由哪一則去記」。
但上傳失敗會從上次確認的位置重送，所以一批的開頭很可能是伺服器早就收過的訊息。那一則會先
把名額佔走，接著因為重複而不寫入，於是整批一筆稽核都沒有 —— 而同一批後面那些真的是新的
訊息，已經被判定「這個模型有人認領了」。**一個真的沒見過的模型，就這樣永遠不會被記下來。**
改成在確定寫進去的那一刻才認領。

**第二件：那道索引會讓還沒清空的資料庫開不了機。** 建唯一索引碰到既有重複值會直接失敗，而
「已存在就跳過」比對的是索引名字，根本不會去看資料。遷移失敗會讓伺服器中止啟動，而且失敗
不會被記錄成「跑過了」—— 每次重開機都再撞一次。正式機是手動清空過才沒事，但開發用的資料
庫、第二套部署、或任何人把備份還原回去都會中。遷移檔現在自己先把重複值收斂掉。

**第三件：那四條新測試其實沒有測到這一版改的東西。** 把應用層的去重整段刪掉，測試還是全綠 ——
因為假的資料庫模擬了那道索引，等於索引一個人扛下了所有斷言。現在假資料庫改成也數「嘗試寫入
幾次」，並補了三個情境：開頭是重送、整批都是重送、以及查詢讀到過期資料因而真的去撞索引。

驗證方式是把修好的邏輯故意弄壞兩次，確認對應的測試會紅；遷移檔則是在正式機的資料庫裡灌進
重複值、整支跑過再回滾，確認它收斂得掉、重跑不會壞、也沒動到其他類型的稽核。

第四件是小事：那句「撞到就安靜放掉」原本沒有指定要看哪一道索引，會把這張表上任何一種重複
都一起吞掉，包含還原備份後流水號沒跟上造成的主鍵衝突。指定清楚。

### 第二輪 review：兩件重要的，其中一件是第一輪修了一半

第一輪找出「測試沒有在測這一版改的東西」並且修好了 —— 但只修了「同一批裡面」那一半。
**「跨批次」那一半，也就是這一版真正在做的事，仍然沒有任何測試擋得住它。** 把那段查詢整個
廢掉、讓它永遠回答「沒有人記過」，33 條測試還是全綠，因為那條測試只數了「表裡有幾筆」，
而假資料庫模擬的索引自己就會讓答案是 1。現在改成數「有沒有去嘗試寫」，兩種弄壞法都會紅。

**第二件：那道收斂重複值的 DELETE，在它專門要處理的那種資料上會跑到天荒地老。** 原本的寫法
要把每一組重複「兩兩配對」展開，而這張表正好是最糟的形狀：二十五萬筆只有六種模型。在灌成
一模一樣形狀的 Postgres 16 上實測，查詢規劃器估出 9,553 萬個配對，跑到 3 分 59 秒還沒結束
被我中止。這個連線池沒有設查詢逾時，而遷移沒跑完伺服器就不會開始服務 —— 它不會失敗，它會
讓伺服器一直起不來。第一輪驗證時只灌了兩筆重複，所以完全沒看出代價。改寫成「每一組只留最
早那一筆」：**527 毫秒**，留下來的是同樣那幾筆。

還有一件小的：模型名稱如果自己帶著 `::`，會被查詢用的鍵切錯半邊，於是每一批都重新嘗試寫入
一次、永遠如此，而索引會安靜地把它擋掉，所以哪裡都看不到。同一個寫法在工作階段查詢上也有，
一起改掉。
## v1.26.134 — 安裝腳本印「已更新」的依據，是它的意圖，不是檔案

靜態審查兩支安裝腳本時找到的。先講好消息：**IR-001、IR-002、IR-003 三條鐵律的原始坑都已經補好了**
（分別在 v1.26.91 改掉「偵測到已設定就跳過寫金鑰」、消音點都補上守門、v1.26.98 把升級日誌
搬到 `~/.ownmind-logs` 這個還原範圍外的位置）。下面是剩下的缺口。

### 缺口一：金鑰寫完沒讀回來確認

`install.ps1` 與 `install.sh` 都是寫檔之後，拿記憶體裡的舊金鑰跟新金鑰比，印出
「API key updated」。**那是這次執行的意圖，不是檔案裡真正的內容。**

而同一支檔案往下三十行，同一把金鑰寫進 `~/.claude.json` 那半邊，是讀回來確認過的，
訊息還特別寫著 `(verified by reading it back)`，註解也點名 IR-001。

於是一次金鑰寫入的兩半，證據標準不一樣。**沒被確認的那一半，正是開場載入、鐵律檢查、
回覆檢查讀金鑰的地方**，也就是換帳號時必須改到的兩個位置之一。

改法：寫完讀回來、解析、比對金鑰。對不上就明說「金鑰不在檔案裡，掛勾會繼續用舊帳號，
自己開檔案看」——不消音、也不中斷安裝。對得上才印原本那三句，並補上
「verified by reading it back」。

### 缺口二：`ConvertTo-Json -Depth 10` 超過深度不會報錯

PowerShell 超出深度時**只發警告**，把超出的分支寫成 `System.Collections.Hashtable`
這個字串。實測 5.1，即使 `$ErrorActionPreference = 'Stop'`：

```
$deep | ConvertTo-Json -Depth 3
→ {"a":{"b":{"c":{"d":"System.Collections.Hashtable"}}}}
```

`settings.json` 在使用者還沒加任何掛勾之前就已經五層深。四個寫入點全部改成
`-Depth 100`（cmdlet 接受的上限）。

這兩個缺口疊起來就是完整的失效鏈：JSON 被截斷寫壞 → 沒有讀回來確認 → 照樣印
「API key updated」。缺口一的修法同時也接住缺口二漏掉的情況。

### 附帶：成功宣告排在證據之前

兩支腳本都是先印「OwnMind installation complete」，自我檢查與產出檢查才跑。
`install.sh` 更明顯：它會在「complete」底下印出「[FAIL] Installation did not complete.」
——同一次執行自我矛盾，而且**宣告先到**。

兩邊都把橫幅移到檢查之後。`install.ps1` 原本包自我檢查的 `try { … } catch { }` 也補上訊息：
自我檢查自己爆掉時，原本一個字都不會說。

### 新增測試

`tests/installer-key-update.test.js` 從 13 項加到 18 項：兩支腳本都要讀回來確認、
都要有「金鑰沒落地」的訊息、深度不准退回 10、橫幅必須排在檢查之後、自我檢查爆掉要說話。
既有那個「真的執行 install.sh 抽出來的寫入區塊」的測試，斷言改成必須看到
`verified by reading it back` ——那句話只有走完讀回來比對那條分支才印得出來，
所以它證明的是程式真的跑過那段，不是原始碼裡有那段。
...
2026-08-10 這台機器，實際把掛勾跑起來、餵它 Claude Code 真正會送的 payload：

| 鐵律 | 作者自己標的觸發詞 | 安裝時會不會出現 |
|---|---|---|
| IR-001 換金鑰／切換帳號後必須自己開設定檔確認 | `install` `setup` `config` `api_key` | **不會** |
| IR-002 `set -e` 腳本裡的 `2>/dev/null` 是紅旗 | `install` `script` `shell` `debug` | **不會** |
| IR-003 回滾／清理前先把日誌搬走 | `cleanup` `回滾` `還原` | 會 |
| IR-004 回話要用白話中文 | `reply` `language` | 不適用（走 Stop 掛勾） |

`bash install.sh --api-key <k>` 的實測結果是：沒有任何輸出，exit 0。

**IR-001 的內容整條就是「安裝腳本印的成功訊息一律不採信，要自己去開設定檔看值」。**
它在它指名的那個操作上，是沉默的。

### 缺陷一：那些觸發詞從來沒有人問過

觸發閘認得四種指令形狀 —— `git commit/reset/rebase/merge`、`git push`、刪除類、部署類 ——
其他一律在讀金鑰之前就 exit 0。標籤沒有寫錯，是**沒有任何觸發類別叫 `install`**，
所以沒有人來問這個標籤。

安裝與換金鑰是這套系統風險最集中的操作之一，而它是唯一一類完全沒有閘門的操作。

### 缺陷二：拿不到鐵律的時候，跟「沒有規則符合」長得一模一樣

```
curl -sf --max-time 3 ... 2>/dev/null
```

三個消音疊在同一行：`-s` 藏掉進度與錯誤，`-f` 讓任何 4xx/5xx 回一個空的 body，
`2>/dev/null` 把剩下的丟掉。於是 `RULES` 是空字串，而空的 `RULES` 走的是
「沒有規則符合這個操作」那條路 —— 也就是沉默。

**伺服器掛掉、金鑰被撤銷、以及真的沒有規則符合，這三件事對掛勾和對使用者都長一樣。**
鐵律是安全機制；一個能把自己無聲關掉的安全機制，比沒有還糟，因為沒有至少看得見。

**這支檔案自己知道這件事。** 同一份檔案裡另一段的註解寫著：這裡不加 `2>/dev/null`，
因為把失敗變成沉默正是那個 bug 藏了好幾週的原因（v1.26.120）。那次修的是憑證讀取，
隔壁最關鍵的那個呼叫沒有跟著改 —— 跟 v1.26.131 是同一種形狀：修好了一支，
沒修隔壁那支。

### 這一版改五件事

1. **新增 `install` 觸發類**。認安裝／設定腳本的檔名（`install.sh`、`setup.sh`、
   `bootstrap.sh`、`update.sh`，含 `.ps1/.bat/.cmd`）與金鑰字樣（`API_KEY`、`credential`）。
   判定放在所有既有判定**之後**，所以沒有任何原本有觸發類的指令會改變分類。
2. **`TRIGGER_TAG_ALIASES` 加上 `install`**，收 `install/setup/config/安裝/設定/api_key/
   credential_rotation/換金鑰/切換帳號`。刻意比其他類窄：不收 `script` 和 `debug`，
   否則每一條講 shell 的規則都會擠進一次不相干的換金鑰。
3. **`detectTriggerFromContext` 也認得 install**。這是 MCP `report_compliance` 用的入口，
   同一個缺口往裡一道門。不補的話，就是在另一個入口重建這一版要關掉的那個落差 ——
   規則在用的詞彙，沒有任何呼叫端問得出來。補上不會啟用任何強制行為：它只會選中
   `verification.trigger` 明寫 `install` 的規則，而今天一條都沒有。
4. **取鐵律失敗會留下紀錄**。body 寫到檔案、狀態碼進變數，失敗時寫一筆
   `iron_rule_fetch_failed` 到活動紀錄，帶著 HTTP 狀態或 curl 的離開碼
   （6 = DNS、7 = 拒絕連線、28 = 逾時）與當下的觸發類別。紀錄不走 stdout ——
   stdout 是要交給 Claude Code 的信封，必須維持格式。
5. **逾時 3 秒 → 5 秒**。這是「提醒最多可以拖住指令多久」的上限，不是正確性旋鈕；
   但 3 秒會讓一條普通的慢線路無聲地把鐵律弄丟，那是同一個缺陷換一條路到達。

### 一個邊界條件，差點又是同一種沉默

`\bAPI[_-]?KEY\b` 抓不到 `OWNMIND_API_KEY`：**底線是單字字元**，所以 `D` 和 `A` 之間
根本不存在 `\b`。而帶前綴正是每一個真實環境變數的樣子，也正是這條規則要管的樣子 ——
唯一抓不到的形狀，剛好就是唯一會出現的形狀。改成守在「前面不是英文字母」，
`therapy_keys` 這類仍然不會誤中。

### 沒有動的地方

- **`npm install` / `pip install` 不觸發**。一個在每次裝相依套件時都跳出來的提醒，
  使用者三天內就學會跳過它，而這兩條規則講的是憑證和安裝腳本，不是抓套件。
- **`.js` 版掛勾的 `|| 'command'` fallback 不變**。兩份掛勾對「沒有觸發類」的處理本來
  就不同（`.sh` 直接退出、`.js` 落到 `command`），那是另一件事，不在這一版的範圍。

## v1.26.131 — 更新不了的那兩台，正好也是沒辦法說出自己更新不了的那兩台

### 量到的事實

2026-08-10 正式機，直接查活動紀錄表、沒有經過任何報表：

| | 版本 | 活動紀錄（全部歷史） |
|---|---|---|
| 七個用 Claude Code 的人 | 1.26.125～1.26.128 | 合計 16,733 筆（其中 8,087 筆是「更新跳過」） |
| Amiee | 1.26.57，8/4 起沒動 | **0 筆** |
| Joanna | 1.26.27，6/18 起沒動 | **0 筆** |

零不是「二十天內零」，是**從來沒有過一筆**。而這兩台每天都有心跳，代表程式確實有啟動，
而且心跳是帶認證的，所以連線位址跟金鑰都是好的。

同一支程式對另外七個人一路正常上傳。所以這不是伺服器擋掉、不是金鑰、不是查詢問題。

### 兩個會造成這個結果的缺陷，都在同一支檔案裡

**第一個：路徑。** 紀錄要寫到哪裡是用 `join(process.env.HOME || '', '.ownmind', 'logs')`
算的，而 **Windows 沒有 `HOME` 這個環境變數**。那個「`|| ''`」讓它變成相對路徑，於是紀錄
被寫到「程式啟動時剛好所在的資料夾」。這一點本身就該修：就算那個資料夾可以寫，結果也是
在使用者的專案裡留下一個他沒要求過、還可能被 commit 進去的紀錄資料夾。

**第二個：順序。** 建資料夾是那個 try 裡的第一件事，排隊上傳在最後。所以只要建資料夾出錯，
整個事件在被排進上傳佇列之前就沒了，而外層的 `catch {}` 什麼都沒說。**一個檔案系統問題不是
把事件降級成「有送出、只是本機沒存」，是把兩份一起刪掉。**

心跳活著是因為它完全不碰檔案。

**這一行這個專案已經付過一次錢。** `mcp/index.js` 自動更新那段上面的註解寫著 v1.17.22：
「Alice（Windows）跟 Bob 卡在舊版的根因：`process.env.HOME` 在 Windows 上是 undefined」。
那次修好了 `index.js`，但隔壁那支負責記錄的檔案沒有跟著改 —— 於是更新不了的機器，同時也
失去了說出「我更新不了」的能力。

用 Claude Code 的人不受影響，而且是兩層不受影響：他們的程式是在一個可以寫入的專案資料夾裡
啟動的，而且他們的活動紀錄還有另一條完全不同的上傳路徑。這個問題從頭到尾只在沒有那條路徑
的機器上看得到。

### 還沒證實的那一段

「那兩台的資料夾真的不能寫」是**推論，不是量到的**。另一個同樣說得通的可能是：那兩台的
程式在 30 秒內就被關掉，而排隊中的紀錄還沒送出去。

這一版三個改動對兩種可能都有效，所以不必先分辨出來才能修；但紀錄在這裡，是因為要分辨
只需要問一個問題：**在她們的電腦上、`%USERPROFILE%` 以外的地方，找不找得到
`.ownmind\logs\*.jsonl`。** 找得到就是後者，找不到就是前者。
...
1. 路徑改成 `HOME || USERPROFILE || os.homedir()`，跟這個專案其他每一支檔案一致。
2. **先排隊、再寫檔**，而且寫檔有自己的 try。兩份紀錄現在各自獨立失敗，而看得到的那一份
   不再是先死的那一份。
3. 「更新成功／失敗／跳過／本來就最新」四種結果改成立刻送出。它們一天最多發生一次，排隊
   一點好處都沒有；而這四種結果都是在程式剛啟動的頭幾秒產生的，正好落在「還沒送出去」的
   那個空窗裡，而某些工具關掉子程式時是直接砍掉、不給訊號，那條「關閉前補送」的路根本
   不會執行。

另外順手把另外三個檔案裡同樣的寫法一起改掉。它們目前呼叫端都有帶參數、碰不到，但這一版
的前提就是這個寫法會一再出現。

## v1.26.130 — 檢測說這台壞了，修復程式在同一台上說它很好

2026-08-10 正式機。兩位使用者、都在 1.26.125，自我檢測每天回報同一句：
...
`self-check.cjs` 從 v1.26.124 開始會比對工作的 actions 跟這次安裝的目錄，所以它抓得到。
但**自我檢測只負責報，不負責修**。真正會修的是 `ensure-scanner-schedule.ps1` ——
每次自動更新都會跑一遍 —— 而它問的是比較弱的問題：

```powershell
if ($task -and $task.State -ne 'Disabled') { "already_registered"; exit 0 }
```

他們兩台的工作**存在、而且是啟用的**，只是驅動另一個安裝目錄。所以修復程式判定「這台很健康」
就返回了，每天一次，在一台同一支程式正在說它壞掉的機器上。

**看得到、修不到。** 升級救不了他們，機器上也沒有別的東西會去碰它。

`Get-ScheduledTask` 是全機器共用的：同名的工作存在，只代表這台機器上有人裝過 OwnMind，
不代表這次安裝註冊過它。這正是 v1.26.124 把 `taskBelongsToInstall()` 抽出來的原因，
而修復端一直沒有跟上。

這一版把「什麼叫健康的排程」寫成一份可以 dot-source 的 PowerShell 規則
（`schedule-health.ps1`），修復前的閘門跟重新註冊後的驗證都走它。三種情況都算壞掉：
被停用（v1.26.79）、屬於別的安裝目錄（這一版）、狀態讀不到 —— 最後一種是為了讓修復端
不會比檢測端寬鬆，不然兩邊又會各說各話。

規則刻意寫成純字串邏輯、不碰任何 Task Scheduler 指令，所以測試可以在 macOS / Linux 上
**真的執行它**，跟 JS 那一份跑同一張案例表。純文字比對抓不到這個 bug：舊的閘門讀起來
完全正確，它只是問錯問題。

### 順帶修掉一個「修復程式永遠修不完」的組合

修復程式本來認 `OWNMIND_DIR` 這個安裝路徑覆寫，而**寫入排程的那支跟自我檢測都從來不認**。
安裝在非預設路徑的機器因此會進入一個永遠繞不出來的迴圈：閘門說這個工作不是我的 → 重新
註冊 → 寫進去的還是預設路徑 → 驗證又說不是我的 → 每天送一次失敗回報。那個覆寫也不可能
當共用值：它是安裝當下才存在的環境變數，每天自動更新跑的時候根本讀不到。三邊現在算同一
個值。

失敗訊息也補上了「這個工作實際指到哪裡」。這個 bug 從 v1.26.79 活到現在，正是因為看得到
的那份回報從來沒寫這件事。

## v1.26.129 — 程式被複製到旁邊執行，而它要用的東西沒跟著搬

### 先講最嚴重的：v1.26.127 會讓記憶完全載不進來

`update.sh` 把 `hooks/lib/*.js` 複製到 `~/.claude/hooks/lib/`，但**從來沒有複製 `shared/`**。
所以任何一支 `import '../../shared/…'` 的模組，在那個副本位置解析出來的是
`~/.claude/shared/`，那個資料夾不存在，模組載入直接死。

v1.26.127 為了發技巧，在 `render-session-context.js` 加了一行 `import shared/tips.js` ——
**那一行把「產生 session 開場內容」這件事整個推進同一個死區**。使用者升到那一版之後，
開新對話會完全沒有記憶、沒有鐵律。這一版在發出去之前修掉。

### 順著這條線挖到一個早就死掉的功能

`conditional-sync.js` 一直都 `import shared/scanners/base.js`，而 hook 呼叫的是
`$SCRIPT_DIR/lib/conditional-sync-cli.js` —— 副本那一份。實測跑它就是
`ERR_MODULE_NOT_FOUND`。

**每一個呼叫點都把 stderr 導去 `/dev/null`**，所以它退化成「`INIT_DATA` 是空的」，
而 hook 把空值當成「API 慢」。「只在記憶有變動時才下載」這個省流量的機制，
每次開對話都白跑一趟，沒有任何人看得出來。

`update.sh` 自己的註解早就講過這件事 —— usage scanner 那一段寫著「需要 shared/ 模組、
所以放在 `$OWNMIND_DIR` 執行」。這一版讓 session hook 跟那句話一致：
`LIB_DIR` 優先指向工作目錄本體（`shared/` 就在旁邊），找不到才退回副本。

守門測試是照真實環境重建的：把那幾支模組複製到一個**沒有 `shared/`** 的資料夾再跑，
斷言它真的會死。任何一個呼叫點改回「跑副本」也會紅。

### 自動更新不再是靜悄悄的

背景更新本來就每天自己跑，但每一種結果都只寫進本機事件紀錄。**成功跟失敗，使用者看到的
是同一件事：什麼都沒有。** 而一個一直失敗的更新，跟一台已經是最新版的機器，從使用者那邊
看完全一樣。

更新跑在脫離 session 的子行程裡，結果在那個對話結束前不會知道，所以訊息走
`banner-pending.jsonl` —— 下一次開對話印出來：

- 成功 → 「ownmind已經自動更新到 x.xx版，升級成功，你不需做任何處理」
- 失敗 → 講卡在哪一步（用白話，不是 `npm` / `pull` 這種字），並問要不要幫忙回報給管理者
- 本來就是最新版 → **什麼都不說**（沉默必須繼續代表「沒事發生」，否則訊息會變成雜訊）

三支更新程式都要接上，不是只接一支：MCP 跟 bash hook 搶同一把鎖，誰先搶到誰做，
Windows 上做事的又是第三支。只接一支的話，「不再靜悄悄」會變成擲骰子。
（`STEP_LABELS` 裡本來就有 `lock`、`update_script_missing`、`outer` 三個 bash hook 根本
不會產生的步驟 —— 那三行本身就是漏接的證據。）

### 「有新版本」廣播改成「你的自動更新壞了」

原本的門檻是 `${SERVER_VERSION}-prev`，也就是「只要不是最新版就提醒」。這個 repo 一天發
好幾版，等於幾乎每個人每天都被提醒 —— 而且它是強制等級的廣播，會佔走 AI 的第一句話。
**更新已經是自動的了，每天叫使用者去做一件已經發生的事，只會訓練他跳過通知。**

改成落後 10 個版本以上才發，內容也從「有新版本」改成「你的自動更新好像沒在運作」，
並附上「回報 ownmind bug」這條出路 —— 因為那正是自動化救不了、只有人能處理的情況。

**只改門檻等於什麼都沒改。** 這些自動廣播從來沒有寫過 `ends_at`，而後台 API 拒絕撤銷
`is_auto` 的廣播，所以 v1.17.0 以來每一則都還活著。落後五版的人照樣命中昨天那一則、
看到的還是舊文案；而抓取是新的排前面、最多顯示三則，真正卡住的人會看到新訊息底下
壓著兩則過期的。所以這支排程現在每次執行都會把自己取代掉的那些收掉，
範圍限定 `is_auto = TRUE` 且 `type = 'upgrade_reminder'`，人手寫的一個都不碰。

## v1.26.128 — 團隊規範送到了 Claude Code 面前，然後被丟掉

上一版把技巧提示接好之後，Vin 說「讓團隊每個人的 AI 自動遵守同一套規範」這句技巧他要留著。
去查它成不成立，結果查出一個更大的東西。

`/api/memory/init` 一直都有回 `team_standards_digest`，而且它放在 `!compact` 的**外面** ——
也就是說，它是**專門為了 hook 這條路**送的（每一個現行呼叫端都帶 `compact=true`）。
但 `hooks/lib/render-session-context.js` 從來沒有讀它。

所以團隊規範只到得了「會呼叫 `ownmind_init` 工具」的那些工具，到不了「用 SessionStart hook
載入記憶」的那些 —— 而 Claude Code 整個在第二類。**規範上傳了、伺服器也送了、AI 沒收到。**

跟 v1.26.127 那個技巧完全同一個形狀：伺服器送了、算繪那一層丟掉了，而唯一看得到的症狀是
「AI 沒有遵守一條它從來沒拿到的規則」—— 那看起來只像 AI 不聽話。

現在 session 開場會列出團隊規範，位置排在鐵律**後面**（衝突時鐵律優先，用順序講，不用寫一句話
說明），並附上「要看全文用 `ownmind_get("standard_detail")`」，因為摘要只有標題，
看得到名字卻讀不到內容等於還是遵守不了。

技巧文案兩句照 Vin 的意思回到原本的寫法：「回報 ownmind bug 直接送到管理者那邊」、
「讓團隊每個人的 AI 自動遵守同一套規範、不會不小心違規」。後面這句現在是送得到的，
它靠的跟鐵律是同一個東西 —— AI 遵守載入的規則，沒有 hook 會擋下違規動作。

## v1.26.127 — 同一份技巧清單抄了兩份，其中幾條講的功能沒有做

OwnMind 每次工具回應都會附一句技巧，從 v1.17.7 起無條件帶上。走 API 的用戶端則是在
`/api/memory/init` 的操作手冊裡拿到同一批。兩份清單各 28 條，一字不差 —— 而**一字不差
正是「複製的清單」在開始走樣前一刻的樣子**。沒有任何程式在比對它們，走樣之後會變成
同一個產品對 MCP 使用者跟 REST 使用者自我介紹得不一樣。

其中幾條講的功能沒有做：

- 「問『哪些鐵律被停用了』可以回顧過去的決策變更」 —— 沒有任何工具讀得到已停用的記憶，
  `ownmind_get` 沒有這個參數，API 只在同步用的 `since` 路徑才回停用列。
- 「OwnMind 會持續進化，AI 會主動建議工作流程與規則改進」 —— 沒有實作、也沒有任何指令要它這麼做。
- 「就算在線上 AI（claude.ai、ChatGPT）也可以匯出與載入記憶」 —— 沒有匯出這條路。

**技巧是產品用自己的口吻對自己下的斷言**，前面掛著真的版本標記，後面接著一次真的操作。
旁邊沒有任何東西標示哪一句只是願望，所以使用者分不出來，代價由他付：跑去找一個不存在的功能。

### 一份清單、兩個消費者

`shared/tips.js` 變成唯一的清單。`mcp/index.js` 從它拿 `getRandomTip`；操作手冊改成內插
`renderTipPool()`，不再把條目重寫一遍。兩邊本來就都有從 `shared/` import，不用動打包。

每一條帶一個**錨點** —— repo 裡讓這句話成立的那個東西：`ownmind_*` 是 `mcp/index.js`
真的有註冊的工具名，`file:<路徑>` 是單一工具涵蓋不到的能力（落在本機的記憶檔、機器與模型
的紀錄、session 壓縮）。`tests/tips-list.test.js` 逐條把錨點解出來，**寫不出「一個不存在
的功能」的技巧**，工具改名也會在改名當下就紅，而不是紅在使用者面前。

清單本身 28 條變 25 條：13 條原封不動、7 條砍掉、8 條改寫、4 條新增。

砍掉的七條裡，四條講的東西不存在（哪些鐵律被停用了、AI 會主動建議流程改進、線上 AI 匯出
載入、2 小時／50% 主動提醒 —— 最後這條確實有寫，但只寫在六份範本的其中兩份，而技巧是每個
用戶端都看得到）。另外三條是 Vin 讀過一遍砍的，那是這件事的另一半：**錨點只證明那個東西
存在，不證明這句話值得拿給人看。** 砍掉的是只有做 OwnMind 的人才在意的 —— 記了哪台機器
哪個模型、鐵律有編號可以直接引用、另一條停用鐵律的說法跟既有那條重複。

補上四條本來沒人提的日常時刻：開工、收工、回報 bug、團隊規範。改寫的八條主要是拿掉內部
說法（不用專案名當例子、不講「多關鍵字查詢」、交接不指定 Codex），以及把話講回它做得到的
範圍 —— 團隊規範原本寫「大家的 AI 自動遵守、不會不小心違規」，但 hook 只讀鐵律、不讀團隊
規範；密鑰原本寫「敏感資訊一律不會寫進記憶」，但寫入時的偵測是**刻意保守**的
（`shared/secret-detect.js` 寧可漏也不要誤擋），相信那句話的人才會把 token 貼進記憶。

### 真正會編出東西來的那個洞：叫它念一句，卻沒給它

Vin 實際遇到的是 AI 講了一句跟 OwnMind 無關的技巧。清單存在不代表那一句是從清單來的 ——
**技巧是掛在 MCP「工具回應」上的，而用 SessionStart hook 載入記憶不是一次工具呼叫。**
`configs/AGENTS.md` 的啟動流程要 AI 在記憶載入後立刻印一句技巧，那條路上從來沒有人給它一句。
指令有、東西沒有，模型就自己補。

所以 `hooks/lib/render-session-context.js` 現在也帶一句，同樣從 `shared/tips.js` 抽：

```
Tip (relay this one verbatim; do not compose your own): ...
```

**叫別人轉述某個東西，只有在那個東西真的存在的時候才安全。**

### 範本改成沿用，而不是自己想一句

五份設定範本原本寫「技巧提示：[隨機一條]」，沒有指定來源。現在改成沿用 OwnMind 給的那句、
不可自行編造、**而且沒給就不要顯示這一行** —— 最後這句才是真正堵住編造的那一條，因為
「沿用」本身不管「沒得沿用的時候怎麼辦」，下一條新路徑一樣會漏。

守門是逐**行**檢查不是逐檔：`configs/AGENTS.md` 有兩個技巧位置，逐檔檢查會被「其中一個還
是對的」滿足，另一個偷偷改回去也不會紅。

`configs/CLAUDE.md` 從來沒有這條指令，這版也沒有加。

## v1.26.126 — 更新紀錄的按鈕做好了六個月，只是沒有人把資料接上去

後台頁尾那顆「系統版本更新紀錄」，點下去一直是空的。彈窗、時間軸樣式、三個語系的字串
都在 v1.20.0 就寫好了，缺的只有資料：`client/src/App.jsx` 往下傳的是字面上的 `[]`，
旁邊的註解寫著「真正的更新紀錄來源是獨立的一件事」，而那件事沒有人做。

所以使用者看到的不是壞掉的畫面，是一個運作良好的空狀態 —— 它誠實地說「目前尚無版本紀錄」，
而檔案裡躺著 299 筆。**空狀態寫得越好，缺資料就越不像 bug。** 這是這個 bug 活六個月的原因。

### 資料來源就是 CHANGELOG.md 本身

沒有新增第二份發版清單。理由跟 v1.26.43 把版號改成跟 server 拿一樣：兩個地方要更新，
其中一個一定會有人忘記，而忘記不會報錯。這份檔案是本來就在維護的那一份。

`src/utils/changelog.js` 解析標題列。檔案裡實際存在三種寫法，往回捲都還讀得到：

```
## v1.26.125 — 標題          （現行，破折號）
## v1.15.4 - 標題            （v1.16.0 以前，ASCII 連字號）
## 2026-03-26 — v1.4.0 標題  （最早期，日期在前）
```

只認第一種的話，284 筆會變成沒有標題的空白列。日期只有最早那批有，所以其餘的 `date` 是空字串，
時間軸就不畫它 —— 空的 `<span>` 會被讀成「這裡少了一個值」，而不是「這個值當初就沒記」。

摘要取的是第一段散文。凡是「結構」而不是句子的東西都跳過去找下一段：圍籬區塊（有幾筆是用一段
log 開頭的，直接取會把堆疊片段當成摘要）、`###` 小標、表格、清單、引用。

小標那條原本寫成「遇到就停」，結果最近三十筆裡有八筆是直接從 `### 修法` 開頭的，
彈窗有四分之一只剩標題。**小標底下那段仍然是這一筆自己的內容**，不是去借別人的，所以改成跳過。

還有六個標題寫成 `## v1.26.98（同版）— …`，那是寫給讀 markdown 的人看的註記。留著會讓標題變成
`（同版）— 回滾失敗時…`，破折號卡在句子中間。現在解析時拿掉 —— 它要講的事情，時間軸上
連續幾列都是同一個版號，本來就看得出來。

**圍籬區塊裡的 `##` 不算標題。** 這一段本身就是反例：上面那三行範例被當成三筆發版讀了進去，
於是時間軸第二列出現一筆憑空生出來的 v1.15.4，標題是「標題（v1.16.0 以前，ASCII 連字號）」，
壓在真正的歷史上面。整份檔案有六個這種引用。

順帶記一件對客戶端有影響的事實：**版號不是唯一鍵。** v1.26.98 底下有六筆、v1.26.87 跟 v1.17.1
各兩筆，所以時間軸的 React key 是「版號 + 位置」，只用版號會讓重複的那幾筆被丟掉。

### 這次動到的是伺服器要讀的檔案

`GET /api/changelog` 掛在 auth 後面，理由跟 `/api/version` 一樣：唯一的消費端在 RequireAuth
底下，加金鑰不花成本；而把發版清單交給未登入的掃描器，等於直接告訴它這台可能還缺哪些修復。

Dockerfile 補了 `COPY CHANGELOG.md`。`src/` 跟 `db/` 是逐個 COPY 的，放在它們之外的檔案不會
進映像 —— 而解析失敗會安靜地退成 `[]`，症狀正好是「開發機上好好的，正式機是空的」。

**但 COPY 只有一半。** `.dockerignore` 有一條 `*.md`，所以那道 COPY 的真正下場不是「檔案沒進去」，
是整個 build 直接失敗：

```
failed to compute cache key: ... "/CHANGELOG.md": not found
```

那份排除清單開頭第二行就寫著「若未來 server code 新增讀檔路徑、要從這份排除清單拿掉」，
而這次就是那個未來，還是漏了。現在加了 `!CHANGELOG.md`，並且把守門的測試從「Dockerfile 裡有沒有
那行 COPY」換成「**每一行從 build context 來的 COPY，來源都不能被 .dockerignore 排除**」——
原本那條在 build 壞掉的狀態下照樣是綠的。這個 repo 沒有 CI 會 build 映像，所以這種錯要等到部署才會撞上。

### 順帶拿掉右下角的「OwnMind 開源計畫」

Vin 要求移除。三個語系的 `footer.copyright` 一起刪掉，沒有留孤兒鍵。頁尾原本是三欄
`justify-between`，少一欄之後按鈕從中間移到右邊，這是預期內的。
...
- 三份 README 開頭的「目前版本」還停在 v1.26.116。連續四個版本（117–120）我都只加了
  條目、沒改那一行 —— 而那條測試存在的理由，正是 v1.26.90／91 也是這樣漏掉的。已補到 v1.26.121。
- v1.26.119 我在註解裡寫了真實的家目錄路徑（含使用者名稱），撞到「產品程式碼不可以出現
  擁有者姓名」那條守門測試。改成 placeholder。

**順手修掉的（一條，本來就紅）**

- 「產品程式碼不可以出現個人鐵律編號」那條守門測試，在 Windows 上**永遠是紅的**：
  它的白名單寫 `src/routes/me.js`，而 `path.relative` 在 Windows 上回的是
  `src
outesme.js`，所以那個唯一有正當理由的例外從來沒被比對到 —— 守門的自己倒了，
  而原因跟它守的東西完全無關。跟 v1.26.119 那批分隔符是同一族。

順帶把 v1.26.120 註解裡引用的個人鐵律編號改成描述規則本身，那正是這條守門測試要求的寫法。

### 剩下的紅燈，確認都不是這次改出來的

拿 v1.26.116（動工前）開 worktree 跑同樣的檔案，逐條對照：

| 測試 | 動工前 | 現在 |
|---|---|---|
| installer-key-update：舊金鑰換新 | 紅 | 紅（本來就紅）|
| lint-event-logger：不可寫路徑 | 紅 | 紅（本來就紅）|
| post-commit：Node repo 的版號提醒 | 紅 | 紅（本來就紅）|
| pre-commit-secret：pathspec magic 改名 ×2 | 紅 | 紅（本來就紅）|
| bare-mount：/dashboard/ ×2 | 紅 | 紅（本機沒有 console build，CI 上有跑 ensure-console-build）|

## v1.26.120 — 半安裝的機器上，鐵律檢查整個沒在跑，而且不會說一聲（#79 的 A 組）

A 組那 12 條紅，你決定當「備援，壞了要修」。修下去發現它真的壞了，而且壞在一個
**沒人會發現**的地方。

`.sh` 掛勾要讀 `~/.claude/settings.json` 拿金鑰，寫法是把路徑**插進 `node -e` 的原始碼**：

```bash
node -e "... readFileSync('$CLAUDE_SETTINGS_WIN', 'utf8') ..." 2>/dev/null
```

只要 `to_win_path` 回的是 `cygpath -m` 的輸出（正斜線），這樣是安全的。問題是
**`path-helpers.sh` 不在的時候，fallback 是原封不動回傳** —— 而 Windows 上那是反斜線路徑，
`C:\Users\...` 送進 JS parser 就是一串跳脫序列，讀檔丟例外、catch 印出空金鑰、
掛勾在空值守衛那裡 `exit 0`。**一台半安裝的機器因此完全沒有鐵律檢查，而且不會說一聲。**
跟 v1.26.94、v1.26.112 同一族。

而 `2>/dev/null` 正是 IR-002 講的那個紅旗：它讓失敗變成沉默。這次它讓一個真的 bug
安靜地活著，而測試給的失敗訊息是**猜的**（「command 沒被抽出來」），跟真正的原因無關。

三個地方一起修：

- 路徑改用 **argv 傳進去**（`process.argv[1]`），不再插進原始碼。反斜線、空白、
  單引號全都免疫 —— 這正是 `path-helpers.sh` 自己的檔頭建議的做法，只是這裡一直沒採用。
- 拿掉憑證讀取那兩個 `2>/dev/null`（IR-002）。node 讀不到那個檔案時，理由要出現在 stderr，
  Claude Code 的 hook 除錯看得到。「還沒設定」這種正常情況仍然由空值守衛保持安靜。
- 測試**不要再把 stderr 倒掉**（原本是 `child.stderr.resume()`，IR-003）。這些斷言只會在
  作者不在的平台上紅，而 `reached: false` 這一個布林值，對「payload 解不開」「更上面的守衛
  先跳出」「直接 crash」三種情況長得一模一樣 —— 而這三種各自會寫出不同的 stderr。
  失敗訊息現在附上 exit code、stdout、stderr。**這一步是整個修法裡最關鍵的**：加上去之後，
  真正的原因兩分鐘就看到了。

### 另外兩把壞掉的尺

- **`chmod 0o500` 在 NTFS 上是空操作**（v1.26.118 已經量過），所以「狀態目錄不可寫」那條
  在 Windows 上根本沒有製造出不可寫的目錄，寫入成功、降級沒發生、測試為了一個它沒在測的理由紅。
  改成**拿一個普通檔案擋在資料夾的位置**：POSIX 給 ENOTDIR、Windows 給 ENOENT，兩邊都是
  「這寫不進去」，正是這條要問的。
- **`node-hook-parity` 的清理會在 Windows 上丟 EPERM**：那個掛勾本來就會啟動更新腳本，
  子行程可能比掛勾多活一下，而 Windows 不准刪掉還有人開著的資料夾。原本 `finally` 裡是
  裸的 `rmSync`，於是**斷言全部通過之後**才失敗，而且掛的名字是
  「takes the lock before announcing the check」—— 跟它完全無關。改成重試、最後仍失敗就
  在 stderr 說一聲（不吞，IR-003），不再拿它去判整條測試死刑。

Windows 上實測：這三個檔案 69 條全綠，`node-hook-parity` 10 條全綠。
...
```
— a file whose tests pass but which never exits: ended=true named=false
```

也就是**時限有正常開火、也把檔案結束掉了**（`ended=true`），只是「報告裡有沒有出現這個檔名」
這個判斷說沒有。所以壞的是判斷，不是被判斷的東西。

Windows 上有兩件事會讓「整條路徑字串比對」失敗，而**我手上的證據不足以分辨是哪一個**
（那條測試原本只回報 true／false，沒有把報告內容帶出來）：

- TAP 會把反斜線跳脫，路徑印出來是 `D:\\a\\ownmind\\…`，跟手上的 `D:\a\ownmind\…` 不是同一個字串。
- `os.tmpdir()` 在 runner 上回的是 8.3 短檔名（`C:\Users\RUNNER~1\…`，這串就寫在上一版
  那個 EBUSY 訊息裡），而執行器報告的是展開後的長名。

兩個都修：比對改成只看最後兩段（那個每次執行都不一樣的暫存資料夾 ＋ 檔名），
並且把反斜線正規化、忽略大小寫。只看檔名會退化成「報告裡有提到某個叫 probe.test.js 的檔案」，
所以留著資料夾那一段，它是唯一的。

另外把報告內容加進失敗訊息裡。**這條測試只會在我不在的平台上紅**，只回報 true／false
等於每次都要再跑一趟 CI 才知道對方到底印了什麼。

驗證：用 Windows 那兩種真實拼法當 fixture，在 Mac 上就跑得到，含反向對照
（別次執行的資料夾不可以被判定成同一個）。改回原本的整串比對，這兩條會紅。

## v1.26.115 — 上一版那條新測試，自己在 Windows 上是紅的

v1.26.114 加了「測試卡住的時候要報出名字」的守門測試。它在 Mac 跟 Linux 上綠，
在 Windows 上紅：`EBUSY: resource busy or locked, rmdir`。

**這正是這個 repo 一路在踩、而且上一版自己還寫在說明裡的那個坑**：在 Mac 上開發，壞在別的平台。

三個原因，各自都足以造成它，三個都修：

- 探針的工作目錄就是等一下要刪掉的那個暫存資料夾。Windows 不准刪掉任何 process 還待在裡面的
  資料夾。改成工作目錄放系統暫存根目錄，檔案路徑本來就是絕對路徑，不受影響。
- 殺掉探針的那段在 Windows 上會走進 fallback，而 fallback 沒有等 process 真的結束就往下走，
  於是資料夾還被佔著就被拿去刪。現在每一條路徑都會等到 exit 事件，並且有 5 秒的保險。
- 刪除本身沒有重試。就算 process 已經被回收，Windows 上檔案 handle 還會多留一下。
  改成重試 20 次、每次間隔 100 毫秒。

驗證：node 20 跟 node 24 上，刪除前探針的狀態一律是已結束（`exitCode` 有值或 `signalCode=SIGKILL`），
不會出現「還活著」。Windows 本身要等 CI 回報。

## v1.26.114 — 測試卡住的時候，它什麼都不說

CI 開起來的第二天，main 上的 macOS 那一欄跑到第 20 秒吐完最後一行，然後二十分鐘不再出聲，
直到 `timeout-minutes` 把它砍掉。GitHub 把這種結果記成 `cancelled`，看起來像有人按了取消。

同一個 commit 重跑一次，74 秒就綠了，於是現場什麼都沒留下。同一天稍早也發生過一次，
那次被下一個 push 蓋掉，所以根本沒被當成「卡住」。**六次裡兩次，兩次都查不到東西。**

### 為什麼會一片安靜

`node --test` 預設沒有任何時限。某個檔案跑不完，它就一直等，而且因為它是**照派工順序**回報結果的，
排在後面的檔案就算跑完也不會印出來。傳到 log 的只有沉默。

這跟這個 repo 早就寫下來的一條規則是同一種：自動化回報「什麼都沒有」，在證明它有能力回報之前，
不算結果。一個卡住的 job，並不能告訴你是哪個測試卡住。

### 這一版做的事

`npm test` 跟 `npm run test:watch` 都加上 `--test-timeout=300000`。

- 每個測試、每個檔案有五分鐘。整套測試 ubuntu 跑 30 秒、macOS 跑 47 秒，正常情況離五分鐘很遠。
- 一旦超過，執行器會印出那個測試的名字（或那個檔案的路徑）跟 `testTimeoutFailure`，
  幾分鐘內就紅，而不是安靜二十分鐘。
- 這個值有上下限，兩邊都會被檢查。往下：太接近正常跑完的時間，慢一點的機器就會被誤殺，
  那比原本的問題還糟。往上：如果比 CI job 自己的 `timeout-minutes` 還大，job 會先被砍掉，
  等於把二十分鐘的沉默原封不動裝回去。上限是從 workflow 讀出來的，不是再抄一份數字。

### 這一版沒做的事

**沒有修好那個卡住。** 原因還是不知道：本機整套跑 5 次、對著疑似卡住的那個檔案在滿載下跑 120 次，
都沒有重現；唯一有完整 log 的那次，重跑就過了。這一版做的是「下一次發生時會有名字」。

而且這張網有破洞。兩個 CI 用到的 node 版本實測結果不一樣：

| 卡住的形狀 | node 20 | node 24 |
|---|---|---|
| 測試永遠不結束 | 自己 2 毫秒就失敗 | 時限攔得住，會報名字 |
| 測試都過了但檔案不退出 | 時限攔得住，會報名字 | 攔不住 |

node 24 上「洩漏 handle」那格還是無解。寫出來，不假裝有守到。

## v1.26.113 — 那個假的 stat 測不到四件事，而失敗還是沒有聲音

v1.26.111 用放進 PATH 的假 `stat` 驅動兩條測試，把方言處理釘在每個平台上 —— 這是最重要的性質，也是那兩條存在的理由。但有四件事在它們的射程外，而每一件都是這個缺陷當初能藏住、或未來能復發的路徑。

### 假的 stat 只會回答測試指定的方言

所以沒有任何一條測試去問「這台機器**真正的** `stat`，這個 helper 在這裡能不能用」。而那正是在 Linux 上被回答「不能」、卻連續九個版本在 macOS 上全綠的那個問題。現在有一條不帶任何 stub、直接問這台機器。

### 那兩條直接呼叫 `lock_age_seconds`，所以沒驗到真正咬人的後果

`acquire_update_lock` 把空的 age 讀成「看不出多舊」，整段 staleness 分支跳過 —— 那才是使用者實際遇到的事。以後改壞 age 而沒有任何呼叫端被跑到，測試一樣會全綠。現在有一條走完整條路：20 分鐘前的 lock 必須真的被接管。

### 沒有解釋的失敗，跟有解釋的失敗長得一模一樣

兩種寫法都答不出來時，`lock_age_seconds` 原本只是回非零。這個缺陷能活九個版本，靠的正是「失敗沒有聲音」—— 兩個 `2>/dev/null` 蓋掉抱怨，函式再默默回非零，沒有任何一層講過話。

現在會往 stderr 說清楚是哪個檔案、兩種寫法都試過了。呼叫端行為完全不變、一樣 fail-closed，只是先說為什麼。方言處理的邏輯一個字都沒動。

### 而檔案不存在是常態，那條路徑必須安靜

`acquire_update_lock` 每次 session 都會問 `.reclaim` 的年紀，而那個檔案通常不存在。少了這條測試，上面那句診斷會變成每個使用者每次啟動都看到的噪音。
...
### 順帶：這個修好之後，Linux 才第一次走得到「接管過期的鎖」那條路

而那條路上有一個既有的競爭窗口。**修掉一個，還剩一個。**

修掉的那個：清掉「上一個接管者死掉留下的標記」時，是先讀年紀、再 `mv` 移開。贏得 `mv` 不等於
移走的就是剛才量到的那一份 —— 中間可能有人贏了同一個 `mv`、清掉它、再放一份**新鮮的**標記回去，
於是這次的 `mv` 移走的是別人正在用的那一份。兩個人同時進了臨界區，而底下那道「重讀年紀」只有在
對方的新鎖已經建好之後才擋得住。現在移開之後會回頭看一眼移走的到底是不是過期的那份，不是就退出。
兩邊實作都補上，各自的 mutation 都會紅。

剩下的那個：八個掛勾同時搶的情境下，`a leaked reclaim marker does not let two shell hooks into
the critical section` 在單核容器裡 60 次還是會紅 1～2 次。加上追蹤重跑，看到的是**四個 process
先後都合法地成為接管者** —— 第一個做完就把標記移除了，後面的人看到「沒有標記、鎖還是過期的」，
於是照規則接手。這不是上面那個洞，是協定本身的形狀：這個檔案開頭就寫著「刪掉再建立沒有原子作法，
實作只能把窗口縮小」。要收掉它得改協定，不是順手改。先量、先寫下來。
...
### 改掉違規的地方，重送，又被同一條規則擋下來

發現於 v1.26.103 那次提交。訊息裡有 `Co-Authored-By`，被 commit-msg 掛勾擋下；拿掉那一行重送，又被擋 —— 這次是 **pre-commit**，理由還是同一個「訊息含 Co-Authored-By」，但訊息裡已經沒有那行字了。

去看 `.git/COMMIT_EDITMSG`，裡面躺的是第一次那份。手動把新訊息蓋上去，才送得出去。
...
```
pre-commit → prepare-commit-msg → 編輯器 → commit-msg
```

`.git/COMMIT_EDITMSG` 要到 pre-commit 跑完之後才會被寫入。所以 pre-commit 去讀那個檔案，讀到的永遠是**上一次**的訊息 —— 上一次成功的 commit，或上一次被擋掉的那份草稿。

寫一支會印出內容的掛勾，連送兩次，量到的就是這樣：

```
PRE-COMMIT 看到 COMMIT_EDITMSG：[FIRST MESSAGE]
COMMIT-MSG  拿到的 $1 內容：    [SECOND MESSAGE]
```

而在一個全新的 repo 裡，第一次 commit 時那個檔案根本不存在，訊息規則等於完全沒跑。

### 兩個方向都是錯的，安靜的那個比較嚴重

1. **該過的被擋**：改好了重送，還是被上一份的內容擋住，畫面上完全看不出為什麼。這次就是這樣被發現的。
2. **該擋的放過**：第一次送乾淨的訊息、留下一份乾淨的檔案，第二次才加進違規內容 —— 檢查對著舊的那份看，直接放行。沒有人會發現，所以它一直活著。

### 為什麼測試沒抓到

讀檔失敗時才會退回去讀環境變數 `GIT_COMMIT_MSG`，而整個倉庫裡出現這個變數的地方只有兩處：那段退路，跟測試檔。測試用的是全新的 repo、沒有 `COMMIT_EDITMSG`，所以讀檔一定失敗、一定走環境變數 —— 那條路產線永遠不會走到。介面的兩端都是測試自己給的。

### 改法：交給真的拿得到訊息的那支掛勾

新增 `hooks/ownmind-git-commit-msg.js`，用 git 傳進來的路徑（`$1`）評估所有 `commit_message` 類型的規則。挑規則看的是**條件類型、不是規則編號** —— 每個人的鐵律編號都不一樣，綁編號等於只對一個人有效。

pre-commit 則是把訊息規則**整個排除**，不是留著讓它「通過」。條件判斷在沒有訊息時會回傳通過，留著就會被算進「N 條規則全部通過 ✓」—— 對一個根本沒跑的檢查宣告通過，比不提它更糟。

### 有一個寫死的檢查刻意留著

第一版把 shell 掛勾裡寫死的 `Co-Authored-By` 比對拿掉，結果開了個洞：規則引擎的比對是**區分大小寫的子字串比對**，寫成 `Co-Authored-By` 的規則match 不到 `Co-authored-by:` —— 而後者正是 git 自己的寫法。寫死那條是整行錨定加不分大小寫，抓得到規則抓不到的。

抓到這件事的是既有的測試，一個字都沒改。

那條寫死的檢查確實是「把某個人的鐵律寫進產品程式碼」，該拿掉，但要跟「訊息比對改成不分大小寫」一起做，而那會改變所有既有規則的比對行為，是另一個決定。列 backlog。

### 審查抓到的一件事，差點讓這一版變成「保護變少」

`~/.ownmind` 本身就是 git 檢出目錄，所以 `git pull` 一落地，`hooks/` 底下的邏輯立刻換新。但 `git-hooks/` 裡那三支是安裝腳本**複製**過去的，而自動更新的路徑是 `git pull` → `npm install` → `update.sh`，從頭到尾不會跑安裝腳本。

這一版剛好把工作從 pre-commit 搬到 commit-msg。原本會發生的事是：使用者馬上拿到不再檢查訊息的新 pre-commit，手上的 commit-msg 卻還是舊版、根本不會去叫新腳本 —— **訊息規則整組靜靜失效**。而且最不容易發現：唯一看起來還在動的那條（署名行）是舊版寫死的比對本來就抓得到的，使用者會以為一切正常。

現在兩支更新腳本都會從檢出目錄把已安裝的 wrapper 重新複製一次（沒安裝的不會幫他裝）。update.ps1 之前根本沒碰過這個目錄。

### 順便修好的三個誤擋

- **`git commit --verbose` 會被誤擋**：`--verbose` 會把暫存區的差異放在剪刀線下面，而且**不是註解**。差異裡只要有一行 `+Co-Authored-By: …`，子字串比對就命中 —— 又變成「擋你的理由是訊息裡沒有的字」。現在剪刀線以下整段切掉。
- **在非 git 目錄執行會 exit 128 而且什麼都不印**：`set -e` 底下 `VAR="$(指令)"` 會把指令的結束碼帶出來、直接中止腳本。git 只會回報「掛勾拒絕了這次 commit」。
- **`#` 開頭的註解字元寫死了**：`core.commentChar` 是可以改的。另外，訊息如果每一行都以註解字元開頭（例如 `git commit -m '#123 修好了'`），現在會照原文判斷，不會整段被當成註解丟掉、讓所有訊息規則靜靜失效。

### 半套安裝不該讓人不能 commit

腳本裡對本專案模組的 import 全部改成可以被 catch 的動態載入。靜態 import 攔不住 —— 模組根本還沒開始執行，node 直接 exit 1，而 wrapper 分不出那是「規則違規」還是「檔案不見了」，結果就是拿著一份堆疊追蹤把人的 commit 擋下來。

### 影響範圍

正式機上用 `commit_message_not_contains` 的活躍規則有 2 條、屬於同 1 個使用者、都會擋 commit，而且都是那個寫死的比對本來就抓得到的同一件事。所以實際上沒有人因此失去保護 —— 但這個機制從來沒有正常運作過，今天誰去寫一條 `commit_message_contains`，拿到的會是一個對著上一次訊息做判斷的檢查。

## v1.26.103 — 搬檔案不算寫檔案

### 一個字都沒改的檔案，換個位置就被當成新寫的

回報 #10：用 `git mv` 搬 22 個已歸檔的 OpenSpec 資料夾，純搬移、內容一個字沒動，commit 被密鑰掃描擋下來，點名兩個檔案含疑似憑證。

兩個檔案的 blob 雜湊跟 HEAD 一模一樣，git 自己回報的狀態是 `R100`。同樣的內容已經在 repo 裡待了好幾個版本，每次 commit 都過。唯一的差別是路徑變了。

### 不是掃描器看錯，是我們把整份檔案遞給它

掃描器拿到的狀態是對的，留下來的路徑也是對的。問題在下一行——它跟 git 要的是**單一路徑**的差異：

```
git diff --cached -U0 -- <新路徑>
```

只指定一個路徑時，git 找不到可以配對的那一側刪除，改名偵測就跑不起來，於是把這個檔案當成全新的，每一行都回報成新增。掃描器沒有誤判，它是被告知「這 400 行剛剛寫進去」。

### 為什麼不照回報建議的比對雜湊

回報建議「blob 雜湊一樣就跳過掃描」。那樣只會蓋住 100% 相同的那一種，而真正更麻煩的另一種不受影響：**搬檔案的同時改其中一行**——剩下 400 行沒動過的內容照樣被當成新寫的，而這時雜湊真的變了，比對雜湊救不了。

改法是把來源路徑一起交給 git，讓它重新配對得起來。純搬移於是一行新增都沒有；搬移加編輯，就只剩真正編輯的那幾行。

兩個 git 呼叫都明寫 `-M`。使用者如果把 `diff.renames` 關掉，git 會把搬移報成不相干的一刪一增，來源路徑消失，整件事退回原本的 bug——他要不要開改名偵測，不該決定他的 commit 會不會被擋。

### 審查抓到的一個回頭洞：git 給的路徑，git 自己會當成樣式讀

把來源路徑交回給 git 的時候，它讀進去的不是「一個檔名」而是「一條比對樣式」，而且 `--` 擋不住這件事。

`:!victim.txt` 是合法檔名，同時也是「排除這個」的樣式。所以一個叫這個名字的檔案被改名時，它會把目的地從自己的差異裡排除掉，git 回傳空的，**同一次 commit 新加進去的密鑰就完全沒被看到**。實測不是推論：掃描器對一行全新的 `sk-proj-…` 回報通過。

這是這次修改唯一可能「掃得比以前少」的方向，所以加上 `--literal-pathspecs`，並補一條測試釘住它。同一個旗標也順手修好反方向的情況：檔名開頭是冒號時，配對會失敗、整份檔案又被當成新的，純搬移再度被擋。

### 刻意沒有多加一個豁免

blob 雜湊豁免先寫了，然後刪掉：配對修好之後，沒有任何一條測試分得出它在不在。在一個會擋人的安全檢查裡，加一個測不到的「跳過掃描」分支是多一個洞，不是多一層保險。這個元件已經為了先前的誤判累積三個逃生口，第四個正是要避免的結果。

### 為什麼這件事比一次 commit 大

被擋住的是已經在 repo 裡好幾個版本的內容，而文件給的出路是 bypass——bypass 一開就是同時關掉其他所有規則。一個誤判會直接換成一段完全沒有密鑰掃描的空窗。

## v1.26.102 — 採集程式停掉，現在會有人被通知

### 這半年修過兩次，今天量到還是有一台是停的

`collector_heartbeat` 全表，2026-08-07：

```
誰            機器               工具          最後回報      幾天前
Amiee Kuo    LAPTOP-RGE2HCSQ    claude-code   08-07 10:24     0.2   ← MCP 寫的
Amiee Kuo    LAPTOP-RGE2HCSQ    cursor        07-27 09:18    11.2   ← 掃描器寫的
Amiee Kuo    LAPTOP-RGE2HCSQ    antigravity   07-27 09:18    11.2
Amiee Kuo    LAPTOP-RGE2HCSQ    opencode      07-27 09:18    11.2
```

她最後一筆真實用量是 94 天前。她的 MCP 停在 1.26.57、掃描器停在 1.26.26，兩個都比帶修復的那版舊，所以 v1.26.79 的自動修復從來沒在那台機器上跑過，也跑不了。

其他十台機器，五個工具的回報時間**在同一秒**。只有壞掉的那台跟自己不一致。
...
| 誰寫的 | 寫幾個工具 | 有沒有上傳用量 |
|---|---|---|
| MCP | 一個，開哪個 IDE 就是哪個 | 沒有 |
| 排程掃描器 | 五個一起 | **有** |

排程死掉之後，人照樣每天有心跳、照樣什麼都沒上傳。用「這個人最近有沒有回報」去判，這台機器是健康的。這正是當初一台機器停了二十天沒人發現的原因。

真正的訊號是**同一台機器內部對不起來**：有東西兩天內回報過，另一個東西七天沒回報。拿上面那份快照去跑，剛好響一次，另外十台全部安靜。

### 誰會收到

**兩邊都收，內容不一樣。**

- **機器的主人** — 「你的用量採集停了」，講哪台機器、哪幾個工具、從哪天起、這段期間用量沒上傳，以及怎麼修。
- **管理員** — 另一封，列出所有人，停最久的排前面，不寫修法（管理員在別人電腦上跑不了）。

修好之後那封通知會提早關掉。它是不能貼掉的、預設掛兩天；沒有這一步，早上修好的人會被自己那句話唸到後天。

### 不會第一眼就吵

第一次看到有問題**不通知**，先記下來，六小時後那台機器還是壞的才講。

原因是這種形狀會自己出現一次：一台放了兩週的電腦重新開機，MCP 馬上有心跳，掃描器要等排程才跑 —— macOS 跟 Linux 半小時、**Windows 兩小時**，接電池的話 Windows 還會自己延後。在那個空檔判，就會對一台好機器送出不能貼掉、掛兩天的通知。而開機掃描是「一部署就跑」，那都是上班時間。

等六小時的代價是真的壞掉的機器晚一個晚上被講。當初那次是二十天。

反過來，**壞了十四天還沒修的會再講一次**。通知本身只掛兩天，不再講的話，那台機器就又回到「沒人知道」的狀態，也就是這個功能要解決的那件事。

### 刻意不抓的

**整台都安靜的機器。** 在這張表裡，那跟「電腦關機」「人請假」「筆電換掉了」長得一模一樣。同一份快照裡 Joanna 整台安靜 4.2 天、Michelle 的舊 Mac 1.2 天，兩個都不是故障。硬猜多抓一種，賠掉的是其他每一則通知的可信度。

**掃描器從來沒跑起來過的機器。** 沒有凍住的紀錄可以比對，這裡看不到，那是安裝自我檢測要管的。
## v1.26.101 — 索引一半的位置花在「講第二次」，專案就是被它擠掉的

v1.26.100 讓索引有了行數預算，然後照筆數平分。平分是中立的，而中立在這裡是錯的，因為兩邊沒被列出來的代價差很多。

**鐵律有第二條通道。** SessionStart 掛勾每次開對話都把整組鐵律連觸發條件一起注入 session，索引這一行多給的只是「連到本地檔案」的連結。

**專案沒有。** 沒被列進索引的專案，就是完全不在 session 裡。

照筆數平分的結果，是把一半的預算花在 session 已經有的東西上：

```
鐵律列出   63 筆   （143 條本來就全部都在 session 裡）
專案列出   63 筆
專案沒列   67 筆
```
...
鐵律固定只拿 20 行，其餘全部給沒有別條路可走的類型。上限套在「告訴分配器每一類要吃幾行」的地方，所以被壓下來的那一類會像任何一個吃不完額度的類型一樣，把剩下的讓出去，分配邏輯本身完全沒動。

同一份資料，改完之後：

```
鐵律列出    20 筆
專案列出   106 筆   （這整件事開始之前是 47 筆）
```

### 為什麼是 20

這個數字沒有東西可以量：規則本來就在 session 裡，索引那一行買到的只有「手上正在處理的那條規則，一步就打得開本地檔案」。20 蓋得住最近改過的。剩下的照樣由「少列了幾筆、去哪裡找」那一行講清楚。

被壓下來的類型，那句話報的仍然是**真正的剩餘筆數**（143 筆列 20 筆就報 123 筆），不是「超過上限的部分」，所以壓縮跟預算壓力兩件事之間只會出現一個誠實的數字。

突變檢查：把上限拿掉、以及數的時候忽略上限，兩種改法都轉紅。

## v1.26.100 — 記憶索引比讀它的那一端還長，後面那半從來沒被載進來過

不是有人回報，是在同一個 session 裡連續看到兩則警告才發現的：

```
WARNING: MEMORY.md is 280 lines and 31.8KB. Only part of it was loaded.
         Keep index entries to one line under ~200 chars.

Error:   over its 200-line read limit ... everything past the limit is silently
         dropped each time the index is loaded ... Rewrite it to under 140 lines.
```

實際量到的檔案是這個形狀：

```
MEMORY.md        284 行、32708 字元
  ## Iron Rules  143 筆
  ## Projects    130 筆
最長的一行        196 字元（305 位元組）
```

行數上限破了。**被切掉的那一段全部是專案**：鐵律排在前面，第 151 行就列完了，所以 143 條鐵律其實都有進來，200 行切下去之後專案只剩 47 筆，另外 83 筆從來沒有進到 session 過。而產生它的程式不知道、讀它的那一端在載入時不出聲、檔案本身也沒有任何記號。它讀起來像一份完整的索引，但它不是。

（讀取端講的「31.8KB」是它的字元數不是位元組數：那份檔案 32708 字元、52222 位元組。所以它量的軸是字元，`140 行 × 200 字`＝28000 字元已經把這一軸壓在觸發警告的門檻底下，不需要另外加位元組上限。）

### 為什麼會長成這樣

索引把每一條鐵律都列出來，可是 SessionStart 掛勾**本來就已經把整組鐵律直接注入 session** 了（143 條，連觸發條件一起）。那 143 行索引沒有多給任何東西，卻正好是把檔案推過上限、開始把專案擠掉的那一段。

這裡沒有哪一個決定是錯的。是索引會跟著使用者的記憶筆數長，而讀它的那一端額度不會。

### 改法

`MEMORY.md` 現在有一個它跨不過去的行數預算，而且會把沒放進來的東西講出來。

- **上限 140 行，在組字串的時候就分配掉，不是事後檢查。** 140 是讀取端自己要求的數字。
- **每一類都挑最近更新的排前面**，所以不用花任何一次工具呼叫就看得到的，是現在還在動的東西。
- **預算按需要分配。** 某一類用不完就把剩下的讓給別類，所以「4 條鐵律 + 300 個專案」跟反過來的使用者，拿到的索引都是有用的。
- **每一行最多 200 字**，標題過長就截斷並補上 `…`，連結不動（截斷連結會指到不存在的檔案）。
- **每一類各自講清楚少列了幾筆、去哪裡找**：

  ```
  ## Projects
  - [最近更新的那個](project_812_recent_one.md) — updated 2026-08-07
  ...
  - 67 more not listed here (line budget): see the project_*.md files in this
    directory, or search with the `ownmind_search` MCP tool.
  ```

拿真實資料試跑：**284 行變成 139 行、32708 字元變成 16800 字元**，273 筆記憶裡列出 126 筆，另外 147 筆被明確標示出來。

**專案從 47 筆變成 63 筆**（專案在索引之外沒有第二條通道）。**鐵律從 143 筆變成 63 筆** —— 這一項是變少的，因為 SessionStart 掛勾本來就把整組鐵律連觸發條件一起注入 session，索引這邊少列不影響規則生效，少的只是那個連到本地檔案的連結。**要不要乾脆把鐵律的額度壓到最小、把預算全給專案，這件事沒有自己決定**，維持按筆數分配、等拍板。

### 沒有動的部分

每一筆記憶還是各自有自己的 `<type>_<id>_<slug>.md`，一個檔都沒少，也沒有任何一筆變得找不到，被限制的只有索引的列表。墓碑刪檔、第一次執行時的備份、同步失敗的警告都沒改。

也沒有新增檔案。原本考慮過把列不下的另外寫成一份溢位索引，後來拿掉了：每個檔名裡本來就有標題的 slug，`ls` 一下就是完整清單，再寫一份只是多一個要同步的東西。

### 為什麼是「組的時候就守住」而不是加一條 lint

事後檢查的 lint 報的是一個已經寫出去、而且已經被截斷讀過一次的檔案。上限要放在產生器裡面，測試則證明它對任何輸入都跨不過去，包含 5000 筆記憶配 400 字標題。

新增 `tests/sync-memory-files.test.js` 28 項（本檔共 49 項）。**突變檢查十種改法全部轉紅**：把上限拿掉、把預算數字調大、把「少列了幾筆」那行刪掉、把排序拿掉、把截斷拿掉、把按需分配換成平均切、把標題換行的清理拿掉、把失敗標記那一行的保留拿掉、把非數字 id 的排序後備拿掉、以及**先全部吐出來再事後切到 140 行**。

其中兩次是自己第一版沒擋住、被指出來才補的：

- 「平均切」第一次沒抓到，是斷言門檻在只有兩類時平均切也會通過，改成檢查「預算有沒有被用完」才擋得住。
- 「事後切」是規格第一條明文禁止的做法（上限要在組字串時就守住，不是事後檢查），但一開始沒有任何測試釘住它。那個改法會讓 `## Projects` 整個標題消失、專案一筆都不列，而測試全綠。現在加上「每個出現的類型都必須有標題而且底下至少有一筆」。

另外三個問題是審查抓到、確認能重現之後才修的：標題含換行時一次 push 會變成好幾行實體（60 筆各含兩個換行 → 189 行）、真正的同步失敗路徑是把標記插進已經寫好的檔案而不經過產生器（剛好用滿的 140 行會被推成 141 行）、以及標題含 emoji 被截斷時會切在 UTF-16 半個字元上（24 種檔名長度裡有 12 種會產生半個壞字）。

完整測試 3474 項、0 失敗。

## v1.26.98 — 那把「一次只准一個人更新」的鎖，其實鎖不住

不是有人回報，是查一個帳號為什麼有 18 筆「升級失敗」時翻出來的。把整個時間窗撈出來看，而不是只篩失敗那幾筆，真相是這樣：
...
01:03:22 | update_failed  | hook
01:03:21 | update_check   | hook   ← 同一秒四筆
```

四個掛勾同時起跑，一個做完，三個撞在一起記成失敗。08-06、08-05 形狀一樣。**升級每次都成功**，那 18 筆是搶輸的那幾個。

### 鎖為什麼鎖不住

有三支程式會跑每日自動更新，共用 `~/.ownmind/.update-lock`：MCP、Node 掛勾、shell 掛勾。真正拿鎖的只有 MCP。

**shell 掛勾** — 兩個問題，任一個都足以讓它不成鎖：

```sh
if [ -d "$OWNMIND_DIR/.git" ] && [ ! -f "$LOCK_FILE" ]; then   # 第 132 行
  ...
  ( touch "$LOCK_FILE" || { log_event "update_failed" "step" "lock"; exit 0; }   # 第 143 行
```

檢查跟建立隔了十行還跨一次 fork，所有人都在第一個建立之前就通過了檢查。而且 **`touch` 對已經存在的檔案不會失敗**，就算順序完全正確，兩個也都會「拿到」。實測：四個並行，四個全進。

**Node 掛勾** — 讀鎖、還新鮮就返回、太舊就刪掉，然後什麼都沒建立。那個函式裡根本沒有「取得」這個動作。

**接手舊鎖的邏輯，三邊都一樣錯** — 看年齡、刪掉、建立。兩個都判定太舊，兩個都刪，第二個刪掉的正是第一個剛拿到的新鎖。同一個 bug，往上一層。

### 今天沒出事，但每天早上都在賭

現在的代價是吵跟白做工。真正的風險是四個程序同時在**同一個工作目錄**跑 `git fetch`、`git stash`、`git pull --rebase`、`npm install`、`update.sh`。有一台機器每天早上都在賭這一把。健康報告裡已經有兩台出現 `upgrade_dirty_tree` 警告，正是併行拉取做到一半會留下的形狀（**只是形狀相符，沒有實證是它造成的**）。

觀測面的錯跟功能面一樣值錢：一個「沒有任何東西失敗時會觸發」的 `update_failed`，害我們去找一個不存在的故障，差點去「修」一條本來就好的升級路徑。

### 改法

協定只留一份 `shared/update-lock.js`，兩支 Node 程式共用。shell 掛勾不能 import（為了拿一把鎖去 spawn 一個 node，比那把鎖值錢），所以它照抄同樣的步驟，測試讓**兩邊都跑同一組情境**。

- 取得＝獨佔式建檔：Node 用 `fs.openSync(file, 'wx')`，shell 用 `set -C` 加 `>` 轉向。不管幾個並行，只有一個會成功，中間沒有空隙。
- 搶輸記 `update_skipped`、原因 `lock_held` — 這是 MCP 本來就在用的事件名稱跟原因，不是失敗。但如果是**建不出鎖、而且現場也沒有鎖檔可以解釋**（唯讀檔案系統、磁碟滿），那仍然記 `update_failed`。

### 接手死掉的鎖：沒有「原子」這個選項

一開始只做了兩步（排隊 + 再讀一次年齡）。送審查時被抓到那還是不夠：清掉「別人留下的排隊檔」本身又是一次「先看再刪」，兩個都看到、兩個都刪，第二個刪掉的正是第一個剛建的。審查者在兩份實作上都重現出來了。

他建議改用 rename。**但 rename 也擋不住** —— 晚到的那個 rename 會把別人剛放的新檔搬走，同一個 bug 往下挪一層。

根本原因是：**在一般檔案系統上，「刪掉一個路徑再重建」做不到原子。** 任何判定「這個檔太舊」的程序，都可能刪到後來才佔住那個路徑的另一個檔。

所以目標改成「不可能有兩個人同時以為自己拿到」，而不是「不可能競爭」。三層：

1. 刪除要先排隊（另一個獨佔檔）。
2. 排到之後**再讀一次年齡**才刪；別人已經放了新鎖就不動它。
3. 拿到之後**把自己寫進去的暗號讀回來**。讀到的不是自己寫的，代表鎖被換掉了 —— 那就是別人拿到，自己退出。

第三層是關鍵：被擠掉的那個自己會發現。清排隊檔也套同一套（搬到只有自己用得到的名字，搬輸的就這輪不接手）。

### 測試

八個並行只能有一個拿到，shell 與 Node 各跑一次。另外附**陽性對照**：先讓舊寫法上場，證明這組測試真的看得到競爭（看不到的話下面全部不算數），以及 `touch` 對已存在的檔案確實會成功。

突變測試前後跑了 20 個，全部要紅。過程中有五個第一輪沒被抓到：八個同時放行的程序會排成一列而不是重疊，racy 測試碰不到那個窗口；而且第三層防護會把第一層的失效**遮起來**（防護做成多層，多層之間會互相掩護）。先懷疑測試不是懷疑程式，補了決定性測試直接把交錯做出來，全部當場死掉。

真實掛勾也並行跑過四份：新版是 1 個 `update_check`＋3 個 `update_skipped`，同一組工具跑舊邏輯是 4 個全進。

## v1.26.98（同版）— 回滾失敗時不要再回報「backup restored」

vin-windows 回報。2026-08-07 19:26 在 Windows 上刻意讓升級失敗，輸出裡連續兩行自相矛盾：
...
還原失敗了，下一行卻說備份已還原。而**這句話會原封不動送到伺服器**，變成後台讀到的診斷紀錄。

Windows 上這是常態不是特例：MCP 的 node 行程握著 `mcp\node_modules` 底下的檔案時，刪除必定失敗。

那晚的廣播因此也寫錯了 ——「自動還原備份」是照抄這句寫死的字串，實際上沒有還原。那台沒事是因為 pull 失敗時什麼都還沒改。

### 收 review 補的五項

送 review（含對抗審查）之後補了五項，每一項都先重現過：

1. **最慘的那種失敗，伺服器完全收不到。** 還原是「先刪掉、再搬回來」。刪掉成功、搬失敗 = 安裝沒了 —— 而回報程式就住在剛被刪掉的那個資料夾裡。實測：目錄還在會寫出檔案，目錄被刪掉則什麼都沒寫、還回報成功。現在開工前先把回報程式複製到 `~/.ownmind` 外面。

2. **Mac 版會拿舊記錄誤判失敗原因。** 它搜整份升級記錄找「被鎖住」字樣，而記錄裡有十分鐘前 npm 留下的舊錯誤。結果磁碟滿了也會被說成「檔案被鎖住，請關掉 Claude Code」。改成只看這次還原自己的錯誤，跟 Windows 版一致。

3. **Windows 版會把畫面訊息格式撐破。** 這兩支程式的輸出是一行一個格式在被讀取的，而 git 的錯誤是多行；PowerShell 的 `2>&1` 還會把它包成物件、印出來更多行。多行塞進 `ERROR:代碼:訊息` 就讀不到了。兩邊都改成摺一行、去控制字元、同一個字數上限。

4. **Windows 版少建一個資料夾。** 記錄檔搬出 `~/.ownmind` 之後，順手把建立 `~/.ownmind/logs` 的那行也帶走了，但升級完成的回報還是往那裡寫，而那個寫法遇到父目錄不存在會直接失敗。補回來。

5. **git status 為什麼失敗還是沒記下來。** 只記了結束代碼。兩邊都改成把 git 自己講的話一起帶上。

跟同版的另一個修正（把 git 的話寫進 detail）重疊的部分整段拿掉，避免同一件事在一個版本裡做兩次。

### dirty-tree 偵測不再把「git 掛了」讀成「工作區乾淨」

`git status --porcelain` 在工作區乾淨時印空的，在 git 自己失敗時也印空的。舊寫法把 stderr 丟掉之後這兩種完全無法區分，而歧義永遠往不安全的方向解：直接對一個從未確認過狀態的工作區 `git pull`。

促成這項的實際觀察：一台 Windows 10 的 `git status --porcelain` 每次都直接當掉、stdout 與 stderr 皆空，而同一支 git 的其他指令全部正常。**那是 git 端的問題，不是 OwnMind 的** —— 但它證明了「git 沒印東西」不能當成「工作區乾淨」。

**副作用**：在 git status 會失敗的機器上，升級從此會停在這一步。這是刻意的，但那些機器會從「看似升級成功」變成「明確失敗」。
## v1.26.98（同版）— 「最常做的專案」是空的，因為沒有人把它送出去

同一次調查的另一半。原本結論是「救不回來」，被 Vin 當場質疑，**質疑是對的**。

### 我當初判斷錯在哪

我看到「伺服器事後重建那條路只有活動記錄可讀，而活動記錄裡沒有專案名稱」，就寫成「修不了」。

但專案名稱**從來就不是未知的**。`mcp/index.js` 從 v1.17.37 就會從 `CLAUDE_PROJECT_DIR`（沒有就用當前目錄）算出來，只是這個值只跟著「AI 主動寫的那份工作記錄」走，其他事件一律沒帶。

所以要修的是**讓它一起送**，不是「從沒有的東西裡變出來」。這跟 IR「宣告無解之前先證明不是自己的 bug」是同一件事，我沒做到。

### 範圍比原本以為的大

那一週的實測：**Vincent 自己 95 場裡有 76 場是事後重建的**，另外四個人是全部。所以這一欄對四個人全空，對第五個人空掉五分之四。

### 改法

- `shared/helpers.js` 新增 `resolveProjectName()`，成為唯一一份推導邏輯（MCP 原本自己有一份，刪掉改用共用的）
- MCP 的活動記錄器、兩支 SessionStart 掛勾，送出的**每一個事件**都帶上專案
- 伺服器重建工作記錄時，從那些事件裡挑**出現最多次**的那個（不是第一個，免得一個誤入的事件把整場改名）

**只送資料夾名稱，不送路徑。** 資料夾名稱是工作脈絡；路徑會透露一個人把檔案放在哪，這個產品不需要那個才能依專案分組。

### 測試

shell 掛勾那份是手工拼 JSON 的，所以特別測了「資料夾名稱裡有引號」時整行還是合法 JSON —— 不合法的話那筆事件不是變醜，是整筆被伺服器丟掉。10 個突變全紅。

## v1.26.98（同版）— 團隊用量頁的「鐵律遵守率」對四個人顯示「尚無資料」，但資料其實在

### 為什麼會空白：兩個不同的原因

`session_logs` 有兩種寫法：

1. **AI 主動記錄** — AI 呼叫 `ownmind_log_session` 時可以附上 `rules_complied` / `rules_skipped`
2. **伺服器事後重建** — AI 從來沒呼叫的話，伺服器會拿活動記錄重建一筆，把同樣的事情記成 `compliance`（每個 `iron_rule_compliance` 事件一筆 `{rule, action}`）

**畫面只讀第一種。** 所以走第二種的人，資料明明在那一列裡，畫面卻顯示「尚無資料」。

2026-08-07 量到的：Michelle 有 52 筆遵守記錄分散在 10 列、Vin-windows-test 有 33 筆在 12 列 —— 兩個人畫面上都是一槓。

**但 Phoebe 跟采瑤是另一回事**：她們的 `iron_rule_compliance` 事件是 **0 筆**。畫面沒騙人，是真的沒有東西可以顯示。

### 改了什麼

讀不到第一種寫法時，改讀第二種。兩種都有的話只算第一種，不會重複計。

`violate` 刻意不計入。這個函式的「觸發數」一直定義成「遵守 + 略過」，而畫面是用「遵守 ÷ 觸發」算百分比；把違規塞進去會無聲地改掉一個大家已經看了好幾個月的數字。列進 backlog 38 讓 Vin 決定要怎麼呈現。

### 同一欄旁邊的「最常做的專案」

當初判定「這半救不回來」，理由是伺服器重建那條路只有活動記錄可讀、而活動記錄裡沒有專案名稱。**那個判定是錯的**，同版另一節已經修掉：專案名稱一直算得出來，只是沒跟著事件送出去。

## v1.26.98（同版）— 密碼偵測把三分之一的程式碼當成金鑰

同一天擋了兩次 commit，兩次都是誤判：

| 它擋下來的 | 實際上是 |
|---|---|
| `.update-lock.reclaim` | 一個檔名 |
| `REASON_MAX_CHARS=300` | 一個變數名稱 |

兩次都跟使用者說「這看起來像 API 金鑰」。

### 先量，再改

拿這個倉庫自己所有追蹤中的檔案，把每一個 20 字以上的字串抓出來當樣本：

**10486 個裡面有 3438 個被誤判 —— 每三個就錯一個。**

而且順手發現一個真的漏洞：**含斜線的金鑰完全抓不到**。舊的規則只要看到「三段以上用 `/` 分隔」就當成檔案路徑放行，而 base64 金鑰正好長那樣。

### 根因：它量錯東西了

舊規則是「20 字以上、字元都在金鑰字元集內」。這條分不出金鑰跟識別字，因為兩者長得一樣。之前已經為此補了三個例外（點分隔路徑、斜線分隔路徑、純符號分隔線），**每一個都是在某次 commit 被誤擋之後才加的**。再加第四個只會重演。

真正的差別是**有沒有單字結構**。金鑰是隨機的；識別字是用 `-`、`_`、`.` 或大小寫接起來的單字。

### 改法

1. 取最長的一段「金鑰形狀字元」。`- _ .` 會切斷它（識別字跟路徑就是這樣組的）；`/ + =` 不會（那是 base64 的字元）。
2. 把那一段拆成單字，算「像單字的字元」佔多少：三個字母以上、有母音、沒有數字。
3. 大部分是單字 → 識別字。否則 → 金鑰形狀。

門檻 0.6 是**量出來的不是喬出來的**：0.5 到 0.8 之間誤判數幾乎不動（1191～1204），但 0.5 會漏掉兩種真金鑰，0.6 是全部留住的最低值。

**結果：同一組樣本 3438 → 333（33% → 3.2%）。** 剩下的 333 個裡有 279 個是 `package-lock.json` 的套件雜湊 —— 那些本來就是亂數，判成金鑰形狀不算錯。只看程式碼跟文件（排除 .json）是 **51 / 9082 = 0.6%**。

真金鑰 13 種全部照抓，含斜線的那種現在也抓得到了。

原本那三個例外裡的兩個直接刪掉 —— 用量的取代用列的之後它們就多餘了（刪掉只讓誤判數從 309 變 318）。

### 測試

保證寫成**比率**不是清單：清單只會裝進已經有人踩過的那幾個，這正是它三個例外還在錯的原因。8 個突變全紅。

順帶一提，這次的測試檔自己夾帶了看不見的控制字元，被 v1.26.96 那條規則當場擋下來。
...
> AI 不可以自己填 confirm_string ——伺服器會用 HTTP 400 擋掉自動填入的送出。

**它不會。** `confirm_string` 只是一個字串欄位，伺服器看到的是「有一個內容正確的字串送到了」，它看不出這幾個字是使用者敲的還是 AI 自己填的。回報者的示範方式是：直接用這種方式送出 bug #17，然後用同樣方式送出 #18。

### 他建議的修法也擋不住

他提議由伺服器出一個一次性隨機片語，使用者照著念。**但去要那個片語的就是 AI**，AI 看得到它、可以自己填回去，伺服器收到的跟今天完全一樣。唯一的差別是片語不能靠記憶猜 —— 可是本來也不用猜，因為它就寫在工具說明裡。

「改成到後台按核可」則是另一個原因不成立：查了認證方式，`POST /me/login` 回傳的就是**AI 手上那把一模一樣的金鑰**。使用者叫得動的端點，AI 全部叫得動。

**只要兩者共用同一把鑰匙，任何形狀的伺服器檢查都分不出他們。**

### 改法：不要假裝擋得住，改成記錄實情

- 新增 `confirmation_declared` 欄位：`user_typed` / `ai_filled` / `unknown`。名字用 `declared`（宣告），免得以後有人把它當成驗證過的東西
- 沒送、送了不認得的值、型別不對 → 一律 `unknown`，**絕不會變成 `user_typed`**。舊版客戶端根本不會送這個欄位，那正好就是這種情況
- 既有資料回填 `unknown`：有些是使用者親手打的、有些不是，而且沒有紀錄可以分辨。填任何一邊都是在編造資料
- 工具說明拿掉那句假保證，改成明講「伺服器只檢查值、看不出是誰打的」，並要求 AI 誠實宣告
- 列表端點會回傳這個欄位，這樣看回報的人才看得到

**這沒有讓那道關卡變成真的，它只是讓紀錄變成真的** —— 而那本來就是「我想知道哪些回報有人看過」需要的東西。

### 沒有做的

把 AI 跟使用者的鑰匙分開（AI 那把只能寫回報、不能核可）。那是動整個權限模型，記進 backlog 36。

### 措辭上的一個取捨

原本那句「不可以自己填」雖然是假保證，但它旁邊還有一個**真的**在做事的設計：**工具說明裡從來沒寫出那個確認片語是什麼**。AI 不知道要打什麼，自然就得去問使用者。

所以誠實化的時候要小心：講「伺服器擋不住」很容易變成「你可以偷跑」。最後的寫法是把責任放回 AI 身上（「這一條靠你」），而不是通知它這道檢查是紙糊的。片語本身仍然不會出現在 AI 讀得到的說明裡，那條既有的測試也保留著。

順帶修了一條**在保護假保證的測試** —— 它斷言說明裡必須有「不可以自己填」那句話，也就是說誰去修 bug #18，誰的測試就會紅。

### 驗證

六種破法各弄壞一次確認會紅：沒宣告時當成 `user_typed`、把 `unknown` 放進可宣告清單、route 不再寫這個欄位、把假保證的說明放回去、migration 回填成 `user_typed`、把「不會擋」那種寫法放回說明裡。
...
原本補的是萬用字元＋幾個副檔名，但那還是白名單，形狀一樣會再漏。實測：906 個被追蹤的檔案，補完也才涵蓋 484 個。改成一行全域規則加例外清單：
...
在拋棄式副本裡驗過：906 個檔案全部有規則、兩個真正的二進位檔仍被正確辨識、跑正規化只有 `.gitattributes` 自己變，**沒有任何檔案內容被改動**。

**`text=auto` 不可以省。** 單寫 `* eol=lf` 會無條件把 `text` 打開，等於關掉 git 的二進位嗅探 —— 回報者在含 `0x0D 0x0A` 的檔案上實測：13 bytes 進、11 bytes 出，壞掉。這個程式碼庫裡就有活例子：`CHANGELOG.md` 內含 4 個真的 NUL 位元組（很久以前那篇講「怎麼處理 NUL」的條目，直接把 NUL 寫進內文），git 現在把它當二進位檔。

1. `.gitattributes` 一行全域規則，Windows 原生格式列為例外（放在後面，最後匹配的贏）
2. 新測試改成守住那個**形狀**：全域規則還在不在、`text=auto` 有沒有被拿掉、例外有沒有被排到前面、git 自己認不認為每個檔案都有規則、二進位檔還是不是二進位。新增檔案已經不可能漏，要擋的是「有人又把它改回清單」
3. `install.sh` 複製掛勾到 `~/.ownmind/git-hooks/` 時去掉 CR，所以已經壞掉的機器跑一次安裝就會修好

### 收 code review 之後補的

- **`*.js` 這條規則反而把兩個含 NUL 位元組的檔案掃進來了。** 明確寫 `text` 屬性會**蓋掉** git 自己的二進位判斷，而其中一個 `tests/install-check-null-byte-sanitize.test.js` 存在的目的就是釘住精確的位元組內容 —— 下一個跑進去的 CR 就會在 commit 時被改掉，等於在這個修正裡新增了一個它自己要消滅的毛病。改成豁免，而且要寫 `-text -eol`：只寫 `-text` 沒用，因為設定 `eol` 本身就會啟用轉換。另一個檔案用原始 NUL 當組合鍵分隔字元，改成 `\0` 跳脫（對引擎完全等價）。兩者都在拋棄式副本裡塞 CRLF 實測過：豁免的檔案位元組不變，一般的 `.js` 的 CR 被拿掉。
- **修復原本漏掉一條升級路徑。** `interactive-upgrade.sh` 只有在找得到憑證時才會重跑 `install.sh`；找不到就退回 `scripts/update.sh`，而那支從來不碰 git 掛勾。現在那條路也會就地修好。
- 安裝腳本的複製改成 `tr -d '\015'`（比 `\r` 可攜）並寫暫存檔再搬移（直接覆寫會先清空現有掛勾，`tr` 中途死掉就留下半個檔案，而 git 下次 commit 照樣執行它）。
- 「清單太短就當失敗」改成點名那三支真的壞過的掛勾，光看數量太寬鬆。

### 驗證

- 八種破法各弄壞一次確認會紅：省掉 `text=auto`、改回白名單、例外排到全域規則前面、拿掉 NUL 豁免、豁免只寫 `-text`、把原始 NUL 放回去、安裝腳本改回 `cp`、改回直接覆寫
- 另補 `.editorconfig`：`.gitattributes` 管 git，管不到編輯器存檔時寫什麼，兩邊不一致會來回打架
...
然後把同一個物件送到伺服器。但伺服器收事件時只讀 `details` 這一個欄位 —— `details` 不存在，所以每一筆存進去都是空的 `{}`。
...
升級腳本其實分得很細，會告訴你是卡在 `lock`／`cd`／`fetch`／`pull`／`npm`／`update_sh` 哪一步。這些字全部沒有離開過使用者自己的電腦。所以「Phoebe 升級失敗 18 次」就是能看到的全部，想知道為什麼只能去她那台機器上翻。

同一個沉默也蓋掉了鐵律觸發（是哪一種觸發）、session 啟動（成功還失敗），以及 v1.26.92 才加的 `edit_reminder_failed` —— 那是一條出生就壞掉的錯誤回報管道。

### 改法

把附加欄位放進 `details`。伺服器那端不用改，它從來都讀對地方。

### 為什麼一直沒被發現

事件名稱一直都是對的。用事件名稱數失敗次數的儀表板完全正常，缺的只有「是哪一步」—— 而這個問題沒有人會問，直到某個人的升級開始一直失敗。Phoebe 的失敗了 18 次，然後發現沒有東西可以看。

### 驗證

- 兩支掛勾的 `log_event` 都是從檔案裡抽出來真的執行，用 JSON.parse 讀回來（不是比對字串）
- 對正式機實跑一次完整來回：送出一筆帶標記的事件、從資料庫讀回來確認 `details` 完整、再刪掉（有做陽性對照，確認那個「剩 0 筆」不是查詢壞掉）
- 兩種破法各弄壞一次確認會紅：改回平鋪、空值時留下逗號

### 收 code review 之後補的

- **漏掉第三份，而且那份正是 Windows 在跑的。** `hooks/ownmind-session-start.js` 用 `...extra` 平鋪，Windows 沒有 Git Bash 時裝的就是它。只修 shell 那兩份的話，同一個事件會因為作業系統不同而有兩種形狀，之後任何人寫查詢都會對 Windows 使用者讀到空白 —— 正是這一版要終結的那種沉默。
- **而且有一條測試在保護那個壞掉的形狀。** `tests/node-hook-reports-init.test.js` 斷言的是平鋪的 `e.status`，它今天會過正是因為程式是壞的；誰去修它反而會看起來像把測試弄壞。一起改成讀 `details.status`。
- **鍵沒有配對到值時，掛勾會永遠卡住。** `shift 2` 在只剩一個參數時會失敗且不位移，迴圈就停不下來 —— session 卡在那裡、畫面上什麼都沒有。實際跑過確認。改成 `while [ $# -gt 1 ]`，多出來的鍵直接丟掉。這是舊有的問題，現行呼叫端碰不到，但這一版是第一次有測試碰得到它。
- **值裡有換行或 tab 會產生非法 JSON。** 整包上傳會被拒絕、事件直接消失。現在送進去之前先濾掉控制字元。同樣是舊問題，但這條管道接下來最可能的用途就是記錄錯誤訊息。
- **「上傳出去的內容跟寫進檔案的一樣」原本是比對原始碼字串。** 改成用假的 curl 把真正送出去的內容攔下來、JSON.parse 讀回來比對。
...
- **原本的檢查抓不到把「已設定就跳過」加回去。** 它斷言的是那句訊息不見了，不是那個判斷不見了 —— 實測把同一個 `grep -q '"ownmind"'` 用另一句話寫回去，13 條測試全綠。現在改成看整個區塊裡不准出現那個判斷。
- **行為測試原本測的是抄寫版。** 它在測試檔裡重打了一份 install.sh 的寫入邏輯來跑，install.sh 自己改壞了測試照樣綠。現在改成把 install.sh 裡那段真的抽出來執行。兩種破法都各弄壞一次確認會紅。
- **中文版與日文版 README 補上。** 這已經是連續第三版只有英文版被改到；v1.26.92 加的檢查現在會擋住這件事。

## v1.26.92 — 你標最多的那批鐵律，AI 改檔案的時候一條都看不到

掛勾只掛在「執行指令」上。改檔案不是執行指令，所以標了「改檔案時要遵守」的鐵律，從來沒有在 AI 動手改檔案的當下出現過。

拿一個實際帳號量：138 條鐵律裡，`trigger:edit` 有 56 條，是這個帳號用最多的標籤；`trigger:write` 有 23 條，第二多。算進同義標籤跟沒標籤（沒標籤的所有觸發都適用）之後，**改一次檔案有 68 條鐵律該出現，實際出現 0 條。**

而且這些不是冷門規則，剛好都是「動手改東西時該注意什麼」那一類：改完要驗證真的寫進去、新加的檢查要先弄壞一次確認它會紅、三份 README 要一起更新。AI 最容易走鐘的地方，正好就是提醒缺席的地方。

### 改法

掛勾多掛一個位置：改檔案的工具（Edit / Write / MultiEdit / NotebookEdit）。

代價是次數。改檔案是一個 session 裡最頻繁的動作，commit 跟部署很少。68 條規則每改一次檔案就列一次，沒有人受得了 —— 然後就會被關掉，關掉等於全部失效。

所以加了節流，以一小時為一個區間：

- **這個小時第一次改檔案** — 完整列出（跟 deploy 現在的做法一樣）
- **同一小時內之後每一次** — 只有一行

```
【OwnMind v1.26.92】AI 改檔案要遵守的鐵律 68 條 · 本小時第 4 次
```

那行字的主詞是 AI，不是讀的人 —— 「68 條鐵律生效中」讀起來像在叫看螢幕的人去遵守，但要遵守的不是他。它說的是規則適用，不是規則被遵守了：掛勾看得到前者，看不到後者，而一句宣稱遵守的話，會剛好在最需要它是真的的時候變成假的。

後面那個「本小時第 4 次」是用來回答它自己引起的疑問：一分鐘前還是一整排、現在只剩一行，看起來像壞了。

### 沒有動到的東西

- **不會多擋任何事。** edit 觸發只產生提醒，完全不會走到驗證引擎 —— 那是這支掛勾裡唯一能發出 `decision: block` 的路徑。
- **指令觸發完全不變。** commit / deploy / delete 的判斷一個字沒改。
- **節流那條不會上網。** 規則數量記在狀態檔裡，所以第二次之後的編輯，前面不會多一趟網路來回。

### 驗證

- 兩支掛勾都用真實的 Edit / Write payload 實跑，對本機 HTTP 伺服器
- 第 1 次完整列出、第 2 次一行且完全沒發出請求、視窗過期後又完整列出、狀態檔壞掉時寧可多列一次
- 任何一次 edit 觸發的輸出都沒有 `decision` 欄位
- 安裝器的註冊連續跑兩次，確認不會重複也不會漏（舊的判斷是「裝過就跳過」，而所有人都是升級，新的那個位置永遠不會裝上去）
- 十三種破法各弄壞一次確認會紅：`.sh` 拿掉工具判斷、`.js` 不看 tool_name、節流也去打 API、視窗永不過期、文案改成宣稱已遵守、別名拿掉 write、安裝器改回舊判斷、讓 edit 也會擋、拿掉抓取失敗的退避、0 條也照樣講一行、視窗不分 session、用原型鏈查表、寫入失敗默默吞掉

### 收 code review 之後補的

- **伺服器連不上時，每次改檔案都會多等三秒，而且什麼都不說。** 抓不到規則就退避五分鐘再試，這段時間不重試也不出聲。原本每次編輯都會重試一次、等滿 3 秒超時，整個斷線期間都這樣。
- **一條規則都對不上的帳號，會被塞「鐵律 0 條」一整個小時。** 完整列出那條路徑本來就會安靜，節流那條漏了。全新帳號正好是最不能忍受這個的人。
- **視窗改成分 session。** 原本整台機器共用一個視窗，所以第二個對話（或子代理）開始工作時只會拿到「第 2 次」這個數字，那 68 條它從來沒看過。列出來的目的就是讓那個 AI 看到，看不到就沒有意義。
- **狀態檔寫不進去時會講出來。** 寫不進去就記不住「已經列過」，於是每次編輯都重列一次加一趟網路，而且沒有任何地方說為什麼 —— 這正是這一系列版本在修的那種失敗。
- **`Edit` 以外的工具名不會再撞到原型鏈。** `constructor`、`toString` 這幾個名字用一般查表會拿到函式（是 truthy），足以騙過「有沒有觸發」的判斷。
- **Windows 沒有 Git Bash 的那條路，其實從來沒跑起來過。** 安裝器把 `.js` 掛勾複製到 `~/.claude/hooks` 再從那裡註冊，但它會 import `../shared/helpers.js`，而 `~/.claude/shared/` 不存在也沒有人建，node 讀到 payload 之前就死了。現在改成註冊 checkout 裡的那份。**這段只有寫、沒有實跑（開發機沒有 PowerShell），要在真的 Windows 上驗過才算修好。**
...
### 掛勾一開始運作，就會踩到四個從來沒跑過的東西

這支掛勾在 `exit 0` 的那一行後面還有一大段程式碼，從來沒有執行過。把它接通等於同時把那些
程式碼第一次開起來，其中四個當場就是錯的：

- **`git push` 會在任何專案被擋下來。** 那段檢查拿 OwnMind 自己的版號去比對「你現在所在
  目錄」的 git tag，比不到就擋。它是給 OwnMind 這個 repo 用的發版關卡，在別人的專案裡會
  叫你去建一個 `v1.26.90` 的標籤。已改成只在 OwnMind 目錄本身生效。
- **`.sh` 的提醒印在裸 stdout，AI 看不到。** PreToolUse 掛勾正常結束時，裸 stdout 只會出現
  在逐字稿模式，不會進到模型的上下文。而那段提醒的內容本身寫著「回應格式要求：AI 的第一行
  必須是…」，一句永遠送不到的指令。同一個檔案裡的攔截與版號關卡早就用對了 JSON 格式，只有
  提醒這條路沒有。已改成一樣包 `hookSpecificOutput`。
- **`.js` 每一次 Bash 呼叫都會連伺服器。** 一般指令判不出觸發類型時會退回 `command`，但提醒
  那段本來就跳過 `command`，抓回來的資料沒人用。等於在每一個 Bash 指令前面加一次網路往返
  （逾時 3 秒），還附送一個空的上下文。已改成不抓、也不輸出。
- **取值沒有檢查型別。** `command` 是陣列或物件時是「有值」，會通過空值檢查，然後以字串化的
  樣子流到後面。已改成只接受字串。
...
掛勾接通之後，本來會連「攔截」一起接通：`deploy`／`delete`（Windows 版連 `commit` 也算）
會把本機快取裡標了「不通過就擋」的鐵律拿來評估，不通過就中止指令。這段程式碼同樣從來沒跑過。

問題在那些標記的來源。v1.26.89 修掉的正是「伺服器在你存規則時，偷偷比對現成範本、把驗證條件
寫進你的規則」，而五個範本全部都是會擋人的。v1.26.89 只停止了新的誤套，已經寫進去的沒有清，
使用者也沒有辦法自己清（backlog 29）。

實際量過一台機器：27 條快取鐵律裡 20 條帶「會擋」的標記。用真實條件跑下去，`git push` 會被
6 條擋住，其中包含「GitLab 優先用個人 PAT」「ima 部署標記命名規則」這種跟 push 毫無關係的
規則——顯示出來的理由是「還沒跑測試」。

**清本機快取沒有用**：那份快取是伺服器的鏡像，每次載入記憶、每次改鐵律都會從 API 重新覆蓋回去。
資料要在伺服器端清，而且使用者要有辦法自己管理，攔截才適合打開。兩件事都列進 backlog（31）。

所以這一版只做提醒：條件沒過還是會照常列出來、不會藏起來，但不會中止你的指令。唯一保留會擋人的
是 OwnMind 自己的版號關卡，而且只在 OwnMind 目錄裡生效。
...
所以實際發生的事是：你寫了一條提醒自己的規則，拿回來的是一條會在某天攔住你的規則，而且沒有任何一句話跟你講過。

### 實際案例（回報 #16）

一條叫「失敗處理不能毀掉診斷線索：回滾／清理前，先把日誌搬到不會被還原的地方」的規則，全文在講「回滾的時候不要把日誌一起刪掉」。它被貼了 `trigger:deploy` 標籤，內文剛好出現一次「測試」兩個字——這樣就被套上了「部署前跑測試」的範本。

以後在完全無關的情境被它擋下來時，畫面上會寫「還沒跑測試」。那句話跟使用者當初寫的東西對不上，被擋的人根本不會知道為什麼。

### 回報者不知道的那一段

同一天早上發現有 8 條鐵律帶著「commit 要同步文件」的驗證條件，其中只有 2 條真的跟文件有關。當時判斷是「手動複製貼上複製錯了」。**不是。就是這個機制放上去的。**

那個範本的條件是「標籤有 commit」＋「內文出現 同步／README／CHANGELOG／FILELIST／文件 任一個」。「文件」兩個字在鐵律裡到處都是。所以不改這裡，手動清掉的那 8 條會再長回來。

### 改法

比對還在，但**只給建議、不動你的規則**。回傳多一個欄位講清楚：比中了哪個範本、叫什麼名字、它會不會擋人、以及「沒有自動套用」。AI 可以直接把那句話轉述給你，要的話再說一聲。

### 沒做

已經被誤套的規則不會自動清掉。目前要清掉只能整包覆寫 metadata，還得自己記得把來龍去脈欄位帶回去——回報者說得對，一般使用者做不到。記進 backlog。

## v1.26.88 — Windows 的升級跑到一半就停了，而且什麼都不說

在 Windows 上跑 `bootstrap.sh` 升級，`install.sh` 會在中間某一行停住。畫面上沒有錯誤、沒有警告，就只是停了。呼叫端接著回滾，然後叫你去看一個它剛剛親手刪掉的日誌檔。

停住的那一行之後，全部沒跑：**自動載入記憶的 SessionStart 掛勾、鐵律的 git hooks、用量掃描的排程、安裝完的自我檢查。**

回報者：`Vin-windows-test`（機器 TANK，Windows 10 + Git Bash + Node 25.8.1），bug #15，附 `bash -x` 追蹤。

### 三層各自都足以把失敗藏起來

**第一層：POSIX 路徑餵給 Windows 版 node。**

Git Bash 底下 `$HOME` 是 `/c/Users/Vin`。當作「參數」傳給 node 的路徑，MSYS 會自動轉；但**寫死在 `node -e` 程式碼字串裡的路徑不會轉**，node.exe 拿到開頭那個斜線，當成磁碟根目錄去解析，變成 `C:\c\Users\Vin\...`，然後 ENOENT。

這個坑早就診斷過一次。`path-helpers.sh` 從 v1.26.7 就存在、寫法完全正確，當時只接到 `interactive-upgrade.sh` 一支。`install.sh` 跟 `update.sh` 從那時起帶著十五處沒轉的路徑。

**第二層：`2>/dev/null` 遇上 `set -e`。**

出事那段結尾是 `" 2>/dev/null`。node 的錯誤訊息被丟掉，`set -e` 讓整支腳本中止，標準輸出跟錯誤輸出加起來一個字都沒有。

**第三層：回滾把它叫你看的那份日誌刪了。**

日誌寫在 `~/.ownmind/logs/`，而回滾就是「刪掉 `~/.ownmind`、把備份搬回來」。訊息說「請看 `~/.ownmind/logs/upgrade-<時間>.log`」，那個檔案在同一個函式裡剛被刪掉。實測 0 bytes。

### 為什麼拖了這麼久沒被發現

升級失敗回滾之後，另一個機制把工作目錄拉到了新版本。所以那台機器的版本號是對的，但沒有任何一個安裝步驟跑完過。

**版本號不能當成「裝好了」的證據。** v1.26.87 那則廣播叫 Windows 同事「更新後記憶載入就會恢復」，實際上升級在 Windows 上根本走不完。

### 改法

- 十五處寫死在 node 程式碼裡的路徑全部改走 `to_win_path`（`cygpath -m`，在 Mac / Linux 上是原封不動回傳，所以對現在正常的平台沒有任何行為改變）
- 安裝腳本裡的 node 錯誤一律寫進日誌，不再丟掉；中止時印出停在哪一行、日誌在哪、最後幾行錯誤
- 升級日誌搬到 `~/.ownmind-logs/`，在回滾動不到的地方
- 安裝結束前檢查該產出的東西在不在（SessionStart 掛勾、鐵律掛勾、git hooks、技能檔、MCP 進入點）；缺了就明講並回傳失敗。同一份清單也接進自我檢查，變成新的 `install_complete` 項目，所以「裝到一半」現在會自己找上門，不用等人發現

### 驗證

四道守衛測試從腳本本身長出清單，不是手寫的：任何新的 `node -e` 只要寫死路徑、或把錯誤丟給 `/dev/null`、或把升級日誌搬回會被刪掉的目錄，都會紅。解析不出來的區塊算失敗，不算跳過。三道都故意弄壞驗過會紅、還原後綠。`install.sh` 在隔離的家目錄裡實跑到底。

Windows 端的實機驗證還沒做——這次的修正在 Mac 上是恆等變換，看不出差別。

### 審查抓到的（都已修）

- **最嚴重的一個是這次新加的檢查自己造成的。** 「裝不完整」原本回傳失敗碼 1，而升級腳本看到任何失敗都會回滾。但回滾只還原 `~/.ownmind`，`~/.claude` 底下的設定、掛勾、技能檔早就被改成新的了——結果是舊程式配新設定，比原本「裝到一半」更糟，而且回滾也生不出缺少的檔案。改成回傳 2，升級腳本認得這個碼、只回報不回滾。
- `set -e` 不會把錯誤處理帶進 shell function 裡，要 `set -eE`。少了那個 E，函式裡出事還是一個字都不印——正是這次要修的那個病。
- 守衛測試被示範了兩種繞過方式：`node --eval` 這個寫法它看不到；而 `claude_settings` 是「照名字放行」的，註解還寫著「另一個測試會管」——那個測試根本沒看它。把 update.sh 的修正原封不動改回去，測試全綠。改成只認「同一個檔案裡有經過 to_win_path 的賦值」，名字不算數。
- 「清單要長出來」的那份清單自己是手寫的。改成 `git ls-files '*.sh'` 之後立刻抓到**同一個 bug 還活在 `hooks/` 裡**：鐵律檢查掛勾用原始路徑讀金鑰，而它在 Windows 上是實際被註冊的那一支——所以那台機器的金鑰一直讀回空字串，掛勾靜靜地什麼都沒做。三支 hook 一起修掉。
- 產出物檢查原本只認一種實作，會讓 `install.ps1` 裝好的機器被判定成壞的、然後廣播一個做了也沒用的解法。改成任一種實作都算數。順帶挖出真的 bug：`install.ps1` 從來沒複製過 `hooks/lib`，只有 `update.ps1` 有——ps1 裝完又沒更新過的機器，掛勾在、它要呼叫的程式不在。
- 目錄不能當證據：`git-hooks` 跟 `hooks/lib` 都是先 `mkdir -p` 再填東西，空目錄也會通過。改成檢查裡面的檔案。
- 升級日誌建不起來時的退路是「寫回會被刪掉的那個目錄」，等於默默把 bug 裝回去，而測試只看賦值那一行、還是會綠。改用 `mktemp -d`。

### 沒做

`install.ps1` 跟 `install.sh` 在 Windows 上走的是兩條不同的路、結果也不同（ps1 跑得完、sh 跑不完），而升級只走 sh。這次沒動 ps1，記進 backlog。


## v1.26.87 — 讀鐵律的三個地方，只有一個跟得上 API 換格式

伺服器的鐵律清單在某個 v1.19.x 版本改成包一層 `{ data: [...] }` 回傳。有三個地方在讀它，當時只有一個被改。

`ownmind-iron-rule-check.js` 在 v1.19.20 修好了，程式碼裡還留著當時的註解。另外兩個沒有，而且壞的方式各不相同：

**`ownmind-git-pre-commit.js` 安靜地拿到零條。** 它用「是陣列才算數、不是就當空的」讀回應，所以每次同步都得到零條鐵律，接著把這個零條寫回快取、蓋掉原本好的資料。回到呼叫端，看到零條就當成「沒有規則要檢查」直接放行，連一行字都不印。

連鎖反應是這樣：快取只要過期一次，那一次還會用記憶體裡的舊資料檢查、看起來完全正常，但磁碟上的快取已經是空的。**下一次 commit，27 條鐵律全部不檢查、畫面全靜音。** 唯一的訊號是那行「all N rules passed ✓」不見了，而沒有人會注意到少了一行字。

**`ownmind-iron-rule-check.sh` 直接爆掉。** 它對包好的物件呼叫 `.filter`，丟 TypeError，整段 node 死掉，輸出又被 `$( )` 吃掉。這一支才是實際寫在使用者 `settings.json` 裡的那個，所以動手前的鐵律提醒從 API 換格式之後就沒再出現過。

### 改法

解析跟「要不要覆蓋快取」抽成共用的 `hooks/lib/iron-rule-sync.js`，兩個都是純函式。解析同時接受包過跟沒包過的格式；覆蓋規則只有一條：**抓不到資料就絕不覆蓋**。空結果幾乎都代表同步出錯，而不是使用者把鐵律全刪了；舊快取至少還擋得住東西，空快取什麼都擋不住。`.sh` 那支照 `.js` 版本 v1.19.20 的寫法補上。

### 驗證

實跑：刪掉快取、跑一次掛勾，修好前寫回 0 條，修好後寫回 27 條。`.sh` 那段解析抽出來當獨立程序餵兩種格式跑過，並重現舊寫法的 TypeError。三條原始碼守衛防止有人把舊寫法寫回去。修正故意弄壞一次，確認測試會紅。

### 附帶：兩個會被編輯器悄悄改掉的隱形字元

`self-check.cjs` 的金鑰格式檢查裡，字元類別中直接放了一個真的 U+FEFF。它能動，但任何「清掉隱形字元」的編輯器動作都會靜靜改掉這段的行為，而 diff 看起來毫無異狀。改成 `\uFEFF`。

原本只掃 `src/` 的「原始碼不得含控制位元組」測試，範圍擴到 `scripts/install-helpers/` 跟 `hooks/`——這兩個目錄已經各出過一次事（v1.26.87 的兩個 NUL、這次的 U+FEFF）——並加上第二道：不得含真的隱形字元。兩道都故意弄壞驗過會紅。


## v1.26.87 — 哪些檢測失敗是新的

安裝後自我檢查會掃出一堆紅燈。使用者的每一台機器都會掃、同一個問題會報好幾次，如果每次都吵一下，使用者很快會假裝沒看到。

新增一份純函式 `evaluateFailures` 決定哪些失敗「第一次出現」值得公告。同一件事只算一次、修好了再掉下來才算新、寫法改了但還是同一個問題就只改紀錄不再問。

### 缺陷：同一升級裡多份報告掃出同一個失敗，每份都公告一次

升級時會上傳多份掃描報告（每台機器一份），同一個失敗會出現在好幾份報告裡。函式應該是把它視為一個失敗、只公告一次。實際上前三台機器上的同一個失敗會產出三筆公告。

根因：`processedKeys` 集合漏了。函式沒有追蹤「這個失敗我已經處理過了」，所以第二份報告、第三份報告到來時還是當新事件處理。

### 改法（初版）

追蹤已處理過的鍵值。一次呼叫裡，每個 `(user_id, machine, check_name)` 只處理第一次出現的報告；後面的報告碰到同一個鍵就跳過。這樣「announce one problem three times」的場景變成只公告一次。

### 缺陷（改法不足）：檢查層級去重允許混合狀態結果

去重在檢查層級粒度過細。如果同一台機器的不同報告（新報告說 pass，舊報告說 fail）來得順序反了，會混雜兩份報告的決策。規格要求每台機器只讀最新一份報告，忽略其他所有舊報告。

### 改法（重構）

去重改在報告層級。追蹤已處理的 `(user_id, machine)` 對，第一份報告（最新）完整處理，同一台機器的後續報告（舊的）一律跳過整份。更明確地體現「每台機器只用最新報告」的規格。在 JSDoc 明文紀錄呼叫端責任：報告必須按時間戳遞減排列（新到舊）。

### 文件修正：JSDoc 與測試覆蓋補齒

模組層 JSDoc 仍記錄舊的檢查層級去重邏輯，與實作不符。改寫為：「此函式只處理每台機器的最新報告，舊報告一律跳過整份」。補充測試覆蓋「新報告含 A 檢查、舊報告含不同 B 檢查」場景，驗證舊報告真的被忽略（包括其全部檢查）。同時在 JSDoc 補上呼叫端義務：報告不含空 checks 陣列（無訊號列）。

### 第二部分：把新失敗寫成一則看得懂的通知

第一部分決定了「哪些失敗是新的」，第二部分把它們渲染成管理員能讀的廣播訊息。

使用者機器多、同一個失敗會重複，如果完整列出，訊息會爆炸長（2000 字上限）。新增純函式 `renderAlertMessage` 做三件事：

1. **多台同因合併一行**：同一個檢查、同一個原因、不同台機器 → 合併成一條「檢查 失敗 — User1（機器A）、User2（機器B）」
2. **超限說明遺漏數**：訊息超過 2000 字就截斷，並補上「另有 N 項未列出」，讓管理員知道有更多問題尚未顯示
3. **單項超限也送**：即便第一條就超限，也截斷後送出而非留空，因為驗證器會拒絕空訊息

### 缺陷：截斷演算法虛假截斷、omitted 計數錯誤

實作採逐筆檢查「再加一條會不會超限」的做法，每次都悲觀保留下一條的頁尾空間。結果是即使剩餘條目全部都裝得下，也會因為保留頁尾空間而拒絕加入，發生虛假截斷。

範例：條目 A = 1981 字、條目 B = 11 字，合併後 1994 字裝得進 2000 字上限。但演算法檢查 A 時，為下一條保留頁尾（約 20 字），判定 1981 + 20 = 2001 > 2000，拒絕條目 B，報告 `omitted: 1`。實際上沒有東西被遺漏，違反規格。

### 改法

改用三步：

1. 全部條目合併，若裝得下就直接回傳，不加頁尾（最常見的情況）
2. 否則用二分法找最大的 k，使得前 k 條 + 頁尾裝得下
3. 若連第一條都裝不下，截斷第一條、加可見標記「…（内容已截斷）」、加頁尾

新增四個測試：條目全部能裝、條目超限、超大單條、empty 版本號。尤其「全部能裝」那項是對虛假截斷的直接覆蓋。

### 第三部分：把決定跟渲染接成一支可執行的 job

前兩部分是純函式，這部分把它們接上資料庫，變成 `runInstallCheckAlerts`。

讀報告的 SQL 帶兩個限制條件、都跟 `evaluateFailures` 的呼叫端義務一一對應：`DISTINCT ON (user_id, machine)` 保證每台機器只留最新一筆；`jsonb_array_length(...) > 0` 排除還沒跑完檢查的 beacon 列（否則它會被當成「這台機器現在什麼問題都沒有」，把所有未解決的問題靜靜標記為已解決）。

決定出的新失敗寫進 `install_check_alert_state`（`ON CONFLICT` upsert，保證重跑冪等）；已修好的標記 `resolved_at`；只是文字改了的更新 `detail`、不觸發公告。廣播只發給 id 最小的那位 super_admin（正式機上有兩位 super_admin，處理這類警告的是資歷最久那位），不是「隨便一位」。沒有 super_admin 時狀態照寫、但不硬發廣播、寫一筆警告日誌。

這支 job 目前沒有任何路由或啟動流程呼叫它，接線留給下一個任務。

### 第四部分：接線 — 上傳當下評估一次，開機時補跑一次

前三部分寫好了決定與渲染邏輯，這部分讓它真的被呼叫，機制才算生效。

`POST /api/debug/install-check` 寫入報告後、回應 `{ ok: true }` 之前，呼叫 `runInstallCheckAlerts`。呼叫包在 try/catch 裡：報告已經寫進資料庫了，警報邏輯再壞也不能讓這次上傳跟著失敗，只把錯誤寫進日誌。

`createDebugRouter` 新增第三個可選參數 `onReportStored`（預設是真正的評估器）。測試藉此換掉評估器、在沒有資料庫的情況下驗證「報告存好後有觸發評估」與「評估失敗不影響回應、報告仍然落地」，既有呼叫端（`src/app.js`）不需要改。
...
上面六部分做完之後，整條分支送了一次完整審查。以下每一件都是「照原樣上線就會出事」。

#### 一、訊息在送到眼前的路上被砍掉，第二項以後根本沒人看得到

這一版存在的理由是「不要無聲截斷」，而它自己就是無聲截斷的受害者。

兩支投遞端（`hooks/lib/render-session-context.js`、`mcp/index.js`）在把廣播放到使用者面前之前，一律做同一件事：取內文前 5 行、用空白接起來、再取前 400 字。而渲染器是照 2000 字寫的，一項就佔 4 行、項與項之間還隔一個空行——第 5 行剛好是那個空行。所以不管內文多短，**第二項以後永遠送不到**。更糟的是「另有 N 項未列出」那句話是內文最後一行，只要發生截斷就一定被砍掉。

沒有改那兩支投遞端：它們負責渲染產品裡的每一則廣播，而且外面已經裝好的舊版客戶端不會跟著改，改了也沒用。改的是伺服器這邊——照使用者真的看得到的那個尺寸來寫訊息。

一項就是一行，檢查名、人與機器、版本、原因、修法用「｜」串在同一行。欄位順序是照「被砍掉時誰該先活下來」排的：識別問題的欄位在前，長敘述在後。最多 4 行內容 + 1 行遺漏數。字數用注水法分配：短的那項用不完的額度讓給長的那項，所以一項超長不會把其他項擠掉。

測試改成先套用投遞端那一行轉換、再斷言，因為存起來的內文本來就不是任何人會讀到的東西。

#### 二、狀態先寫、廣播後寫，中間出事就永遠靜音

原本流程是把新失敗全部標記成「已公告」，然後才寫廣播。中間任何一個閃失（連線斷掉、連線池逾時）都會讓這批失敗從此讀起來像是已經講過了，而廣播根本不存在。第一次掃描歷史資料時剛好撞上，兩筆躺了兩個月的 WSL 失敗就會第三次消失。

改成：先認領、再廣播、廣播失敗就把認領放掉再往上拋。

第一版只保護了廣播那一行，認領迴圈本身還在 try 外面——同一個缺陷往前挪一個查詢而已。第一次正式掃描會撈到那兩筆躺著的 WSL 失敗（Adam、Vin-windows-test），Adam 那筆認領成功寫進去，接著連線池在 Vin-windows-test 那筆逾時，錯誤被路由吞掉，廣播從來沒寫成，但 Adam 那筆從此讀起來就是「已公告」，永遠不會再被撈出來。現在從第一次認領到寫廣播全部在同一個 try 裡面，catch 只放掉**這一輪真的認領到的那些**——沒認領到的不能動，它可能屬於上一次真的發成功的公告，清掉會變成同一件事講兩次。

`releaseClaims` 本身保證不會拋：每一筆放行各自包 try/catch，連裡面那行錯誤日誌也包起來。它是在 catch 裡跑的，如果它自己拋，會把真正該往上報的錯誤換掉，而且後面還沒放掉的認領就全部留在「已公告」——又回到同一個缺陷。

#### 三、兩支同時跑的評估會把同一件事講兩次

每一次上傳都會就地跑一次完整評估，兩筆上傳同時到就是常態。兩支都在對方寫入之前讀到同一份狀態，於是各自算出同一批新失敗、各自發一則廣播。

認領改用條件式 upsert（`ON CONFLICT ... WHERE announced_at IS NULL OR resolved_at IS NOT NULL`）加 `RETURNING`：拿不到列就代表別人已經認領走了，這一筆不進我的廣播。這跟第二點的順序是一起設計的——認領必須在廣播之前（才擋得住競爭），廣播失敗必須放掉認領（才不會永久靜音）。

#### 四、一台時鐘不準的機器可以讓自己永遠不再報警

判斷「哪一筆是最新報告」原本用 `install_check_logs.ts`，那個值是客戶端自己送上來的，只驗過「解析得動」。一台把時鐘設到 2027 年的 Windows 機器上傳一筆全綠報告之後，它之後每一筆真實報告排序都比它舊、都被忽略——那台機器從此永遠不會再報出任何失敗，而且沒有任何跡象。改成用資料庫給的 `l.id`。

#### 五、七天太久

這則廣播是 `warning` 且不可暫緩，session 起始那條路徑也沒有冷卻時間，等於接下來每一次新對話 AI 的第一句話都在講它。七天會被學會略過，那正是這一版要打破的沉默。改成 48 小時。

#### 六、只有一項、而且那一項被截斷時，頁尾會寫「另有 0 項未列出」

一句宣布自己沒有遺漏任何東西的遺漏說明。頁尾改成只在真的有遺漏時才出現（該項被切這件事，截斷標記已經講了）。順手把該檔案裡的簡體「内」改成「內」。

另外 `tests/install-check-alert-migration.test.js` 補進 `FILELIST.md`（其餘九個新檔都在，只漏這一個）。

**驗證**：六件事各自的測試都先拿舊版程式跑過一次、確認真的變紅（訊息類 7 紅、job 類 6 紅），再對新版跑綠。

### 第八部分：「放掉認領」看不到的那一種失敗

第七部分的第二點還留了一個洞。放掉認領這件事是由客戶端決定的，而客戶端只看得到「我這個查詢有沒有拿到回應」。

認領已經在資料庫那邊寫進去了、回應在回程掉了（連線被重置），這種情況下客戶端看到的是失敗，手上卻沒有這筆認領的紀錄——不會去放掉，也不知道要放掉。那筆失敗就永遠標記成已公告，而廣播從來沒寫成。這正是這一版要修的那個病，只是換了一個入口。

改成把所有認領跟寫廣播放進同一個資料庫交易（`withTransaction`）。要回滾的是資料庫自己，它是唯一知道那筆認領到底有沒有寫成的一方。既然有交易，客戶端那套「放掉認領」就整組拿掉了——留著會讓人以為還有第二道保險，實際上那道保險蓋不到最需要它的那一種失敗。

job 的介面多了一個可注入的 `withTransaction`（預設就是 `src/utils/db.js` 那一支，命名跟 `src/routes/setup.js` 一致），測試才餵得進一個會模擬回滾的假交易；兩個呼叫端（`src/routes/debug.js`、`src/index.js`）的用法完全沒變。

`resolved` 跟 `detail` 兩種更新刻意留在交易外面：它們跟認領無關、各自獨立，而且最常見的情況是「沒有新失敗要公告」——那條路徑根本不開交易，它們還是得寫。

認領迴圈改成照 `(user_id, machine, check_name)` 排序後才逐筆認領。以前每一筆認領各自 commit，鎖只活一個查詢；現在整批鎖要撐到交易結束，兩支同時跑的評估若以相反順序鎖同一組列就會死鎖。順序原本來自客戶端上傳的 checks 陣列，也就是「順序由外面決定」——固定成自己排的順序，這一整類問題就不存在。

**驗證**：三個新測試（廣播炸掉整批回滾、認領中途炸掉整批回滾、認領跟廣播確實在同一個交易裡）先對 `3698823` 跑過一次，4 紅。另外兩道守門測試（預設真的是 `withTransaction`、認領順序不跟客戶端走）各自把對應的程式碼弄壞一次確認會紅，再改回來。全套測試綠。

### 第九部分：金鑰只在環境變數裡，MCP 好好的，其他全部瞎掉

前八部分做的是「檢測失敗會主動找上門」。這一部分補的是一個從來沒被當成失敗看待的失敗。

金鑰可以放在檔案（`~/.claude/settings.json`、`~/.claude/settings.local.json`、`~/.claude.json`），也可以只存在於環境變數裡。MCP 的環境是 AI 工具餵給它的，所以兩種都能動。但是掃描器是工作排程器／launchd 起的、SessionStart 掛勾是掛勾執行器起的——兩者都繼承不到那個 shell，只讀得到檔案。

`resolve-credentials.cjs` 從 v1.26.82 起就把這件事回報成 `background_safe: false`。而它唯一的下場，是掃描器自己的 log 多寫一行——一份沒有人會去看的 log，寫在一台掃描器早就不回報了的機器上。安裝自我檢查把這個旗標收進來、然後從來沒看過它一眼。所以那台機器的掃描器跟記憶載入是死的，而每一份報告都寫「健康」。

Vin 2026-08-06 拍板：自動修，但不准無聲，而且要留退出的路。

新增 `scripts/install-helpers/ensure-key-file.cjs`，寫法完全比照 v1.26.86 的 `ensure-session-hook.cjs`：先寫 `.tmp` 再 rename（中斷的執行不會留下半個設定檔）、讀不懂的設定檔一律拒改並回報原因（不然是為了修一個只會安靜的問題去毀掉使用者其他所有設定）、合法 JSON 但不是物件同樣拒改（陣列會通過 `typeof` 檢查、然後在 stringify 時安靜地丟掉具名屬性，那條路會「成功」但什麼都沒寫）、`~/.ownmind/.no-key-file` 是使用者的退出開關。它的工作是把只存在於環境變數的金鑰補寫進 `~/.claude/settings.json` 的 `mcpServers.ownmind.env`，網址同理但只在網址也只存在於環境變數時才寫——已經寫在檔案裡的那個才是設定值，環境變數那個可能是舊的。

自我檢查加一項 `background_credentials`：先跑修復、再回報結果。已經在檔案裡、或這次補寫成功，都是 pass，detail 會說是哪一種；使用者自己退出是 warn；修復失敗是 fail，帶原因跟做得到的 fix。這個三分法是有作用的，不是措辭問題——本版的警告機制廣播新的 fail、故意不理 warn，所以修不好會找到人，刻意退出不會吵人。完全沒有金鑰也歸 warn，因為 `api_key_format` 已經在報同一件事了，同一把缺席的金鑰報兩次只是噪音。

四支安裝／更新腳本（`install.sh`、`install.ps1`、`scripts/update.sh`、`scripts/update.ps1`）照既有寫法呼叫它並印出那一行摘要，標籤跟著結束碼走——失敗不會印成 `[ OK ]`。

**驗證**：`tests/ensure-key-file.test.js` 把修復程式當獨立程序真的跑，子程序的環境重新造過（不然開發機自己的 `OWNMIND_API_KEY` 會讓「完全沒有金鑰」那個案例偷偷變成別的案例）。「先 .tmp 再 rename」用佔住 `.tmp` 路徑的方式驗——「沒有留下殘骸」這件事直接寫檔也成立，分不出兩者。破壞法：四支腳本各刪一次呼叫、helper 五處各壞一次、自我檢查三處各壞一次，十二次全部確認會紅再改回來。全套測試綠。

## v1.26.86 — 修理工自己從來沒上過工，以及版號提醒管到別人家

### 缺陷一：設定修復程式從來沒有真的執行過

v1.26.82 起，安裝程式裡就有一段「把 SessionStart 設定修成正確樣子」的修復邏輯。四個版本都假設它有在跑。

它沒有。證據一直躺在正式機上：采瑤 16:38 升級到 v1.26.84 之後，她的設定仍然是舊的單一 `null` 觸發時機。修復程式在，卻一次都沒動過她的檔案。

原因是那段邏輯用 PowerShell 把設定檔轉成 JSON 字串、塞給另一個程式當參數、再收回一個字串來判斷。這條傳遞鏈只要任何一環斷掉，結果就只是「不等於成功」，不會有任何錯誤訊息。壞得無聲無息，跟它要修的那個掛勾一模一樣。

### 改法

字串傳遞鏈整條拆掉。新增一支獨立的修復程式（`ensure-session-hook.cjs`），設定檔由它自己讀、自己改、自己寫回（先寫暫存檔再改名，中斷不會留下半套檔案）。四支安裝／更新腳本一律改成呼叫它，並把它回報的一行結果直接印在升級畫面上：修了、沒事、使用者自訂不碰、或**錯誤原因**。從此失敗會說話。

讀不懂的設定檔一律拒絕動（直接回報錯誤退出），不會為了修一個掛勾把使用者其他的設定毀掉。內容是 JSON 但不是設定物件（例如陣列）也一樣拒絕——那種檔案改了會假裝成功、實際什麼都沒寫進去。

對抗審查後再補三刀：升級畫面的標籤跟著結束代碼走，修復失敗不會再被印成 `[ OK ]`；家目錄含空格（`C:\Users\Jane Doe`）的未加引號舊設定不再被誤判成「使用者自訂」而永遠漏修；認定「這是我們的設定」看的是掛勾檔名，路徑不同是要修的漂移、不是要尊重的自訂。

第二輪審查又攔下一個會讓整版白做的錯：install.ps1 呼叫完修復程式之後，還會把記憶體裡的舊設定快照寫回檔案，等於當場蓋掉剛修好的東西——新裝的 Windows 機器會拿到沒有掛勾的設定、畫面卻是綠色 OK。修復程式已移到整個流程的最後一步，並加上「它必須是最後一個寫這個檔的人」的順序測試。同輪也發現使用者關閉自動掛勾的開關（`~/.ownmind/.no-session-hook`）在這次改版中失效了；現在改由修復程式自己遵守這個開關，四支腳本一體適用，關掉的人會看到 `opted_out` 而不是被每天重新裝回去。

### 驗證

測試把這支程式當獨立程序真的執行，餵的是采瑤跟 Adam 兩台機器逐字真實的壞設定，驗證修完的樣子；並且用破壞法確認「安裝腳本忘記呼叫它」會讓測試變紅（前四版就是死在這裡，這一環特別上保險）。

### 缺陷二：版號提醒拿自己的版本管別人的專案（bug #13）

commit 後的版號提醒（「版本 X 還沒打 tag」）讀的是 OwnMind 自己的版本號，卻去查使用者正在 commit 的那個專案有沒有對應 tag。在 OwnMind 自己的 repo 裡兩個數字剛好一樣所以看不出來；在 Go / Rust 這類沒有 package.json 的專案，每次 commit 都會被叫去打一個完全不相干的 tag。照做還會建出錯誤的 tag。

更深的傷害：同一則無關提醒每天噴，使用者很快學會無視所有 OwnMind 前綴的輸出，連真正該看的警告一起略過。

### 改法

版本號改成從「正在 commit 的那個專案」的 package.json 讀。專案沒有 package.json、或沒有版本欄位，整段提醒閉嘴。

改了來源就多了一個口子：那個 package.json 是別人 repo 裡的東西，不可信。所以版本字串只接受正常版號格式，查 tag 改用不經過 shell 的呼叫方式——惡意 repo 不能藉 version 欄位在 commit 的當下執行指令。這是對抗審查抓到的，測試用一個帶指令的假版號實際驗過打不進來。

### 驗證

在臨時 git repo 實跑掛勾：Go 型專案安靜、Node 專案提醒的是它自己的版本（不是 OwnMind 的）、tag 已存在時安靜。


## v1.26.85 — 掛勾指向一個永遠跑不起來的副本

### 缺陷

安裝程式把 SessionStart 掛勾指到 `~/.claude/hooks/ownmind-session-start.js`。那支程式用相對路徑載入 `../shared/`，從那個位置算出來是 `~/.claude/shared/`，**那個資料夾不存在**。

Node 直接以 ERR_MODULE_NOT_FOUND 結束，一行都沒跑到。沒有人會看掛勾的結束代碼，所以什麼都不會說。

這是 v1.26.80～84 一路修完之後，最後一層。Adam 的機器每一項都對了：四個觸發時機、走 Node、檔案存在、版本最新，載入次數仍然是 0。

**對照組是意外來的。** 采瑤 的 AI 當初手寫路徑時寫成 `~/.ownmind/hooks/`，那裡的相對路徑解得開，所以她一路都是通的。兩台唯一的差別就是那個目錄名。

### 改法

Windows 的掛勾一律指向 `~/.ownmind/hooks/`。舊的 `.claude/hooks` 路徑會在升級時自動換掉。

### 驗證

拿 Adam 當天真實的設定跑真正的更新程式：四筆全部從 `.claude/hooks` 改成 `.ownmind/hooks`；已經在正確路徑的機器不會被動到。


## v1.26.84 — 記憶載入成功了也不會有人知道，而且 Windows 少做八件事

### 缺陷一：儀表壞的

Windows 走的 Node 版掛勾，載入記憶之後只寫本機檔案，**沒有回報給伺服器**。bash 版一直都有回報，移植過去的時候漏掉這一行。

後果比漏一筆紀錄嚴重：

- 一台正常運作的 Windows 電腦，在伺服器上跟一台壞掉的長得一模一樣
- v1.26.81 新加的「記憶有沒有載入」檢查，正是讀這筆紀錄。所以**每台健康的 Windows 都會被判成「從來沒載入過」**，而且永遠不會變

一個對健康機器亂叫的警報，會教會所有人忽略它。

發現的時機是驗證 v1.26.82 的時候：Adam 重開 Claude Code 後，伺服器看得到他的 MCP，卻看不到那筆「自動載入」——而那正是整個 Windows 修復要拿來判定成敗的數字。

### 缺陷二：引號決定了誰的設定

v1.26.82 的「不覆蓋使用者自訂的指令」保護，比對的是我們產出的**完整字串**。

采瑤 的電腦上，她的 AI 手動寫過一行可以運作但**沒加引號**的指令。我們把它讀成「使用者自訂」，於是升級不去動它——她就一直停在一個觸發時機，只有開全新對話會載入記憶，繼續舊對話、清空、壓縮都不會。

**引號是寫法，不是意圖。** 現在的判斷改成：一行指令如果整件事就是執行我們自己的掛勾檔，不管怎麼寫都算我們的；一旦有多餘參數、包了別的程式、或指向別的檔案，才是使用者真的改過，不碰。

### 缺陷三：Windows 的掛勾少做八件事

v1.26.80 把 Windows 改走 Node 版掛勾，那是對的，但那支比 bash 版少做八件事，
其中兩件使用者會直接感覺到：**廣播通知從來不會出現、記憶檔從來不會寫進專案目錄。**

當時記成待辦 26，並註明「不准用『夠用了』把它結案」。這版補齊：

| bash 版做的事 | Node 版現在 |
|---|---|
| 補印上次累積的訊息 | ✅ |
| 補送合規紀錄 | ✅ |
| 每日更新檢查 | ✅（跟 MCP 共用同一把鎖，不會互相蓋） |
| 補送自我檢查報告 | ✅ |
| 用 sync_token 跳過重複下載 | ✅（順帶拿到帳號指紋保護） |
| **顯示廣播** | ✅ |
| 用共用的排版模組輸出 | ✅（兩個平台讀到的格式從此一致） |
| **同步記憶檔到專案目錄** | ✅ |

排版改用共用模組而不是各寫一份，是因為兩份佈局必然會走鐘；今天兩邊已經不一樣了。

### 驗證

拿兩台機器當天真實的設定跑真正的更新程式：

- 采瑤：一筆沒有觸發時機的指令 → 四筆完整的
- Adam：設定已經正確 → 完全不動（否則每天都會重寫一次）
- Mac：不受影響
- 回報那條：本機實際執行一次掛勾，三秒後在正式機資料庫查得到，來源標記為 hook
- 八項補齊：起一個假伺服器、實際執行掛勾程式，斷言真的送出去的請求與真的寫出來的檔案


## v1.26.83 — Windows 掛勾成功了也不說，害驗證看不見、檢測永遠誤判

### 缺陷

驗證 v1.26.82 時當場發現：Adam 重開了 Claude Code，伺服器看得到他的 MCP 動靜，
卻看不到記憶載入的紀錄。原因不是沒載入，是 **Windows 走的那支掛勾只把紀錄寫在本機，
從來不上傳**。bash 版一直都會上傳，改寫成 Node 版時漏了這一行。

兩個後果，第二個比第一個嚴重：掛勾好的機器跟死的機器在伺服器端長得一樣；
而昨天新加的「記憶有沒有載入」檢測讀的正是這個紀錄，所以**每台健康的 Windows
機器都會被永遠誤判成壞的**。狼來了喊多了，檢測就沒人信了。

### 改法

掛勾載入成功或失敗時，補上傳一筆到伺服器。射後不理、三秒逾時，絕不拖慢對話啟動。

### 驗證

測試起一個假伺服器、跑真的掛勾程式，斷言那筆真的送到；再在本機跑一次真掛勾，
一秒後那筆出現在正式機資料庫，來源標記正確。

---

## v1.26.82 — 金鑰找錯地方，害掃描器、記憶載入、自我檢查一起死

### 缺陷

Adam 的電腦：MCP 每天正常上傳資料，但用量掃描器 7/15 停、升級回報 7/8 停、記憶從來沒載入過、
剛才跑自我檢查三個項目失敗說「金鑰是空的」。

一個原因。**除了 MCP 以外，每個元件都只去 `~/.claude/settings.json` 找金鑰，而他的金鑰不在那裡。**

MCP 不讀檔案，金鑰是 Claude Code 直接餵進它的執行環境的，所以它照常運作。這是最糟的組合：
唯一能發出警報的那個元件，正好是唯一沒事的那個。

他的實際狀況是網址在 `~/.claude.json`（Claude Code 現在放 MCP 設定的地方），
金鑰在 `OWNMIND_API_KEY` 環境變數。

### 改法

「去哪裡找金鑰」收進一個檔案，四個會因為找不到就停擺的元件共用：掃描器、兩支
SessionStart 掛勾、安裝後的自我檢查。找的地方依序是兩個設定檔、Claude Code 的設定檔、
環境變數。

### 一個容易漏掉的區別

**環境變數不等於檔案。** 用量掃描器是排程叫起來的，不會繼承你 shell 的環境變數。
在環境變數裡找到金鑰，只證明 MCP 能動，不代表掃描器能動。

所以多回報一個判斷：金鑰是不是存在「背景程式讀得到」的地方。不是的話會明白講出來，
而不是報成健康。Adam 的情況正是如此。

### 連帶補上：快取現在記得自己是誰的

Adam 曾經裝過別人的金鑰。伺服器查過沒有痕跡（他那台從有紀錄以來只出現在自己帳號下），
但本機的記憶快取檔**不記錄它屬於哪個帳號**：換金鑰之後，前一個帳號的個人資料跟鐵律
會留在硬碟上繼續被讀，沒有任何地方看得出來。

掃描器在 v1.26.69 就為同一個危險修過它的進度檔，記憶快取一直沒有。現在寫入時蓋上
帳號指紋（伺服器＋金鑰的雜湊，不存金鑰本身），對不上就整份拒收、重新下載。
既有機器上沒有指紋的舊快取一律當別人的處理，代價是多下載一次。

### 順帶修掉的誤導

掃描器找不到金鑰時的錯誤訊息只寫「~/.claude/settings.json」，把看到訊息的人指向
金鑰最不可能在的那個地方。現在三個位置都列出來。


## v1.26.81 — 加一項檢查：記憶到底有沒有真的載入

### 缺陷

安裝後的自我檢查有九項：檔案在不在、版本對不對、金鑰能不能用、排程活不活著、
用量資料有沒有真的傳到伺服器。**沒有一項在問這個產品最核心的那件事：使用者的記憶跟鐵律，
到底有沒有進到他的 AI 裡。**

所以六台機器三個月沒有載入過任何東西，沒有任何地方發出聲音。不是難查，是沒人在查。

### 改法

第十項檢查，照現有那項「用量有沒有傳到」的做法：**結論由伺服器給，不由本機自己說。**
一台機器對自己的健康狀況做的報告，正是最不能信的那一份。

伺服器回答：這個帳號最近一次「自動載入」是什麼時候、七天內幾次。而且把「自動載入」跟
「AI 自己去呼叫」分開算，後者不算數，那是 AI 剛好記得，不是功能會動。

本機同時收集四樣東西，不管結論是什麼都收，因為那是別人機器出問題時唯一的線索：

- 設定裡那行指令的原文
- 那行指令要跑的檔案在不在
- 這台的 `bash` 解析到哪裡（在 System32 就是 WSL，會特別標出來）
- 這台的 `node` 解析到哪裡

這四樣是這週查那六台機器時，一項一項手動挖出來的。以後每個人每次升級都會自動附上。

### 三種不算失敗的情況

沒有金鑰、連不上伺服器、伺服器版本太舊，都只是提醒不是失敗。把離線的機器判成壞掉，
只會教會大家忽略這項檢查。

### 更大的洞：這個檢查以前一季才跑一次

完整的自我檢查只在「安裝」跟「手動跑升級」時執行。而沒有人會手動跑升級。

Adam 上一次完整回報是 **2026-05-29**。之後兩個多月，他的電腦每天自動更新、什麼都沒說，
而他的用量掃描器早在七月就死了。

**而且他五月那份回報裡就已經有答案。** 裡面寫著這台電腦的 `bash` 指向 WSL，正是 Windows
記憶載入失敗的原因，而那個原因花了整整一週才查出來。資料一直都在，只是沒有人看，
也沒有再更新過。

所以自動更新現在也會跑一次自我檢查並上傳，每天。跳過唯一那項會掃描所有本機資料庫的
檢查（掃描器本來就有自己的排程），其餘九項照跑，背景執行，不擋更新。

六台 Windows 電腦的環境紀錄現在攤開來看是這樣，全部是它們自己回報的：

| | `bash` 指到哪 |
|---|---|
| Adam、Eric、Joanna、采瑤 | WSL（四台都沒裝 Git Bash） |
| Amiee、測試機 | 有 Git Bash，但系統目錄的 WSL 版排在前面 |

---

## v1.26.80 — Windows 上的記憶自動載入，從來沒有成功過一次

### 缺陷

每次新對話開始時，OwnMind 會自動把使用者的記憶跟鐵律載入 AI 的工作環境。這件事在 Windows 上**從來沒有發生過**。
...
沒有任何錯誤訊息。使用者看到的是「AI 好像不太記得我的規則」，而不是「載入失敗」。

### 兩個原因

**一、安裝時選對了，之後每天被改回去。**

安裝腳本在找不到 bash 的時候會註冊 node 版指令，這是對的。但每天跑的更新腳本認得那筆設定（比對字串 `ownmind-session-start`，node 版也中），發現它少了四個觸發時機，就整組刪掉、重寫成**寫死 bash** 的四筆。

所以正確的設定只活到第一次自動更新。而每台機器每天都會更新。

**二、判斷「有沒有 bash」這件事本身就是錯的問題。**

Windows 10/11 的 `bash.exe` 在 System32，那是 WSL 的啟動器。找得到不代表跑得動：指令會進 WSL 執行，`~` 是 WSL 的家目錄，掛勾檔根本不在那裡。

這個專案本來就知道這件事（`find-git-bash.ps1` 就是為了避開 WSL 寫的），但只用在升級驗證那一步，沒用在真正需要它的地方。

### 改法

指令怎麼下，收進**一個**檔案，四支安裝／更新腳本共用。Windows 一律走 node 絕對路徑，Mac / Linux 維持原本那串 bash 完全不動。

一個關鍵細節：**光看「四個觸發時機齊不齊」修不到任何人。** 那六台機器現在都是四個齊、但全是 bash。判斷條件必須連指令內容一起看，否則這一版發出去等於沒發。

另外，使用者自己改過的指令不會被覆蓋。以前只要跟預設不同就會被每天蓋一次，使用者永遠吵不贏。

### 補丁：手動跑升級也要能修好（同版，不另外發版）

第一版的修復只做在自動更新那條路上。但壞掉的那幾台，OwnMind 幾乎不啟動，也就幾乎不會
自動更新；他們最可能的動作是手動跑一次升級，而那條路走的是安裝腳本。

安裝腳本原本的判斷是「有沒有設定」，有就跳過。**所有壞掉的機器都「有」，只是內容是錯的**，
所以那條路永遠修不到任何一台。現在安裝腳本也會檢查內容對不對，不對就換掉。

跟今天其他幾個缺陷同一個形狀：修復蓋在故障不會經過的路上。

### 已知還沒補的

Windows 走的 node 版掛勾比 bash 版少做八件事，其中**廣播通知**跟**記憶檔同步**是使用者會感覺到的。這一版先讓記憶跟鐵律載入得了（原本是完全沒有），其餘列入待辦 26。

### 驗證

沒有 Windows 機器可以測，所以拿真正的更新腳本、在假造成 Windows 的環境下，對著那六台機器現在的設定實際跑：四筆 bash 全部換成 node，第二次執行不再改動；Mac 的設定完全沒被碰到；使用者自己改的指令留著；連 helper 檔案不存在時也不會把其他掛勾一起弄壞。

---

## v1.26.79 — 自動更新現在會檢查排程還活著，死了就接回去

### 缺陷

用量收集靠一支排程程式，每兩小時去讀本機資料庫。這支排程只在**安裝的時候**註冊一次，之後沒有任何地方再看它一眼。

它死掉的時候，畫面上不會出現錯誤。用量欄位只會變成「尚無資料」，看起來就像這個人今天沒工作。

實際發生過：某台機器的排程從 7/15 起就沒再跑過，**三個星期沒有人發現**。那台機器的 OwnMind 一直是好的，每天自動更新、每天回報，只有排程不見了。

### 為什麼原本的修復救不到

Windows 的手動升級腳本從 v1.26.65 起就會重新註冊排程。但只有手動執行升級指令才會走到那裡，而沒有人會手動跑升級。

自動更新走的是另一條路，那條路從來沒看過排程。**修復蓋在一條故障不會經過的路上。**

### 改法

**一、自動更新每次都問一次排程還在不在，不在就接回去。**

三個作業系統都做：macOS 問 launchd、Linux 問 systemd、Windows 問工作排程器。已經活著就完全不碰，避免把好好的排程重設。

**二、「存在」不等於「會跑」。** Windows 的工作排程器可以有一個被停用的工作，查詢得到、永遠不會執行。對使用者來說跟沒有一樣，所以停用也算壞掉。Linux 同時問「現在有沒有在跑」跟「重開機後會不會回來」。

**三、修不好就往伺服器回報。** 這件事的本質是「壞掉沒人知道」，所以修復失敗一定要留下痕跡，不能只印在沒人看的視窗上。修復失敗不會讓整個更新算失敗，因為檔案同步本身是成功的。

**四、安裝時也要驗證。** macOS / Linux 的安裝腳本原本只看註冊指令有沒有報錯就宣告成功，跟 Windows 從 v1.17.12 起就會回頭確認的做法不一致。現在三邊一致：註冊完再問一次系統。

### 驗證

在真的 launchd 上做過陰陽對照，不是只跑模擬：先確認排程活著時完全不動它，再把 agent 真的拆掉、確認 launchd 查不到，然後跑修復、由 launchd 回答它回來了。

---

## v1.26.78 — 欄位只有 10 個字，值有 11 個字，整批活動紀錄被退回

### 缺陷

`activity_logs.source` 這個欄位限制 10 個字。而寫進去的值是：

| 值 | 幾個字 | 誰寫的 |
|---|---|---|
| `system_auto` | 11 | AI 每次自動偵測到「有遵守鐵律」就送一次 |
| `session_audit` | 13 | 對話稽核 |
| `system_server_auto` | 18 | 伺服器自己補的那筆 |

**從這個欄位建立以來，這三個值一次都沒進去過。** 證據是這張表自己：三萬一千筆裡，來源只有 `mcp`、`hook`、`api`、`e2e-test` 四種。

### 比「掉一筆」嚴重

上傳活動紀錄是整批送的，而整個迴圈只包在一個錯誤處理裡。**一筆放不進去，整批都被退回。** 客戶端存起來重送，再送再失敗。

看得到的後果在統計儀表板：鐵律遵守率、活動筆數，都是用那些從來沒進來的資料算的。

### 改法

**一、把欄位放寬到 64 個字。**

不是把字串改短。因為這些值也在**已經裝在別人機器上的舊版**裡，有人還停在 v1.26.29。改短伺服器這邊的字串，那些機器照樣失敗。

放到 64 而不是剛好夠用的 24，因為「照今天最長的那個字串來設」就是在等明天那個更長的。

**二、每一筆各自處理。** 一筆被拒絕就只有那一筆，而且會記下它是哪種事件、來源是什麼。以前只有一句沒頭沒尾的「整批上傳失敗」，這就是它躲了這麼久的原因。

### 老實說一件事

這個上傳路徑沒辦法從測試裡驅動（它直接抓資料庫連線，不是傳進去的），所以「一筆壞掉不影響其他筆」這件事只能用讀程式碼的方式檢查，比真的跑一次弱。要改成可注入是另一件事，已列進待辦。

### 今天第三個

三個缺陷都是部署之後才現形，測試全綠時一個都看不到，而且是同一個形狀：**測試把介面的兩端都造假的時候，它量的是那兩個假的彼此同意。**

---

## v1.26.77 — 升級後的自我檢查，從來就問不到伺服器

### 缺陷

2026-08-06 把一台真的機器升到 v1.26.76，升級自己跑的檢查說：

```
問不到伺服器：伺服器回 401
掃描本身有跑，只是這個檢查沒辦法確認另一端。
```

掃描上傳成功。**只有「回頭問伺服器我的資料到了嗎」這一步失敗。**

### 為什麼

問伺服器的時候用的是 `X-API-Key` 這個標頭，而伺服器只認 `Authorization: Bearer`，其他一概不看。

所以這個功能**從 v1.26.72 加進來就是壞的**，每一次呼叫都是 401。

### 為什麼一直沒發現

v1.26.72 寫完的時候有拿正式機測過一次。當時正式機是 v1.26.67、還沒有這個端點，所以回的是 404，檢查也就正確地回報「這台伺服器還沒有這個功能」。

**404 是在驗證身分之前就回的。** 第一台有機會回 401 的伺服器，是一小時前才部署上去的那台。

### 為什麼測試沒抓到

兩邊各自造假：

- 客戶端的測試把「發請求」換成假的，所以沒有任何標頭被讀過
- 伺服器端的測試把「驗身分」換成假的，所以沒有任何標頭被檢查過

兩邊各自的測試都全過，但它們對不上。

**測試如果把一個介面的兩端都造假，它就看不見這個介面壞掉。** 這是兩小時內第二個同樣形狀的缺陷，前一個是 v1.26.76 的心跳 SQL。

### 這次怎麼確定修好了

直接拿兩個標頭打正式機：舊的回 401，新的回 200、拿回五個工具的紀錄。

另外加了一條測試，它會去讀伺服器那支驗證身分的程式碼，所以兩邊以後不能再默默走散。

---

## v1.26.76 — 心跳寫不進去，資料庫連收都不收

### 缺陷

正式機在 2026-08-06 03:16 升上 v1.26.75。八秒後開始噴：

```
heartbeat update failed: inconsistent types deduced for parameter $2
```

每一次心跳都失敗。**所有機器的收集器都變成回報不了**「我還活著」。

### 為什麼

v1.26.73 為了把「一個人一個工具最多 20 台」的上限做進同一次往返，把這段寫入從 `INSERT ... VALUES` 改成 `INSERT ... SELECT ... WHERE`。

這兩種寫法認欄位型別的方式不一樣：

| 寫法 | 參數的型別怎麼來 |
|---|---|
| `INSERT ... VALUES` | 從它要寫進去的那個欄位 |
| `INSERT ... SELECT` | 先把 SELECT 當成獨立查詢分析，沒有指定就是「未知」，最後當成 text |

而同一個參數在下面的 `WHERE tool = $2` 又被認成 varchar。同一個參數兩種答案，Postgres 直接拒收整句。

### 為什麼測試沒抓到

這個專案所有測試都給這支程式一個假的資料庫，SQL 被推進一個陣列就結束，**沒有任何懂 SQL 的東西讀過它**。一百條這種形狀的測試也分不出這句話是對的還是壞的。

跟 v1.26.71 那次是同一類問題：量測工具本身看不到它該量的那個失敗。

### 這次怎麼確定修好了

三個方式，不是只靠自己的測試：

1. 測試加了斷言，並且**故意拿掉一個型別標註驗證它會變紅**
2. 把**沒修的**那句話拿到正式機的資料庫上試（只解析、不執行），重現一模一樣的錯誤訊息
3. 把**修好的**那句話拿到同一個資料庫上試，通過

順便掃了全部程式碼還有沒有同樣形狀的寫法：找到一處，它的每個參數都只出現一次、不會有第二種認法，也一樣拿到資料庫上驗過才放過。

---

## v1.26.75 — 上一版那一欄，查得比需要的多幾十倍

### 缺陷

v1.26.74 讓「最近活動」改讀三個來源，寫法卻讓其中兩個來源跟著工作紀錄的筆數跑：**一個人這段期間有 50 筆工作紀錄，就查 50 次**。而 token 用量是整個產品裡最大的一張表。

沒有人會看到錯的數字。這一版修的是它為了算出那個數字所做的白工。

### 改法

改成一個人算一次。排序也改成直接指名畫面上那一欄，不再把同一段查詢寫第二遍（寫第二遍就等於再跑一次）。

### 這是怎麼發現的

發版前跑的對抗審查。四個發現，一個要修、一個保留但把理由寫進程式碼、兩個是確認沒問題。

保留的那個：統計儀表板的「最近活動」不跟著日期篩選走，跟它旁邊的計數不同調。這是刻意的 — 那一頁列的是所有人，這一欄的意思一直是「最後一次聽到這個人是什麼時候」。套上日期區間，會讓「這段期間沒動靜的人」變成空白，而那正是有人打開這頁要找的人。

---

## v1.26.74 — 「最近活動」現在真的是最近活動

### Vin 看到什麼

2026-08-06，他人正在用 Claude 工作，團隊用量頁說他最近一次活動是凌晨 00:20，差了八小時。

**數字是對的，意思是錯的。**

### 為什麼

那一欄讀的是「最近一次工作紀錄」。而工作紀錄這件事，AI 是在**對話快結束的時候**才寫一次。所以一個長時間的工作對話，那一欄顯示的是它**開始**的時間，而且在結束之前不會再動。

**一個人一次坐下來工作得越久，這一欄就錯得越多** — 剛好跟它該有的行為相反。

統計儀表板頁有同一個毛病的輕症版：它讀「AI 呼叫 OwnMind 工具」的時間，而一個長時間寫程式的對話可能一次都不會呼叫。
...
|---|---|
| 工作紀錄 | 對話結束時 |
| MCP 活動 | AI 呼叫 OwnMind 工具時 |
| Token 用量 | 收集器上傳時，**唯一一個在工作中途會動的** |

Token 用量記的是**訊息發生的時間**、不是上傳的時間，所以收集器 30 分鐘一次的排程只影響「多久看得到」，不影響「它說是幾點」。

兩個頁面現在讀同一組來源。同一個問題兩個頁面給兩個答案，本身就是一個缺陷。

### 沒有動

團隊用量頁**誰會出現在名單上**，還是由工作紀錄決定。時間的意思跟名單的組成是兩個問題，一起改會在修第一個的時候偷偷改掉第二個。

---

## v1.26.73 — 一台電腦一列，不要一個人一列

### 缺陷

`collector_heartbeat` 是「一個人一個工具一列」。所以一個人不管有幾台電腦，就只有五格；每掃一次，就把上一台的機器名字、版本、作業系統、狀態全部蓋掉。

2026-08-05 在正式機上親眼看到：11:50 的時候，Vin 的 `claude-code` 記的是 TANK、另外四個記的是 Vincent.local。12:30 在 Windows 那台手動掃一次之後，**五個全部變成 TANK，Mac 的狀態從資料庫裡消失，而且沒有留下它曾經回報過的任何痕跡。**

**代價不是好不好看的問題。一台電腦上的收集器死掉，只要同一個人另一台還活著，就完全看不出來** — 心跳是新的、用量也在流。系統設定頁只看得到最後寫入的那一台，第二台根本不會出現。

### 為什麼是現在

v1.26.72 的自測可以問伺服器「我的資料到了嗎」，但在一個人有兩台電腦的帳號上，它能給的最好答案只有「伺服器把這個記在另一台電腦上」。那是聳肩，不是答案。

而且它擋著你要的「自動收環境參數」：兩台電腦的診斷資訊互相蓋掉，比不收還糟。

### 改了什麼

鍵值變成 `(user_id, tool, machine)`。機器名字先變成不可為空、預設 `unknown`，因為 Postgres 認為兩個空值互不相同，一個不回報主機名稱的客戶端會每次心跳都插一列新的、永無止境。

更新的時候不再寫入機器名字。它現在是識別的一部分，而**那一行寫入就是兩台電腦互相抹掉的完整機制**。

### 跟著變好的

- 自測現在會在同一個工具的好幾列裡面找**自己那一列**，所以「收到了」代表的是這台、不是這個帳號。一台上傳一直失敗的電腦，再也不能拿隔壁那台的心跳當自己的成功證明。
- `team-overview.js` 免費變好。它那段查詢本來就是繞著這個限制寫的，旁邊還留著一行「未來要…」的註解。
- 三個「一個工具顯示一個版本」的查詢改成取最新那台。不改的話，每個有兩台電腦的人都會出現兩次。

### 審查抓到的：我自己開了一個新的洞

**這個改動把「客戶端自己講的字串」放進了資料的識別碼裡。** 在這之前，識別碼沒有任何一部分是客戶端能控制的，所以沒有任何帳號有辦法讓這張表長大。現在一台每次開機主機名稱都不一樣的電腦，每次心跳都會插一列新的，而且原本的頻率限制完全擋不住，因為每一列都是「第一次」。

加了上限：一個人一個工具最多 20 台。而且**上限只擋新機器**，已經在名單上的機器永遠可以繼續更新，所以撞到上限不會害一台真的在回報的電腦變啞巴。

另外，migration 原本是先把舊的唯一性拿掉、再建新的。反過來了：先建新的，讓資料庫自己證明沒有重複，有重複就整筆回滾，而不是留下一張完全沒有唯一性的表。

還有一條記進 backlog：兩台主機名稱一樣的電腦，在伺服器眼裡是同一台。這不是這版造成的（在這之前一個人的所有電腦本來就共用一列，範圍更大），但它是同一個問題剩下的那個洞。

### 系統設定頁改成依電腦分組

畫了三種給 Vin 挑，他選了「依機器分組」。

一台電腦一個小標題（名字 + macOS/Windows + 最後回報多久前），工具列在底下。**一台電腦的狀態取它最糟的那個工具**，不是平均 — 一台其他都正常、只有一個收集器死掉的電腦，正是這整件事要讓它看得見的情況，平均掉就等於放回去。清單順序是壞的排前面。

順帶修掉一個 React 的 key 衝突：原本用工具名稱當 key，一個人有兩台電腦就會撞。

---

## v1.26.72 — 升級完之後，機器會回頭問伺服器「我的資料到了嗎」

### 為什麼

這一週修掉的每一個收集器缺陷都長一樣：機器以為自己在正常運作、伺服器手上什麼都沒有、而且沒有任何一層說得出來。v1.26.66 那個是這樣過了十一週。

**真正的問題是：要診斷收集器所需要的證據，只存在於那台出問題的機器上，而沒有人會去那裡看。** Amiee 的 codex 從 2026-05-05 到現在一次都沒回報過，到今天還卡在「要碰到她的筆電才查得到」。

### 加了什麼

在唯一有人在現場、而且正在注意的那一刻（安裝或升級的最後）問一個問題：

1. 跑一次掃描
2. 問伺服器：**你現在手上有這個帳號、這台機器的什麼東西**
3. 印出一般人看得懂、而且知道下一步要做什麼的結果

第 2 步是這件事的重點。掃描器本來就會拿到每一批的「收到幾筆」回覆，那從來就不夠：那只代表一個請求成功了，不代表伺服器最後手上真的有東西；而且「沒東西可送」的那幾輪它什麼都不會說，偏偏壞掉的機器每一輪都是那樣。

### 五種判定

| 判定 | 意思 | 等級 |
|---|---|---|
| 收到了 | 伺服器手上有這個工具、來自這台機器、剛剛才更新 | 正常 |
| 沒裝 | 這台沒有這個工具，本來就沒東西可收 | 正常 |
| 記在別台 | 伺服器上這個工具記的是另一台電腦 | 提醒 |
| 認不出是誰 | 伺服器有近期紀錄，但不知道是哪台電腦送的 | 提醒 |
| 沒送到 | 這台掃了，伺服器上卻沒有這台的近期紀錄 | 失敗 |
| 讀不到 | 這台讀不到這個工具的資料 | 失敗 |

「記在別台」是 backlog 第 14 條的可見版本：`collector_heartbeat` 是一個人一個工具只有一格，所以一個人有兩台電腦就會互相蓋掉。**這版不修它、也不等它**，只是讓它看得見。現在的狀況是第二台完全不存在、一點痕跡都沒有。

### 兩條不能違反的規則

**絕對不能弄壞安裝。** 網路不通、伺服器版本太舊、另一個掃描正在跑、掃太久，全部只是提醒，安裝照常完成。一個會弄壞它要檢查的東西的診斷工具，比它要抓的缺陷還糟。

**真的沒送到就要 exit 1。** 上面每一層都在讀離開代碼，而這一週那些缺陷之所以能一直躲著，就是因為每一層都回報成功。

### 審查抓到最重要的一條

「認不出是誰」這一格原本會判成「收到了」，而且旁邊還寫了理由：不知道是誰不代表就是別人。

那個理由是錯的，而且錯出的正好是這整件事要防的那一種。伺服器上一個人一個工具只有一格，所以一筆「沒寫機器名字」的新紀錄，跟「別台電腦寫的」根本分不出來。**結果就是：一台上傳其實一直在失敗的電腦，會把隔壁那台的心跳當成自己成功的證據。**

改成第五種判定「認不出是誰」，算提醒。

另外兩條也修了：這個端點沒有設不准快取（每個成員打的網址一模一樣、靠 header 分身分，中間任何一層快取都可能把 A 的資料送給 B）；還有 API 金鑰有機會混在網路錯誤訊息裡被印出來（量過：金鑰裡有換行的時候，fetch 丟出來的錯誤會原封不動帶著它）。

### 放在哪裡

`self-check.cjs` 從 v1.17.63 起就在每次安裝結束時跑，它原本的八項檢查全部在問「東西有沒有裝好、能不能登入」。這次加的是第九項 `usage_roundtrip`，是唯一一項在問「資料到底有沒有在流」。它跟著既有的報告一起寫本機 log、一起上傳給伺服器，所以管理者那邊也自動看得到。

另外 `hooks/ownmind-selfcheck.js` 是同一套邏輯的單獨指令，要診斷某一台機器的時候只要跑這一行，不用照著一份步驟做。

### 沒有做的

Vin 另外要的「自動收上來的環境參數跟除錯資訊」不在這版。**要先解決一個人的兩台電腦互相蓋掉的問題**，不然收上來的診斷資訊會是兩台混在一起的，比不收還糟。順序是 backlog 14，然後才是那個。

---

## 未發版 — install.sh 的 scanner 清單改成用掃的，不要用手寫的

`install.sh` 裡有一段複製 scanner 模組的程式，檔名是手寫的五個，是 2026-04-22 跟用量追蹤系統一起寫的。`shared/scanners/` 之後長到十一個檔案，沒有人回來加。

**沒有任何使用者受影響，而「怎麼確定沒有」才是這條真正的內容。**

那段複製的來源是 `$OWNMIND_DIR/shared/scanners`，目的地是 `$HOME/.ownmind/shared/scanners`，而 `OWNMIND_DIR` 就是 `$HOME/.ownmind`，同一個資料夾。`safe_cp` 用 `-ef` 比對之後直接跳過，整段是死碼。真正把檔案送到位的是上面的 `git clone` / `git pull`。

第一次讀到這張清單的結論是「少六個檔案、每台 Mac 的收集器都開不起來」，而且還做了一個「驗證」去支持它：手動建一個只放五個檔案的資料夾，跑起來當然壞。**那個資料夾是驗證的人自己造的，安裝腳本不會產生它。** 反證一直在眼前，好幾台機器的收集器每天都在回報。

改成 glob，並且加了 `tests/install-scanner-module-list.test.js` 盯著。這段複製是給「`OWNMIND_DIR` 不等於 `~/.ownmind`」的情況留的，那一天它必須是對的；而且一張看起來很權威的過期清單，會讓下一個讀到的人付出同樣的代價。

---

## v1.26.71 — 最後一個「應用程式不開著就讀不到」的收集器

### 缺陷

v1.26.70 修好了 Tier 2，同時記下一件沒做的事：`shared/scanners/opencode.js` 有自己一份一模一樣的 `sqlite3 -readonly`。這版就是把那一份收掉。

一樣的對照實驗，2026-08-06 在 Mac 上量的。把資料庫複製到空目錄，旁邊什麼都沒有：

```
sqlite3 -json -readonly "<複本>"   → unable to open database file (14)
sqlite3 -json          "<複本>"   → [{"count(*)":1205}]
```

`PRAGMA journal_mode` 對正在用的檔案回 `wal`，所以是同一種資料庫、同一個失敗。

**第一次量這件事，量的人自己把結果污染了。** 對著正在用的檔案下 `-readonly` 看起來是成功的，這個結果一度被讀成「OpenCode 沒受影響」。它會成功，是因為前一行的 `PRAGMA journal_mode` 已經用讀寫模式開過一次，順手建出了 `-readonly` 需要的 `opencode.db-shm`。在那道探針之前列出來的目錄裡，一個附屬檔都沒有。**量測工具自己製造了它接著回報成「本來就這樣」的條件。**

### 為什麼這個比 Tier 2 嚴重

Tier 2 掉的是「那天有沒有用」的天數，還能從別的天數推。OpenCode 是 Tier 1，掉的是每一則訊息的 token 明細，其他所有數字都是從它算出來的，**沒有第二個來源可以推回來**。

而且它比編輯器更常是關著的。OpenCode 是每做一件事開一次的終端機程式，Cursor 則是一開就開一整天。

### 什麼東西沒有風險

游標是 `(time_created, id)` 複合鍵，而且只會前進到真的讀進來的那幾筆。所以就算只讀到一半，代價是延遲、不是掉資料，剩下的下一輪會補上。這跟平常增量讀取靠的是同一個性質。

### 修法

把 v1.26.70 的 fallback 抽成 `shared/scanners/sqlite-cli.js`，兩邊共用。**一份實作、一份測試、下次再發現問題只有一個地方要改。**

兩個呼叫端的差異保留、不平均掉：

- **輸出上限。** Tier 2 讀三筆遙測資料、給 10 MB。Tier 1 一次掃描要吐出游標之後的每一則訊息，給 100 MB。把 OpenCode 降下來會在「第一次掃描一段長歷史」時安靜地截斷。
- **安裝提示。** OpenCode 找不到 sqlite3 CLI 時，會印出指名 OpenCode 的安裝指令。ENOENT 必須原封不動傳到它手上。

### 審查時發現的另一半：它根本不會說自己壞了

這個 adapter 把所有失敗都吞成 `{ events: [] }` 加一個心跳，而且**沒有帶原因**。上層看到「沒事件、沒 session、沒檔案數、沒跳過清單」，就自己推導出 `no_new_activity`。

**所以資料庫讀不到的時候，回報上去是「他今天沒用 OpenCode」。** 這正是 v1.26.69 要消滅的假健康訊號，只是這一個 adapter 沒補到。也因為這樣，上面那個修復就算生效了，在真實機器上也看不出來，沒有任何管道可以說「它開始能讀了」。

現在會回 `unreadable`、`no_install` 或 `sqlite_missing`。而且跟 v1.26.69 幫 Cursor 加的一樣，會多問一句「檔案在不在」，因為 `sqlite3` 對「應用程式關著的資料庫」跟「路徑上根本沒東西」講的是同一句話，把後者當成前者，就是把「你沒在用 OpenCode」講成「你的 OpenCode 壞了」。

### 沒有修、但寫下來的一件事

審查指出游標有一個會**永久跳過**某筆資料的路徑：兩則訊息的時間戳精確到毫秒都一樣、分兩次寫入、而 id 的排序跟寫入順序相反，那麼先讀到後面那則就會把前面那則永遠排除掉。

量過了：這台機器 1205 則訊息裡，同一毫秒的配對是 0 筆。而 id 是 `msg_` 加一段從時間推出來的前綴，跨毫秒的排序是對的。**但同一毫秒之內就由後面那段隨機碼決定順序，所以這個機制是真的存在的，只是條件湊不到。** 而且它不是這版造成的，掃描器直接讀正在使用中的資料庫，一樣會遇到。

改法會動到 Tier 1 的資料寫入語意，那是整個產品最吃重的一條路徑，要自己一版、自己一套驗證。已列進 backlog 第 21 條。

### 成本

需要走 fallback 的那幾輪，多複製一次資料庫。這台機器上最大的一個量過：13 MB、24 毫秒。另外三個是 3.2 MB、1.2 MB、512 KB。**沒有加大小限制**，24 毫秒沒有什麼好擋的。

### 順手修的

v1.26.70 自己的 `spec.md` 跟 `proposal.md` 還在寫 `immutable=1` 跟 `pathToFileURL`，那是量錯、在發版前就被換掉的第二版設計。tasks 檔記了這個更正，spec 沒有跟上，等於那一版的正式規格描述了一份不存在的實作。這版一起改掉，並且把真正上線的「附屬檔要一起複製」補成獨立的 scenario。
...
`https://legacy-server.example/ownmind/dashboard`（**沒有尾斜線**）回 `301 Location: /dashboard/`。那是絕對路徑，瀏覽器把 `/ownmind` 前綴丟掉、落到 `https://legacy-server.example/dashboard/` —— 一個叫「Vin WorkSpace」、跟 OwnMind 毫無關係的頁面。而且它回 200，不是 404，所以使用者會以為自己到了某個真的存在的地方。`/ownmind/admin` 一模一樣。
...
實測數據（legacy-server.example、視窗寬 968px）：wrapper clientWidth 662、table scrollWidth 841、「查看」按鈕的 x 座標 1079（在裁切區外）。
...
同一個病灶 `src/middleware/first-run-redirect.js` 有兩處：`/setup` 跟 `/admin/login` 都是絕對路徑。今天在 legacy-server.example 沒事是因為 nginx 剛好連沒帶前綴的 `/setup` 也代理，但那是巧合而不是設計。這次一起改成相對。用同一個工具函式 `relativeRedirectTarget()` — 這是 v1.26.46 建立 legacy-admin-mount 時已經抽出來的、有 10 條單元測試背書。
...
**列了沒修**：`GET /ownmind/dashboard`（沒有結尾斜線）會 301 到 `/dashboard/`、把 `/ownmind` 前綴丟掉，因為 `serve-static` 是用 Express 看到的路徑去組那個轉址。實測正式機上 `https://legacy-server.example/dashboard/` 也有被 proxy、會回 200，所以目前沒有使用者看得到的破口。`src/app.js` 對 `/me` 已經用相對的 `301 → 'me/'` 解掉同一種形狀。為了讓這次的 diff 只修一個缺陷，留著另辦。
...
2. `cd /srv/ownmind && git pull origin main`
