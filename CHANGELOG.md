...
```
MEMORY.md        283 行、52 KB
  ## Iron Rules  143 筆
  ## Projects    130 筆
最長的一行        305 字
```

兩個上限都破了。也就是說**索引後面那一半，已經有一段時間沒有進到 session 裡**，而產生它的程式不知道、讀它的那一端在載入時不出聲、檔案本身也沒有任何記號。它讀起來像一份完整的索引，但它不是。
...
拿真實資料試跑：**284 行 52 KB 變成 140 行 28 KB，最長的一行 196 字**，273 筆記憶裡列出 127 筆，另外 146 筆被明確標示出來。
...
新增 `tests/sync-memory-files.test.js` 12 項。**突變檢查**：把上限拿掉、把預算數字調大、把「少列了幾筆」那行刪掉、把排序拿掉、把截斷拿掉、把按需分配換成平均切，六種改法各試一次，**六種全部轉紅**。其中「平均切」第一次沒被抓到，是因為當時的斷言門檻在只有兩類的情況下平均切也會通過，改成檢查「預算有沒有被用完」之後才真的擋得住。
