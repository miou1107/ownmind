...
## 尚未發版 — js-yaml 升到 4.3.2，這一則掛在直接用的套件上，但打不到

你不用做任何事，下次同步 OwnMind 就會換掉舊版本。

GitHub 這次報的高風險跟前一批不一樣。前一批那三個套件都躲在相依樹深處，js-yaml 是根目錄
直接宣告的，讀鐵律檔頭的那段程式就是拿它在解析，所以這次查得比較仔細。

結論是打不到。`src/utils/iron-rule-frontmatter.js` 載入的時候指定了 `JSON_SCHEMA`，那個
schema 裡沒有 merge、也沒有 omap，三則通報要繞過的計數器根本不會被叫到。拿 CVE-2026-84375
的攻擊寫法實測，0.8 毫秒就被「重複的鍵」擋掉。

版本還是升上去了。那一行 schema 參數是唯一擋在載入器跟團隊規範中間的東西，而團隊規範從
別人的帳號同步過來，改一個字它就不見了。
...
停在舊版本；4.1.1 那次就是這樣拖了很久。

那條測試是整份檔案裡唯一綁在特定通報上的，上一次升到 4.3.1 沒跟著拉，所以把 package.json
改回 4.3.0 也照樣全綠。這次一起補上。
