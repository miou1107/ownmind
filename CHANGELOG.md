...
**背景：** 2026-05-23 / 2026-05-24 連續 4 次 AI 想存 type=`env` 的「bot-server.example 遠端訪問方式總覽」記憶被擋。追根究柢、被擋的內容只是「密鑰名稱字串」（如 `anydesk.bot_legacy-host.unattended_password`）跟 IP／連線編號等非敏感資訊、實際密碼存在 OwnMind 密鑰管理工具裡。v1.19.1 引入的 value-side keyword 偵測太寬鬆（「只要出現 password 字樣就擋」）導致誤判率高。
...
|------|----------|----------|
| `anydesk.bot_legacy-host.unattended_password`（密鑰名稱 reference） | ❌ 誤擋 | ✅ 放行 |
...
`process.env.MY_PASSWORD` 跟 `anydesk.bot_legacy-host.unattended_password` 這類「點分隔識別字路徑」（每段是合法變數名、用 `.` 串接）也被長度啟發式（≥ 20 字純英數字）誤抓。v1.19.13 加負向條件、識別字路徑樣式直接放行。真實密鑰（JWT、AWS、GitHub PAT、OpenAI key）有專屬 regex 抓、不依賴此啟發式。
...
- `npm test` 1706 / 1706 全綠（+31 個 v1.19.13 新測試含 bot-server.example 真實案例 regression、+ code review 後追加 I-1 PII 不洩漏 / I-2 雙段 base64 不放過 / I-3 snake_case 仍擋 共 11 條）
...
Eric / Michelle / Adam（user role）現在可以開 https://legacy-server.example/ownmind/me/，
