...
## v1.30.27 — Windows 上每回一句話就跳出一個空白視窗

你不用做任何事，下次自動更新就會換過去。更新之後，這個視窗就不會再出現。

Claude 每回你一句話，OwnMind 就會在背景叫你電腦上的 Claude，檢查那句話有沒有違反你的規矩。
這個檢查本來就該在背後安靜地跑。但在 Windows 上，背景程式再開另一個程式的時候，
要明講「不要給它視窗」。OwnMind 兩個地方都沒講，所以每回一句話，桌面上就多一個
空白的 Claude 視窗，要等檢查跑完才會關掉。

這一版把兩個地方都補上，也各加了一條測試盯著。拿掉修正再跑，這兩條測試會失敗。

### 順手修的：終端機設成「強制彩色」時，同步檢查會看錯版本

有些終端機會設定「一律輸出彩色」。在這種終端機裡跑同步檢查，數字會被加上顏色碼，
檢查程式讀回來就認不出是數字，結果把版本比較回報成失敗，規則數量也多出一段亂碼。
現在改成先把數字轉成純文字再輸出，另外加了一條測試，專門在強制彩色的情況下跑一次。

## v1.30.26 — 刪掉一份沒有人在跑的規矩檢查程式

你不用做任何事，下次自動更新就會換過去。畫面上不會有任何差別。

OwnMind 在你每次下指令、每次改檔案的時候，會先拿你的規矩檢查一遍。這個檢查以前有兩份：
一份用 Node 寫，一份用 shell 寫。從 v1.30.15 起，安裝程式在每一個作業系統上都只註冊 Node 那份，
shell 那份沒有任何地方會叫它。

但它還留在程式庫裡，安裝程式還在複製它，測試還在測它。**兩份同一件事的程式，改的時候要改兩次，
而沒有人在跑的那一份就是會慢慢爛掉的那一份。** 這個庫已經為此付過兩次代價：一次是它自己抄了一份
指令分類表、跟正本對不起來，另一次是它在 Windows 上被註冊成一個根本跑不動的指令，而畫面上還印
「已修好」。

這一版把它刪掉，連同兩個只為它存在的檔案：一支四行的分類包裝、以及一支讓 shell 有辦法呼叫
攔截器的命令列程式。升級的時候，OwnMind 會順手把你電腦上那幾份舊的複本也清掉。

### 順手發現的兩件事

**同一個錯誤，兩個地方印不一樣的名字。** 缺 git 的時候，安裝腳本印 `preflight_missing_git`，
而檢查程式印 `preflight_git`。你照畫面上的字去搜尋會搜不到。四個地方現在都印同一個字，
而且有測試在盯。

**有一個專門救老機器的搬家機制，從 v1.30.15 起就沒有在跑了。** 它寫在 shell 那份裡面：
偵測到你的設定檔裡還沒有開場掛勾，就自己去拉一次最新版、裝好、然後跟你說一聲。Node 那份
從來沒有這段。那份不再被註冊之後，這個搬家就跟著停了，而當時沒有任何東西講。

刪掉那份程式不會讓任何人少拿到東西——那件事在半年前就已經發生。真正的問題是：如果現在還有
機器停在那個舊狀態，它已經沒有任何自動的路可以走出來。要不要把這個搬家做進 Node 那份，
是另外一個決定。

## v1.30.25 — 這一版修的六件事，共通點是「壞掉了，但畫面上看不出來」

你不用做任何事，下次自動更新就會換過去。

### 安裝說明裡那行指令，你貼下去會先跳一段紅字

README 教你複製的那一行，開頭會噴一段紅色錯誤，然後才開始裝。`scripts/bootstrap.ps1` 的開頭
藏了三個看不見的字，PowerShell 把它們當成指令名稱的一部分，於是說找不到這個指令。

那三個字本來有用：Windows PowerShell 5.1 少了它們，就會把檔案裡的中文讀成亂碼。所以這一版
先把那個檔案剩下的四句中文註解翻成英文，整份檔案只剩英文字之後，才把它們拿掉。兩件事綁在一起
測，中文放回去測試就會紅。

### 連不上伺服器的時候，訊息只有「連線失敗」四個字

記錄這次對話的時候連續失敗三次，訊息每次都一模一樣，而同一時間用別的工具打同一個網址是成功的。
你手上一個工具會失敗、一個會成功，中間卻沒有任何東西可以比對。

現在那句話會告訴你三件事：程式打的是哪個網址、等了幾毫秒、以及真正的原因——連線逾時、SSL 壞掉、
對方把連線切斷，程式以前把這些全都吞掉了。順帶修好一個更安靜的毛病：專門抓網路問題的那段程式
只看最外層，真正的錯誤碼卻藏在裡面一層，所以程式根本沒把某些斷線當成斷線，也就不會自動切去
離線運作。

### 如果你的電腦沒裝 git，安裝程式跑一半會清掉你的資料

有同事照著 README 那行裝，那台電腦沒有 git。程式先把他整個 `~/.ownmind` 備份走，然後死在第一個
git 指令上，畫面只剩 Windows 原生的「無法辨識」訊息：沒有錯誤代碼、沒有回報，還留下一個沒用的
備份資料夾。

從頭到尾沒有任何一段程式問過「這台電腦上有沒有 git」。現在升級跟安裝都會先問一次，而且是在搬動
任何東西之前：缺什麼一次講完，每一樣都附上那個平台的完整安裝指令，然後停住。這一版 Windows 跟
Mac／Linux 兩邊一起補，兩邊的 Node 版本下限也跟安裝程式對齊。

### 你在改檔案、檢查工具壞掉的時候，它會默默放你過去

檢查規矩的程式如果自己出錯，它會安靜地結束、什麼都不印，而這跟「檢查過了、沒有問題」在畫面上
長得一模一樣。你的編輯照樣寫進去，沒有人告訴你這次沒檢查。

現在改檔案這條路上出錯，它會講一句「這次沒檢查到」，用的是另外兩個地方本來就在用的同一句話。
執行指令那條路維持原樣不出聲，因為每一個指令都印一次，久了就沒有人會看。

### 每次開新對話，有一支程式印完就整個當掉

它印出來的東西是對的，所以你看不出來，但它的結束狀態是當掉。任何會去看結束狀態的程式，都會把
這次開場當成失敗。

它在網路連線還沒收乾淨的時候就強制結束自己。這一版改成先讓它自己結束，卡住超過 0.3 秒才強制。
三種網路狀況都量過：正常的時候現在還比以前快。

### 十五個測試會亮綠燈，其實什麼都沒檢查

它們現在沒壞，可是東西真的壞掉的時候，它們還是綠的——唯一的 assert 藏在 if 裡面、比對的清單是
空的、或者整個測試只寫了一句「沒有拋錯」。其中一個抓中英夾雜的測試，只要功能壞掉就會自動通過。

這一版把十五個全部改掉，每一個都留下一句話說明它原本藏了什麼。另外有一個測試從寫出來那天起就
沒在任何一次自動檢查裡跑過，因為它要用到的東西沒被裝進去；現在自動檢查會裝了。
