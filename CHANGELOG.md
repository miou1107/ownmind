...
- 接手死掉的鎖要先排隊（另一個獨佔檔），拿到之後**再讀一次年齡**才刪。排隊期間如果別人已經放了新鎖，那就不算舊了，不動它，晚到的自己去搶、搶不到就走。
- 搶輸記 `update_skipped`、原因 `lock_held` — 這是 MCP 本來就在用的事件名稱跟原因，不是失敗。

### 測試

八個並行只能有一個拿到，shell 與 Node 各跑一次。另外附**陽性對照**：先讓舊寫法上場，證明這組測試真的看得到競爭（看不到的話下面全部不算數），以及 `touch` 對已存在的檔案確實會成功。

八個突變全部要紅。第一輪有兩個沒被抓到（Node 的排隊與二次確認）——八個同時放行的程序會排成一列而不是重疊，racy 測試碰不到那個窗口。先懷疑測試不是懷疑程式，補了兩個直接把交錯做出來的決定性測試，兩個突變當場死掉。
