...
2026-08-10 正式機。九個人，七個在最新版、二十天內各有 41～5531 筆活動紀錄。剩下兩位：
一位卡在 1.26.57（8/4 之後沒動）、一位卡在 1.26.27（6/18 之後沒動），二十天內的活動紀錄
都是 **0 筆**。而這兩台每天都有心跳，代表程式有啟動。

零筆不是「很少」。從 v1.17.22 以來，這兩台送出的每一筆「更新成功／失敗／跳過」全部沒有
離開過那台電腦。**最需要被診斷的兩個人，正好是唯二診斷不了的。**

### 心跳活下來、其他全死的原因

心跳是立刻送出、而且完全不碰檔案系統。活動紀錄則是在同一個 try 裡做兩件事，順序是：

```js
appendFileSync(filePath, ...);   // 先寫本機
buffer.push(entry);              // 再排隊上傳
```

而那個路徑來自 `join(process.env.HOME || '', '.ownmind', 'logs')`。

**Windows 沒有 `HOME` 這個環境變數。** 那個「`|| ''`」讓路徑變成相對路徑，於是紀錄被寫到
「程式被啟動時剛好所在的資料夾」；那個資料夾不能寫的時候，建資料夾就會出錯 —— 而因為本機
寫入排在前面，出錯直接跳過了下面的排隊上傳，外層的 `catch {}` 什麼都沒說。

所以一個檔案系統問題不是把事件降級成「有送出、只是本機沒存」。它是把兩份一起刪掉，安靜地。
...
3. 「更新成功／失敗／跳過／本來就最新」四種結果改成立刻送出。它們一天最多發生一次，排隊
   一點好處都沒有；而某些工具關掉子程式時是直接砍掉、不給訊號，那條「關閉前補送」的路
   根本不會執行。
