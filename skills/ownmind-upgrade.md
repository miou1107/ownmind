...
| Darwin / Linux | 否（首次安裝 or pre-v1.17.6） | `curl -fsSL https://legacy-server.example/ownmind/bootstrap.sh \| bash` |
| Windows_NT / 找不到 uname | 是 | `powershell -ExecutionPolicy Bypass -File $HOME\.ownmind\scripts\bootstrap.ps1` |
| Windows_NT / 找不到 uname | 否 | `iwr -useb https://legacy-server.example/ownmind/bootstrap.ps1 \| iex` |
