...
2. 如果有更新 → 自動 pull + 更新 skill → 顯示更新了什麼：
   ```
   【OwnMind v1.8.0】版本更新：偵測到新版本，已自動更新：
      - 新增 xxx 功能
      - 修正 xxx 問題
   ```
...
6. 如果有新 commit → 顯示提醒：
   ```
   【OwnMind v1.8.0】更新提醒：遠端有新版本（{N} 個 commit），建議更新：
      - {commit message 1}
      - {commit message 2}
   要我幫你更新嗎？
   ```
...
### 讀取特定記憶時
```
【OwnMind v1.8.0】記憶操作：已調閱「ring-linebot」專案記憶
```

### 搜尋時
```
【OwnMind v1.8.0】記憶操作：搜尋「SSH 相關規則」→ 找到 2 筆相關記憶
```

### 寫入時
```
【OwnMind v1.8.0】記憶操作：已儲存新鐵律 IR-008「部署前必須檢查環境變數」
```

### 更新時
```
【OwnMind v1.8.0】記憶操作：已更新「ring-linebot」專案進度
```

### 停用時
```
【OwnMind v1.8.0】記憶操作：已停用 IR-003（原因：改用其他測試策略）
```

### 交接時
```
【OwnMind v1.8.0】交接：已建立 → 目標：Codex
...
### 接手交接時
```
【OwnMind v1.8.0】交接：接手 ← 來源：Claude Code @ MacBook Pro
   - 狀態：webhook handler 重構做到一半
   - 待完成：error handling、測試
   - 注意：parser 的 signature 驗證不要動
   確認接手嗎？
...
### 彙整時
```
【OwnMind v1.8.0】彙整建議：本次 session 有以下值得記錄的事項
...
### 密鑰存取時
```
【OwnMind v1.8.0】密鑰存取：正在取得「line-channel-secret」...
```

### 更新記憶時
```
【OwnMind v1.8.0】記憶操作：已更新「檔案命名規則」
   舊版：檔案名稱要大寫
   新版：檔案名稱要小寫
   原因：統一 Linux 路徑規範
```
...
```
【OwnMind v1.8.0】行為觸發：你提醒過「SSH 不要頻繁登入登出」，已強制遵守
```
...
2. 如果有規範版本更新 → 強制顯示更新通知：
   ```
   【OwnMind v1.8.0】行為觸發：📋 團隊規範有更新，已強制套用最新版
...
4. 如果版本不一致 → 清除快取，重新載入最新版，並顯示：
   ```
   【OwnMind v1.8.0】行為觸發：⚠️ 團隊規範「{title}」已更新至 {version}，重新載入最新版
      更新內容：{changelog}
   ```
...
6. 按規則執行，並顯示：
   ```
   【OwnMind v1.8.0】行為觸發：已載入團隊規範「Git 治理規範」(v2026-03-27T14:30)，按規範執行
   ```
...
- AI 必須主動偵測衝突並發出提醒，不可靜默覆蓋：
  ```
  【OwnMind v1.8.0】衝突偵測：團隊規範「{title}」與你的個人鐵律 IR-{XXX} 衝突
     → 依規定：團隊規範為最高準則，已按團隊規範執行
     → 你的個人規則「{rule_title}」在此情境下被覆蓋
  ```
- 即使使用者指示違反團隊規範，AI 也必須先提醒：
  ```
  【OwnMind v1.8.0】行為觸發：⚠️ 你的操作違反團隊規範「{title}」，此為組織最高準則，無法直接跳過
...
1. 顯示警告：
   ```
   【OwnMind v1.8.0】記憶操作：⚠️ 你即將關閉團隊規範「{title}」
      - 此規範由管理員設定，優先級高於個人規則
      - 關閉後只影響你自己，不影響其他成員
      請輸入「我確認」以確認操作
   ```
...
4. 顯示：
   ```
   【OwnMind v1.8.0】記憶操作：已關閉團隊規範「{title}」（僅限你個人，團隊其他人不受影響）
   ```
5. **關閉後的持續提醒（強制）：**
   使用者關閉團隊規範後，每次 OwnMind 被觸發時，都必須顯示提醒：
   ```
   【OwnMind v1.8.0】行為觸發：⚠️ 你已關閉團隊規範「{title}」，此規範目前未套用於你
   ```
...
**非 admin 嘗試寫入時：**
```
【OwnMind v1.8.0】記憶操作：無法新增團隊規範，此類型僅限管理員操作
```
...
1. 顯示即將寫入的內容摘要：
   ```
   【OwnMind v1.8.0】記憶操作：⚠️ 你即將新增團隊規範，此規範將套用到所有成員
      - 標題：{title}
      - 觸發條件：{triggers}
      - 影響範圍：全體成員
      請輸入「我確認」以確認操作
...
3. 修改既有團隊規範時，同樣需要確認，提示改為：
   ```
   【OwnMind v1.8.0】記憶操作：⚠️ 你即將修改團隊規範「{title}」，變更將同步到所有成員
      請輸入「我確認」以確認操作
   ```
...
```
【OwnMind v1.8.0】記憶操作：⚠️ Context 已超過 40%，AI 品質可能開始下降
   ✅ 鐵律已重新載入
...
```
【OwnMind v1.8.0】記憶操作：⚠️ 規則「{title}」已被遺漏 {N} 次，落地率偏低
...
**自評格式（強制）：**
```
【OwnMind v1.8.0】規則自評：本次 session 規則遵守狀況
...
**AI 收到 stale 警告時：**
```
【OwnMind v1.8.0】記憶操作：⚠️ 記憶狀態已被其他工具更新，建議 re-init 取得最新版
```

**AI 收到 409 時：**
```
【OwnMind v1.8.0】記憶操作：❌ 寫入被拒（狀態已變更），正在 re-init...
```
...
刷新後顯示：
```
【OwnMind v1.8.0】鐵律確認：鐵律已重新載入，防護持續中
```
