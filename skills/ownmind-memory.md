...
**版本號來自 MCP tool 回傳（`server_version` 欄位），不在 skill 中硬編碼。以下用 `vX.X.X` 代表。**

**MCP tool 回傳已自動附帶版本標記（`【OwnMind vX.X.X】{類型}：`）和技巧提示，AI 不需要重複加。以下格式範例中標記為「MCP 自動」的由程式處理，標記為「AI 手動」的需要 AI 在輸出時自己加。**
...
| 鐵律攔截 | 驗證不通過，block 操作 |
| 鐵律觸發 | AI 即將違反鐵律時主動停下 |
| 鐵律確認 | context 過長重新載入鐵律 |
| 合規回報 | 回報遵守/跳過/違反 |
| 強制注意 | 歷史違反記錄的警告 |
| 違規自省 | Session 結束時的 commit 稽核 |
| 規則自評 | Session 結束統計遵守率 |
| 自我學習 | 違反率分析 + 動態調整 |
| 學習回顧 | Session 結束前回顧新知識 |
| 衝突偵測 | 鐵律 vs 團隊規範矛盾 |
| 彙整建議 | 主動建議值得存的經驗 |
| 行為觸發 | 偵測重複模式、規範更新 |
| 個人偏好 | get("profile") |
| 工作原則 | get("principle") |
| 編碼標準 | get("coding_standard") |
| 團隊規範 | get("team_standard") |
| 專案記憶 | get("project") |
| 環境設定 | get("env") |
| 記憶搜尋 | search |
| 記憶寫入 | save / update / disable |
| 建立交接 | handoff_create |
| 載入交接 | init active_handoff |
| 接受交接 | handoff_accept |
| 進度紀錄 | log_session |
| 密鑰管理 | secret 讀寫 |
| 自動升級 | hook 偵測缺 hook |
| 版本更新 | 自動更新完成 |
| 週報摘要 | init weekly_summary |
| 記憶健檢 | init memory_health |
| 待審記憶 | init pending_review |
| Commit 前檢查 | git pre-commit hook |
| Commit 後稽核 | git post-commit hook |
| 錯誤回報 | MCP tool / hook 失敗 |
| 技巧提示 | 每次操作後隨機 |
| Context 提醒 | context 快用完 |

**規則：**
- 禁止使用舊格式（【OwnMind】、【OwnMind 觸發】等）
- 版本號從 MCP tool 回傳的 `server_version` 讀取，不硬編碼在 skill 中
- 每次操作後附上一行：`【OwnMind vX.X.X】技巧提示：...`（MCP 自動）
...
6. 如果有新 commit → 顯示提醒：
   ```
   【OwnMind vX.X.X】版本更新：遠端有新版本（{N} 個 commit），建議更新：
      - {commit message 1}
      - {commit message 2}
   要我幫你更新嗎？
   ```
...
### 讀取特定記憶時
```
【OwnMind vX.X.X】專案記憶：已調閱「ring-linebot」專案記憶
```

### 搜尋時
```
【OwnMind vX.X.X】記憶搜尋：搜尋「SSH 相關規則」→ 找到 2 筆相關記憶
```

### 寫入時
```
【OwnMind vX.X.X】記憶寫入：已儲存新鐵律 IR-008「部署前必須檢查環境變數」
```

### 更新時
```
【OwnMind vX.X.X】記憶寫入：已更新「ring-linebot」專案進度
```

### 停用時
```
【OwnMind vX.X.X】記憶寫入：已停用 IR-003（原因：改用其他測試策略）
```

### 交接時
```
【OwnMind vX.X.X】建立交接：已建立 → 目標：Codex
...
### 接手交接時
```
【OwnMind vX.X.X】接受交接：接手 ← 來源：Claude Code @ MacBook Pro
   - 狀態：webhook handler 重構做到一半
   - 待完成：error handling、測試
   - 注意：parser 的 signature 驗證不要動
   確認接手嗎？
...
### 密鑰存取時
```
【OwnMind vX.X.X】密鑰管理：正在取得「line-channel-secret」...
```

### 更新記憶時
```
【OwnMind vX.X.X】記憶寫入：已更新「檔案命名規則」
   舊版：檔案名稱要大寫
   新版：檔案名稱要小寫
   原因：統一 Linux 路徑規範
```
...
```
【OwnMind vX.X.X】鐵律觸發：你提醒過「SSH 不要頻繁登入登出」，已強制遵守
```
...
行為要求：
- 收到 enforcement_alerts 時，**必須**用【OwnMind vX.X.X】強制注意：格式完整顯示所有 alerts
...
2. 如果有規範版本更新 → 強制顯示更新通知：
   ```
   【OwnMind vX.X.X】行為觸發：📋 團隊規範有更新，已強制套用最新版
...
4. 如果版本不一致 → 清除快取，重新載入最新版，並顯示：
   ```
   【OwnMind vX.X.X】行為觸發：⚠️ 團隊規範「{title}」已更新至 {version}，重新載入最新版
      更新內容：{changelog}
   ```
...
6. 按規則執行，並顯示：
   ```
   【OwnMind vX.X.X】行為觸發：已載入團隊規範「Git 治理規範」(v2026-03-27T14:30)，按規範執行
   ```
...
- AI 必須主動偵測衝突並發出提醒，不可靜默覆蓋：
  ```
  【OwnMind vX.X.X】衝突偵測：團隊規範「{title}」與你的個人鐵律 IR-{XXX} 衝突
     → 依規定：團隊規範為最高準則，已按團隊規範執行
     → 你的個人規則「{rule_title}」在此情境下被覆蓋
  ```
- 即使使用者指示違反團隊規範，AI 也必須先提醒：
  ```
  【OwnMind vX.X.X】行為觸發：⚠️ 你的操作違反團隊規範「{title}」，此為組織最高準則，無法直接跳過
...
1. 顯示警告：
   ```
   【OwnMind vX.X.X】記憶寫入：⚠️ 你即將關閉團隊規範「{title}」
      - 此規範由管理員設定，優先級高於個人規則
      - 關閉後只影響你自己，不影響其他成員
      請輸入「我確認」以確認操作
   ```
...
4. 顯示：
   ```
   【OwnMind vX.X.X】記憶寫入：已關閉團隊規範「{title}」（僅限你個人，團隊其他人不受影響）
   ```
5. **關閉後的持續提醒（強制）：**
   使用者關閉團隊規範後，每次 OwnMind 被觸發時，都必須顯示提醒：
   ```
   【OwnMind vX.X.X】行為觸發：⚠️ 你已關閉團隊規範「{title}」，此規範目前未套用於你
   ```
...
**非 admin 嘗試寫入時：**
```
【OwnMind vX.X.X】記憶寫入：無法新增團隊規範，此類型僅限管理員操作
```
...
1. 顯示即將寫入的內容摘要：
   ```
   【OwnMind vX.X.X】記憶寫入：⚠️ 你即將新增團隊規範，此規範將套用到所有成員
      - 標題：{title}
      - 觸發條件：{triggers}
      - 影響範圍：全體成員
      請輸入「我確認」以確認操作
...
3. 修改既有團隊規範時，同樣需要確認，提示改為：
   ```
   【OwnMind vX.X.X】記憶寫入：⚠️ 你即將修改團隊規範「{title}」，變更將同步到所有成員
      請輸入「我確認」以確認操作
   ```
...
```
【OwnMind vX.X.X】Context 提醒：⚠️ Context 已超過 40%，AI 品質可能開始下降
   ✅ 鐵律已重新載入
...
```
【OwnMind vX.X.X】自我學習：⚠️ 規則「{title}」已被遺漏 {N} 次，落地率偏低
...
**自評格式（強制）：**
```
【OwnMind vX.X.X】規則自評：本次 session 規則遵守狀況
...
**AI 收到 stale 警告時：**
```
【OwnMind vX.X.X】記憶寫入：⚠️ 記憶狀態已被其他工具更新，建議 re-init 取得最新版
```

**AI 收到 409 時：**
```
【OwnMind vX.X.X】記憶寫入：❌ 寫入被拒（狀態已變更），正在 re-init...
```
