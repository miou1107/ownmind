...
API_URL="https://legacy-server.example/ownmind"
API_KEY=""
...
if [ -f "$CLAUDE_SETTINGS" ]; then
  ...
  API_URL=$(node -e "
    try {
      const s = JSON.parse(require('fs').readFileSync('$CLAUDE_SETTINGS', 'utf8'));
      console.log(s.mcpServers?.ownmind?.env?.OWNMIND_API_URL || 'https://legacy-server.example/ownmind');
    } catch { console.log('https://legacy-server.example/ownmind'); }
  " 2>/dev/null)
fi
