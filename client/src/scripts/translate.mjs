...
//
// 環境變數（任一組可用）：
//   TRANSLATE_API_BASE  — OpenAI 相容 API base URL（預設 https://legacy-server.example/llm-switch/v1）
...
const API_BASE = process.env.TRANSLATE_API_BASE || 'https://legacy-server.example/llm-switch/v1';
...
async function main() {
  ...
  if (!API_KEY) {
    ...
    console.log('   export TRANSLATE_API_BASE=<api base、預設 https://legacy-server.example/llm-switch/v1>');
