 ...
 *   （例 anydesk.bot_legacy-host.unattended_password）會被舊邏輯誤判、實際上那只是
...
export function detectSecretLike(value, options = {}) {
  ...
  if (!options.skip_keyword) {
    ...
    //     - 「anydesk.bot_legacy-host.unattended_password」這種密鑰「名稱」reference
  ...
  //    v1.19.13：點分隔識別字路徑（例 anydesk.bot_legacy-host.unattended_password、
 ...
 *
 * 不命中（過濾掉的誤判型）：
 *   - 「anydesk.bot_legacy-host.unattended_password」沒 :／= 後綴、不命中
 ...
 * 真實密鑰名稱 reference（`anydesk.bot_legacy-host.unattended_password`、
